...
export class JntOrdersComponent implements OnInit {
  ...
  getAll() {
    ...
    this.spin.show();
    this.jntOrders = [];
    this.service.findConsignmentSpark(this.searhform.getRawValue()).subscribe(res => {      //scanType: null this.searhform.getRawValue()
      // res.forEach(x => {
  ...
  downloadExcel() {
    ...
    this.jntOrders.forEach(x => {
      res.push({
        ...
        "Event": x.scan_Type == null ? 'Ready to Print' : x.scan_Type,
