...
public interface ConsignmentWebhookRepository extends JpaRepository<ConsignmentWebhookEntity, Long> {
	...
	@Query(value = "  SELECT * FROM tblconsignmentwebhook\r\n"
			+ "	WHERE CUSTOMER_REFERENCE_NUMBER = :customerRreferenceNumber and TYPE IN (:type) ", 
			nativeQuery = true)
	public List<ConsignmentWebhookEntity> findByCustomerReferenceNumberAndType(
			@Param("customerRreferenceNumber") String customerRreferenceNumber, 
			@Param("type") List<String> type);
	
	@Query(value = "  SELECT TOP 1 * FROM tblconsignmentwebhook\r\n"
			+ "	WHERE CUSTOMER_REFERENCE_NUMBER = :customerRreferenceNumber ORDER BY EVENT_TIME_EPOCH DESC", 
			nativeQuery = true)
	public ConsignmentWebhookEntity findOrdersByType (@Param("customerRreferenceNumber") String customerRreferenceNumber);
	
	@Query(value = "  SELECT type FROM tblconsignmentwebhook\r\n"
