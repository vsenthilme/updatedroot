...
//		log.info("convertStringToDate-------> : " + date);
	 ...
	 * @return
	 */
	public static String convertDate3String(Date date) {
...
//		generateNext7Days ("2024-03-27");
		
		convertDate3String (new Date());
	 ...
	 * @throws ParseException
	 */
	public static List<String> generateNext7Days (String currentDate) throws ParseException {
		List<String> next7Dates = new ArrayList<>();
		...
		for (int i = 0; i <= 7; i ++) {
			...
			boolean isFriday = (c.get(Calendar.DAY_OF_WEEK) == Calendar.FRIDAY);
			currentDate = sdf.format(c.getTime());
			if (!isFriday) {
				System.out.println(currentDate + "," + isFriday);
				next7Dates.add(currentDate);
			}
			counter ++;
