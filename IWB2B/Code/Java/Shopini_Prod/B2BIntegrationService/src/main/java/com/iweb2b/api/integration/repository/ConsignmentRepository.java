...
public interface ConsignmentRepository extends JpaRepository<ConsignmentEntity, Long>,
	...
	//-----------For AJEX-----------------------------------------
	@Query(value = "SELECT REFERENCE_NUMBER FROM tblconsignment3 where CUSTOMER_REFERENCE_NUMBER=:customerReferenceNumber", nativeQuery = true)
	...
	@Query(value ="select tc.AWB_3RD_PARTY, tc.CREATED_AT, tc.REFERENCE_NUMBER \n"+
			...
			"and tc.AWB_3RD_PARTY not in (select TRACKING_NO FROM tblshopiniwebhook where LMD_SYNCED_STATUS = 1 group by TRACKING_NO) \n" +
