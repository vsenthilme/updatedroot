  ...
  intercept(
    ...
    return next.handle(request).pipe(catchError(error => {
      if (error.status === 401 || error.status == 400 || error.status == 415) {
        if (request.url.includes('/mv-master-service/auth-token') || (error.error != null && error.error.error.includes("Order is not completely Processed"))) {
...
console.log(apiName)
  ...
  private addTokenHeader(request: HttpRequest<any>, token: string) {
    ...
    if (request.url.includes('/mv-master-service/login/validate'))
{
  ...
  return request.clone({ params: new HttpParams().append("authToken", token) });
   ...
   return request.clone({ params: new HttpParams().append("authToken", token).append('loginUserID', 'Admin') });
  }
