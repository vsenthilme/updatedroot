...
export class ConsignmentNewComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog, public spinner: NgxSpinnerService,public serivce: ConsignmentNewService,public cs: CommonService, private messageService: MessageService,
  ...
  getAll(){
    this.spinner.show();
    this.sub.add(this.serivce.Getall().subscribe((res: any[]) => {
  ...
  delete(): void {
    ...
    const dialogRef = this.dialog.open(DeleteComponent, {
      ...
      width: '35%',
      maxWidth: '80%',
      data: this.consignmentNew1[0].orderId
    });
console.log(this.consignmentNew1[0].languageId)
  ...
  deleterecord(orderId: any) {
    this.spinner.show();
    this.sub.add(this.serivce.Delete(orderId).subscribe((res) => {
  ...
  onChange() {
    console.log(this.selectedconsignmentNew1.length)
  ...
  downloadexcel( ) {
    ...
    this.consignmentNew1.forEach(x => {
      res.push({
       ...
       "Origin Address":x.originDetailsAddressLine1,
       "Destination Address":x.destinationDetailsAddressLine1,
