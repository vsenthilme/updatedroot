...
export class StoreTableComponent implements OnInit {
  ...
  deletetedStoreNumber: any [] = [];
  submit(){

    this.deletetedStoreNumber.forEach(element => {
      this.sub.add(this.storageService.Update({
        availability: 'Vacant'
      }, element).subscribe(storageResult => {
  
      }))
    });
 //console.log(this.dataSource.data)
 this.dialogRef.close({ data: this.dataSource.data, totalRent: this.rentTotal, storeChanged: this.storeChanged});
  ...
  removeEmail(index:any,element:any) {
    this.deletetedStoreNumber.push(element.storeNumber)
