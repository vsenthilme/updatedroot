...
export class AgreementComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    private upload: CustomerService
  ) { }
...
this.comaStoreNumber = this.finalStoreNumber.map(element=>element).join(",");
    ...
    // pdfMake.createPdf(dd).download('Agreement- ' + res.agreementNumber);
      ...
      //   (dataUrl) =>{
      //     this.pdfFile = dataUrl;
      //   }
      //   );
      //   this.upload.uploadfile(this.pdfFile, 'document').subscribe((resp: any) => {
      //     this.toastr.success("Document uploaded successfully.", "Notification", {
      //       timeOut: 2000,
      //       progressBar: false,
      //     });
      //     this.upload.documentStorage({
      //       fileDescription: resp.file,
      //       customerName: 'LC0000000251'
      //     }).subscribe(res => {})
      //   })
