...
export class CommonInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (bearerToken) {
      ...
      if (req.url.split('/')[1] == 'us-master-service' && req.url.split('/')[2] == 'doc-storage' &&  req.method == 'GET') {
        authReq = req.clone({
          url: `${url}${req.url}`,
      ...
      else if (req.url.split('/')[1] == 'us-master-service' && req.url.split('/')[2] == 'doc-storage' && req.method == 'POST') {
        authReq = req.clone({
          url: `${url}${req.url}`,
          params: new HttpParams()
        });
