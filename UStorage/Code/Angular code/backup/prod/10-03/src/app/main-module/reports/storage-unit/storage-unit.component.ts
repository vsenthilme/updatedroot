...
export class StorageUnitComponent implements OnInit {
  ...
  showFiller = false;
  displayedColumns: string[] = [ 'codeId', 'description', 'phase', 'doorType', 'priceMeterSquare', 'weekly', 'monthly', 'halfYearly', 'yearly', 'availability', 'linkedAgreement'];
...
dropdownlist() {
  ...
  this.cas.getalldropdownlist([
    ...
    this.storageNumberList = this.cas.foreachlist3(results[0], this.cas.dropdownlist.trans.storageunit.key);
    this.doortypeList = this.cas.foreachlist1(results[1], this.cas.dropdownlist.setup.doortype.key);
  }, (err) => {
  ...
  filtersearch() {
    ...
    this.spin.show();
    this.sub.add(this.service.getStorageunitTime(this.form.getRawValue()).subscribe(res => {
