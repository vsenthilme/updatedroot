...
dataList: any[] = [];
  ...
  getData(){
    this.sub.add(this.service.getpaymentdue({}).subscribe(res => {
        res.agreementDetails.forEach(element => {
            if(parseInt(element.dueDays) < 7){
                console.log(7)
              //  this.lessThanSeven.push(element)
                this.lessThanSeven = this.lessThanSeven + (element.lastPaidVoucherAmount != null ? parseInt(element.lastPaidVoucherAmount) : 0.00);
                
            };
            if(parseInt(element.dueDays) > 7 && parseInt(element.dueDays) < 14){
               // this.lessThanFourteen.push(element.dueDays)
                this.lessThanFourteen = this.lessThanFourteen + (element.lastPaidVoucherAmount != null ? parseInt(element.lastPaidVoucherAmount) : 0.00);
            };
            if(parseInt(element.dueDays) >= 14 && parseInt(element.dueDays) < 21){
                //this.greaterThanTweentyEight.push(element.dueDays)
                this.lessThanTweentyOne = this.lessThanTweentyOne + (element.lastPaidVoucherAmount != null ? parseInt(element.lastPaidVoucherAmount) : 0.00);
            };
            if(parseInt(element.dueDays) >= 21 && parseInt(element.dueDays) < 28){
            //  this.lessThanTweentyEight.push(element.dueDays)
               this.lessThanTweentyEight = this.lessThanTweentyEight + (element.lastPaidVoucherAmount != null ? parseInt(element.lastPaidVoucherAmount) : 0.00);
            };
            if(parseInt(element.dueDays) >= 28){
              //  this.greaterThanTweentyEight.push(element.dueDays)
                this.greaterThanTweentyEight = this.greaterThanTweentyEight + (element.lastPaidVoucherAmount != null ? parseInt(element.lastPaidVoucherAmount) : 0.00);
            };
        });
        this.dataList.push(
            {
  ...
  bindChart(array){
    this.chartOptions = {
        ...
        tooltip: {
            ...
            pointFormat: '<span style="color:{point.color}">{point.name}</span>: <b>{point.y:.3f}KWD</b><br/>'
        },
