...
public class ImBasicData1V2Specification implements Specification<ImBasicData1V2> {
	...
	@Override
     public Predicate toPredicate(Root<ImBasicData1V2> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (searchImBasicData1.getBarcodeId() != null && !searchImBasicData1.getBarcodeId().isEmpty()) {
            final Path<Group> group = root.<Group> get("barcodeId");
            predicates.add(group.in(searchImBasicData1.getBarcodeId()));
        }

        if (searchImBasicData1.getMaterialNo() != null && !searchImBasicData1.getMaterialNo().isEmpty()) {
            final Path<Group> group = root.<Group> get("materialNo");
            predicates.add(group.in(searchImBasicData1.getMaterialNo()));
        }

        if (searchImBasicData1.getPriceSegment() != null && !searchImBasicData1.getPriceSegment().isEmpty()) {
            final Path<Group> group = root.<Group> get("priceSegment");
            predicates.add(group.in(searchImBasicData1.getPriceSegment()));
        }

        if (searchImBasicData1.getArticleNo() != null && !searchImBasicData1.getArticleNo().isEmpty()) {
            final Path<Group> group = root.<Group> get("articleNo");
            predicates.add(group.in(searchImBasicData1.getArticleNo()));
        }

        if (searchImBasicData1.getGender() != null && !searchImBasicData1.getGender().isEmpty()) {
            final Path<Group> group = root.<Group> get("gender");
            predicates.add(group.in(searchImBasicData1.getGender()));
        }

        if (searchImBasicData1.getColor() != null && !searchImBasicData1.getColor().isEmpty()) {
            final Path<Group> group = root.<Group> get("color");
            predicates.add(group.in(searchImBasicData1.getColor()));
        }

        if (searchImBasicData1.getSize() != null && !searchImBasicData1.getSize().isEmpty()) {
            final Path<Group> group = root.<Group> get("size");
            predicates.add(group.in(searchImBasicData1.getSize()));
        }
        if (searchImBasicData1.getNoPairs() != null && !searchImBasicData1.getNoPairs().isEmpty()) {
            final Path<Group> group = root.<Group> get("noPairs");
            predicates.add(group.in(searchImBasicData1.getNoPairs()));
        }
