...
public class StagingLineV2Specification implements Specification<StagingLineEntityV2> {
    ...
    public Predicate toPredicate(Root<StagingLineEntityV2> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (searchStagingLine.getMaterialNo() != null && !searchStagingLine.getMaterialNo().isEmpty()) {
            final Path<Group> group = root.<Group>get("materialNo");
            predicates.add(group.in(searchStagingLine.getMaterialNo()));
        }

        if (searchStagingLine.getPriceSegment() != null && !searchStagingLine.getPriceSegment().isEmpty()) {
            final Path<Group> group = root.<Group>get("priceSegment");
            predicates.add(group.in(searchStagingLine.getPriceSegment()));
        }

        if (searchStagingLine.getArticleNo() != null && !searchStagingLine.getArticleNo().isEmpty()) {
            final Path<Group> group = root.<Group>get("articleNo");
            predicates.add(group.in(searchStagingLine.getArticleNo()));
        }
        if (searchStagingLine.getGender() != null && !searchStagingLine.getGender().isEmpty()) {
            final Path<Group> group = root.<Group>get("gender");
            predicates.add(group.in(searchStagingLine.getGender()));
        }

        if (searchStagingLine.getColor() != null && !searchStagingLine.getColor().isEmpty()) {
            final Path<Group> group = root.<Group>get("color");
            predicates.add(group.in(searchStagingLine.getColor()));
        }

        if (searchStagingLine.getSize() != null && !searchStagingLine.getSize().isEmpty()) {
            final Path<Group> group = root.<Group>get("size");
            predicates.add(group.in(searchStagingLine.getSize()));
        }
        if (searchStagingLine.getNoPairs() != null && !searchStagingLine.getNoPairs().isEmpty()) {
            final Path<Group> group = root.<Group>get("noPairs");
            predicates.add(group.in(searchStagingLine.getNoPairs()));
        }
        if (searchStagingLine.getBarcodeId() != null && !searchStagingLine.getBarcodeId().isEmpty()) {
            final Path<Group> group = root.<Group>get("barcodeId");
            predicates.add(group.in(searchStagingLine.getBarcodeId()));
        }
