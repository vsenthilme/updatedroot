...
public class PutAwayLineV2Specification implements Specification<PutAwayLineV2> {
    ...
    public Predicate toPredicate(Root<PutAwayLineV2> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		if (searchPutAwayLine.getInboundOrderTypeId() != null && !searchPutAwayLine.getInboundOrderTypeId().isEmpty()) {
            final Path<Group> group = root.<Group> get("inboundOrderTypeId");
            predicates.add(group.in(searchPutAwayLine.getInboundOrderTypeId()));
        }

        if (searchPutAwayLine.getConfirmedBy() != null && !searchPutAwayLine.getConfirmedBy().isEmpty()) {
            final Path<Group> group = root.<Group> get("confirmedBy");
            predicates.add(group.in(searchPutAwayLine.getConfirmedBy()));
        }
        if (searchPutAwayLine.getMaterialNo() != null && !searchPutAwayLine.getMaterialNo().isEmpty()) {
            final Path<Group> group = root.<Group>get("materialNo");
            predicates.add(group.in(searchPutAwayLine.getMaterialNo()));
        }

        if (searchPutAwayLine.getPriceSegment() != null && !searchPutAwayLine.getPriceSegment().isEmpty()) {
            final Path<Group> group = root.<Group>get("priceSegment");
            predicates.add(group.in(searchPutAwayLine.getPriceSegment()));
        }

        if (searchPutAwayLine.getArticleNo() != null && !searchPutAwayLine.getArticleNo().isEmpty()) {
            final Path<Group> group = root.<Group>get("articleNo");
            predicates.add(group.in(searchPutAwayLine.getArticleNo()));
        }
        if (searchPutAwayLine.getGender() != null && !searchPutAwayLine.getGender().isEmpty()) {
            final Path<Group> group = root.<Group>get("gender");
            predicates.add(group.in(searchPutAwayLine.getGender()));
        }

        if (searchPutAwayLine.getColor() != null && !searchPutAwayLine.getColor().isEmpty()) {
            final Path<Group> group = root.<Group>get("color");
            predicates.add(group.in(searchPutAwayLine.getColor()));
        }

        if (searchPutAwayLine.getSize() != null && !searchPutAwayLine.getSize().isEmpty()) {
            final Path<Group> group = root.<Group>get("size");
            predicates.add(group.in(searchPutAwayLine.getSize()));
        }
        if (searchPutAwayLine.getNoPairs() != null && !searchPutAwayLine.getNoPairs().isEmpty()) {
            final Path<Group> group = root.<Group>get("noPairs");
            predicates.add(group.in(searchPutAwayLine.getNoPairs()));
        }
