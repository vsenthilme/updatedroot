...
public class ProformaInvoiceLineSpecification implements Specification<ProformaInvoiceLine> {
    ...
    public Predicate toPredicate(Root<ProformaInvoiceLine> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (findProformaInvoiceLine.getProformaHeaderId() != null && !findProformaInvoiceLine.getProformaHeaderId().isEmpty()) {
            final Path<DeferredImportSelector.Group> group = root.<DeferredImportSelector.Group> get("proformaHeaderId");
            predicates.add(group.in(findProformaInvoiceLine.getProformaHeaderId()));
        }
