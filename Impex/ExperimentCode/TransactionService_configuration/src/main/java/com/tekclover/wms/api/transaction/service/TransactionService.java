...
public class TransactionService extends BaseService {
    ...
    //                List<InboundOrderLinesV2> sqlInboundLineList = inboundOrderLinesV2Repository.getOrderLinesByOrderTypeId(dbOBOrder.getOrderId(), dbOBOrder.getInboundOrderTypeId());
            ...
            if (inboundList != null && !inboundList.isEmpty()) {
                ...
                for (InboundIntegrationHeader inbound : spList) {
                    ...
                    String companyCodeId = inbound.getCompanyCode();
                    String plantId = inbound.getBranchCode();
                    String languageId = inbound.getLanguageId();
                    String warehouseId = inbound.getWarehouseID();
                    ...
                    try {
                        ...
                        InboundHeaderV2 inboundHeader = inboundOrderProcess(companyCodeId, plantId, languageId, warehouseId, refDocNumber, inbound);
     ...
     * @throws Exception
     */
    private InboundHeaderV2 inboundOrderProcess(String companyCodeId, String plantId, String languageId, String warehouseId,
                                                String refDocNumber, InboundIntegrationHeader inbound) throws Exception {
        String profile = getProfile(companyCodeId, plantId, languageId, warehouseId);
        log.info("Profile: " + companyCodeId + "|" + plantId + "|" + languageId + "|" + warehouseId + "|" + profile);
        if (profile != null) {
            profile = profile.strip().toLowerCase();
            switch (profile) {
                case "walkaroo":
                    return inboundOrderProcessingService.processInboundReceivedV3(refDocNumber, inbound);
                case "impex":
                    return inboundOrderProcessingService.processInboundReceivedV4(refDocNumber, inbound);
                default:
                    return null;
            }
        }
