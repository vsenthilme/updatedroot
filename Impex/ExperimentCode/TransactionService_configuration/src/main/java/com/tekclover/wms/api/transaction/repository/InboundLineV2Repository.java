...
//            + "im.WH_ID in (:warehouseId) AND "
    ...
    //====================================================Impex-V4======================================================================
    boolean existsByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndRefDocNumberAndPreInboundNoAndLineNoAndItemCodeAndManufacturerNameAndStatusIdAndDeletionIndicator(
            String languageId, String companyCode, String plantId, String warehouseId, String refDocNumber,
            String preInboundNo, Long lineNo, String itemCode, String manufacturerName, Long statusId, Long deletionIndicator);

    @Query(value = "SELECT CASE WHEN COUNT(*) = SUM(CASE WHEN status_id = 24 THEN 1 ELSE 0 END) THEN 'TRUE' ELSE 'FALSE' END AS result \n" +
            "FROM tblinboundline where ref_doc_no = :refDocNo and pre_ib_no = :preInboundNo \n" +
            "and c_id = :companyCode and plant_id = :plantId and lang_id = :languageId and wh_id = :warehouseId \n" +
            "and is_deleted = 0 ", nativeQuery = true)
    public String getInboundLinesCountStatusForInboundConfirm(@Param("companyCode") String companyCode,
                                                              @Param("plantId") String plantId,
                                                              @Param("languageId") String languageId,
                                                              @Param("warehouseId") String warehouseId,
                                                              @Param("refDocNo") String refDocNo,
