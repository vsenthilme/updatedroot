...
//        PickupLineV2 pickupLine = pickupLineV2Repository.findTopByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndItemCodeAndManufacturerNameAndAlternateUomAndBagSizeAndDeletionIndicatorAndPickedStorageBinNotOrderByPickupConfirmedOnDesc(companyCodeId, plantId, languageId, warehouseId, itemCode, manufacturerName, alternateUom, bagSize, 0L, directReceiptStorageBin);
     ...
     * @throws Exception
     */
    public List<PickupLineV2> createPickupLineCBMV4(@Valid List<AddPickupLine> newPickupLines, String loginUserID) throws Exception {
        log.info("login UserId : {}", loginUserID);
        Long STATUS_ID = 0L;
        Long HEADER_STATUS_ID = 0L;
        String companyCodeId = null;
...
//                            Double ALLOC_QTY = inventory.getAllocatedQuantity() - dbPickupLine.getAllocatedQty();
                            ...
                            //==========================CBM====================================//
                            Double[] cbm = cbmService.storageBinCapacityExpansion(companyCodeId, plantId, languageId, warehouseId, itemCode,
                                                                                  dbPickupLine.getManufacturerName(), dbPickupLine.getPickedStorageBin(),
                                                                                  dbPickupLine.getPickConfirmQty(), loginUserID);
                            if (cbm != null && cbm.length > 1) {
                                inventory.setCbmPerQuantity(String.valueOf(cbm[0]));
                                inventory.setCbm(String.valueOf(cbm[1]));
                            }
                            //==================================================================//
