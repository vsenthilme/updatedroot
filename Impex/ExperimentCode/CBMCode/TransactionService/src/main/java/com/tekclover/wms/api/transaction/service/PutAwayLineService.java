...
//        PutAwayLineV2 dbPutawayLine = putAwayLineV2Repository.findTopByCompanyCodeAndPlantIdAndWarehouseIdAndLanguageIdAndItemCodeAndManufacturerNameAndStatusIdAndAlternateUomAndBagSizeAndDeletionIndicatorOrderByCreatedOn(companyCodeId, plantId, warehouseId, languageId, itemCode, manufacturerName, 20L, alternateUom, bagSize, 0L);
    ...
    @Transactional
    public List<PutAwayLineV2> putAwayLineConfirmCBMV4(@Valid List<PutAwayLineV2> newPutAwayLines, String loginUserID) {
        List<PutAwayLineV2> createdPutAwayLines = new ArrayList<>();
        log.info("newPutAwayLines to confirm : " + newPutAwayLines);
        ...
        try {
            for (PutAwayLineV2 newPutAwayLine : newPutAwayLines) {
                ...
                itemCode = newPutAwayLine.getItemCode();
                manufacturerName = newPutAwayLine.getManufacturerName();
                putAwayNumber = newPutAwayLine.getPutAwayNumber();
                companyCode = newPutAwayLine.getCompanyCode();
                plantId = newPutAwayLine.getPlantId();
                languageId = newPutAwayLine.getLanguageId();
                warehouseId = newPutAwayLine.getWarehouseId();
                refDocNumber = newPutAwayLine.getRefDocNumber();
                preInboundNo = newPutAwayLine.getPreInboundNo();
                proposedBin = newPutAwayLine.getProposedStorageBin();
                confirmedBin = newPutAwayLine.getConfirmedStorageBin();
                Double putAwayQty = newPutAwayLine.getPutAwayQuantity();
                Double putAwayConfirmQty = newPutAwayLine.getPutawayConfirmedQty();

                log.info("proposedBin, confirmedBin: " + proposedBin + "|" + confirmedBin);

                BeanUtils.copyProperties(newPutAwayLine, dbPutAwayLine, CommonUtils.getNullPropertyNames(newPutAwayLine));

                PutAwayHeaderV2 putAwayHeader = putAwayHeaderService.getPutawayHeaderV2(companyCode, plantId, warehouseId, languageId, putAwayNumber);
                log.info("putawayHeader: " + putAwayHeader);
                ...
                if (putAwayHeader != null) {
                    ...
                    manufacturerName = manufacturerName != null ? manufacturerName : putAwayHeader.getManufacturerName();
                    proposedBin = proposedBin != null ? proposedBin : putAwayHeader.getProposedStorageBin();
                    putAwayQty = putAwayQty != null ? putAwayQty : putAwayHeader.getPutAwayQuantity();
                } else {
                    dbPutAwayLine.setCreatedOn(new Date());
                }

                StorageBinV2 proposedStorageBin = storageBinService.getStorageBinV2(companyCode, plantId, languageId, warehouseId, proposedBin);
                StorageBinV2 confirmedStorageBin = storageBinService.getStorageBinV2(companyCode, plantId, languageId, warehouseId, confirmedBin);
                ...
                if(confirmedStorageBin == null) {
                    throw new BadRequestException("ConfirmedBin Un Available");
                }

                boolean capacityCheck = cbmService.capacityCheck(imBasicData1, confirmedStorageBin);
                if(!capacityCheck) {
                    boolean itemCapacityCheck = cbmService.itemCapacityCheck(imBasicData1);
                    boolean binCapacityCheck = cbmService.binCapacityCheck(confirmedStorageBin);
                    if (itemCapacityCheck && !binCapacityCheck) {
                        throw new BadRequestException("Selected Bin is not under Capacity Check since Item is Capacity Enabled. Kindly Select a Capacity Enabled Bin!");
                    }
                    if (!itemCapacityCheck && binCapacityCheck) {
                        throw new BadRequestException("Selected ItemCode is not under Capacity Check since bin is capacity Enabled. Kindly Select a Capacity Enabled Item!");
                    }
                }
                if(capacityCheck) {
                    log.info("CBM Initiated----> " + capacityCheck);
                    boolean pass = !proposedBin.equalsIgnoreCase(confirmedBin) || (proposedBin.equalsIgnoreCase(confirmedBin) && putAwayConfirmQty < putAwayQty);
                    if (pass) {
                        cbmService.storageBinCapacityExpansion(companyCode, plantId, languageId, warehouseId, proposedStorageBin, imBasicData1, putAwayConfirmQty, loginUserID);
                        cbmService.storageBinCapacityReduction(companyCode, plantId, languageId, warehouseId, confirmedStorageBin, imBasicData1, putAwayConfirmQty, loginUserID);
                    } else {
                        cbmService.updateStorageBinStatus(companyCode, plantId, languageId, warehouseId, confirmedStorageBin, confirmedBin, loginUserID);
                    }
                }
                dbPutAwayLine.setLevelId(String.valueOf(confirmedStorageBin.getFloorId()));

                StagingLineEntityV2 dbStagingLineEntity = stagingLineService.getStagingLineForPutAwayLineV2(companyCode, plantId, languageId, warehouseId, preInboundNo, refDocNumber,
                                                                                                            newPutAwayLine.getLineNo(), itemCode, manufacturerName);
                log.info("StagingLine: " + dbStagingLineEntity);
                if (dbStagingLineEntity != null) {
                    dbPutAwayLine.setManufacturerFullName(dbStagingLineEntity.getManufacturerFullName());
                    dbPutAwayLine.setMiddlewareId(dbStagingLineEntity.getMiddlewareId());
                    dbPutAwayLine.setMiddlewareHeaderId(dbStagingLineEntity.getMiddlewareHeaderId());
                    dbPutAwayLine.setMiddlewareTable(dbStagingLineEntity.getMiddlewareTable());
                ...
                Optional<PutAwayLineV2> existingPutAwayLine = putAwayLineV2Repository.findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndGoodsReceiptNoAndPreInboundNoAndRefDocNumberAndPutAwayNumberAndLineNoAndItemCodeAndProposedStorageBinAndConfirmedStorageBinInAndDeletionIndicator(
                        languageId, companyCode, plantId, warehouseId, newPutAwayLine.getGoodsReceiptNo(), preInboundNo, refDocNumber,
                        putAwayNumber, newPutAwayLine.getLineNo(), itemCode, proposedBin, Arrays.asList(confirmedBin), 0L);
                log.info("Existing putawayline already created : " + existingPutAwayLine);

                if (existingPutAwayLine.isEmpty()) {
                    //Lead Time calculation
                    String leadTime = putAwayLineV2Repository.getleadtime(companyCode, plantId, languageId, warehouseId, putAwayNumber, new Date());
                    dbPutAwayLine.setReferenceField1(leadTime);
                    log.info("LeadTime: " + leadTime);
                    ...
                    if (createdPutAwayLine != null && putAwayConfirmQty > 0L) {

                        if(!capacityCheck) {
                            confirmedStorageBin.setStatusId(1L);
                            storageBinService.updateStorageBinV2(confirmedBin, confirmedStorageBin, companyCode, plantId, languageId, warehouseId, loginUserID);
                        }

                        if (putAwayHeader != null) {
                            log.info("putawayConfirmQty, putawayQty: " + putAwayConfirmQty + "|" + putAwayQty);
                            ...
                            if (putAwayConfirmQty < putAwayQty) {
                                Double dbAssignedPutawayQty = putAwayHeader.getReferenceField2() != null ? Double.valueOf(putAwayHeader.getReferenceField2()) : putAwayHeader.getPutAwayQuantity();
                                Double dbPutawayQty = putAwayLineV2Repository.getPutawayCnfQuantity(companyCode, plantId, languageId, warehouseId,
                                                                                                    refDocNumber, preInboundNo, itemCode, manufacturerName,
                                                                                                    createdPutAwayLine.getLineNo());
                                dbPutawayQty = dbPutawayQty != null ? dbPutawayQty : 0D;
                                ...
                                if (dbPutawayQty <= dbAssignedPutawayQty) {
                                    if (proposedBin.equalsIgnoreCase(confirmedBin)) {
                                        log.info("New PutawayHeader Create Initiated---> ");
                                        PutAwayHeaderV2 newPutAwayHeader = new PutAwayHeaderV2();
                                        BeanUtils.copyProperties(putAwayHeader, newPutAwayHeader, CommonUtils.getNullPropertyNames(putAwayHeader));
                                        ...
                                        Double putawaycnfQty = newPutAwayHeader.getReferenceField3() != null ? Double.valueOf(newPutAwayHeader.getReferenceField3()) : 0D;
                                        putawaycnfQty = putawaycnfQty + createdPutAwayLine.getPutawayConfirmedQty();
                                        newPutAwayHeader.setReferenceField3(String.valueOf(putawaycnfQty));
