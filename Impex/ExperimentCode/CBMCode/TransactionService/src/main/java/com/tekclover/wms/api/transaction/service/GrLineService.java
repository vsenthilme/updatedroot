...
//    }
    ...
    public List<GrLineImpl> findGrLineSQLV2(SearchGrLineV2 searchGrLine) throws ParseException, java.text.ParseException {
        ...
        List<GrLineImpl> results = grLineV2Repository.findGrLine(
                ...
                searchGrLine.getMaterialNo(),
                searchGrLine.getPriceSegment(),
                searchGrLine.getArticleNo(),
                searchGrLine.getGender(),
                searchGrLine.getColor(),
                searchGrLine.getSize(),
                searchGrLine.getNoPairs(),
    ...
    public List<PackBarcode> generatePackBarcodeV2(String companyCode, String languageId, String plantId,
                                                   Double acceptQty, Double damageQty, String warehouseId, String loginUserID) {
        try {
            log.info("c_id,plant,lang,wh: " + companyCode + ", " + plantId + ", " + languageId + ", " + warehouseId);
            AuthToken authTokenForIDMasterService = authTokenService.getIDMasterServiceAuthToken();
            long NUM_RAN_ID = 6;
            List<PackBarcode> packBarcodes = new ArrayList<>();
     ...
     */
    @Transactional
    public List<GrLineV2> createGrLineNonCBMV2(@Valid List<AddGrLineV2> newGrLines, String loginUserID) throws Exception {
        ...
        try {
            ...
            for (AddGrLineV2 newGrLine : newGrLines) {
                ...
                for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
                    ...
                    if (packBarcode.getQuantityType() == null) { //Code for INDUS MEGA FOOD
                        dbGrLine.setQuantityType("A");
                    }
                    if (packBarcode.getBarcode() == null) { //Code for INDUS MEGA FOOD
                        long NUM_RAN_ID = 6;
                        String packBarCode = getNextRangeNumber(NUM_RAN_ID, newGrLine.getCompanyCode(), newGrLine.getPlantId(),
                                                                newGrLine.getLanguageId(), newGrLine.getWarehouseId());
                        dbGrLine.setPackBarcodes(packBarCode);
                    }
                    ...
                    dbGrLine.setBatchSerialNumber(newGrLine.getBatchSerialNumber());
                    ...
                    if (oldGrLine == null || oldGrLine.isEmpty()) {
                        ...
                        if (createdGRLine != null && !createGrLineError) {
                            // Record Insertion in PUTAWAYHEADER table - Indus ib_typ 6L

                            boolean passOrdTypId = createdGRLine.getInboundOrderTypeId() != null &&
                                    (createdGRLine.getInboundOrderTypeId().equals(IB_QH_ORD_TYP_ID) ||
                                            createdGRLine.getInboundOrderTypeId().equals(IB_FG_ORD_TYP_ID) ||
                                            createdGRLine.getInboundOrderTypeId().equals(IB_SFG_ORD_TYP_ID));

                            if (createdGRLine.getCompanyCode() != null && createdGRLine.getCompanyCode().equalsIgnoreCase(COMPANY_CODE) && passOrdTypId) {
                                createInboundQualityHeader(createdGRLine, loginUserID);
                                createInventoryNonCBMV2(createdGRLine);
                            }
                            // Record Insertion in PUTAWAYHEADER table - Indus ib_typ other than 6L and all other company other than Indus
                            if (createdGRLine.getCompanyCode() != null) {
                                if ((createdGRLine.getCompanyCode().equalsIgnoreCase(COMPANY_CODE) && !passOrdTypId) ||
                                        !createdGRLine.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
...
//            }
    ...
    @Transactional
    public List<GrLineV2> createGrLineV3(@Valid List<AddGrLineV2> newGrLines, String loginUserID) throws Exception {
        List<GrLineV2> createdGRLines = new ArrayList<>();
        String companyCode = null;
        String plantId = null;
        String languageId = null;
...
//                        }
                    ...
                    if (createdGRLine != null && !createGrLineError) {
                        // Record Insertion in PUTAWAYHEADER table
                        createPutAwayHeaderV3(createdGRLine, newGrLine.getPutAwayNumber(), loginUserID);
                    }
                }
//                }
                log.info("Records were inserted successfully...");
     ...
     * @param fetch
     */
    public void createPutAwayHeaderFromInboundQualityLine(FetchImpl fetch) {
        List<GrLineV2> grLines = null;
        if (fetch.getBatchSerialNumber() == null) {
            grLines = grLineV2Repository.findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndRefDocNumberAndPreInboundNoAndItemCodeAndLineNoAndDeletionIndicator(
                    fetch.getLanguageId(), fetch.getCompanyCodeId(), fetch.getPlantId(), fetch.getWarehouseId(),
                    fetch.getRefDocNumber(), fetch.getPreInboundNo(), fetch.getItemCode(), fetch.getLineNo(), 0L);
        }
        if (fetch.getBatchSerialNumber() != null) {
            grLines = grLineV2Repository.findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndRefDocNumberAndPreInboundNoAndItemCodeAndLineNoAndBatchSerialNumberAndDeletionIndicator(
                    fetch.getLanguageId(), fetch.getCompanyCodeId(), fetch.getPlantId(), fetch.getWarehouseId(),
                    fetch.getRefDocNumber(), fetch.getPreInboundNo(), fetch.getItemCode(), fetch.getLineNo(), fetch.getBatchSerialNumber(), 0L);
        }
        if (grLines != null && !grLines.isEmpty()) {
            for (GrLineV2 grLine : grLines) {
                grLine.setStorageSectionId(fetch.getStorageSectionId());
                if (fetch.getStatusId().equals(95L)) {
                    grLine.setInboundOrderTypeId(2L);
                }
                createIMFPutAwayHeaderNonCBMV2(grLine, fetch.getLoginUserID());
            }
        }
    }
     ...
     * @param loginUserID
     */
    private void createIMFPutAwayHeaderNonCBMV2(GrLineV2 createdGRLine, String loginUserID) {
        ...
        try {
            ...
            storageBinPutAway.setWarehouseId(warehouseId);
            storageBinPutAway.setStorageSectionIds(storageSectionIds);
//            if(createdGRLine.getStorageSectionId() != null) {
//                if (createdGRLine.getStorageSectionId().equalsIgnoreCase(ST_SEC_ID_PFG)) {
//                    storageBinPutAway.setStorageSectionId(ST_SEC_ID_FG);
//                } else if (createdGRLine.getStorageSectionId().equalsIgnoreCase(ST_SEC_ID_PSFG)) {
//                    storageBinPutAway.setStorageSectionId(ST_SEC_ID_SFG);
//                } else if(createdGRLine.getStorageSectionId().equalsIgnoreCase(ST_SEC_ID_CS)) {
//                    storageBinPutAway.setStorageSectionId(ST_SEC_ID_CS);
//                } else if(createdGRLine.getStorageSectionId().equalsIgnoreCase(ST_SEC_ID_DS)) {
//                    storageBinPutAway.setStorageSectionId(ST_SEC_ID_DS);
//                } else {
//                    storageBinPutAway.setStorageSectionId(createdGRLine.getStorageSectionId());
//                }
//            }
//            if(createdGRLine.getStorageSectionId() == null) {
//                storageBinPutAway.setStorageSectionId(ST_SEC_ID_DS);                    //Hard code default dry store
//            }
            ...
            if (createdGRLine != null) {
                ...
                if (createdGRLine.getInboundOrderTypeId() == 1 || createdGRLine.getInboundOrderTypeId() == 3 ||
                        ...
                        createdGRLine.getInboundOrderTypeId() == 6 || createdGRLine.getInboundOrderTypeId() == 7 || createdGRLine.getInboundOrderTypeId() == 8) {
                    binClassId = 1L;
                }
                ...
                List<String> inventoryStorageBinList = inventoryService.getInventoryBinForPutAwayCreate(companyCode, plantId, languageId, warehouseId,
                                                                                                        itemCode, binClassId);
                ...
                if (createdGRLine.getInboundOrderTypeId() == 2) {
                    ...
                    StorageBinV2 proposedBinClass7Bin = storageBinService.getStorageBinBinClassId7(storageBinPutAway);
                    ...
                    if (proposedBinClass7Bin == null) {
                        binClassId = 2L;
                        log.info("BinClassId : " + binClassId);
                        StorageBinV2 stBin = storageBinService.getStorageBinByBinClassIdV2(warehouseId, binClassId,
...
//                    storageBinPutAway.setStatusId(0L);
                    ...
                    if (createdGRLine.getQuantityType().equalsIgnoreCase("A")) {
                        ...
                        if (putAwayHeader.getProposedStorageBin() == null) {
                            PutAwayHeaderV2 existingBinItemCheck = putAwayHeaderService.getPutawayHeaderExistingBinItemCheckV2(companyCode, plantId, languageId, warehouseId, itemCode);
...
//                            log.info("existingProposedPutawayStorageBin: " + existingProposedPutawayStorageBin);
                            log.info("BinClassId: " + binClassId);
                            storageBinPutAway.setStorageSectionId(createdGRLine.getStorageSectionId() != null ? createdGRLine.getStorageSectionId() : "3");
//                            storageBinPutAway.setStorageBin(existingProposedPutawayStorageBin);
                ...
                putAwayHeader.setStorageSectionId(createdGRLine.getStorageSectionId());
                if (createdGRLine.getParentProductionOrderNo() != null) {
                    putAwayHeader.setParentProductionOrderNo(createdGRLine.getParentProductionOrderNo());
                } else {
                    GrHeaderV2 grHeader = grHeaderService.getGrHeaderForReversalV2(companyCode, plantId, languageId, warehouseId, createdGRLine.getRefDocNumber(), createdGRLine.getPreInboundNo());
                    putAwayHeader.setParentProductionOrderNo(grHeader.getParentProductionOrderNo());
                }
...
//        }
     ...
     * @throws Exception
     */
    private void createPutAwayHeaderV3(GrLineV2 createdGRLine, String putAwayNumber, String loginUserID) throws Exception {
        try {
            String itemCode = createdGRLine.getItemCode();
            String companyCode = createdGRLine.getCompanyCode();
            String plantId = createdGRLine.getPlantId();
            ...
            String nextPANumber = getNextRangeNumber(NUM_RAN_CODE_PA_NO, companyCode,
                                                     plantId, languageId, warehouseId, authTokenForIDMasterService.getAccess_token());
...
//                PutAwayHeaderV2 dbPutAwayHeader = putAwayHeaderService.getPutAwayHeaderV3(
//                        companyCode, plantId, languageId, warehouseId, createdGRLine.getPackBarcodes(), dates[0], dates[1]);
//                log.info("dbPutAwayHeader : " + dbPutAwayHeader);
//                if (dbPutAwayHeader != null && dbPutAwayHeader.getAssignedUserId() != null && !dbPutAwayHeader.getAssignedUserId().isBlank()) {
//                    putAwayHeader.setAssignedUserId(dbPutAwayHeader.getAssignedUserId());
//                } else {
//                    outerloop1:
//                    for (String user : hhtUserList) {
//                        log.info("1.pickerIp: " + companyCode + "," + plantId + "," + languageId + "," + warehouseId + "," + user + "," + dates[0] + "," + dates[1]);
//                        assignPicker = putAwayHeaderV2Repository.getAssignPickerV3(
//                                companyCode, plantId, languageId, warehouseId, user, dates[0], dates[1]);
//                        log.info("1.assignPicker : " + assignPicker);
//                        if (assignPicker == null) {
//                            putAwayHeader.setAssignedUserId(user);
//                            break outerloop1;
//                        }
//                    }
//                    if (putAwayHeader.getAssignedUserId() == null) {
//                        log.info("2.pickerIp-->20: " + companyCode + "," + plantId + "," + languageId + "," + warehouseId + "," + hhtUserList + "," + dates[0] + "," + dates[1]);
//                        assignPicker = putAwayHeaderV2Repository.getAssignPickerV3(
//                                companyCode, plantId, languageId, warehouseId, hhtUserList, 20L, dates[0], dates[1]);
//                        log.info("2.assignPicker : " + assignPicker);
//                        if (assignPicker != null) {
//                            putAwayHeader.setAssignedUserId(assignPicker);
//                        } else {
//                            outerloop:
//                            for (String userId : hhtUserList) {
//                                log.info("3.pickerIp-->19: " + companyCode + "," + plantId + "," + languageId + "," + warehouseId + "," + userId + "," + dates[0] + "," + dates[1]);
//                                assignPicker = putAwayHeaderV2Repository.getAssignPickerV3(
//                                        companyCode, plantId, languageId, warehouseId, userId, 19L, dates[0], dates[1]);
//                                log.info("3.assignPicker : " + assignPicker);
//                                if (assignPicker == null) {
//                                    putAwayHeader.setAssignedUserId(userId);
//                                    break outerloop;
//                                }
//                            }
//                            if (putAwayHeader.getAssignedUserId() == null) {
//                                log.info("4.pickerIp-->20: " + companyCode + "," + plantId + "," + languageId + "," + warehouseId + "," + hhtUserList + "," + dates[0] + "," + dates[1]);
//                                assignPicker = putAwayHeaderV2Repository.getInboundAssignUserV3(
//                                        companyCode, plantId, languageId, warehouseId, hhtUserList, 20L, dates[0], dates[1]);
//                                log.info("4.assignPicker : " + assignPicker);
//                                if (assignPicker != null) {
//                                    putAwayHeader.setAssignedUserId(assignPicker);
//                                }
//                            }
//                            if (putAwayHeader.getAssignedUserId() == null) {
//                                log.info("5.pickerIp--->19: " + companyCode + "," + plantId + "," + languageId + "," + warehouseId + "," + hhtUserList + "," + dates[0] + "," + dates[1]);
//                                assignPicker = putAwayHeaderV2Repository.getInboundAssignUserV3(
//                                        companyCode, plantId, languageId, warehouseId, hhtUserList, 19L, dates[0], dates[1]);
//                                log.info("5.assignPicker : " + assignPicker);
//                                if (assignPicker != null) {
//                                    putAwayHeader.setAssignedUserId(assignPicker);
//                                } else {
//                                    log.info("6.assignPicker : " + hhtUserList.get(0));
//                                    putAwayHeader.setAssignedUserId(hhtUserList.get(0));
//                                }
//                            }
//                        }
//                    }
//                }
//            }
//            log.info("AssignUser : " + putAwayHeader.getAssignedUserId());
            statusDescription = stagingLineV2Repository.getStatusDescription(19L, createdGRLine.getLanguageId());
            putAwayHeader.setStatusDescription(statusDescription);
            putAwayHeader.setDeletionIndicator(0L);
            putAwayHeader.setCreatedBy(loginUserID);
     ...
     */
    public void assignBinner(String companyCode, String plantId, String languageId, String warehouseId, String parentProductionOrderNo) throws Exception {
        try {
            List<HHTUser> hhtUserIdList = preOutboundHeaderV2Repository.getHHTUserListV3(companyCode, plantId, languageId, warehouseId, 5L);
            log.info("hhtUserList: " + hhtUserIdList.size());
            log.info("assign Binner: " + parentProductionOrderNo);
            List<String> hhtUserList = new ArrayList<>();
            List<String> absentHhtUserList = new ArrayList<>();
            if (hhtUserIdList != null && !hhtUserIdList.isEmpty()) {
                for (HHTUser dbHhtUser : hhtUserIdList) {
            ...
            com.tekclover.wms.api.transaction.model.enterprise.Warehouse entWarehouse =
                    enterpriseSetupService.getWarehouse(warehouseId, companyCode, plantId, languageId, getEnterpriseAuthToken());
            ...
            int i = 0;
            int j = 0;
            List<PutAwayHeaderV2> putAwayHeaderV2List = putAwayHeaderService.getPutAwayHeaderV3(companyCode, plantId, languageId, warehouseId, parentProductionOrderNo);
            log.info("putAwayHeaderV2List : " + putAwayHeaderV2List.size());
            if (putAwayHeaderV2List != null && !putAwayHeaderV2List.isEmpty()) {
                for (PutAwayHeaderV2 putAwayHeader : putAwayHeaderV2List) {
                    if (hhtUserList != null && !hhtUserList.isEmpty()) {
                        log.info("hhtUserList count: " + hhtUserList.size());
                        putAwayHeaderV2Repository.updateAssignUser(putAwayHeader.getPutAwayNumber(), hhtUserList.get(i));
                        log.info("AssignUser : " + putAwayHeader.getAssignedUserId());
                        j = j + 1;
                        if (j == inboundQaCheck) {
                            j = 0;
        ...
        try {
            ...
            for (AddGrLineV2 newGrLine : newGrLines) {
                ...
                for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
                    ...
                    //===========================================Handle MfrName==============================================================//
                    if (newGrLine.getCompanyCode() != null && newGrLine.getManufacturerName() == null) {
                        newGrLine.setManufacturerName(getMfrName(newGrLine.getCompanyCode()));
                        dbGrLine.setManufacturerName(getMfrName(newGrLine.getCompanyCode()));
                        if (dbGrLine.getManufacturerPartNo() == null) {
                            dbGrLine.setManufacturerPartNo(getMfrName(newGrLine.getCompanyCode()));
                        }
                        if (dbGrLine.getManufacturerCode() == null) {
                            dbGrLine.setManufacturerCode(getMfrName(newGrLine.getCompanyCode()));
                        }
                    }
                    //=========================================================================================================//
                    ...
                    dbGrLine.setAcceptTotalCbm(packBarcode.getAcceptTotalCbm());
            ...
            if (createdGRLine.getInterimStorageBin() != null) {
                ...
                if (storageBin != null) {
                    ...
                    if (!capacityCheck && storageBinCapacityCheck) {
                        ...
                        throw new BadRequestException("Storage Bin Capacity Check is enabled whereas item capacity check is disabled ");
                    }
                    if (capacityCheck && !storageBinCapacityCheck) {
                        // Exception Log
                        createGrLineLog8(itemCodeCapacityCheck, "Item Capacity Check is enabled whereas Storage Bin capacity check is disabled.");
                        throw new BadRequestException("item Capacity Check is enabled whereas Storage Bin capacity check is disabled ");
                    }
...
//                        }
            ...
            if (createdGRLine.getReferenceDocumentType() != null) {
                ...
                putAwayHeader.setReferenceDocumentType(getInboundOrderTypeDesc(
                        createdGRLine.getCompanyCode(),
                        createdGRLine.getPlantId(),
                        createdGRLine.getLanguageId(),
                        createdGRLine.getWarehouseId(),
                        createdGRLine.getInboundOrderTypeId()));
            }
            putAwayHeader.setProposedHandlingEquipment(createdGRLine.getPutAwayHandlingEquipment());
            putAwayHeader.setCbmQuantity(createdGRLine.getCbmQuantity());
            ...
            putAwayHeader.setBusinessPartnerCode(createdGRLine.getBusinessPartnerCode());
            putAwayHeader.setTransferOrderDate(dbPreInboundHeader.getTransferOrderDate());
            putAwayHeader.setSourceBranchCode(dbPreInboundHeader.getSourceBranchCode());
            putAwayHeader.setSourceCompanyCode(dbPreInboundHeader.getSourceCompanyCode());
            putAwayHeader.setIsCompleted(dbPreInboundHeader.getIsCompleted());
            ...
            createStockMovement(createdGRLine, createdinventory);
            /*----------------INVENTORYMOVEMENT table Update---------------------------------------------*/
//                createInventoryMovementV2(createdGRLine, createdinventory.getStorageBin());
        }
//            if (cbm == 0D) {
...
//        }
     ...
     * @param createdInventory
     */
    public void createStockMovement(GrLineV2 createdGRLine, InventoryV2 createdInventory) {

        StockMovement stockMovement = new StockMovement();

        stockMovement.setItemCode(createdGRLine.getItemCode());
        stockMovement.setRefDocNumber(createdGRLine.getRefDocNumber());
        stockMovement.setDocumentDate(new Date());

        List<InventoryV2> inventoryV2List = inventoryService.getCountOfCbmValue(createdGRLine.getItemCode(), createdGRLine.getBusinessPartnerCode());

        Double opningStock = 0.0;
        Double outboundQty = stockMovement.getOutboundQty();
        Double closingStock = (stockMovement.getOpeningStock() + stockMovement.getInboundQty()) - outboundQty;

        for (InventoryV2 inventoryV2 : inventoryV2List) {
            opningStock += inventoryV2.getThreePLCbm();
        }
        stockMovement.setOpeningStock(opningStock);
        stockMovement.setClosingStock(closingStock);
        stockMovement.setInboundQty(createdGRLine.getThreePLCbm());
        stockMovement.setDescription(createdInventory.getReferenceField8());
        stockMovement.setStockUom(createdGRLine.getThreePLUom());
        stockMovement.setStatusId(17L);
    ...
    public InventoryV2 createInventoryNonCBMV2(GrLineV2 createdGRLine) {
        try {
            ...
            if (dbInventory != null) {
                ...
                Double INV_QTY = inventory.getInventoryQuantity() != null ? inventory.getInventoryQuantity() : 0D;
                Double ALLOC_QTY = inventory.getAllocatedQuantity() != null ? inventory.getAllocatedQuantity() : 0D;
                ...
                log.info("Inventory - INV_QTY,ALLOC_QTY,TOT_QTY: " + INV_QTY + ", " + ALLOC_QTY + ", " + TOT_QTY);
...
//                inventory.setInventoryId(System.currentTimeMillis());
            ...
            if (dbInventory == null) {
                ...
                // ST_BIN ---Pass WH_ID/BIN_CL_ID=3 in STORAGEBIN table and fetch ST_BIN value and update
                AuthToken authTokenForMastersService = authTokenService.getMastersServiceAuthToken();
                StorageBinV2 storageBin = storageBinService.getStorageBinByBinClassIdV2(
                        createdGRLine.getWarehouseId(), 3L,
                        createdGRLine.getCompanyCode(),
                        createdGRLine.getPlantId(),
                        createdGRLine.getLanguageId());
                log.info("storageBin: " + storageBin);
                ...
                if (dbInventory != null) {
                    ...
                    inventory.setReferenceField4(round(TOT_QTY));
                    log.info("After - Inventory - INV_QTY,ALLOC_QTY,TOT_QTY: " + INV_QTY + ", " + ALLOC_QTY + ", " + TOT_QTY);
                }
                if (dbInventory == null) {
    ...
    private InventoryV2 createInventoryV2(GrLineV2 createdGRLine) {
        ...
        try {
            ...
            if (dbInventory != null) {
                ...
                //3PL
                List<PreInboundLineEntity> preInboundHeader = preInboundLineRepository.findByRefDocNumberAndDeletionIndicator(
                        createdGRLine.getRefDocNumber(), 0L);

                inventory.setThreePLPartnerId(preInboundHeader.get(0).getBusinessPartnerCode());
                ...
                String threePLUom = createdGRLine.getThreePLUom();
                Double threePLCbm = createdGRLine.getThreePLCbm();

                if (threePLCbm != 0.0) {
                    Double threePLCbmPerQty = threePLCbm / dbInventory.getInventoryQuantity();
                    inventory.setThreePLCbmPerQty(threePLCbmPerQty);
                } else {
                    inventory.setThreePLCbmPerQty(0.0);
                }
...
//                inventory.setInventoryId(System.currentTimeMillis());
            ...
            if (dbInventory == null) {
                ...
                if (threePLCbm != 0.0 && dbInventory.getInventoryQuantity() != 0.0) {
                    Double threePLCbmPerQty = threePLCbm / dbInventory.getInventoryQuantity();
                    inventory.setThreePLCbmPerQty(threePLCbmPerQty);
                } else {
                    inventory.setThreePLCbmPerQty(0.0);
...
//            List<IImbasicData1> imbasicdata1 = imbasicdata1Repository.findByItemCode(inventory.getItemCode());
                ...
                // INV_QTY
                if (dbInventory != null) {
                    inventory.setInventoryQuantity(round(dbInventory.getInventoryQuantity() + createdGRLine.getGoodReceiptQty()));
                    log.info("Inventory Qty = inv_qty + gr_qty: " + dbInventory.getInventoryQuantity() + ", " + createdGRLine.getGoodReceiptQty());
                    inventory.setReferenceField4(round(inventory.getInventoryQuantity()));
                    log.info("Inventory Total Qty: " + inventory.getInventoryQuantity());   //Allocated Qty is always 0 for BinClassId 3
                }
                if (dbInventory == null) {
                    inventory.setInventoryQuantity(round(createdGRLine.getGoodReceiptQty()));
...
//                inventory.setInventoryId(System.currentTimeMillis());
    ...
    public void deleteGrLineV2(String companyCodeId, String plantId, String languageId,
        ...
        if (grLine != null) {
            ...
            inventoryService.deleteInventory(companyCodeId, plantId, languageId, warehouseId, packBarcodes, refDocNumber, itemCode, loginUserID);

            stockMovementService.deleteStockMovement(companyCodeId, plantId, languageId, warehouseId, refDocNumber, itemCode, loginUserID);
            grLine.setDeletionIndicator(1L);
            grLine.setUpdatedBy(loginUserID);
     ...
     * @return
     */
    public String generateBatchSerial(String companyCode, String plantId, String languageId, String warehouseId, String storageMethod) {

        if (storageMethod.equalsIgnoreCase("Batch")) {
            long NUM_RAN_CODE = 18;     //BATCH_SERIAL_NO Code in Number Range Table
            String BATCH_SERIAL_NO = getNextRangeNumber(NUM_RAN_CODE, companyCode, plantId, languageId, warehouseId);
            log.info("Internal Batch Serial Number :" + BATCH_SERIAL_NO);
            return BATCH_SERIAL_NO;
        }
        if (storageMethod.equalsIgnoreCase("Serial")) {
            long NUM_RAN_CODE = 19;     //BATCH_SERIAL_NO Code in Number Range Table
            String BATCH_SERIAL_NO = getNextRangeNumber(NUM_RAN_CODE, companyCode, plantId, languageId, warehouseId);
            log.info("Internal Batch Serial Number :" + BATCH_SERIAL_NO);
            return BATCH_SERIAL_NO;
        }
        throw new BadRequestException("Storage Method should be either Batch or serial: " + storageMethod);
    }
     ...
     * @param loginUserID
     */
    private void createInboundQualityHeader(GrLineV2 createdGRLine, String loginUserID) {
        InboundQualityHeader inboundQualityHeader = new InboundQualityHeader();
        BeanUtils.copyProperties(createdGRLine, inboundQualityHeader, CommonUtils.getNullPropertyNames(createdGRLine));
        inboundQualityHeader.setCompanyCodeId(createdGRLine.getCompanyCode());
        inboundQualityHeader.setBatchSerialNumber(createdGRLine.getBatchSerialNumber());
        inboundQualityHeader.setReceivedQuantity(createdGRLine.getGoodReceiptQty());
        inboundQualityHeader.setReferenceField6(createdGRLine.getManufacturerName());
        inboundQualityHeader.setReferenceField9(String.valueOf(createdGRLine.getLineNo()));

        mfgService.postInboundQualityHeader(inboundQualityHeader, loginUserID);
    }
    ...
    //==============================================walkaroo-v3==============================================================
    public List<GrLineV2> getGrLineV3(String companyCode, String plantId, String languageId,
                                      String warehouseId, String parentProductionOrderNo) {

        return grLineV2Repository.findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndParentProductionOrderNoAndDeletionIndicatorOrderByCreatedOnDesc(
                languageId,
                companyCode,
                plantId,
                warehouseId,
            ...
            for (AddGrLineV2 newGrLine : newGrLines) {
                ...
                for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
                    ...
                    if (oldGrLine == null || oldGrLine.isEmpty()) {
                        ...
                        if (createdGRLine != null && !createGrLineError) {
                            createPutAwayHeaderNonCBMV4(companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName, preInboundNo, refDocNumber, createdGRLine, loginUserID);
                        }
                    }
                }
                log.info("Records were inserted successfully...");
     ...
     * @throws Exception
     */
    private void createPutAwayHeaderNonCBMV4(String companyCode, String plantId, String languageId,
                                             String warehouseId, String itemCode, String manufacturerName,
                                             String preInboundNo, String refDocNumber,
                                             GrLineV2 createdGRLine, String loginUserID) throws Exception {
        ...
        try {
            ...
            if (createdGRLine != null) {
                ...
                String nextPANumber = getNextRangeNumber(NUMBER_RANGE_CODE, companyCode, plantId, languageId, warehouseId, idMasterToken);
...
//                List<String> inventoryStorageBinList = inventoryService.getPutAwayHeaderCreateInventoryV4(companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName, createdGRLine.getBarcodeId(), createdGRLine.getBatchSerialNumber(), packBarCode, alternateUom, bagSize, binClassId);
    ...
    public InventoryV2 createInventoryNonCBMV4(String companyCode, String plantId, String languageId,
        ...
        try {
            InventoryV2 createdinventory = null;
//            InventoryV2 dbInventory = getInventoryBinClassId3V4(companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName, createdGRLine.getBatchSerialNumber(), createdGRLine.getBarcodeId(), createdGRLine.getAlternateUom(), createdGRLine.getBagSize());
...
//                inventory.setInventoryId(System.currentTimeMillis());
     ...
     * @return
     */
    public InventoryV2 getInventoryBinClassId3(String companyCode, String plantId, String languageId, String warehouseId,
                                               String itemCode, String manufacturerName, String batchSerialNumber, String barcodeId) {
        InventoryV2 dbInventory = null;
        if (barcodeId == null && batchSerialNumber != null) {
            dbInventory = inventoryV2Repository.findTopByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndItemCodeAndManufacturerNameAndPackBarcodesAndBinClassIdAndBatchSerialNumberAndDeletionIndicatorOrderByInventoryIdDesc(
                    companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName, batchSerialNumber, 3L, batchSerialNumber, 0L);
        } else if (barcodeId != null && batchSerialNumber == null) {
            dbInventory = inventoryV2Repository.findTopByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndItemCodeAndBarcodeIdAndManufacturerNameAndPackBarcodesAndBinClassIdAndDeletionIndicatorOrderByInventoryIdDesc(
                    companyCode, plantId, languageId, warehouseId, itemCode, barcodeId, manufacturerName, "99999", 3L, 0L);
        } else if (barcodeId != null && batchSerialNumber != null) {
            dbInventory = inventoryV2Repository.findTopByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndItemCodeAndBarcodeIdAndManufacturerNameAndPackBarcodesAndBinClassIdAndBatchSerialNumberAndDeletionIndicatorOrderByInventoryIdDesc(
                    companyCode, plantId, languageId, warehouseId, itemCode, barcodeId, manufacturerName, batchSerialNumber, 3L, batchSerialNumber, 0L);
        } else {
            dbInventory = inventoryV2Repository.findTopByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndItemCodeAndManufacturerNameAndPackBarcodesAndBinClassIdAndDeletionIndicatorOrderByInventoryIdDesc(
                    companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName, "99999", 3L, 0L);
        }
        return dbInventory;
    }
     ...
     * @return
     */
    public InventoryV2 getInventoryBinClassId3V4(String companyCode, String plantId, String languageId, String warehouseId, String itemCode, String manufacturerName,
                                                 String batchSerialNumber, String barcodeId, String alternateUom, Double bagSize) {
        String packBarCode = batchSerialNumber != null ? batchSerialNumber : "99999";
        InventoryV2 dbInventoryV2 = new InventoryV2();
        IInventoryImpl inventory = inventoryV2Repository.getInboundInventoryV4(companyCode, plantId, languageId, warehouseId, barcodeId, batchSerialNumber,
                                                           itemCode, manufacturerName, packBarCode, null, 3L, alternateUom);
        if(inventory != null) {
     ...
     * @return
     */
    public InventoryV2 getInventoryBinClassId1V4(String companyCode, String plantId, String languageId, String warehouseId, String itemCode, String manufacturerName,
                                                 String batchSerialNumber, String barcodeId, String alternateUom, Double bagSize, String storageBin, String packBarCode) {
        InventoryV2 dbInventoryV2 = new InventoryV2();
        IInventoryImpl inventory = inventoryV2Repository.getInboundInventoryV4(companyCode, plantId, languageId, warehouseId, barcodeId, batchSerialNumber,
                                                           itemCode, manufacturerName, packBarCode, storageBin, 1L, alternateUom);
        if(inventory != null) {
            BeanUtils.copyProperties(inventory, dbInventoryV2, CommonUtils.getNullPropertyNames(inventory));
            return dbInventoryV2;
        }
     ...
     * @return
     */
    public InventoryV2 getInventoryBinClassId1V4(String companyCode, String plantId, String languageId, String warehouseId,
    ...
    //===============================================================CBM=========================================================================//
    @Transactional
    public List<GrLineV2> createGrLineCBMV4(@Valid List<AddGrLineV2> newGrLines, String loginUserID) throws Exception {
        ...
        try {
            ...
            for (AddGrLineV2 newGrLine : newGrLines) {
                ...
                for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
                    ...
                    Double recDamageQty = 0D;
                    Double variance = 0D;
                    Double invoiceQty = dbGrLine.getOrderQty() != null ? dbGrLine.getOrderQty() : 0D;
                    Double acceptQty = dbGrLine.getAcceptedQty() != null ? dbGrLine.getAcceptedQty() : 0D;
                    Double damageQty = dbGrLine.getDamageQty() != null ? dbGrLine.getDamageQty() : 0D;
                    ...
                    double grQty = dbGrLine.getGoodReceiptQty() != null ? dbGrLine.getGoodReceiptQty() : 0;
                    ImBasicData1V2 imBasicData1 = cbmService.getImBasicData1(companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName);
                    if(imBasicData1 != null) {
                        capacityCheck = cbmService.itemCapacityCheck(imBasicData1);
                    }
                    if(capacityCheck && (dbGrLine.getCbm() == null || dbGrLine.getCbmQuantity() == null)) {
                        double cbmPerQty = cbmService.itemCBM(imBasicData1);
                        double totalCBM = cbmPerQty * grQty;
                        dbGrLine.setCbm(totalCBM);
                        dbGrLine.setCbmQuantity(cbmPerQty);
                    }
                    dbGrLine.setConfirmedQty(grQty);
                    dbGrLine.setDeletionIndicator(0L);
                    dbGrLine.setCreatedBy(loginUserID);
                    dbGrLine.setUpdatedBy(loginUserID);
                    dbGrLine.setConfirmedBy(loginUserID);
...
//                    if (oldGrLine == null || oldGrLine.isEmpty()) {
                        ...
                        if (createdGRLine != null && !createGrLineError) {
                            if(capacityCheck) {
                                ...
                                createPutAwayHeaderCBMV4(companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName, preInboundNo, refDocNumber, createdGRLine, loginUserID);
                            } else {
                                createPutAwayHeaderNonCBMV4(companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName, preInboundNo, refDocNumber, createdGRLine, loginUserID);
                            }
                        }
//                    }
     ...
     * @throws Exception
     */
    private void createPutAwayHeaderCBMV4(String companyCode, String plantId, String languageId, String warehouseId,
                                          String itemCode, String manufacturerName, String preInboundNo, String refDocNumber,
                                          GrLineV2 createdGRLine, String loginUserID) throws Exception {

        try {
            String proposedStorageBin = createdGRLine.getInterimStorageBin();
            Double orderCBM = createdGRLine.getCbm() != null ? createdGRLine.getCbm() : 0D;
            Double cbm = orderCBM;
            Double cbmPerQuantity = createdGRLine.getCbmQuantity() != null ? createdGRLine.getCbmQuantity() : 0D;
            Double grQty = createdGRLine.getGoodReceiptQty() != null ? createdGRLine.getGoodReceiptQty() : 0D;
            Double putAwayQty = 0D;
            boolean binCapacityCheck = false;
            ...
            while (cbm > 0) {
                if (createdGRLine != null) {
                    ...
                    putAwayHeader.setCompanyCodeId(companyCode);
                    putAwayHeader.setPutAwayUom(createdGRLine.getOrderUom());
                    putAwayHeader.setPutAwayQuantity(grQty);
                    ...
                    StorageBinV2 inventoryStorageBin = storageBinService.getInventoryBinOrderByRemainingVolume(companyCode, plantId, languageId, warehouseId, itemCode,
                                                                                                                   manufacturerName, binClassId, cbmPerQuantity);
                    log.info("Inventory StorageBin: " + inventoryStorageBin);
                    ...
                    if (createdGRLine.getInterimStorageBin() != null) {                         //Direct Stock Receipt - Fixed Bin - Inbound OrderTypeId - 5
                        ...
                        putAwayHeader.setPutAwayQuantity(createdGRLine.getGoodReceiptQty());
                        if (storageBin != null) {
                            binCapacityCheck = cbmService.binCapacityCheck(storageBin);
                            if (!binCapacityCheck) {
                                throw new BadRequestException("Storage Bin is not capacity Enabled since item is capacity Enabled: " + proposedStorageBin);
                            }
                            cbmService.storageBinCapacityReduction(companyCode, plantId, languageId, warehouseId, storageBin, cbmPerQuantity, cbm, loginUserID);
                            putAwayHeader.setPutAwayQuantity(grQty);
                            putAwayHeader.setProposedStorageBin(proposedStorageBin);
                            putAwayHeader.setLevelId(String.valueOf(storageBin.getFloorId()));
                        } else {
                            throw new BadRequestException("Invalid Bin : " + proposedStorageBin);
                        }
                    }
                    //BinClassId - 7 - Return Order(Sale Return)
                    if (createdGRLine.getInboundOrderTypeId() == 2) {
                    ...
                    if (createdGRLine.getInterimStorageBin() == null && putAwayHeader.getProposedStorageBin() == null) {
                        log.info("BinClassId : " + binClassId);
                        if (inventoryStorageBin != null) {
                            if (createdGRLine.getQuantityType().equalsIgnoreCase("A")) {
                                Double[] qty = cbmService.storageBinCapacityReduction(companyCode, plantId, languageId, warehouseId, inventoryStorageBin, cbm, orderCBM, cbmPerQuantity, loginUserID);
                                cbm = qty[1];
                                putAwayQty = qty[0];
                                putAwayHeader.setPutAwayQuantity(putAwayQty);
                                putAwayHeader.setCbm(String.valueOf(cbm));
                                putAwayHeader.setProposedStorageBin(inventoryStorageBin.getStorageBin());
                                putAwayHeader.setLevelId(String.valueOf(inventoryStorageBin.getFloorId()));
                                log.info("Existing CBM ProposedBin, GrQty: " + inventoryStorageBin.getStorageBin() + "|" + putAwayQty + "|" + cbm);
                            }
                            if (createdGRLine.getQuantityType().equalsIgnoreCase("D")) {
                                storageBinPutAway.setBinClassId(7L);
                                StorageBinV2 proposedBin = storageBinService.getStorageBinByBinClassIdV4(storageBinPutAway);
                    ...
                    //Last Picked Bin as Proposed Bin If it is empty
                    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    if (putAwayHeader.getProposedStorageBin() == null && (inventoryStorageBin == null)) {

                        double proposalVolume = propertiesConfig.getPickupLineBinPartialAllocationStrategy().equalsIgnoreCase("1") ? cbm : cbmPerQuantity;
                        //statergy 1 means empty bin; else occupied bin still has the volume to store the current cbm
                        Long statusId = propertiesConfig.getPickupLineBinAllocationStrategy().equalsIgnoreCase("1") ? 0L : 1L;

                        StorageBinV2 lastPickedBin = storageBinService.getLastPickedBin(companyCode, plantId, languageId, warehouseId,
                                                                                        itemCode, manufacturerName, binClassId, statusId, proposalVolume);
                        log.info("PickupLineForLastBinCheckV2: " + lastPickedBin);
                        if (lastPickedBin != null) {
                            Double[] qty = cbmService.storageBinCapacityReduction(companyCode, plantId, languageId, warehouseId, lastPickedBin, cbm, orderCBM, cbmPerQuantity, loginUserID);
                            cbm = qty[1];
                            putAwayQty = qty[0];
                            putAwayHeader.setPutAwayQuantity(putAwayQty);
                            putAwayHeader.setCbm(String.valueOf(cbm));
                            putAwayHeader.setProposedStorageBin(lastPickedBin.getStorageBin());
                            putAwayHeader.setLevelId(String.valueOf(lastPickedBin.getFloorId()));
                            log.info("LastPick CBM Bin: " + lastPickedBin.getStorageBin() + "|" + putAwayQty + "|" + cbm);
                        }
                    }

                    //Confirmed Putaway line Bin as Proposed Bin
                    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    if (putAwayHeader.getProposedStorageBin() == null && (inventoryStorageBin == null)) {

                        double proposalVolume = propertiesConfig.getPutAwayBinPartialAllocationStrategy().equalsIgnoreCase("1") ? cbm : cbmPerQuantity;
                        Long statusId = 1L;
                        StorageBinV2 lastConfirmedBin = storageBinService.getConfirmedPutAwayLineBin(companyCode, plantId, languageId, warehouseId,
                                                                                                     itemCode, manufacturerName, binClassId, statusId, proposalVolume);
                        log.info("LastConfirmedBin: " + lastConfirmedBin);
                        if (lastConfirmedBin != null) {
                            Double[] qty = cbmService.storageBinCapacityReduction(companyCode, plantId, languageId, warehouseId, lastConfirmedBin, cbm, orderCBM, cbmPerQuantity, loginUserID);
                            cbm = qty[1];
                            putAwayQty = qty[0];
                            putAwayHeader.setPutAwayQuantity(putAwayQty);
                            putAwayHeader.setCbm(String.valueOf(cbm));
                            putAwayHeader.setProposedStorageBin(lastConfirmedBin.getStorageBin());
                            putAwayHeader.setLevelId(String.valueOf(lastConfirmedBin.getFloorId()));
                            log.info("LastConfirmed CBM Bin [putawayline]: " + lastConfirmedBin.getStorageBin() + "|" + putAwayQty + "|" + cbm);
                        }
                    }

                    //status Id 19 Proposed PutAway Header Bin as Proposed Bin
                    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    if (putAwayHeader.getProposedStorageBin() == null && (inventoryStorageBin == null)) {

                        double proposalVolume = propertiesConfig.getPutAwayBinPartialAllocationStrategy().equalsIgnoreCase("1") ? cbm : cbmPerQuantity;
                        Long statusId = 0L;
                        StorageBinV2 lastConfirmedBin = storageBinService.getProposedPutAwayHeaderBin(companyCode, plantId, languageId, warehouseId,
                                                                                                     itemCode, manufacturerName, binClassId, statusId, proposalVolume);
                        log.info("LastConfirmedBin: " + lastConfirmedBin);
                        if (lastConfirmedBin != null) {
                            Double[] qty = cbmService.storageBinCapacityReduction(companyCode, plantId, languageId, warehouseId, lastConfirmedBin, cbm, orderCBM, cbmPerQuantity, loginUserID);
                            ...
                            putAwayHeader.setProposedStorageBin(lastConfirmedBin.getStorageBin());
                            putAwayHeader.setLevelId(String.valueOf(lastConfirmedBin.getFloorId()));
                            log.info("LastProposed CBM Bin [putawayHeader]: " + lastConfirmedBin.getStorageBin() + "|" + putAwayQty + "|" + cbm);
                        }
                    }
                    ...
                    if (putAwayHeader.getProposedStorageBin() == null && (inventoryStorageBin == null)) {
                        ...
                        if (createdGRLine.getQuantityType().equalsIgnoreCase("A")) {
                            //proposed bin not in putAway header
                            double proposalVolume = propertiesConfig.getPutAwayBinPartialAllocationStrategy().equalsIgnoreCase("1") ? cbm : cbmPerQuantity;
                            Long statusId = 0L;
                            StorageBinV2 proposedEmptyBin = storageBinService.getProposedEmptyBin(companyCode, plantId, languageId, warehouseId, binClassId, statusId, proposalVolume);
                            log.info("proposedEmptyBin: " + proposedEmptyBin);
                            if (proposedEmptyBin != null) {
                                Double[] qty = cbmService.storageBinCapacityReduction(companyCode, plantId, languageId, warehouseId, proposedEmptyBin, cbm, orderCBM, cbmPerQuantity, loginUserID);
                                cbm = qty[1];
                                putAwayHeader.setPutAwayQuantity(qty[0]);
                                putAwayHeader.setCbm(String.valueOf(cbm));
                                putAwayHeader.setProposedStorageBin(proposedEmptyBin.getStorageBin());
                                putAwayHeader.setLevelId(String.valueOf(proposedEmptyBin.getFloorId()));
                                log.info("Proposed Empty CBM Bin [not present in putawayHeader statusId 19]: " + proposedEmptyBin.getStorageBin() + "|" + grQty + "|" + cbm);
                            } else {
                                StorageBinV2 stBin = getReserveBin(warehouseId, 2L, companyCode, plantId, languageId);
                                log.info("A --> CBM reserveBin: " + stBin.getStorageBin());
                                putAwayHeader.setProposedStorageBin(stBin.getStorageBin());
                    ...
                    /*----------------Inventory tables Create---------------------------------------------*/
                    createInventoryCBMV4(companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName, refDocNumber, createdGRLine);
     ...
     * @return
     */
    public InventoryV2 createInventoryCBMV4(String companyCode, String plantId, String languageId,
                                            String warehouseId, String itemCode, String manufacturerName,
                                            String refDocNumber, GrLineV2 createdGRLine) {
        try {
            InventoryV2 createdinventory = null;
            ...
            if (dbInventory != null) {
                ...
                //=========================================================CBM================================================//
                double cbmPerQty = createdGRLine.getCbmQuantity() != null ? createdGRLine.getCbmQuantity() : getCBMImBasicData1(companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName);
                double invCBM = inventory.getReferenceField4() * cbmPerQty;
                inventory.setCbm(String.valueOf(invCBM));
                inventory.setCbmPerQuantity(String.valueOf(cbmPerQty));
                //============================================================================================================//
