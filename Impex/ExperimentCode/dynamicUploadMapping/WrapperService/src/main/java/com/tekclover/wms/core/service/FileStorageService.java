...
//================================================================================================================
	...
	public Map<String, String> processAsnOrders(String companyCodeId, String plantId, String languageId,
		...
		try {
			// Read the file and validate columns
			List<String> validationErrors = validateExcelFile(file);
			if (!validationErrors.isEmpty()) {
				throw new ValidationException(String.join(", ", validationErrors));
			}
		}  catch (Exception ex) {
			throw new BadRequestException("Error uploading file: " + ex.getMessage());
		}
...
//				List<ASNV2> asnV2Orders = prepAsnData(companyCodeId, plantId, languageId, warehouseId, loginUserId, allRowsList);
	...
	private List<String> validateExcelFile(MultipartFile file) {
		...
		try (InputStream inputStream = file.getInputStream()) {
			...
			String[] expectedHeaders = {"asnNumber", "lineReference", "sku", "barcodeId", "skuDescription", "batchSerialNumber",
					"containerNumber", "supplierCode", "supplierName", "supplierPartNumber", "manufacturerName", "manufacturerCode",
					"expectedDate", "expectedQty", "uom", "origin", "Brand", "packQty", "orderTypeId"};
			for (int i = 0; i < expectedHeaders.length; i++) {
				if (headerRow.getCell(i) == null || !headerRow.getCell(i).getStringCellValue().equals(expectedHeaders[i])) {
					errors.add("Missing or incorrect column header at position " + (i + 1));
				}
			}
			...
			for (int i = 1; i <= sheet.getLastRowNum(); i++) {
				...
				// Validate asnNumber (should not be empty)
				Cell asnCell = row.getCell(0);
				if (asnCell == null || asnCell.getStringCellValue().trim().isEmpty()) {
					errors.add("Row " + (i + 1) + ": asnNumber cannot be empty.");
				}

				// Validate lineReference (should be numeric and greater than 0)
				Cell lineCell = row.getCell(1);
				if (lineCell == null || lineCell.getCellType() != CellType.NUMERIC || lineCell.getNumericCellValue() <= 0) {
					errors.add("Row " + (i + 1) + ": Invalid lineReference");
				}

				// Validate sku (should not be empty)
				Cell skuCell = row.getCell(2);
				if (skuCell == null || skuCell.getStringCellValue().trim().isEmpty()) {
					errors.add("Row " + (i + 1) + ": sku cannot be empty.");
				}

				// Validate barcodeId (should not be empty)
				Cell barCodeIdCell = row.getCell(3);
				if (barCodeIdCell == null || barCodeIdCell.getStringCellValue().trim().isEmpty()) {
					errors.add("Row " + (i + 1) + ": barcodeId cannot be empty.");
				}

				// Validate skuDesc (should not be empty)
				Cell skuDescCell = row.getCell(4);
				if (skuDescCell == null || skuDescCell.getStringCellValue().trim().isEmpty()) {
					errors.add("Row " + (i + 1) + ": skuDesc cannot be empty.");
				}

				// Validate batchSerialNumber (should not be empty)
				Cell batchSnoCell = row.getCell(5);
				if (batchSnoCell == null || batchSnoCell.getCellType() != CellType.NUMERIC || batchSnoCell.getNumericCellValue() <= 0) {
					errors.add("Row " + (i + 1) + ": Invalid batchSerialNumber");
				}

				// Validate containerNumber (should not be empty)
				Cell containerNoCell = row.getCell(6);
				if (containerNoCell == null || containerNoCell.getStringCellValue().trim().isEmpty()) {
					errors.add("Row " + (i + 1) + ": containerNumber cannot be empty.");
				}

				// Validate supplierCode (should not be empty)
				Cell supplierCodeCell = row.getCell(7);
				if (supplierCodeCell == null || supplierCodeCell.getCellType() != CellType.NUMERIC) {
					errors.add("Row " + (i + 1) + ": supplierCode cannot be empty.");
				}

				// Validate supplierName (should not be empty)
				Cell supplierNameCell = row.getCell(8);
				if (supplierNameCell == null || supplierNameCell.getStringCellValue().trim().isEmpty()) {
					errors.add("Row " + (i + 1) + ": supplierName cannot be empty.");
				}

				// Validate supplierPartNumber (should not be empty)
				Cell supplierNoCell = row.getCell(9);
				if (supplierNoCell == null || supplierNoCell.getCellType() != CellType.NUMERIC) {
					errors.add("Row " + (i + 1) + ": supplierPartNumber cannot be empty.");
				}

				// Validate manufacturerName (should not be empty)
				Cell manufacturerNameCell = row.getCell(10);
				if (manufacturerNameCell == null || manufacturerNameCell.getStringCellValue().trim().isEmpty()) {
					errors.add("Row " + (i + 1) + ": manufacturerName cannot be empty.");
				}

				// Validate manufacturerCode (should not be empty)
				Cell manufacturerCodeCell = row.getCell(11);
				if (manufacturerCodeCell == null || manufacturerCodeCell.getCellType() != CellType.NUMERIC) {
					errors.add("Row " + (i + 1) + ": manufacturerCode cannot be empty.");
				}

				// Validate expectedDate (should be numeric and greater than 0)
				Cell expectedDateCell = row.getCell(12);
				if (expectedDateCell == null || expectedDateCell.getCellType() != CellType.NUMERIC) {
					errors.add("Row " + (i + 1) + ": expectedDate cannot be empty");
				}

				// Validate expectedQty (should be numeric and greater than 0)
				Cell expectedQtyCell = row.getCell(13);
				if (expectedQtyCell == null || expectedQtyCell.getCellType() != CellType.NUMERIC || expectedQtyCell.getNumericCellValue() <= 0) {
					errors.add("Row " + (i + 1) + ": Invalid expectedQty. It should be a number.");
				}

				// Validate uom (should not be empty)
				Cell uomCell = row.getCell(14);
				if (uomCell == null || uomCell.getStringCellValue().trim().isEmpty()) {
					errors.add("Row " + (i + 1) + ": uom cannot be empty.");
				}

				// Validate origin (should not be empty)
				Cell originCell = row.getCell(15);
				if (originCell == null || originCell.getStringCellValue().trim().isEmpty()) {
					errors.add("Row " + (i + 1) + ": origin cannot be empty.");
				}

				// Validate brand (should not be empty)
				Cell brandCell = row.getCell(16);
				if (brandCell == null || brandCell.getStringCellValue().trim().isEmpty()) {
					errors.add("Row " + (i + 1) + ": brand cannot be empty.");
				}

				// Validate packQty (should be numeric and greater than 0)
				Cell packCell = row.getCell(17);
				if (packCell == null || packCell.getCellType() != CellType.NUMERIC) {
					errors.add("Row " + (i + 1) + ": Invalid packQty. It should be a number.");
				}
				// Validate orderTypeId (should be numeric and greater than 0)
				Cell orderCell = row.getCell(18);
				if (orderCell == null || orderCell.getCellType() != CellType.NUMERIC || orderCell.getNumericCellValue() <= 0) {
					errors.add("Row " + (i + 1) + ": Invalid orderTypeId. It should be a positive number.");
				}
			}
		} catch (IOException e) {
