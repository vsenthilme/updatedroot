...
//				apiLine.setManufacturerCode(stockReceiptLine.getManufacturerCode());
                if (stockReceiptLine.getManufacturerCode() != null) {
                    apiLine.setManufacturerCode(stockReceiptLine.getManufacturerCode());
                } else {
                ...
                if (stockReceiptLine.getManufacturerShortName() != null) {
                    apiLine.setManufacturerName(stockReceiptLine.getManufacturerShortName());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
...
//				apiLine.setManufacturerCode(b2bTransferInLine.getManufacturerCode());
                ...
                if (b2bTransferInLine.getManufacturerCode() != null) {
                    apiLine.setManufacturerCode(b2bTransferInLine.getManufacturerCode());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
                        apiLine.setManufacturerCode(getMfrName(COMPANY_CODE));
                ...
                if (b2bTransferInLine.getManufacturerName() != null) {
                    apiLine.setManufacturerName(b2bTransferInLine.getManufacturerName());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
                        apiLine.setManufacturerName(getMfrName(COMPANY_CODE));
...
//					}
    ...
    private InboundOrderV2 saveInterWarehouseTransferInV2(InterWarehouseTransferInV2 interWarehouseTransferInV2) {
        try {
            ...
            apiHeader.setTransferOrderNumber(interWarehouseTransferInHeaderV2.getTransferOrderNumber());
            apiHeader.setBranchCode(interWarehouseTransferInHeaderV2.getToBranchCode());
            if(interWarehouseTransferInHeaderV2.getInboundOrderTypeId() != null) {
                apiHeader.setInboundOrderHeaderId(interWarehouseTransferInHeaderV2.getInboundOrderTypeId());
            } else {
                apiHeader.setInboundOrderTypeId(4L);                // Hardcoded Value 4
            }
//			apiHeader.setRefDocumentType("WMS to WMS");
            ...
            for (InterWarehouseTransferInLineV2 iwhTransferLineV2 : interWarehouseTransferInLinesV2) {
                ...
                //validation for bagSize performing BagUom
//                checkUom(iwhTransferLineV2.getUom(), iwhTransferLineV2.getAlternateUom(), iwhTransferLineV2.getBagSize());
                ...
                apiLine.setManufacturerPartNo(MFR_NAME);
                if(interWarehouseTransferInHeaderV2.getInboundOrderTypeId() != null) {
                    apiLine.setInboundOrderTypeId(interWarehouseTransferInHeaderV2.getInboundOrderTypeId());
                } else {
                apiLine.setInboundOrderTypeId(4L);
			}

                if(iwhTransferLineV2.getUom() != null) {
                    AlternateUomImpl alternateUom = getUom(apiHeader.getCompanyCode(), apiHeader.getBranchCode(), apiHeader.getLanguageId(),
                                                           apiHeader.getWarehouseID(), apiLine.getItemCode(), iwhTransferLineV2.getUom());
                    if(alternateUom == null) {
                        throw new BadRequestException("AlternateUom is not available for this item : " + apiLine.getItemCode());
                    }
                    if (alternateUom != null) {
                        ...
                        apiLine.setAlternateUom(alternateUom.getAlternateUom());
                        apiLine.setBagSize(alternateUom.getAlternateUomQty());
                    apiLine.setNoBags(iwhTransferLineV2.getExpectedQty());
//                        double orderQty = getQuantity(iwhTransferLineV2.getExpectedQty(), alternateUom.getAlternateUomQty());
//                        apiLine.setExpectedQty(orderQty);
//                        apiLine.setOrderedQty(orderQty);
                    }
                }
...
//====================================================OutBound V2===========================================================================
    ...
    public DeliveryConfirmationV3 postDeliveryConfirmationV3(DeliveryConfirmationV3 deliveryConfirmationV3) throws Exception {
        ...
        OutboundOrderV2 savedSoHeader = saveDeliveryConfirmationV3(deliveryConfirmationV3);                                // Without Nongo
    ...
    private OutboundOrderV2 saveSOV2(ShipmentOrderV2 shipmentOrder, boolean isRerun) throws ParseException {
        try {
            ...
            Optional<Warehouse> warehouse =
                    warehouseRepository.findByCompanyCodeIdAndPlantIdAndLanguageIdAndDeletionIndicator(
                            soHeader.getCompanyCode(), soHeader.getBranchCode(), LANG_ID, 0L);
            // Warehouse ID Validation
...
//			apiHeader.setRefDocumentType("WMS to Non-WMS");                        // Hardcoded value "SO"
            ...
            apiHeader.setRefDocumentType(getOutboundOrderTypeDesc(apiHeader.getCompanyCode(), apiHeader.getBranchCode(),
                                                                  LANG_ID, apiHeader.getWarehouseID(), apiHeader.getOutboundOrderTypeID()));
...
//                        apiLine.setOrderedQty(orderQty);
     ...
     * @throws Exception
     */
    private OutboundOrderV2 saveDeliveryConfirmationV3(@Valid DeliveryConfirmationV3 deliveryConfirmationV3) throws Exception {
        try {
            List<OutboundLine> lines = outboundLineService.deliveryConfirmationV3(deliveryConfirmationV3);
...
//			apiHeader.setRefDocumentType("PURCHASE RETURN");                        // Hardcoded value "RETURNPO"
            ...
            for (ReturnPOLineV2 rpoLine : returnPOLines) {
                ...
                apiLine.setOrderId(apiHeader.getOrderId());
                if(apiHeader.getOutboundOrderTypeID() != null) {
                    apiLine.setOutboundOrderTypeID(apiHeader.getOutboundOrderTypeID());
                } else {
                apiLine.setOutboundOrderTypeID(2L);
                }

                barcodeId = rpoLine.getBarcodeId() != null ? rpoLine.getBarcodeId() : rpoLine.getSku();
                apiLine.setBarcodeId(barcodeId);

                if(rpoLine.getUom() != null) {
                    AlternateUomImpl alternateUom = getUom(apiHeader.getCompanyCode(), apiHeader.getBranchCode(), apiHeader.getLanguageId(),
                                                           apiHeader.getWarehouseID(), apiLine.getItemCode(), rpoLine.getUom());
                    if(alternateUom == null) {
                        throw new BadRequestException("AlternateUom is not available for this item : " + apiLine.getItemCode());
                    }
                    if (alternateUom != null) {
                        ...
                        apiLine.setAlternateUom(alternateUom.getAlternateUom());
                        apiLine.setBagSize(alternateUom.getAlternateUomQty());
                        apiLine.setNoBags(rpoLine.getReturnQty());
//                        double orderQty = getQuantity(rpoLine.getReturnQty(), alternateUom.getAlternateUomQty());
//                        apiLine.setExpectedQty(orderQty);
//                        apiLine.setOrderedQty(orderQty);
                    }
                }
    ...
    private OutboundOrderV2 saveIWHTransferV2(InterWarehouseTransferOutV2 iWhTransferOutV2) throws ParseException {
        try {
            InterWarehouseTransferOutHeaderV2 interWhTransferOutHeader = iWhTransferOutV2.getInterWarehouseTransferOutHeader();
            ...
            if(interWhTransferOutHeader.getWarehouseId() != null && !interWhTransferOutHeader.getWarehouseId().isBlank()) {
                apiHeader.setWarehouseID(interWhTransferOutHeader.getWarehouseId());
            } else {
            Optional<Warehouse> warehouse =
                    warehouseRepository.findByCompanyCodeIdAndPlantIdAndLanguageIdAndDeletionIndicator(
            ...
            if(interWhTransferOutHeader.getOrderType() != null) {
                apiHeader.setOutboundOrderTypeID(Long.valueOf(interWhTransferOutHeader.getOrderType()));
            } else {
            apiHeader.setOutboundOrderTypeID(1L);                             // Hardcoded Value "1"
            }
//			apiHeader.setRefDocumentType("WMS to WMS");                            // Hardcoded value "WH to WH"
            ...
            for (InterWarehouseTransferOutLineV2 iwhTransferLine : interWarehouseTransferOutLines) {
                ...
                barcodeId = iwhTransferLine.getBarcodeId() != null ? iwhTransferLine.getBarcodeId() : iwhTransferLine.getSku();
                apiLine.setBarcodeId(barcodeId);

                if(iwhTransferLine.getUom() != null) {
                    AlternateUomImpl alternateUom = getUom(apiHeader.getCompanyCode(), apiHeader.getBranchCode(), apiHeader.getLanguageId(),
                                                           apiHeader.getWarehouseID(), apiLine.getItemCode(), iwhTransferLine.getUom());
                    if(alternateUom == null) {
                        throw new BadRequestException("AlternateUom is not available for this item : " + apiLine.getItemCode());
                    }
                    if (alternateUom != null) {
                        ...
                        apiLine.setAlternateUom(alternateUom.getAlternateUom());
                        apiLine.setBagSize(alternateUom.getAlternateUomQty());
                    apiLine.setNoBags(iwhTransferLine.getExpectedQty());
//                        double orderQty = getQuantity(iwhTransferLine.getExpectedQty(), alternateUom.getAlternateUomQty());
//                        apiLine.setExpectedQty(orderQty);
//                        apiLine.setOrderedQty(orderQty);
                    }
                }
...
//			}
    ...
    private StockAdjustment saveStockAdjustment(StockAdjustment stockAdjustment) {
        try {
            ...
            BeanUtils.copyProperties(stockAdjustment, dbStockAdjustment, CommonUtils.getNullPropertyNames(stockAdjustment));
            ...
            if (stockAdjustment.getWarehouseId() != null) {
                ...
                Optional<Warehouse> dbWarehouse =
                        warehouseRepository.findByCompanyCodeIdAndPlantIdAndLanguageIdAndDeletionIndicator(
                                ...
                                stockAdjustment.getLanguageId() != null ? stockAdjustment.getLanguageId() : LANG_ID, 0L);
                log.info("dbWarehouse: " + dbWarehouse);
                validateWarehouseId(dbWarehouse.get().getWarehouseId());
...
//			dbStockAdjustment.setOrderProcessedOn(new Date());
     ...
     * @return
     */
    public Double getBagSize(String companyCodeId, String plantId, String languageId,
                             String warehouseId, String itemCode) {
        return inventoryService.getBagSize(companyCodeId, plantId, languageId, warehouseId, itemCode, MFR_NAME, 1L, ALTERNATE_UOM);
    }
     ...
     * @return
     */
    public void checkUom (String uom, String alternateUom, Double bagSize) {
        boolean pass = (uom != null && uom.equalsIgnoreCase(ALTERNATE_UOM) && (bagSize == null || bagSize == 0)) ||
                (alternateUom != null && alternateUom.equalsIgnoreCase(ALTERNATE_UOM) && (bagSize == null || bagSize == 0));
        if(pass) {
            throw new BadRequestException("Bag size is mandatory for Bag Unit of measurement");
        }
    }
}