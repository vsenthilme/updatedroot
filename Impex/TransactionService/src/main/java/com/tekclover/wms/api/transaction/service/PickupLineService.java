...
//                dbPickupLine.setBarcodeId(dbOrderManagementLine.getBarcodeId());
            ...
            if (existingPickupLine == null || existingPickupLine.isEmpty()) {
                String leadTime = pickupLineV2Repository.getleadtime(dbPickupLine.getCompanyCodeId(), dbPickupLine.getPlantId(),
                                                                     ...
                                                                     dbPickupLine.getPickupNumber(), dbPickupLine.getBarcodeId(), new Date());
...
//                dbPickupLine.setBarcodeId(dbOrderManagementLine.getBarcodeId());
            ...
            if (existingPickupLine == null || existingPickupLine.isEmpty()) {
                try {
                    ...
                    languageId = dbPickupLine.getLanguageId();
                    warehouseId = dbPickupLine.getWarehouseId();
                    String leadTime = pickupLineV2Repository.getleadtime(companyCodeId, plantId, languageId, warehouseId, pickupNumber,
    ...
    public List<PickupLineV2> createPickupLineNonCBMV4(@Valid List<AddPickupLine> newPickupLines, String loginUserID) throws Exception {
        ...
        try {
            ...
            for (PickupLineV2 dbPickupLine : createdPickupLineList) {
                ...
                //Update Inventory
                boolean pass = allocatedBarCode != null && pickedBarCode != null && allocatedBarCode.equalsIgnoreCase(pickedBarCode);
                log.info("allocated, picked barcode, result" + allocatedBarCode + "|" + pickedBarCode + "|" + pass);
                if(pass) {
...
//                        itemCode, manufacturerName, partnerCode, dbPickupLine.getActualHeNo(), dbPickupLine.getAssignedPickerId(),
//                        dbPickupLine.getLineNumber(), STATUS_ID, statusDescription, new Date());
//                log.info("outboundLine updated using Stored Procedure: ");
...
//                            Double ALLOC_QTY = inventory.getAllocatedQuantity() - dbPickupLine.getAllocatedQty();
                    ...
                    if (dbPickupLine.getPickUom().equalsIgnoreCase(dbPickupLine.getAlternateUom())) {
                        inventoryQty = calculateUOMInventory(dbPickupLine.getAllocatedQty(), dbPickupLine.getPickConfirmQty(), dbPickupLine.getBagSize(), inventory.getInventoryQuantity(), inventory.getAllocatedQuantity());
                    } else {
                        inventoryQty = calculateInventory(dbPickupLine.getAllocatedQty(), dbPickupLine.getPickConfirmQty(), dbPickupLine.getBagSize(), inventory.getInventoryQuantity(), inventory.getAllocatedQuantity());
                    }
    ...
    public void modifyInventoryForNonMatchingBarcodeIdV4(String companyCodeId, String plantId, String languageId, String warehouseId,
        ...
        if(allocatedInventory != null) {
            if (dbPickupLine.getPickConfirmQty() > 0D) {
                ...
                double[] inventoryQty = null;
                if (dbPickupLine.getPickUom().equalsIgnoreCase(dbPickupLine.getAlternateUom())) {
                    inventoryQty = calculateInventoryUOMUnAllocate(dbPickupLine.getPickConfirmQty(), dbPickupLine.getBagSize(), allocatedInventory.getInventoryQuantity(), allocatedInventory.getAllocatedQuantity());
                } else {
                    inventoryQty = calculateInventoryUnAllocate(dbPickupLine.getPickConfirmQty(), dbPickupLine.getBagSize(), allocatedInventory.getInventoryQuantity(), allocatedInventory.getAllocatedQuantity());
                }
                if (inventoryQty != null && inventoryQty.length > 3) {
        ...
        if (inventory != null) {
            if (dbPickupLine.getPickConfirmQty() > 0D) {
                try {
                    ...
                    log.info("UOM,ALTUOM: " + dbPickupLine.getPickUom() + "|" + dbPickupLine.getAlternateUom());
                    if (dbPickupLine.getPickUom().equalsIgnoreCase(dbPickupLine.getAlternateUom())) {
                        inventoryQty = calculateInventoryUOMAllocate(dbPickupLine.getPickConfirmQty(), dbPickupLine.getBagSize(), inventory.getInventoryQuantity(), inventory.getAllocatedQuantity());
                    } else {
                        inventoryQty = calculateInventoryAllocate(dbPickupLine.getPickConfirmQty(), dbPickupLine.getBagSize(), inventory.getInventoryQuantity(), inventory.getAllocatedQuantity());
                    }
                    if (inventoryQty != null && inventoryQty.length > 3) {
                        inventory.setInventoryQuantity(inventoryQty[0]);
                        inventory.setAllocatedQuantity(inventoryQty[1]);
