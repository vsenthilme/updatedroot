...
public interface GrLineV2Repository extends JpaRepository<GrLineV2, Long>, JpaSpecificationExecutor<GrLineV2>,
    ...
    @Query(value = "SELECT \n" +
            ...
            "(COALESCE(:materialNo, null) IS NULL OR (gl.MATERIAL_NO IN (:materialNo))) and\n" +
            "(COALESCE(:priceSegment, null) IS NULL OR (gl.PRICE_SEGMENT IN (:priceSegment))) and\n" +
            "(COALESCE(:articleNo, null) IS NULL OR (gl.ARTICLE_NO IN (:articleNo))) and\n" +
            "(COALESCE(:gender, null) IS NULL OR (gl.GENDER IN (:gender))) and\n" +
            "(COALESCE(:color, null) IS NULL OR (gl.COLOR IN (:color))) and\n" +
            "(COALESCE(:size, null) IS NULL OR (gl.SIZE IN (:size))) and\n" +
            "(COALESCE(:noPairs, null) IS NULL OR (gl.NO_PAIRS IN (:noPairs))) and \n" +
    ...
    List<GrLineV2> findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndRefDocNumberAndPreInboundNoAndItemCodeAndLineNoAndDeletionIndicator(
            String languageId, String companyCodeId, String plantId, String warehouseId, String refDocNumber,
            String preInboundNo, String itemCode, Long lineNo, Long deletionIndicator);

    List<GrLineV2> findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndRefDocNumberAndPreInboundNoAndItemCodeAndLineNoAndBatchSerialNumberAndDeletionIndicator(
            String languageId, String companyCodeId, String plantId, String warehouseId, String refDocNumber,
            String preInboundNo, String itemCode, Long lineNo, String batchSerialNumber, Long deletionIndicator);

    //============================================Walkaroo-v3=============================================================
    List<GrLineV2> findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndParentProductionOrderNoAndDeletionIndicatorOrderByCreatedOnDesc(
            String languageId, String companyCode, String plantId, String warehouseId, String parentProductionOrderNo, long l);

    //=================================================Impex==============================================================
    @Query(value = "select sum(gr_qty) + :grQty \n" +
            "from tblgrline where c_id = :companyCode and plant_id = :plantId and lang_id = :languageId and \n" +
            "wh_id = :warehouseId and REF_DOC_NO = :refDocNumber and PRE_IB_NO = :preInboundNo and \n" +
            "gr_no = :goodsReceiptNo and pal_code = :palletCode and case_code = :caseCode and \n" +
            "itm_code = :itemCode and mfr_name = :manufacturerName and \n" +
