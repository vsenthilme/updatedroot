...
//			periodicheaderEntity.setPeriodicLine(listPeriodicLineEntity);
    ...
    public PeriodicHeaderV2 updatePeriodicHeaderV2(String companyCode, String plantId, String languageId, String warehouseId,
                                                   ...
                                                   PeriodicHeaderEntityV2 updatePeriodicHeader) throws Exception {
        // Update Line Details
        try {
            List<PeriodicLineV2> lines = periodicLineService.updatePeriodicLineForMobileCountV4(updatePeriodicHeader.getPeriodicLine(), loginUserID);
            log.info("Lines Updated : " + lines);
    ...
    public PeriodicHeaderEntityV2 processStockCountReceived(CycleCountHeader cycleCountHeader) {
        ...
        if (cycleCountHeader.getWarehouseId() != null) {
            newPeriodicHeaderV2.setWarehouseId(cycleCountHeader.getWarehouseId());
        } else {
            // Get Warehouse
            Optional<Warehouse> dbWarehouse =
                    warehouseRepository.findByCompanyCodeIdAndPlantIdAndLanguageIdAndDeletionIndicator(
                            ...
                            cycleCountHeader.getLanguageId() != null ? cycleCountHeader.getLanguageId() : LANG_ID,
                            0L
                    );
        ...
        newPeriodicHeaderV2.setLanguageId(cycleCountHeader.getLanguageId() != null ? cycleCountHeader.getLanguageId() : LANG_ID);
...
//            listPeriodicHeaderEntity.add(periodicheaderEntity);
     ...
     * @return
     */
    public PeriodicHeaderEntityV2 processStockV4(String companyCode, String plantId, String languageId,
                                                 String warehouseId, String loginUserId, List<PeriodicLineV2> periodicLines) {
        ...
        newPeriodicHeaderV2.setCompanyCode(companyCode);
        newPeriodicHeaderV2.setPlantId(plantId);
        newPeriodicHeaderV2.setLanguageId(languageId);
        newPeriodicHeaderV2.setWarehouseId(warehouseId);

        String idMasterAuthToken = getIDMasterAuthToken();
        NUMBER_RANGE_CODE = 14L;
        String nextRangeNumber = getNextRangeNumber(NUMBER_RANGE_CODE, companyCode, plantId, languageId, warehouseId, idMasterAuthToken);
        newPeriodicHeaderV2.setCycleCountNo(nextRangeNumber);
        newPeriodicHeaderV2.setReferenceCycleCountNo(nextRangeNumber);
        ...
        // STATUS_ID - HardCoded Value "70"
        newPeriodicHeaderV2.setStatusId(70L);
        statusDescription = getStatusDescription(70L, languageId);
        ...
        newPeriodicHeaderV2.setDeletionIndicator(0L);
        newPeriodicHeaderV2.setCreatedBy(loginUserId);
        newPeriodicHeaderV2.setCountedBy(loginUserId);
        newPeriodicHeaderV2.setCreatedOn(new Date());
        newPeriodicHeaderV2.setCountedOn(new Date());
        PeriodicHeaderV2 createdPeriodicHeaderV2 = periodicHeaderV2Repository.save(newPeriodicHeaderV2);
        log.info("createdPeriodicHeaderV2 : " + createdPeriodicHeaderV2);

        for (PeriodicLineV2 dbPeriodicLine : periodicLines) {
            PeriodicLineV2 periodicLineV2 = new PeriodicLineV2();
            BeanUtils.copyProperties(dbPeriodicLine, periodicLineV2, CommonUtils.getNullPropertyNames(dbPeriodicLine));

            StorageBinV2 dbStorageBin = storageBinService.getStorageBinV2(companyCode, plantId, languageId, warehouseId, dbPeriodicLine.getStorageBin());
            if (dbStorageBin != null) {
                periodicLineV2.setStorageSectionId(dbStorageBin.getStorageSectionId());
                periodicLineV2.setLevelId(String.valueOf(dbStorageBin.getFloorId()));
            }

            periodicLineV2.setCompanyCode(companyCode);
            periodicLineV2.setPlantId(plantId);
            periodicLineV2.setWarehouseId(warehouseId);
            periodicLineV2.setLanguageId(languageId);
            periodicLineV2.setCycleCountNo(nextRangeNumber);
            periodicLineV2.setReferenceNo(nextRangeNumber);
            if (dbPeriodicLine.getItemDesc() == null) {
                Optional<ImBasicData1V2> imBasicData1 = imBasicData1V2Repository.findByItemCodeAndCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndManufacturerPartNoAndDeletionIndicator(
                        dbPeriodicLine.getItemCode(), companyCode, plantId, languageId, warehouseId, dbPeriodicLine.getManufacturerName(), 0L);
            ...
            periodicLineV2.setStatusId(70L);
            periodicLineV2.setStatusDescription(statusDescription);
            if(periodicLineV2.getCompanyDescription() == null) {
                periodicLineV2.setCompanyDescription(description.getCompanyDesc());
                periodicLineV2.setPlantDescription(description.getPlantDesc());
                periodicLineV2.setWarehouseDescription(description.getWarehouseDesc());
            }

            if (periodicLineV2.getBarcodeId() == null) {
                List<String> barcode = stagingLineV2Repository.getPartnerItemBarcode(dbPeriodicLine.getItemCode(), companyCode, plantId, warehouseId,
                                                                                     dbPeriodicLine.getManufacturerName(), languageId);
                log.info("Barcode : " + barcode);
                if (barcode != null && !barcode.isEmpty()) {
                    periodicLineV2.setBarcodeId(barcode.get(0));
                }
        ...
        List<PeriodicLineV2> createdPeriodicLine = periodicLineV2Repository.saveAll(periodicLineV2s);
        log.info("createdPeriodicLines: " + createdPeriodicLine);
