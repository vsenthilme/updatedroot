...
public class ImBatchSerialSpecification implements Specification<ImBatchSerial> {
    ...
    public Predicate toPredicate(Root<ImBatchSerial> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (searchImBatchSerial.getSequenceIndicator() != null && !searchImBatchSerial.getSequenceIndicator().isEmpty()) {
            final Path<DeferredImportSelector.Group> group = root.<DeferredImportSelector.Group>get("sequenceIndicator");
            predicates.add(group.in(searchImBatchSerial.getSequenceIndicator()));
        }
