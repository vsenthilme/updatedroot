...
public interface OutboundRepository extends JpaRepository<OutboundHeader, Long>,
    ...
    Stream<OutboundHeaderStream> findAllOutBoundHeaderData (
    ...
    @Query(value = "select status_text from tblstatusid where \n" +
            "status_id in (:statusId) and wh_id in (:warehouseId) and lang_id in (:languageId) and is_deleted = 0", nativeQuery = true)
