			...
			if (inventory != null) {
				if (dbPickupLine.getAllocatedQty() > 0D) {
					try {
						...
						//audit log - allocated qty
						inventoryService.createAuditLogRecord(dbPickupLine.getCompanyCodeId(), dbPickupLine.getPlantId(),
								warehouseId, loginUserID, "INVENTORY", "INVENTORY",
								"INV_UOM", String.valueOf(dbPickupLine.getAllocatedQty()), String.valueOf(ALLOC_QTY),
								dbPickupLine.getPickedPackCode(), dbPickupLine.getItemCode(), dbPickupLine.getPickedStorageBin(),
								dbPickupLine.getStockTypeId(), dbPickupLine.getSpecialStockIndicatorId(),dbPickupLine.getRefDocNumber());

						//audit log - inventory qty
						inventoryService.createAuditLogRecord(dbPickupLine.getCompanyCodeId(), dbPickupLine.getPlantId(),
								warehouseId, loginUserID, "INVENTORY", "INVENTORY",
								"INV_UOM", String.valueOf(inventory.getInventoryQuantity()), String.valueOf(INV_QTY),
								dbPickupLine.getPickedPackCode(), dbPickupLine.getItemCode(), dbPickupLine.getPickedStorageBin(),
								dbPickupLine.getStockTypeId(), dbPickupLine.getSpecialStockIndicatorId(),dbPickupLine.getRefDocNumber());
