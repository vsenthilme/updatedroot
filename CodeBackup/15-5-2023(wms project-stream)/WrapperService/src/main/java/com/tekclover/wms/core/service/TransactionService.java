...
import com.tekclover.wms.core.repository.OutboundRepository;
import com.tekclover.wms.core.repository.ReportRepository;
...
//	}
	 ...
	 * @return
	 */
//	public StreamingResponseBody findStreamGrHeader(SearchGrHeader searchGrHeader) {
//		Stream<GrHeaderStream> outboundHeaderStream = streamGrHeader(searchGrHeader);
//		StreamingResponseBody responseBody = httpResponseOutputStream -> {
//			try (Writer writer = new BufferedWriter(new OutputStreamWriter(httpResponseOutputStream))) {
//				JsonGenerator jsonGenerator = new JsonFactory().createGenerator(writer);
//				jsonGenerator.writeStartArray();
...
//	public Stream<GrHeaderStream> streamGrHeader(SearchGrHeader searchGrHeader) {
	public List<GrHeaderStream> streamGrHeader(SearchGrHeader searchGrHeader) {
		jdbcTemplate.setFetchSize(50);
		/**
		 * Gr Header
		 * String languageId
		...
		StringBuilder sb = new StringBuilder();
		sb.append ("Select LANG_ID, C_ID, PLANT_ID, WH_ID, PRE_IB_NO, REF_DOC_NO ");
		sb.append ("Select LANG_ID, C_ID, PLANT_ID, WH_ID, PRE_IB_NO, REF_DOC_NO, ");
		sb.append ("STG_NO, GR_NO, PAL_CODE, CASE_CODE, IB_ORD_TYP_ID, STATUS_ID, ");
		sb.append ("GR_MTD, CONT_REC_NO, DOCK_ALL_NO, CONT_NO, VEH_NO, EA_DATE, ");
		sb.append ("GR_DATE, IS_DELETED, REF_FIELD_1, REF_FIELD_2, REF_FIELD_3, REF_FIELD_4, ");
		sb.append ("REF_FIELD_5, REF_FIELD_6, REF_FIELD_7, REF_FIELD_8, REF_FIELD_9, REF_FIELD_10, ");
		sb.append ("GR_CTD_BY, GR_CTD_ON, GR_UTD_BY, GR_UTD_ON, GR_CNF_BY, GR_CNF_ON ");
		sb.append ("from tblgrheader ");
		sb.append ("WHERE WH_ID = :warehouseId AND IS_DELETED = 0 " );
		javax.persistence.Query sqlquery = entityManager.createNativeQuery(sb.toString());
				sqlquery.setParameter("warehouseId", searchGrHeader.getWarehouseId());
				String query = sqlquery.toString();*/
//		Stream<GrHeaderStream> grHeaderStream = jdbcTemplate.queryForStream(
		//						+ "where IS_DELETED = 0 and WH_ID in (?) and status_id in (?)",new Object[] {110,1},
		List<GrHeaderStream> grHeaderStream = jdbcTemplate.query(
				"Select LANG_ID, C_ID, PLANT_ID, WH_ID, PRE_IB_NO, REF_DOC_NO, "
						...
						+ "from tblgrheader "
						+ "where IS_DELETED = 0 AND WH_ID IN (?,?,?,?,?) AND STATUS_ID IN (?,?,?,?,?,?,?,?,?,?)",
	 ...
	 * @return
	 */
//	public List<StorageBinStream> streamStorageBin() {
...
//		List<StorageBinStream> storageBinStream = jdbcTemplate.query(
//				"Select ST_BIN, WH_ID, FL_ID, ST_SEC_ID, SPAN_ID, STATUS_ID, "
//						+ "CTD_BY, CTD_ON "
//						+ "from tblstoragebin "
//						+ "where IS_DELETED = 0 ",new Object[] {},
...
//	public Stream<OutboundHeader> findOutboundHeaderNewStream(FindOutboundHeader searchOutboundHeader)
	public List<com.tekclover.wms.core.model.transaction.stream.OutboundHeader> findOutboundHeaderNewStream(FindOutboundHeader searchOutboundHeader)
			throws org.springframework.expression.ParseException, java.text.ParseException {
...
//		Stream<OutboundHeader> spec = reportRepository.findAllOutBoundHeaderData(searchOutboundHeader.getWarehouseId(),
...
//				searchOutboundHeader.getStartOrderDate(),searchOutboundHeader.getEndOrderDate());
		List<com.tekclover.wms.core.model.transaction.stream.OutboundHeader> outboundHeaderList  = Lists.newArrayList();
		try (Stream<com.tekclover.wms.core.model.transaction.stream.OutboundHeader> spec = outboundRepository.findAllOutBoundHeaderData(searchOutboundHeader.getWarehouseId(),
				searchOutboundHeader.getRefDocNumber(), searchOutboundHeader.getPartnerCode(), searchOutboundHeader.getOutboundOrderTypeId(),
				searchOutboundHeader.getStatusId(), searchOutboundHeader.getSoType(),
				searchOutboundHeader.getStartRequiredDeliveryDate(), searchOutboundHeader.getEndRequiredDeliveryDate(),
				searchOutboundHeader.getStartDeliveryConfirmedOn(), searchOutboundHeader.getEndDeliveryConfirmedOn(),
