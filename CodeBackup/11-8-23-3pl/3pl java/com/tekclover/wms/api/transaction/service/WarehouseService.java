...
//	}
	...
	private InboundOrderV2 saveASNV2 (ASNV2 asnv2) {
		try {
			...
			// Warehouse ID Validation
//			validateWarehouseId (asnV2Header.getWareHouseId());

			// Checking for duplicate RefDocNumber
			InboundOrderV2 dbApiHeader = orderService.getOrderByIdV2(asnV2Header.getAsnNumber());

			if (dbApiHeader != null) {
				throw new InboundOrderRequestException("ASNV2 is already posted and it can't be duplicated.");
			}
			...
			for (ASNLineV2 asnLineV2 : asnLineV2s) {
				...
				// EA_DATE
				try {
					Date reqDelDate = DateUtils.convertStringToDate(asnLineV2.getExpectedDate());
					apiLine.setExpectedDate(reqDelDate);
				} catch (Exception e) {
					throw new BadRequestException("Date format should be MM-dd-yyyy");
				}
	...
	private InboundOrderV2 saveSOReturnV2 (SaleOrderReturnV2 soReturnV2) {
		try {
			...
			// Checking for duplicate RefDocNumber
			InboundOrderV2 dbApiHeader = orderService.getOrderByIdV2(soReturnHeaderV2.getTransferOrderNumber());
			if (dbApiHeader != null) {
				throw new InboundOrderRequestException("Return Order Reference is already posted and it can't be duplicated.");
			}
			...
			InboundOrderV2 apiHeader = new InboundOrderV2();
			apiHeader.setReturnOrderReference(soReturnHeaderV2.getTransferOrderNumber());
			apiHeader.setBranchCode(soReturnHeaderV2.getBranchCode());
			apiHeader.setOrderId(soReturnHeaderV2.getBranchCode());
			...
			for (SOReturnLineV2 soReturnLineV2 : salesOrderReturnLinesV2) {
				...
				apiLine.setSourceBranchCode(soReturnLineV2.getStoreID());
	...
	private InboundOrderV2 saveInterWarehouseTransferInV2 (InterWarehouseTransferInV2 interWarehouseTransferInV2) {
		try {
			...
			// Warehouse ID Validation
			//	validateWarehouseId (interWarehouseTransferInHeaderV2.getToWhsId());

			// Checking for duplicate RefDocNumber
			InboundOrderV2 dbApiHeader = orderService.getOrderByIdV2(interWarehouseTransferInHeaderV2.getTransferOrderNumber());
			if (dbApiHeader != null) {
				throw new InboundOrderRequestException("InterWarehouseTransferInV2 is already posted and it can't be duplicated.");
			}
			...
			// Get Warehouse
//			AuthToken authToken = authTokenService.getIDMasterServiceAuthToken();
...
//			Warehouse warehouse = idMasterService.getWarehouse(interWarehouseTransferInHeaderV2.getToCompanyCode(), 
//					interWarehouseTransferInHeaderV2.getToBranchCode(), authToken.getAccess_token());
