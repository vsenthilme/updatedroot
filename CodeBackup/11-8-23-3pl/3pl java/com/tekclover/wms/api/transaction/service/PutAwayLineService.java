...
public class PutAwayLineService extends BaseService {
	...
	private InventoryV2 createInventory (PutAwayLineV2 createdPALine) {
		...
		if(createdPALine.getCbm() == null) { createdPALine.setCbm("0"); }
		if(createdPALine.getCbmQuantity() == null) { createdPALine.setCbmQuantity(0D); }
			...
			for (PutAwayLineV2 newPutAwayLine : newPutAwayLines) {
				...
				if(existingPutAwayLine.isEmpty()) {
					...
					if (createdPutAwayLine != null && createdPutAwayLine.getPutawayConfirmedQty() > 0L ) {
						...
						//v2 code
						if(createdPutAwayLine.getCbm() == null) { createdPutAwayLine.setCbm("0"); }
						if(createdPutAwayLine.getCbmQuantity() == null) { createdPutAwayLine.setCbmQuantity(0D); }
