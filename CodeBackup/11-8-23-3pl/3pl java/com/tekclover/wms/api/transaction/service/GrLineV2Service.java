...
public class GrLineV2Service extends BaseService {
	 ...
	 */
	public List<GrLineV2> findGrLine(SearchGrLine searchGrLine) throws ParseException {
	...
	public List<GrLineV2> createGrLine (@Valid List<AddGrLineV2> newGrLines, String loginUserID)
		...
		try {
			String warehouseId = null; String languageId = null;
			...
			for (AddGrLineV2 newGrLine : newGrLines) {
				...
				for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
					...
					if(newGrLine.getThreePlCbm() == null) {newGrLine.setThreePlCbm(0D);}
					if(newGrLine.getGoodReceiptQty() == null) {newGrLine.setGoodReceiptQty(0D);}
					dbGrLine.setCbmQuantity(newGrLine.getThreePlCbm() / newGrLine.getGoodReceiptQty());

					dbGrLine.setThreePlBillStatus("GR");
					if(newGrLine.getThreePlHeight() != null) {
						dbGrLine.setThreePlHeight(Math.cbrt(newGrLine.getThreePlHeight()));
					}
					if(newGrLine.getThreePlLength() != null) {
						dbGrLine.setThreePlLength(Math.cbrt(newGrLine.getThreePlLength()));
					}
					if(newGrLine.getThreePlWidth() != null) {
						dbGrLine.setThreePlWidth(Math.cbrt(newGrLine.getThreePlWidth()));
					}
					...
					List<GrLineV2> oldGrLine = grLineV2Repository.findByGoodsReceiptNoAndItemCodeAndLineNoAndLanguageIdAndCompanyCodeAndPlantIdAndRefDocNumberAndPackBarcodesAndWarehouseIdAndPreInboundNoAndCaseCodeAndDeletionIndicator(
			...
			for (GrLineV2 grLine : createdGRLines) {
				 ...
				 */
				List<GrHeaderV2> grHeaders = grHeaderService.getGrHeaderV2(grLine.getWarehouseId(), grLine.getGoodsReceiptNo(),
				...
				for (StagingLineEntityV2 stagingLineEntity : stagingLineEntityList) {
					...
					//v2 code
					if(stagingLineEntity.getRec_accept_qty() == null) { stagingLineEntity.setRec_accept_qty(0D); }
					if(grLine.getAcceptedQty() == null) { grLine.setAcceptedQty(0D); }
					if(stagingLineEntity.getRec_damage_qty() == null) { stagingLineEntity.setRec_damage_qty(0D);}
					if(grLine.getDamageQty() == null) { grLine.setDamageQty(0D);}
	...
	private void createPutAwayHeader (GrLineV2 createdGRLine, String loginUserID) {
		...
		if(createdGRLine.getThreePlCbm() != null) {
			cbm = createdGRLine.getThreePlCbm();
		}
				...
				if (!stBinInventoryList.isEmpty()) {
					...
					if (createdGRLine.getThreePlCbm() != null) {
						...
						if (stBinList != null && stBinList.size() > 0) {
							...
							if(stBinList.get(0).getTotalVolume() == null) {stBinList.get(0).setTotalVolume("0");}
							if(stBinList.get(0).getAllocatedVolume() == null) {stBinList.get(0).setAllocatedVolume(0D);}
							if(stBinList.get(0).getOccupiedVolume() == null) {stBinList.get(0).setOccupiedVolume("0");}
							if(stBinList.get(0).getRemainingVolume() == null) {stBinList.get(0).setRemainingVolume("0");}
							...
							putAwayHeader.setPutAwayQuantity((remainingVolume/createdGRLine.getThreePlCbm()) * createdGRLine.getGoodReceiptQty());
						}
...
//		if(createdGRLine.getInventoryQuantity() == null) { createdGRLine.setInventoryQuantity(0D); }
//		if(createdGRLine.getCbm() == null) { createdGRLine.setCbm(0D); }
//		inventory.setCbmPerQuantity(String.valueOf(createdGRLine.getCbm() / createdGRLine.getInventoryQuantity()));
	 ...
	 * @throws InvocationTargetException
	 */
	public GrLineV2 updateGrLine (String warehouseId, String preInboundNo, String refDocNumber, String goodsReceiptNo, String palletCode, String caseCode, String packBarcodes, Long lineNo, String itemCode,
		...
		GrLineV2 dbGrLine = getGrLine(warehouseId, preInboundNo, refDocNumber,
									...
									updateGrLine.getCompanyCode(),
									updateGrLine.getPlantId(),
									updateGrLine.getLanguageId());
