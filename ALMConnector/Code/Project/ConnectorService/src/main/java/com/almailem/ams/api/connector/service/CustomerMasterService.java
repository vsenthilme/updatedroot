...
<<<<<<< HEAD
        ...
        //almailem
        UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(getMasterServiceApiUrl() + "masterorder/master/customer");
        ...
        log.info("almailem result : " + result.getStatusCode());

        authToken = authTokenService.getAmgharaMastersServiceAuthToken();
        headers.add("Authorization", "Bearer " + authToken.getAccess_token());
        //Amghara
        builder = UriComponentsBuilder.fromHttpUrl(getAmgharaMasterServiceApiUrl() + "masterorder/master/customer");
        entity = new HttpEntity<>(customer, headers);
        result = getRestTemplate().exchange(builder.toUriString(), HttpMethod.POST, entity, WarehouseApiResponse.class);
        log.info("amghara result : " + result.getStatusCode());
