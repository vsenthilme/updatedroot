...
import com.tekclover.wms.api.masters.transaction.model.impl.OrderStatusReportImpl;
import com.tekclover.wms.api.masters.transaction.model.impl.OutBoundLineImpl;
import com.tekclover.wms.api.masters.transaction.model.impl.ShipmentDispatchSummaryReportImpl;
import com.tekclover.wms.api.masters.transaction.model.impl.StockMovementReportImpl;
...
import com.tekclover.wms.api.masters.transaction.model.inbound.putaway.v2.InboundReversalInput;
import com.tekclover.wms.api.masters.transaction.model.integration.IntegrationApiResponse;
import com.tekclover.wms.api.masters.transaction.model.outbound.*;
import com.tekclover.wms.api.masters.transaction.model.outbound.ordermangement.OrderManagementLine;
import com.tekclover.wms.api.masters.transaction.model.outbound.ordermangement.UpdateOrderManagementLine;
import com.tekclover.wms.api.masters.transaction.model.outbound.ordermangement.v2.OrderManagementLineV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.outboundreversal.AddOutboundReversal;
import com.tekclover.wms.api.masters.transaction.model.outbound.outboundreversal.OutboundReversal;
import com.tekclover.wms.api.masters.transaction.model.outbound.outboundreversal.v2.OutboundReversalV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.pickup.PickupHeader;
import com.tekclover.wms.api.masters.transaction.model.outbound.pickup.PickupLine;
import com.tekclover.wms.api.masters.transaction.model.outbound.pickup.UpdatePickupHeader;
import com.tekclover.wms.api.masters.transaction.model.outbound.pickup.v2.PickupHeaderV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.pickup.v2.PickupLineV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.preoutbound.v2.OutboundIntegrationHeaderV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.quality.QualityHeader;
import com.tekclover.wms.api.masters.transaction.model.outbound.quality.QualityLine;
import com.tekclover.wms.api.masters.transaction.model.outbound.quality.v2.QualityHeaderV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.quality.v2.QualityLineV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.v2.OutboundHeaderV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.v2.OutboundLineOutput;
import com.tekclover.wms.api.masters.transaction.model.outbound.v2.OutboundLineV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.v2.SearchOutboundLineV2;
import com.tekclover.wms.api.masters.transaction.model.report.FindImBasicData1;
import com.tekclover.wms.api.masters.transaction.model.report.SearchOrderStatusReport;
import com.tekclover.wms.api.masters.transaction.model.report.StockMovementReport;
import com.tekclover.wms.api.masters.transaction.model.report.StockMovementReport1;
import com.tekclover.wms.api.masters.transaction.model.warehouse.inbound.confirmation.AXApiResponse;
import com.tekclover.wms.api.masters.transaction.model.warehouse.outbound.confirmation.*;
import com.tekclover.wms.api.masters.transaction.repository.*;
import com.tekclover.wms.api.masters.transaction.repository.specification.*;
...
public class OutboundLineService extends BaseService {
    ...
    @Autowired
    private TPropertiesConfig TPropertiesConfig;
...
//	@Scheduled(cron = "0 0/32 11 * * *")
    public void scheduleStockMovementReport() throws Exception {
        ...
        List<TImBasicData1> resultsTImBasicData1 = imbasicdata1RepositoryT.findAll(spec);
        log.info("resultsImBasicData1 : " + resultsTImBasicData1.size());

        List<String> itemCodeList = resultsTImBasicData1.stream().map(TImBasicData1::getItemCode).collect(Collectors.toList());
    ...
    public List<OutboundLine> deliveryConfirmation(String warehouseId, String preOutboundNo, String refDocNumber,
        ...
        if (axapiResponse.getStatusCode() != null && axapiResponse.getStatusCode().equalsIgnoreCase("200")) {
            try {
                ...
                TStorageBin TStorageBin = mastersService.getStorageBin(warehouseId, BIN_CLASS_ID, authTokenForMastersService.getAccess_token());
                String movementDocumentNo = refDocNumber;
                String stBin = TStorageBin.getStorageBin();
...
//								}
                        ...
                        AuthToken authTokenForMastersService = TAuthTokenService.getMastersServiceAuthToken();
                        Long BIN_CLASS_ID = 5L;
                        TStorageBin TStorageBin = mastersService.getStorageBin(outboundLine.getWarehouseId(), BIN_CLASS_ID, authTokenForMastersService.getAccess_token());
...
//	}
    ...
    private OutboundReversal createOutboundReversal(Warehouse warehouse, String reversalType, String refDocNumber,
        ...
        AuthToken idmasterAuthToken = TAuthTokenService.getIDMasterServiceAuthToken();
...
//				pickupHeader.getItemCode(), loginUserID, updateOrderManagementLine);
    ...
    private Inventory updateInventory1(PickupLine pickupLine, Long statusId) {
        ...
        TStorageBin TStorageBin = mastersService.getStorageBin(pickupLine.getWarehouseId(), BIN_CLASS_ID, authTokenForMastersService.getAccess_token());
        Inventory inventory = inventoryService.getInventory(pickupLine.getWarehouseId(), pickupLine.getPickedPackCode(),
                pickupLine.getItemCode(), TStorageBin.getStorageBin());
        if (inventory != null) {
            Double INV_QTY = inventory.getInventoryQuantity() - pickupLine.getPickConfirmQty();
            inventory.setInventoryQuantity(INV_QTY);
...
//			inventoryRepository.delete(inventory);
    ...
    private AXApiResponse postInterwarehouseShipment(OutboundHeader confirmedOutboundHeader,
         ...
         * Posting to AX_API
         */
        AXAuthToken authToken = TAuthTokenService.generateAXOAuthToken();
        AXApiResponse apiResponse = warehouseIdService.postInterWarehouseShipmentConfirmation(interWarehouseShipment,
        ...
        response.setApiUrl(TPropertiesConfig.getAxapiServiceIWHouseShipmentUrl());
    ...
    private AXApiResponse postShipment(OutboundHeader confirmedOutboundHeader, List<OutboundLine> confirmedOutboundLines) {
         ...
         */
        AXAuthToken authToken = TAuthTokenService.generateAXOAuthToken();
        AXApiResponse apiResponse = warehouseIdService.postShipmentConfirmation(shipment, authToken.getAccess_token());
        log.info("apiResponse : " + apiResponse);
        ...
        response.setResponseCode(apiResponse.getStatusCode());
        response.setResponseText(apiResponse.getMessage());
        response.setApiUrl(TPropertiesConfig.getAxapiServiceShipmentUrl());
        response.setTransDate(new Date());
        integrationApiResponseRepository.save(response);
        return apiResponse;
    }
    ...
    private AXApiResponse postReturnPO(OutboundHeader confirmedOutboundHeader,
         ...
         */
        AXAuthToken authToken = TAuthTokenService.generateAXOAuthToken();
        AXApiResponse apiResponse = warehouseIdService.postReturnPOConfirmation(returnPO, authToken.getAccess_token());
        log.info("apiResponse : " + apiResponse);
        ...
        response.setResponseText(apiResponse.getMessage());
        response.setTransDate(new Date());
        response.setApiUrl(TPropertiesConfig.getAxapiServiceReturnPOUrl());
        integrationApiResponseRepository.save(response);
    ...
    private AXApiResponse postSalesOrder(OutboundHeader confirmedOutboundHeader,
         ...
         */
        AXAuthToken authToken = TAuthTokenService.generateAXOAuthToken();
        AXApiResponse apiResponse = warehouseIdService.postSaleOrderConfirmation(salesOrder, authToken.getAccess_token());
        log.info("apiResponse : " + apiResponse);
        ...
        response.setResponseCode(apiResponse.getStatusCode());
        response.setResponseText(apiResponse.getMessage());
        response.setApiUrl(TPropertiesConfig.getAxapiServiceSalesOrderUrl());
        response.setTransDate(new Date());
        integrationApiResponseRepository.save(response);
...
//        StatusId idStatus = idmasterService.getStatus(47L, pickupHeader.getWarehouseId(), idmasterAuthToken.getAccess_token());
    ...
    private InventoryV2 updateInventory1V2(PickupLineV2 pickupLine, Long statusId) {
        ...
        TTStorageBinV2 storageBin = mastersService.getStorageBin(pickupLine.getCompanyCodeId(), pickupLine.getPlantId(), pickupLine.getLanguageId(),
