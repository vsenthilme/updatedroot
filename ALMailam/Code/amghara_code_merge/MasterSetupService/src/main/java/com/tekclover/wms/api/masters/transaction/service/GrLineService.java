...
import com.tekclover.wms.api.masters.transaction.model.IKeyValuePair;
...
import com.tekclover.wms.api.masters.transaction.model.errorlog.ErrorLog;
import com.tekclover.wms.api.masters.transaction.model.impl.GrLineImpl;
import com.tekclover.wms.api.masters.transaction.model.inbound.InboundLine;
import com.tekclover.wms.api.masters.transaction.model.inbound.gr.*;
import com.tekclover.wms.api.masters.transaction.model.inbound.gr.v2.AddGrLineV2;
import com.tekclover.wms.api.masters.transaction.model.inbound.gr.v2.GrHeaderV2;
import com.tekclover.wms.api.masters.transaction.model.inbound.gr.v2.GrLineV2;
import com.tekclover.wms.api.masters.transaction.model.inbound.gr.v2.SearchGrLineV2;
import com.tekclover.wms.api.masters.transaction.model.inbound.inventory.Inventory;
import com.tekclover.wms.api.masters.transaction.model.inbound.inventory.InventoryMovement;
import com.tekclover.wms.api.masters.transaction.model.inbound.inventory.v2.IInventoryImpl;
import com.tekclover.wms.api.masters.transaction.model.inbound.inventory.v2.InventoryV2;
import com.tekclover.wms.api.masters.transaction.model.inbound.preinbound.v2.PreInboundHeaderV2;
import com.tekclover.wms.api.masters.transaction.model.inbound.putaway.PutAwayHeader;
import com.tekclover.wms.api.masters.transaction.model.inbound.putaway.v2.PutAwayHeaderV2;
import com.tekclover.wms.api.masters.transaction.model.inbound.putaway.v2.PutAwayLineV2;
import com.tekclover.wms.api.masters.transaction.model.inbound.staging.StagingLineEntity;
import com.tekclover.wms.api.masters.transaction.model.inbound.staging.v2.StagingLineEntityV2;
import com.tekclover.wms.api.masters.transaction.model.inbound.v2.InboundLineV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.pickup.v2.PickupLineV2;
import com.tekclover.wms.api.masters.transaction.repository.*;
import com.tekclover.wms.api.masters.transaction.repository.specification.GrLineSpecification;
import com.tekclover.wms.api.masters.transaction.repository.specification.GrLineV2Specification;
...
public class GrLineService extends BaseService {
    @Autowired
    private TStorageBinRepository TStorageBinRepository;
    ...
    @Autowired
    private TAuthTokenService TAuthTokenService;

    @Autowired
    private MastersService mastersService;

    @Autowired
    private TImBasicData1Repository imbasicdata1RepositoryT;
    ...
    public List<PackBarcode> generatePackBarcode(Long acceptQty, Long damageQty, String warehouseId, String loginUserID) {
        AuthToken authTokenForIDMasterService = TAuthTokenService.getIDMasterServiceAuthToken();
        long NUM_RAN_ID = 6;
    ...
    public List<GrLine> createGrLine(@Valid List<AddGrLine> newGrLines, String loginUserID)
        ...
        try {
             ...
             */
            AuthToken authTokenForIDService = TAuthTokenService.getIDMasterServiceAuthToken();
...
//							createdGRLine.getItemCode(), 1L, 0L);
            ...
            AuthToken authTokenForMastersService = TAuthTokenService.getMastersServiceAuthToken();
            if (!stBinInventoryList.isEmpty()) {
                ...
                TStorageBin[] TStorageBin = mastersService.getStorageBin(storageBinPutAway, authTokenForMastersService.getAccess_token());
                log.info("storagebin -----------> : " + TStorageBin);
                if (TStorageBin != null && TStorageBin.length > 0) {
                    putAwayHeader.setProposedStorageBin(TStorageBin[0].getStorageBin());
                }
            } else {
                ...
                if (createdGRLine.getQuantityType().equalsIgnoreCase("A")) {
                    TStorageBin[] TStorageBinEMPTY =
                            mastersService.getStorageBinByStatus(createdGRLine.getWarehouseId(), 0L, authTokenForMastersService.getAccess_token());
                    log.info("storageBinEMPTY -----------> : " + TStorageBinEMPTY);
                    List<TStorageBin> TStorageBinEMPTYList = Arrays.asList(TStorageBinEMPTY);
                    List<String> stBins = TStorageBinEMPTYList.stream().map(TStorageBin::getStorageBin).collect(Collectors.toList());
                    ...
                    if (stBins != null && stBins.size() > 2000) {
                        ...
                        TStorageBin[] TStorageBin = getStorageBinForSplitedList(splitedList, storageSectionIds,
                                createdGRLine.getWarehouseId(), authTokenForMastersService.getAccess_token());

                        // Provided Null else validation
                        log.info("storageBin2 -----------> : " + TStorageBin);
                        if (TStorageBin != null && TStorageBin.length > 0) {
                            putAwayHeader.setProposedStorageBin(TStorageBin[0].getStorageBin());
                        } else {
                            Long binClassID = 2L;
                            TStorageBin stBin = mastersService.getStorageBin(createdGRLine.getWarehouseId(), binClassID, authTokenForMastersService.getAccess_token());
                            putAwayHeader.setProposedStorageBin(stBin.getStorageBin());
                        }
                    } else {
                        TStorageBin[] TStorageBin = getStorageBin(storageSectionIds, stBins, createdGRLine.getWarehouseId(), authTokenForMastersService.getAccess_token());
                        if (TStorageBin != null && TStorageBin.length > 0) {
                            putAwayHeader.setProposedStorageBin(TStorageBin[0].getStorageBin());
                        } else {
                            Long binClassID = 2L;
     ...
     * @return
     */
    private TStorageBin[] getStorageBinForSplitedList(List<List<String>> splitedList, List<String> storageSectionIds, String warehouseId, String authToken) {
        for (List<String> list : splitedList) {
            TStorageBin[] TStorageBin = getStorageBin(storageSectionIds, list, warehouseId, authToken);
            if (TStorageBin != null && TStorageBin.length > 0) {
                return TStorageBin;
            }
        }
     ...
     * @return
     */
    private TStorageBin[] getStorageBin(List<String> storageSectionIds, List<String> stBins, String warehouseId, String authToken) {
        StorageBinPutAway storageBinPutAway = new StorageBinPutAway();
        storageBinPutAway.setStorageBin(stBins);
        storageBinPutAway.setStorageSectionIds(storageSectionIds);
        storageBinPutAway.setWarehouseId(warehouseId);
        TStorageBin[] TStorageBin = mastersService.getStorageBin(storageBinPutAway, authToken);
        return TStorageBin;
    }
    ...
    private Inventory createInventory(GrLine createdGRLine) {
        ...
        AuthToken authTokenForMastersService = TAuthTokenService.getMastersServiceAuthToken();
        TStorageBin TStorageBin = mastersService.getStorageBin(createdGRLine.getWarehouseId(), 3L, authTokenForMastersService.getAccess_token());
        inventory.setStorageBin(TStorageBin.getStorageBin());

        List<IImbasicData1> imbasicdata1 = imbasicdata1RepositoryT.findByItemCode(inventory.getItemCode());
        ...
        if (TStorageBin != null) {
            inventory.setReferenceField10(TStorageBin.getStorageSectionId());
            inventory.setReferenceField5(TStorageBin.getAisleNumber());
            inventory.setReferenceField6(TStorageBin.getShelfId());
            inventory.setReferenceField7(TStorageBin.getRowId());
        }
...
//        while (true) {
            ...
            if (createdGRLine != null) {
                ...
                if (createdGRLine.getInterimStorageBin() != null) {                         //Direct Stock Receipt - Fixed Bin - Inbound OrderTypeId - 5
                    ...
                    TTStorageBinV2 storageBin = null;
                    try {
                ...
                if (createdGRLine.getInboundOrderTypeId() == 2) {
                    ...
                    TTStorageBinV2 proposedBinClass7Bin = mastersService.getStorageBinBinClassId7(storageBinPutAway, authTokenForMastersService.getAccess_token());
                    ...
                    if (proposedBinClass7Bin == null) {
                        binClassId = 2L;
                        log.info("BinClassId : " + binClassId);
                        TTStorageBinV2 stBin = mastersService.getStorageBin(
                    ...
                    if (stBinInventoryList != null) {
                        ...
                        if (inventoryStorageBinList != null && !inventoryStorageBinList.isEmpty()) {
                            if(createdGRLine.getQuantityType().equalsIgnoreCase("A")) {
                                ...
                                TTStorageBinV2 proposedExistingBin = mastersService.getExistingStorageBinNonCbm(storageBinPutAway, authTokenForMastersService.getAccess_token());
...
//                        }
                ...
                if (putAwayHeader.getProposedStorageBin() == null && (stBinInventoryList == null || stBinInventoryList.isEmpty())) {
                    ...
                    storageBinPutAway.setStatusId(0L);
                    TTStorageBinV2 proposedNonCbmStorageBin = null;
                    ...
                    if (existingBinPutAwayLineItemCheck != null) {
                        ...
                        if (existingBinPutAwayLineItemCheck.getLevelId() != null) {
                            ...
                            storageBinPutAway.setBin(proposedStorageBin);
                            TTStorageBinV2 getLevelIdForProposedBin = mastersService.getaStorageBinV2(storageBinPutAway, authTokenForMastersService.getAccess_token());
        ...
        try {
            ...
            for (AddGrLineV2 newGrLine : newGrLines) {
                ...
                for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
                    ...
                    TImBasicData1 itemCodeCapacityCheck = mastersService.getImBasicData1ByItemCodeV2(imBasicData, authTokenForMastersService.getAccess_token());
...
//                inventoryV2Repository.findByWarehouseIdAndItemCodeAndBinClassIdAndDeletionIndicator(warehouseId, itemCode, binClassId, 0L);
                ...
                List<TTStorageBinV2> stBinList = new ArrayList<>();
                List<TTStorageBinV2> filterStorageBinList = new ArrayList<>();
                ...
                if (createdGRLine.getInterimStorageBin() != null) {
                    ...
                    TTStorageBinV2 storageBin = mastersService.getStorageBinV2(proposedStorageBin, warehouseId, companyCode, plantId, languageId, authTokenForMastersService.getAccess_token());
                    log.info("InterimStorageBin: " + storageBin);
                    if (storageBin != null) {
                ...
                if (createdGRLine.getInterimStorageBin() == null && putAwayHeader.getProposedStorageBin() == null) {
                    if (stBinInventoryList != null) {
                        ...
                        if (stBinInventoryList != null) {
                            if (capacityCheck) {
                                stBinList = TStorageBinRepository.getStorageBinListByCompanyBranch(itemCode, warehouseId, companyCode, plantId, languageId,
                        ...
                        if (capacityCheck) {
                            ...
                            for (TTStorageBinV2 TStorageBinV2 : stBinList) {                   //filter capacity enabled bin
                                storageBinCapacityCheck = TStorageBinV2.isCapacityCheck();
                                if (TStorageBinV2.getRemainingVolume() != null && !TStorageBinV2.getRemainingVolume().equals("")) {
                                    remainingVolume = Double.valueOf(TStorageBinV2.getRemainingVolume());
                                }
                                if (storageBinCapacityCheck && cbmPerQuantity < remainingVolume) {
                                    filterStorageBinList.add(TStorageBinV2);
                                }
                            }
                            log.info("capacity check storagebin -----------> : " + filterStorageBinList);
                            if (filterStorageBinList != null && filterStorageBinList.size() > 0) {
                                filterStorageBinList.stream().sorted(Comparator.comparing(TTStorageBinV2::getRemainingVolume)).collect(Collectors.toList());

                                TTStorageBinV2 proposedBin = filterStorageBinList.get(0);
...
//                }
                ...
                if (putAwayHeader.getProposedStorageBin() == null && (stBinInventoryList == null || filterStorageBinList == null || filterStorageBinList.isEmpty())) {
                    ...
                    TTStorageBinV2 proposedCBMStorageBin = null;
                    TTStorageBinV2 proposedCbmPerQtyStorageBin = null;
                    ...
                    if (lastPickedStorageBinList != null && !lastPickedStorageBinList.isEmpty()) {
                        ...
                        if (!capacityCheck) {
                            ...
                            TTStorageBinV2 proposedNonCbmLastPickStorageBin = mastersService.getStorageBinNonCbmLastPicked(storageBinPutAway, authTokenForMastersService.getAccess_token());
                            log.info("proposedNonCbmLastPickStorageBin: " + proposedNonCbmLastPickStorageBin);
                            if (proposedNonCbmLastPickStorageBin != null) {
...
//        }
    ...
    public void updateStorageBin(Double remainingVolume, Double occupiedVolume, Double allocatedVolume,
        ...
        TTStorageBinV2 modifiedStorageBin = new TTStorageBinV2();
        modifiedStorageBin.setRemainingVolume(String.valueOf(remainingVolume));
        modifiedStorageBin.setAllocatedVolume(allocatedVolume);
        ...
        TTStorageBinV2 updateTStorageBinV2 = mastersService.updateStorageBinV2(storageBin,
                modifiedStorageBin,
                companyCode,
                plantId,
                languageId,
     ...
     * @param authToken
     */
    public void emptyStorageBinUpdate(TTStorageBinV2 storageBin, Double cbm, String loginUserID, String authToken) {
     ...
     * @param authToken
     */
    public Double emptyCbmPerQtyStorageBinUpdate(TTStorageBinV2 storageBin, Double cbm, String loginUserID, String authToken) {
     ...
     * @return
     */
    private String proposedEmptyStorageBin(boolean capacityCheck, TTStorageBinV2[] storageBin, Double cbm, Double
        ...
        if (capacityCheck) {
            ...
            if (stBinList != null && !stBinList.isEmpty()) {
                filterStorageBinList = stBinList.stream().filter(n -> Double.valueOf(n.getRemainingVolume()) >= finalCbm).sorted(Comparator.comparing(TTStorageBinV2::getRemainingVolume)).collect(Collectors.toList());
                ...
                if (filterStorageBinList == null || filterStorageBinList.isEmpty() || filterStorageBinList.size() == 0) {
                    filterStorageBinList = stBinList.stream().filter(n -> Double.valueOf(n.getRemainingVolume()) >= finalCbmPerQty).sorted(Comparator.comparing(TTStorageBinV2::getRemainingVolume)).collect(Collectors.toList());
        ...
        if (!capacityCheck) {
            ...
            if (stBinList != null && !stBinList.isEmpty()) {
                ...
                TTStorageBinV2 updateStorageBinStatus = stBinList.get(0);
     ...
     * @return
     */
    private TTStorageBinV2[] getStorageBinForSplitedListV2(List<List<String>> splitedList, Long
            binClassId, String companyCodeId, String plantId, String languageId, String warehouseId, String authToken) {
        for (List<String> list : splitedList) {
            TTStorageBinV2[] storageBin = getStorageBinV2(binClassId, list, companyCodeId, plantId, languageId, warehouseId, authToken);
            if (storageBin != null && storageBin.length > 0) {
                return storageBin;
            }
        }
     ...
     * @return
     */
    private TTStorageBinV2[] getStorageBinV2(Long binClassId, List<String> stBins, String companyCodeId, String plantId,
        ...
        storageBinPutAway.setLanguageId(languageId);
        storageBinPutAway.setWarehouseId(warehouseId);
        TTStorageBinV2[] storageBin = mastersService.getStorageBinV2(storageBinPutAway, authToken);
        return storageBin;
    }
    ...
    private InventoryV2 createInventoryNonCBMV2(GrLineV2 createdGRLine) {
        ...
        try {
            ...
            if (dbInventory == null) {
                ...
                // ST_BIN ---Pass WH_ID/BIN_CL_ID=3 in STORAGEBIN table and fetch ST_BIN value and update
                AuthToken authTokenForMastersService = TAuthTokenService.getMastersServiceAuthToken();
                TStorageBin TStorageBin = mastersService.getStorageBin(
                        createdGRLine.getCompanyCode(),
                        createdGRLine.getPlantId(),
                        createdGRLine.getLanguageId(),
                ...
                if (TStorageBin != null) {
                    ...
                    inventory.setReferenceField6(TStorageBin.getShelfId());
                    inventory.setReferenceField7(TStorageBin.getRowId());
                    inventory.setLevelId(String.valueOf(TStorageBin.getFloorId()));
                }
...
//            inventory.setPackBarcodes(createdGRLine.getPackBarcodes());
    ...
    private void createGrLineLog8(TImBasicData1 TImBasicData1, String error) {

        ErrorLog errorLog = new ErrorLog();
        errorLog.setOrderTypeId(TImBasicData1.getUomId());
        errorLog.setOrderDate(new Date());
        errorLog.setLanguageId(TImBasicData1.getLanguageId());
        errorLog.setCompanyCodeId(TImBasicData1.getCompanyCodeId());
        errorLog.setPlantId(TImBasicData1.getPlantId());
        errorLog.setWarehouseId(TImBasicData1.getWarehouseId());
        errorLog.setErrorMessage(error);
        errorLog.setCreatedBy("MSD_API");
        errorLog.setCreatedOn(new Date());
        exceptionLogRepo.save(errorLog);
    ...
    private void createGrLineLog9(TTStorageBinV2 TStorageBinV2, String error) {

        ErrorLog errorLog = new ErrorLog();
        errorLog.setOrderTypeId(TStorageBinV2.getStorageBin());
        errorLog.setOrderDate(new Date());
        errorLog.setLanguageId(TStorageBinV2.getLanguageId());
        errorLog.setCompanyCodeId(TStorageBinV2.getCompanyCodeId());
        errorLog.setPlantId(TStorageBinV2.getPlantId());
        errorLog.setWarehouseId(TStorageBinV2.getWarehouseId());
        errorLog.setErrorMessage(error);
        errorLog.setCreatedBy("MSD_API");
        errorLog.setCreatedOn(new Date());
        exceptionLogRepo.save(errorLog);
