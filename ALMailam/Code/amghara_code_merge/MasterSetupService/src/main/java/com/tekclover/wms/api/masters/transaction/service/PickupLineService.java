...
import com.tekclover.wms.api.masters.transaction.model.inbound.inventory.*;
...
import com.tekclover.wms.api.masters.transaction.model.outbound.OutboundHeader;
import com.tekclover.wms.api.masters.transaction.model.outbound.OutboundLine;
import com.tekclover.wms.api.masters.transaction.model.outbound.UpdateOutboundLine;
import com.tekclover.wms.api.masters.transaction.model.outbound.ordermangement.v2.OrderManagementLineV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.pickup.*;
...
import com.tekclover.wms.api.masters.transaction.model.outbound.pickup.v2.SearchPickupLineV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.preoutbound.PreOutboundHeader;
import com.tekclover.wms.api.masters.transaction.model.outbound.preoutbound.v2.PreOutboundHeaderV2;
import com.tekclover.wms.api.masters.transaction.model.outbound.quality.AddQualityHeader;
...
import com.tekclover.wms.api.masters.transaction.repository.specification.PickupLineSpecification;
import com.tekclover.wms.api.masters.transaction.repository.specification.PickupLineV2Specification;
...
public class PickupLineService extends BaseService {
    ...
    @Autowired
    private TImPartnerService TImPartnerService;
...
//				+ ",preOutboundNo:" + preOutboundNo + ",refDocNumber:" + refDocNumber + ",partnerCode:" + partnerCode + " doesn't exist.");
    ...
    public List<PickupLine> createPickupLine(@Valid List<AddPickupLine> newPickupLines, String loginUserID)
        ...
        for (PickupLine dbPickupLine : createdPickupLineList) {
                        ...
                        if (INV_QTY == 0) {
                            ...
                            try {
                                ...
                                if (inventoryByStBin == null) {
                                    TStorageBin dbTStorageBin = mastersService.getStorageBin(inventory.getStorageBin(),
                                            dbPickupLine.getWarehouseId(), authTokenForMastersService.getAccess_token());
                                    dbTStorageBin.setStatusId(0L);
                                    dbTStorageBin.setWarehouseId(dbPickupLine.getWarehouseId());
                                    mastersService.updateStorageBin(inventory.getStorageBin(), dbTStorageBin, loginUserID,
            ...
            TStorageBin TStorageBin = mastersService.getStorageBin(dbPickupLine.getWarehouseId(), BIN_CLASS_ID,
                    authTokenForMastersService.getAccess_token());
            Inventory existingInventory = inventoryService.getInventory(dbPickupLine.getWarehouseId(),
                    dbPickupLine.getPickedPackCode(), dbPickupLine.getItemCode(), TStorageBin.getStorageBin());
            if (existingInventory != null) {
                ...
                if (dbPickupLine.getStatusId() == 50L) {
                    try {
                        ...
                        newInventory.setStorageBin(TStorageBin.getStorageBin());
                        ...
                        List<IImbasicData1> imbasicdata1 = imbasicdata1RepositoryT
                        ...
                        if (TStorageBin != null) {
                            newInventory.setReferenceField10(TStorageBin.getStorageSectionId());
                            newInventory.setReferenceField5(TStorageBin.getAisleNumber());
                            newInventory.setReferenceField6(TStorageBin.getShelfId());
                            newInventory.setReferenceField7(TStorageBin.getRowId());
                        }
            ...
            if (dbPickupLine.getStatusId() == 50L) {
                ...
                try {
                    ...
                    stBin = TStorageBin.getStorageBin();
                    movementQtyValue = "P";
                    inventoryMovement = createInventoryMovement(dbPickupLine, subMvtTypeId, movementDocumentNo, stBin,
                            movementQtyValue, loginUserID);
                        ...
                        if (INV_QTY == 0) {
                            ...
                            try {
                                ...
                                if (inventoryByStBin == null || (inventoryByStBin != null && inventoryByStBin.getReferenceField4() == 0)) {
                                    TTStorageBinV2 dbStorageBin = mastersService.getStorageBinV2(inventory.getStorageBin(),
                                            dbPickupLine.getWarehouseId(),
                                            dbPickupLine.getCompanyCodeId(),
                                            dbPickupLine.getPlantId(),
                                            dbPickupLine.getLanguageId(),
...
//                dbPickupLine.setBarcodeId(dbOrderManagementLine.getBarcodeId());
        ...
        for (PickupLineV2 dbPickupLine : createdPickupLineList) {
            ...
            TTStorageBinV2 capStorageBin = TStorageBinRepository.getStorageBin(
                    dbPickupLine.getCompanyCodeId(), dbPickupLine.getPlantId(), dbPickupLine.getLanguageId(),
                ...
                if (dbPickupLine.getAllocatedQty() > 0D) {
                    try {
                        ...
                        if (capacityCheck) {
                            ...
                            TTStorageBinV2 dbStorageBin = mastersService.getStorageBinV2(inventory.getStorageBin(), dbPickupLine.getWarehouseId(), authTokenForMastersService.getAccess_token());
...
//        inventoryMovement.setMovementDocumentNo(movementDocumentNo);
     ...
     */
//    @Transactional
    public TImPartner updatePickupLineForBarcodeV2(UpdateBarcodeInput updateBarcodeInput) {
        ...
        if (updateBarcodeInput != null) {
            ...
            TImPartner updateBarcode = TImPartnerService.updateImPartner(companyCodeId, plantId, languageId, warehouseId, itemCode, manufacturerName, barcodeId, loginUserID);
