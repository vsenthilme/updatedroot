...
import com.tekclover.wms.api.masters.transaction.model.cyclecount.periodic.PeriodicLine;
import com.tekclover.wms.api.masters.transaction.model.cyclecount.perpetual.PerpetualLine;
import com.tekclover.wms.api.masters.transaction.model.dto.BusinessPartner;
import com.tekclover.wms.api.masters.transaction.model.dto.TImBasicData1;
import com.tekclover.wms.api.masters.transaction.model.dto.TStorageBin;
...
import com.tekclover.wms.api.masters.transaction.model.impl.StockReportImpl;
import com.tekclover.wms.api.masters.transaction.model.inbound.InboundLine;
import com.tekclover.wms.api.masters.transaction.model.inbound.gr.GrHeader;
...
import com.tekclover.wms.api.masters.transaction.model.inbound.inventory.SearchInventory;
...
import com.tekclover.wms.api.masters.transaction.model.report.*;
import com.tekclover.wms.api.masters.transaction.repository.*;
import com.tekclover.wms.api.masters.transaction.repository.specification.StockMovementReportNewSpecification;
import com.tekclover.wms.api.masters.transaction.repository.specification.StockReportOutputSpecification;
import com.tekclover.wms.api.masters.transaction.repository.specification.TransactionHistoryReportSpecification;
...
public class ReportsService extends BaseService {
    ...
    @Autowired
    TStorageBinRepository storagebinRepositoryT;

    @Autowired
    TImBasicData1Repository imbasicdata1RepositoryT;
    ...
    public Page<InventoryReport> getInventoryReport(List<String> warehouseId, List<String> itemCode, String storageBin,
        ...
        try {
            ...
            if (stSectionIds != null) {
                TStorageBin[] dbTStorageBin = mastersService.getStorageBinBySectionId(warehouseId.get(0), stSectionIds,
                        authTokenForMastersService.getAccess_token());
                List<String> stBins = Arrays.asList(dbTStorageBin).stream().map(TStorageBin::getStorageBin)
            ...
            for (Inventory dbInventory : inventoryList) {
                 ...
                 * values
                 */
                TImBasicData1 TImBasicData1 = mastersService.getImBasicData1ByItemCode(dbInventory.getItemCode(),
                        dbInventory.getWarehouseId(), authTokenForMastersService.getAccess_token());
                log.info("imBasicData1 : " + TImBasicData1);

                if (TImBasicData1 != null) {
                    reportInventory.setDescription(TImBasicData1.getDescription());
                }
                 ...
                 * ST_SEC_ID values
                 */
                TStorageBin stBin = mastersService.getStorageBin(dbInventory.getStorageBin(), warehouseId.get(0),
                        authTokenForMastersService.getAccess_token());
...
//	}
    ...
    private List<StockMovementReport> fillData(List<InventoryMovement> inventoryMovementSearchResults) {
        ...
        for (InventoryMovement inventoryMovement : inventoryMovementSearchResults) {
             ...
             * and fetch MFR_SKU values
             */
            TImBasicData1 TImBasicData1 = mastersService.getImBasicData1ByItemCode(inventoryMovement.getItemCode(),
                    inventoryMovement.getWarehouseId(), authTokenForMastersService.getAccess_token());
            stockMovementReport.setManufacturerSKU(TImBasicData1.getManufacturerPartNo());

            // ITEM_TEXT
            stockMovementReport.setItemText(TImBasicData1.getDescription());
    ...
    public List<ShipmentDeliveryReport> getShipmentDeliveryReport(String warehouseId, String fromDeliveryDate,
        ...
        try {
            ...
            for (OutboundLine outboundLine : outboundLineSearchResults) {
                 ...
                 * MFR_PART
                 */
                TImBasicData1 TImBasicData1 = mastersService.getImBasicData1ByItemCode(outboundLine.getItemCode(),
                        warehouseId, authTokenForMastersService.getAccess_token());
                if (TImBasicData1 != null) {
                    shipmentDelivery.setManfCode(TImBasicData1.getManufacturerPartNo());
                }
    ...
    private double getInventoryQty(String warehouseId, String itemCode, Long stockTypeId,
        ...
        try {
            ...
            if (!stBinInventoryList.isEmpty()) {
                ...
                List<TStorageBin> storagebinList = storagebinRepositoryT
                ...
                if (storagebinList != null && !storagebinList.isEmpty()) {
                    List<String> storageBinList = storagebinList.stream().map(TStorageBin::getStorageBin)
                            .collect(Collectors.toList());
...
//		specialCount = getNormalNSpecialCount(warehouseId, beginningOfMonth, new Date(), "S");
        ...
        long statusCount = storagebinRepositoryT.countByWarehouseIdAndStatusIdAndDeletionIndicator(warehouseId, 0L, 0L);
        binStatus.setStatusEqualToZeroCount(statusCount);

        statusCount = storagebinRepositoryT.countByWarehouseIdAndStatusIdNotAndDeletionIndicator(warehouseId, 0L, 0L);
