...
import com.tekclover.wms.api.masters.transaction.model.dto.IInventory;
...
import com.tekclover.wms.api.masters.transaction.model.impl.InventoryImpl;
...
import com.tekclover.wms.api.masters.transaction.model.inbound.inventory.v2.SearchInventoryV2;
import com.tekclover.wms.api.masters.transaction.repository.ErrorLogRepository;
import com.tekclover.wms.api.masters.transaction.repository.InventoryMovementRepository;
import com.tekclover.wms.api.masters.transaction.repository.InventoryRepository;
import com.tekclover.wms.api.masters.transaction.repository.InventoryV2Repository;
import com.tekclover.wms.api.masters.transaction.repository.specification.InventorySpecification;
import com.tekclover.wms.api.masters.transaction.repository.specification.InventoryV2Specification;
...
//			dbInventoryMovement.setRefDocNumber("");
    ...
    public void createAuditLogRecord(String companyCodeId, String plantId, String warehouseId,
        ...
        TAuditLog TAuditLog = new TAuditLog();

        TAuditLog.setCompanyCode(companyCodeId);

        TAuditLog.setPlantID(plantId);

        TAuditLog.setWarehouseId(warehouseId);

        TAuditLog.setFiscalYear(DateUtils.getCurrentYear());

        TAuditLog.setObjectName(objectName);

        TAuditLog.setTableName(tableName);

        TAuditLog.setRefDocNumber(refDocNumber);

        // MOD_FIELD
        TAuditLog.setModifiedField(modifiedField);

        // OLD_VL
        TAuditLog.setOldValue(oldValue);

        // NEW_VL
        TAuditLog.setNewValue(newValue);

        // CTD_BY
        TAuditLog.setCreatedBy(loginUserID);

        // CTD_ON
        TAuditLog.setCreatedOn(new Date());

        // UTD_BY
        TAuditLog.setUpdatedBy(loginUserID);

        // UTD_ON
        TAuditLog.setUpdatedOn(new Date());

        TAuditLog.setReferenceField1(packBarcodes);
        TAuditLog.setReferenceField2(itemCode);
        TAuditLog.setReferenceField3(storageBin);
        TAuditLog.setReferenceField4(String.valueOf(stockTypeId));
        TAuditLog.setReferenceField5(String.valueOf(specialStockIndicatorId));

        TAuditLogService.createAuditLog(TAuditLog, loginUserID);
    }
