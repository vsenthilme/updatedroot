...
//            return obList.toArray(new OrderManagementLineV2[obList.size()]);
    ...
    // POST - findOrderManagementLine - assignment tab
    public OrderManagementLineV2[] findOrderManagementLinesV2(SearchOrderManagementLineV2 searchOrderManagementLine,
                                                              String authToken) throws ParseException {
        try {
            HttpHeaders headers = new HttpHeaders();
            headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
            ...
            UriComponentsBuilder builder = UriComponentsBuilder
                    .fromHttpUrl(getTransactionServiceApiUrl() + "ordermanagementline/v2/findOrderManagementLines");
            HttpEntity<?> entity = new HttpEntity<>(searchOrderManagementLine, headers);
            ResponseEntity<OrderManagementLineV2[]> result = getRestTemplate().exchange(builder.toUriString(),
                    HttpMethod.POST, entity, OrderManagementLineV2[].class);
            log.info("result : " + result.getStatusCode());
...
//            return obList.toArray(new OutboundReversalV2[obList.size()]);
    ...
    // POST
    public void doCancelLinesBatchV2(List<ReversalInput> outboundReversalInput, String loginUserID, String authToken) {
        try {
            HttpHeaders headers = new HttpHeaders();
            headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
            headers.add("User-Agent", "ClassicWMS RestTemplate");
            ...
            UriComponentsBuilder builder = UriComponentsBuilder
                    .fromHttpUrl(getTransactionServiceApiUrl() + "outboundline/v2/cancel/batch")
                    .queryParam("loginUserID", loginUserID);
            HttpEntity<?> entity = new HttpEntity<>(outboundReversalInput, headers);
