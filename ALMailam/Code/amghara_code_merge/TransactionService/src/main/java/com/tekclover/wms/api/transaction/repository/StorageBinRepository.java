...
public interface StorageBinRepository extends JpaRepository<StorageBin, Long>,
    ...
    @Query(value = "SELECT top 1 st_bin FROM tblstoragebin WHERE bin_cl_id = :binclassId and c_id = :companyCode and plant_id = :plantId and \n" +
            ...
            "is_deleted = 0 and st_bin <> :dsrBin order by st_bin", nativeQuery = true)
    public String getStorageBinV2(@Param("companyCode") String companyCode,
                                  @Param("plantId") String plantId,
                                  @Param("languageId") String languageId,
                                  @Param("warehouseId") String warehouseId,
                                  @Param("binclassId") Long binclassId,
                                  @Param("statusId") Long statusId,
                                  @Param("dsrBin") String dsrBin);
