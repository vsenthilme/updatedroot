...
public class GrLineService extends BaseService {
    ...
    String REC_AL_B2 = "REC-AL-B2"; //Direct stock receipt bin
            ...
            for (AddGrLineV2 newGrLine : newGrLines) {
                ...
                for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
                    ...
                    if (oldGrLine == null || oldGrLine.isEmpty()) {
                        ...
                        log.info("PutawayHeader create started...! " + new Date());
                        if (createdGRLine != null && !createGrLineError) {
                            ...
                            if (createdGRLine.getInboundOrderTypeId() == 2 || createdGRLine.getQuantityType().equalsIgnoreCase("D")) {
                                binClassId = 7L;
                            }

                            // Record Insertion in PUTAWAYHEADER table
                            createPutAwayHeaderNonCBMV2(createdGRLine, binClassId, loginUserID);
                            log.info("PutawayHeader created successfully...! " + new Date());
                        }
                    }
                }
                log.info("Records were inserted successfully...");
...
////        }
//    }
    private void createPutAwayHeaderNonCBMV2(GrLineV2 createdGRLine, Long binClassId, String loginUserID) throws java.text.ParseException {
        String itemCode = createdGRLine.getItemCode();
        String companyCode = createdGRLine.getCompanyCode();
        String plantId = createdGRLine.getPlantId();
        String languageId = createdGRLine.getLanguageId();
        ...
        AuthToken authTokenForIDMasterService = authTokenService.getIDMasterServiceAuthToken();
            ...
            if (createdGRLine != null) {
                ...
                if (createdGRLine.getInterimStorageBin() != null) {                         //Direct Stock Receipt - Fixed Bin - Inbound OrderTypeId - 5
                    ...
                    try {
                        storageBin = getStorageBinV2(companyCode, plantId, languageId, warehouseId, proposedStorageBin);
                    } catch (Exception e) {
                        throw new BadRequestException("Invalid StorageBin");
                    }
                    log.info("InterimStorageBin: " + storageBin);
                ...
                if(createdGRLine.getInterimStorageBin() == null) {
                    StorageBin dbProposedBin = getProposedStorageBin(companyCode, plantId, languageId, warehouseId, itemCode, createdGRLine.getManufacturerName(), binClassId);
                    log.info("ProposedBin : " + dbProposedBin);
                    putAwayHeader.setProposedStorageBin(dbProposedBin.getStorageBin());
                    putAwayHeader.setLevelId(String.valueOf(dbProposedBin.getFloorId()));
                }
                log.info("Proposed Storage Bin: " + putAwayHeader.getProposedStorageBin());
                log.info("Proposed Storage Bin level/Floor Id: " + putAwayHeader.getLevelId());
                putAwayHeader.setProposedHandlingEquipment(createdGRLine.getPutAwayHandlingEquipment());
...
//        }
    ...
    public StorageBin getProposedStorageBin(String companyCode, String plantId, String languageId, String warehouseId,
                                            String itemCode, String manufacturerName, Long binClassId) {
        try {
            log.info("GetProposedBin:---> " + companyCode + "|" + plantId + "|" + languageId + "|" + warehouseId + "|" + itemCode + "|" + manufacturerName);

            //Checking Inventory Table
            String storageBin = inventoryV2Repository.getPutAwayHeaderCreateInventoryV2(companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName, PACK_BARCODE, binClassId);
            log.info("Proposed InventoryBin: " + storageBin);
            if (storageBin != null) {
                return getStorageBinV2(companyCode, plantId, languageId, warehouseId, storageBin);
            }

            //Checking last picked bin in pickupline table
            ...
            if (pickupLine != null) {
                ...
                return getStorageBinV2(companyCode, plantId, languageId, warehouseId, pickupLine.getPickedStorageBin());
            }

            //checking putawayline bin(already confirmed bin) in putawayline table
            ...
            if (putAwayLine != null) {
                ...
                return getStorageBinV2(companyCode, plantId, languageId, warehouseId, putAwayLine.getConfirmedStorageBin());
            }

            //checking putawayheader bin(already proposed bin) in putawayheader table
            ...
            if(putAwayHeader != null) {
                ...
                return getStorageBinV2(companyCode, plantId, languageId, warehouseId, putAwayHeader.getProposedStorageBin());
            }

            //get empty storage bin with status id 0 and bin not proposed in putaway header and direct stock receipt bin
            String emptyBin = storageBinRepository.getStorageBinV2(companyCode, plantId, languageId, warehouseId, binClassId, 0L, REC_AL_B2);
            log.info("Proposed EmptyBin: " + emptyBin);
            if(emptyBin != null) {
                return getStorageBinV2(companyCode, plantId, languageId, warehouseId, emptyBin);
            }

            //Reserve Bin Proposed
            StorageBinV2 reserveBin = getStorageBinByBinClassIdV2(warehouseId, RESERVE_BIN_CLASS_ID, companyCode, plantId, languageId);
            log.info("Proposed ReserveBin: " + reserveBin);
            if(reserveBin != null) {
                log.info("Proposed ReserveBin-Bin: " + reserveBin.getStorageBin());
                return getStorageBinV2(companyCode, plantId, languageId, warehouseId, reserveBin.getStorageBin());
            }
    ...
    public StorageBinV2 getStorageBinByBinClassIdV2(String warehouseId, Long binClassId, String companyCodeId, String plantId, String languageId) {
        Optional<StorageBinV2> storagebin = storageBinRepository.findTopByBinClassIdAndCompanyCodeIdAndPlantIdAndWarehouseIdAndLanguageIdAndDeletionIndicator(
                binClassId,
                companyCodeId,
                plantId,
                warehouseId,
