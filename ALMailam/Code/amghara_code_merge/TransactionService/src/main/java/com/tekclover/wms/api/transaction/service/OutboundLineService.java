...
//                }
    ...
    public void outboundReversalOml(String companyCodeId, String plantId, String languageId, String warehouseId,
                                    String refDocNumber, String preOutboundNo, String itemCode, String manufacturerName,
                                    Long lineNumber, Long orderTypeId, String loginUserID) throws Exception {
        try {
            statusDescription = getStatusDescription(96L, languageId);
            List<PickupLineV2> pickupLines = pickupLineService.getPickupLineForCancelV2(companyCodeId, plantId, languageId, warehouseId, refDocNumber,
                    preOutboundNo, itemCode, manufacturerName, lineNumber, orderTypeId);
            log.info("PickupLine--------for cancel-------> : " + pickupLines.size());
            if (pickupLines != null && !pickupLines.isEmpty()) {
                for(PickupLineV2 pickupLine : pickupLines) {
                    InventoryV2 inventory = inventoryService.getOutboundReversalInventoryV2(companyCodeId, plantId, languageId, warehouseId,
                    ...
                    outboundLineV2Repository.outboundReversalStatusUpdate(companyCodeId, plantId, languageId, warehouseId, refDocNumber,
                            preOutboundNo, itemCode, manufacturerName, lineNumber, orderTypeId, 96L, statusDescription, loginUserID, new Date());
                }
            } else {
                List<OrderManagementLineV2> orderManagementLines = orderManagementLineService.getReversalOrderManagementLinesV2(companyCodeId, plantId, languageId, warehouseId, refDocNumber,
                        preOutboundNo, itemCode, manufacturerName, lineNumber, orderTypeId);
                log.info("OrderManagementLine----for cancel-----> : " + orderManagementLines);
            ...
            //Header statusCheck
            Long headerStatusUpdateCheck = preOutboundHeaderV2Repository.checkReversalHeaderStatusUpdate(companyCodeId, plantId, languageId, warehouseId, refDocNumber, preOutboundNo, 96L);
            log.info("headerStatusUpdateCheck : " + headerStatusUpdateCheck);
            if(headerStatusUpdateCheck == 1) {
                outboundLineV2Repository.outboundReversalHeaderStatusUpdate(companyCodeId, plantId, languageId, warehouseId, refDocNumber, preOutboundNo,  96L, statusDescription, loginUserID, new Date());
            }
     ...
     * @throws Exception
     */
    public List<OutboundReversalV2> batchOutboundReversalV2(List<ReversalInput> outboundReversalInputList, String loginUserID) throws Exception {
        log.info("OutboundReversal Input: " + outboundReversalInputList);
        if(outboundReversalInputList != null && !outboundReversalInputList.isEmpty()) {
            for (ReversalInput outboundReversalInput : outboundReversalInputList){
                outboundReversalOml(outboundReversalInput.getCompanyCodeId(), outboundReversalInput.getPlantId(), outboundReversalInput.getLanguageId(), outboundReversalInput.getWarehouseId(),
                        outboundReversalInput.getRefDocNumber(), outboundReversalInput.getPreOutboundNo(), outboundReversalInput.getItemCode(), outboundReversalInput.getManufacturerName(),
                        outboundReversalInput.getLineReference(), outboundReversalInput.getOrderTypeId(), loginUserID);
            }
        }
        return null;
    }
