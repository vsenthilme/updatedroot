...
//            + "im.WH_ID in (:warehouseId) AND "
    ...
    //getHeaderConfirmation Status
    @Query(value = "SELECT CASE WHEN SUM(CASE WHEN status_id = :statusId AND ref_field_2 = 'true' THEN 1 ELSE 0 END) = COUNT(ref_doc_no) THEN 1 ELSE 0 END AS are_counts_equal \n" +
            "FROM tblinboundline WHERE ref_doc_no = :refDocNo AND pre_ib_no = :preInboundNo AND c_id = :companyCode AND plant_id = :plantId AND \n" +
            "lang_id = :languageId AND wh_id = :warehouseId AND is_deleted = 0 ", nativeQuery = true)
    public Long getHeaderConfirmStatus(@Param("companyCode") String companyCode,
                                       @Param("plantId") String plantId,
                                       @Param("languageId") String languageId,
                                       @Param("warehouseId") String warehouseId,
                                       @Param("refDocNo") String refDocNo,
