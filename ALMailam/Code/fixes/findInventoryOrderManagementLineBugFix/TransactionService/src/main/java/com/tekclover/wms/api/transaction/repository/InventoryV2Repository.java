...
//            @Param(value = "binClassId") Long binClassId,
    ...
    @Query(value = "select max(inv_id) inventoryId into #inv from tblinventory \n"
            + "WHERE is_deleted = 0 " +
            "AND C_ID = :companyCodeId and PLANT_ID = :plantId AND LANG_ID = :languageId AND WH_ID = :warehouseId and ITM_CODE = :itemCode AND MFR_NAME = :manufacturerName \r\n" +
            "group by itm_code,mfr_name,st_bin,plant_id,wh_id,c_id,lang_id \n" +
...
//            + "COALESCE(INV_QTY,0)+COALESCE(ALLOC_QTY,0) referenceField4, \n"
    ...
    @Query (value = "SELECT max(INV_ID) inventoryId into #inv FROM TBLINVENTORY WHERE is_deleted = 0 \n" +
            "AND C_ID = :companyCodeId and PLANT_ID = :plantId AND LANG_ID = :languageId AND WH_ID = :warehouseId and ITM_CODE = :itemCode AND MFR_NAME = :manufacturerName \r\n" +
            "GROUP BY ITM_CODE,MFR_NAME,ST_BIN,PLANT_ID,WH_ID,C_ID,LANG_ID \r\n"
