...
public class PutAwayHeaderSpecification implements Specification<PutAwayHeader> {
    ...
    public Predicate toPredicate(Root<PutAwayHeader> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		 ...
		 if (searchPutAwayHeader.getAssignedUserId() != null && !searchPutAwayHeader.getAssignedUserId().isEmpty()) {
        	 final Path<Group> group = root.<Group> get("assignedUserId");
        	 predicates.add(group.in(searchPutAwayHeader.getAssignedUserId()));
         }
