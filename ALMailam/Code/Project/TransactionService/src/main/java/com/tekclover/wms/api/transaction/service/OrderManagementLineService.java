...
public class OrderManagementLineService extends BaseService {
	 ...
	 * @throws InvocationTargetException
	 */
	public OrderManagementLine doUnAllocation(String warehouseId, String languageId, String preOutboundNo, String refDocNumber,
	 ...
	 * @return
	 */
	public List<OrderManagementLine> doAssignPicker(List<AssignPicker> assignPickers, String assignedPickerId, String languageId,
			String loginUserID) {
		...
		for (AssignPicker assignPicker : assignPickers) {
			...
			if (dupPickupHeader ==  null) {
				...
				AuthToken idmasterAuthToken = authTokenService.getIDMasterServiceAuthToken();
				StatusId idStatus = idmasterService.getStatus(48L, warehouseId, languageId, idmasterAuthToken.getAccess_token());
				...
				if (dbOrderManagementLine.getPickupNumber() == null) {
					...
					String PU_NO = getNextRangeNumber(NUM_RAN_CODE, dbOrderManagementLine.getWarehouseId(),
							dbOrderManagementLine.getCompanyCodeId(),
...
//		for (Inventory stBinInventory : finalInventoryList) {
	...
	public OrderManagementLine updateAllocation(OrderManagementLine orderManagementLine, List<String> storageSectionIds,
		...
		outerloop: for (IInventory stBinWiseInventory : finalInventoryList) {
			...
			for (Inventory stBinInventory : listInventoryForAlloc) {
				...
				StatusId idStatus = idmasterService.getStatus(STATUS_ID, orderManagementLine.getWarehouseId(), orderManagementLine.getLanguageId(), idmasterAuthToken.getAccess_token());
				orderManagementLine.setStatusId(STATUS_ID);
	...
	private OrderManagementLine updateOrderManagementLine(OrderManagementLine orderManagementLine) {
		AuthToken idmasterAuthToken = authTokenService.getIDMasterServiceAuthToken();
		StatusId idStatus = idmasterService.getStatus(47L, orderManagementLine.getWarehouseId(), orderManagementLine.getLanguageId(), idmasterAuthToken.getAccess_token());
