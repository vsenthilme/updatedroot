...
export class HhtuserNewComponent implements OnInit {
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.warehouseId).subscribe(res => {
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code,this.data.warehouseId).subscribe(res => {
