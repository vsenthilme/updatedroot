...
export class StratergyComponent implements OnInit {
  ...
  openDialog(data: any = 'New'): void {
    ...
    let paramdata = "";
    paramdata = this.cs.encrypt({ pageflow: data, code: data != 'New' ? this.selectedstorageclass[0].strategyTypeId : null });
...
if(res){
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.deleterecord(this.selectedstorageclass[0].strategyTypeId);
