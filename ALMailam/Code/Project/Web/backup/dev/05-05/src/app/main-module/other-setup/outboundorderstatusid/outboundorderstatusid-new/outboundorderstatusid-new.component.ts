...
@Component({
  selector: 'app-outboundorderstatusid-new',
  templateUrl: './outboundorderstatusid-new.component.html',
  styleUrls: ['./outboundorderstatusid-new.component.scss']
})
export class OutboundorderstatusidNewComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    outboundOrderStatus: [,Validators.required],
    outboundOrderStatusId: [,Validators.required],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.outboundOrderStatusId.disable();
      ...
      this.form.controls.outboundOrderStatus.disable();
        this.form.disabled;
      this.fill();
  }
  }
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.warehouseId,this.data.languageId,this.data.plantId,this.data.companyCodeId).subscribe(res => {
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code,this.data.warehouseId,this.data.languageId,this.data.plantId,this.data.companyCodeId).subscribe(res => {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.outboundOrderStatusId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
