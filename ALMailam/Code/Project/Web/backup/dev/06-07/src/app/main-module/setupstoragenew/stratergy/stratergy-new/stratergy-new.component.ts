...
export class StratergyNewComponent implements OnInit {
  ...
  selectedStratergyType(e, row){
    this.masterService.searchstrategy({
      ...
      this.stratergyNoList = [];
      res.forEach(element => {
        this.stratergyNoList.push({
    ...
    console.log(row)
    row.controls.sequenceNo.patchValue(noofStratergy.length);
  }


  selectedStratergyNo(e) {
    const optionsCopy = [...this.stratergyNoList]; // Create a copy of the original options array

    for (const selectedValue of e.value) {
      const index = optionsCopy.findIndex(option => option.value === selectedValue); // Find the index of the selected value

      if (index !== -1) {
        optionsCopy.splice(index, 1); // Remove the selected value from the optionsCopy array
      }
    }

    this.stratergyNoList = optionsCopy; // Assign the modified optionsCopy array back to the original options array
    console.log(this.stratergyNoList)
  }
