...
export class UserRoleNewComponent implements OnInit {
  ...
  checkUncheckAll(menu: any, event: any) {
    if (event.checked) { this.menuList.find(x => x.mainMenu == menu).Menu.forEach((x: any) => { x.createUpdate = true, x.delete = true, x.view = true, x.rowSelectAll =  true}) }
    else { this.menuList.find(x => x.mainMenu == menu).Menu.forEach((x: any) => { x.createUpdate = false, x.delete = false, x.view = false, x.rowSelectAll = false }) }
  }
