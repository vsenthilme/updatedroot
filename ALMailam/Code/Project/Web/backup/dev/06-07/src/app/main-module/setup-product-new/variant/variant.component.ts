...
screenid=3011;
  ...
  openDialog(data: any = 'New'): void {
    ...
    let paramdata = "";
    paramdata = this.cs.encrypt({ pageflow: data, code: data != 'New' ? this.selectedvariant[0].variantId : null,companyId: data != 'New' ? this.selectedvariant[0].companyId : null,languageId: data != 'New' ? this.selectedvariant[0].languageId : null,plantId: data != 'New' ? this.selectedvariant[0].plantId : null,warehouseId: data != 'New' ? this.selectedvariant[0].warehouseId : null,levelId: data != 'New' ? this.selectedvariant[0].levelId : null });
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.deleterecord(this.selectedvariant[0].variantId,this.selectedvariant[0].companyId,this.selectedvariant[0].languageId,this.selectedvariant[0].plantId,this.selectedvariant[0].warehouseId,this.selectedvariant[0].levelId,);
  ...
  deleterecord(id: any,companyId:any,languageId:any,plantId:any,warehouseId:any,levelId:any) {
    this.spin.show();
    this.sub.add(this.service.Delete(id,companyId,languageId,plantId,warehouseId,levelId).subscribe((res) => {
