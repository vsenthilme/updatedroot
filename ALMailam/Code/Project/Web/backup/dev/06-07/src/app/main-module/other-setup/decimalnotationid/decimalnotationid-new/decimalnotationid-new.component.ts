...
@Component({
  selector: 'app-decimalnotationid-new',
  templateUrl: './decimalnotationid-new.component.html',
  styleUrls: ['./decimalnotationid-new.component.scss']
})
export class DecimalnotationidNewComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    createdOn: [],
    createdOnFE: [],
    decimalNotation: [,Validators.required],
    decimalNotationId: [,Validators.required],
    deletionIndicator: [],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.decimalNotationId.disable();
      this.form.controls.companyCodeId.disable();
      this.form.controls.plantId.disable();
      this.form.controls.warehouseId.disable();
      this.form.controls.languageId.disable();
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.warehouseId,this.data.companyCodeId,this.data.plantId,this.data.languageId).subscribe(res => {
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code,this.data.warehouseId,this.data.companyCodeId,this.data.plantId,this.data.languageId).subscribe(res => {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.decimalNotationId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
