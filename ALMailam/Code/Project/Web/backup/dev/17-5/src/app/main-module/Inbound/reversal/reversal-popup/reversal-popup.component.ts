...
export class ReversalPopupComponent implements OnInit {
  ...
  getpalletid() {
    this.spin.show();
    this.sub.add(this.service.search({ refDocNumber: [this.refDocNumber] }).subscribe(res => {
      this.spin.hide();
  ...
  reverse() {
    ...
    this.spin.show();
    this.sub.add(this.service.reverse(this.refDocNumber, this.packBarcodes).subscribe(res => {
      this.spin.hide();
