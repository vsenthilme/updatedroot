...
export class ApprovalidNewComponent implements OnInit {
  ...
  dropdownlistSuperAdmin(){
    ...
    this.cas.getalldropdownlist([
      ...
      this.cas.dropdownlist.setup.storageclassid.url,
    this.cas.dropdownlist.setup.plantid.url,
    this.cas.dropdownlist.setup.approvalList.url,
    ]).subscribe((results) => {
      this.languageidList = this.cas.foreachlist2(results[0], this.cas.dropdownlist.setup.languageid.key);
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code,this.auth.warehouseId,this.auth.languageId,this.auth.plantId,this.auth.companyId,this.data.approvalLevel,this.data.approvalProcessId).subscribe(res => {
