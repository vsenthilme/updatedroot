...
@Component({
  selector: 'app-handlingequipmentid-new',
  templateUrl: './handlingequipmentid-new.component.html',
  styleUrls: ['./handlingequipmentid-new.component.scss']
})
export class HandlingequipmentidNewComponent implements OnInit {
 ...
 // dialogRef: any;
  ...
  form = this.fb.group({
    companyCodeId: [,Validators.required],
        ...
        createdOnFE: [],
        deletionIndicator: [],
        handlingEquipmentDescription: [,Validators.required],
        handlingEquipmentId: [,Validators.required],
        handlingEquipmentNumber: [,Validators.required],
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId,this.data.handlingEquipmentNumber).subscribe(res => {
  ...
  onPlantChange(value){
      this.masterService.searchWarehouse({companyCodeId: this.form.value.companyCodeId.value, plantId: value.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
        this.warehouseidList = [];
        res.forEach(element => {
          this.warehouseidList.push({value: element.warehouseId, label: element.warehouseId + '-' + element.warehouseDesc});
        });
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(),this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId,this.data.handlingEquipmentNumber).subscribe(res => {
