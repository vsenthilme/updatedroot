...
@Component({
  selector: 'app-barcodesubtypeid-new',
  templateUrl: './barcodesubtypeid-new.component.html',
  styleUrls: ['./barcodesubtypeid-new.component.scss']
})
export class BarcodesubtypeidNewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.barcodeSubTypeId.disable();
      ...
      if (this.data.pageflow == 'Display')
        this.form.disable();
        this.form.controls.barcodeTypeId.disable();
      this.fill();
  }
  }
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.barcodeTypeId,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId).subscribe(res => {
  ...
  dropdownlistSuperAdmin(){
    ...
    this.cas.getalldropdownlist([
     ...
     this.cas.dropdownlist.setup.barcodetypeid.url,
  ...
  //  this.barcodetypeList=this.cas.foreachlist2(results[3],this.cas.dropdownlist.setup.barcodetypeid.key);
  // this.plantidList = this.cas.forLanguageFilter(results[4], this.cas.dropdownlist.setup.plantid.key);
 ...
 this.masterService.searchPlant({companyCodeId: [this.form.controls.companyCodeId.value], languageId: [this.form.controls.languageId.value]}).subscribe(res => {
  ...
  this.masterService.searchbartype({companyCodeId: this.form.controls.companyCodeId.value, languageId: [this.form.controls.languageId.value],plantId:this.form.controls.plantId.value,warehouseId:this.form.controls.warehouseId.value}).subscribe(res => {
    this.barcodetypeList = [];
    res.forEach(element => {
      this.barcodetypeList.push({value: element.barcodeTypeId, label: element.barcodeTypeId + '-' + element.barcodeType});
    });
  });
  ...
  dropdownlist(){
    ...
    //this.plantidList = this.cas.forLanguageFilter(results[3], this.cas.dropdownlist.setup.plantid.key);
    this.barcodetypeList=this.cas.forLanguageFilter(results[4],this.cas.dropdownlist.setup.barcodetypeid.key);
   this.masterService.searchbartype({companyCodeId: this.auth.companyId, languageId: [this.auth.languageId],plantId:this.auth.plantId,warehouseId:this.auth.warehouseId}).subscribe(res => {
    this.barcodetypeList = [];
    res.forEach(element => {
      this.barcodetypeList.push({value: element.barcodeTypeId, label: element.barcodeTypeId + '-' + element.barcodeType});
    });
  ...
  onLanguageChange(value){
    ...
    this.masterService.searchbartype({companyCodeId: this.form.controls.companyCodeId.value, languageId: [value.value],plantId:this.form.controls.plantId.value,warehouseId:this.form.controls.warehouseId.value}).subscribe(res => {
      this.barcodetypeList = [];
      res.forEach(element => {
        this.barcodetypeList.push({value: element.barcodeTypeId, label: element.barcodeTypeId + '-' + element.barcodeType});
      });
  ...
  onCompanyChange(value){
    ...
    this.masterService.searchbartype({companyCodeId: value.value, languageId: [this.form.controls.languageId.value],plantId:this.form.controls.plantId.value,warehouseId:this.form.controls.warehouseId.value}).subscribe(res => {
      this.barcodetypeList = [];
      res.forEach(element => {
        this.barcodetypeList.push({value: element.barcodeTypeId, label: element.barcodeTypeId + '-' + element.barcodeType});
      });
  ...
  onPlantChange(value){
      ...
      this.masterService.searchbartype({companyCodeId: this.form.controls.companyCodeId.value, languageId: [this.form.controls.languageId.value],plantId:value.value,warehouseId:this.form.controls.warehouseId.value}).subscribe(res => {
      this.barcodetypeList = [];
      res.forEach(element => {
        this.barcodetypeList.push({value: element.barcodeTypeId, label: element.barcodeTypeId + '-' + element.barcodeType});
      });
  ...
  onwarehouseChange(value){
    ...
    this.masterService.searchbartype({companyCodeId: this.form.controls.companyCodeId.value, languageId: [this.form.controls.languageId.value],plantId:this.form.controls.plantId.value,warehouseId:value.value}).subscribe(res => {
    this.barcodetypeList = [];
    res.forEach(element => {
      this.barcodetypeList.push({value: element.barcodeTypeId, label: element.barcodeTypeId + '-' + element.barcodeType});
    });
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(),this.data.code,this.data.barcodeTypeId,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId).subscribe(res => {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.barcodeSubTypeId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
