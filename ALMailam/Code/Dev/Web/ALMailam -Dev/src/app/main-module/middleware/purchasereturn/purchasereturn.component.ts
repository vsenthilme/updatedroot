...
export class PurchasereturnComponent implements OnInit {
  ...
  search(ispageload = false) {
    ...
    this.service.purchasereturn(this.form.value).subscribe(
      (res) => {
        this.spin.hide();
  ...
  reload() {
    ...
    this.sub.add(this.service.purchasereturn({ companyCode:[this.auth.companyId],branchCode:[this.auth.plantId], }).subscribe(res => {
  ...
  downloadexcel() {
    ...
    this.cs.exportAsExcel(res, 'PURCHASE RETURN ');
  }
 
  openConfirm(data: any) {
    let paramdata = this.cs.encrypt({ code: data, pageflow: 'Edit' });
    this.router.navigate(['/main/middleware/purchasereturnlines/' + paramdata]);
  }
}
