...
export class CommonInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if(req.url.split('/')[1] != 'wms-spark-service' || req.url.split('/')[1] != 'wms-spark-service'){
       bearerToken = this.authService.getToken(req.url.split('/')[1]);//sessionStorage.getItem('token');
    }
    ;
