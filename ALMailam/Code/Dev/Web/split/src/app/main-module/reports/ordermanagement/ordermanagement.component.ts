 ...
 orderManagement: any;
  ...
  filtersearch() {
    ...
    this.sub.add(this.reportService.getordermanagement(this.searhform.getRawValue()).subscribe(res => {
      ...
      this.calculateTotals(this.orderManagement)
      this.table = true;
      this.search = false;
  ...
  handleSearch1(filterInput){
    if (filterInput) {
      this.calculateTotals(this.orderManagementTag.filteredValue);
    }
    else {
      this.calculateTotals(this.orderManagement);
  ...
  calculateTotals(data?: any[]): void {
   ...
   this.totalQty1=0;
    ...
    if (data && data.length > 0) {
      ...
      this.totalAllocatedQuantity = data.reduce((total, item) => total + (item.inventoryQty != null ? item.inventoryQty : 0), 0);
      this.totalQty1=data.reduce((total, item) => total + (item.allocatedQty != null ? item.allocatedQty : 0), 0);
