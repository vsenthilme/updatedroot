...
@Component({
  ...
  styleUrls: ['./uom-new.component.scss']
})
export class UomNewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
     ...
     // this.form.controls.palletizationLevelReference.disable();
      ...
      this.form.controls.languageId.disable();
      this.form.controls.plantId.disable();
      this.form.controls.uomType.disable();
      this.form.controls.companyCodeId.disable();
 ...
 this.masterService.searchWarehouse({languageId: [this.data.languageId],companyCodeId:this.data.companyCodeId,plantId:this.data.plantId}).subscribe(res => {
  ...
  if (this.data.code) {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.uomId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
