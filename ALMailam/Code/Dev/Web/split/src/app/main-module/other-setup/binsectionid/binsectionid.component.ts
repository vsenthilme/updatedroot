...
import { BinsectionidNewComponent } from './binsectionid-new/binsectionid-new.component';
import { BinsectionidService } from './binsectionid.service';
import { CommonApiService } from 'src/app/common-service/common-api.service';

@Component({
  selector: 'app-binsectionid',
  templateUrl: './binsectionid.component.html',
  styleUrls: ['./binsectionid.component.scss']
})
export class BinsectionidComponent implements OnInit {
screenid=3110;
  advanceFilterShow: boolean;
  @ViewChild('Setupbinsec') Setupbinsec: Table | undefined;
  binsections: any;
  selectedbinsections : any;
  ...
  constructor(public dialog: MatDialog,
   ...
   // private excel: ExcelService,
    ...
    private service: BinsectionidService) { }
    toggleFloat() {
    ...
    applyFilterGlobal($event: any, stringVal: any) {
      this.Setupbinsec!.filterGlobal(($event.target as HTMLInputElement).value, stringVal);
    }
    RA: any = {};
    ngOnInit(): void {
    ...
    multicountryList:any[]=[];
      ...
      res.forEach((x: { binSectionId: string; binSection: string, }) => {
        this.multidateformatList.push({
          value: x.binSectionId,
          label: x.binSectionId + '-' + x.binSection,
        });
    });
  ...
  console.log(this.selectedbinsections)
    if (data != 'New')
    if (this.selectedbinsections.length === 0) {
    ...
    const dialogRef = this.dialog.open(BinsectionidNewComponent, {
      disableClose: true,
      width: '55%',
      maxWidth: '80%',
      data: { pageflow: data, code: data != 'New' ? this.selectedbinsections[0].binSectionId : null,warehouseId: data != 'New' ? this.selectedbinsections[0].warehouseId : null,languageId: data != 'New' ? this.selectedbinsections[0].languageId : null,companyCodeId: data != 'New' ? this.selectedbinsections[0].companyCodeId : null,plantId: data != 'New' ? this.selectedbinsections[0].plantId : null}
    });
  ...
  deleteDialog() {
    ...
    const dialogRef = this.dialog.open(DeleteComponent, {
      ...
      maxWidth: '80%',
      position: { top: '9%', },
      data: this.selectedbinsections[0].code,
    });
  
    dialogRef.afterClosed().subscribe(result => {
  
      if (result) {
        this.deleterecord(this.selectedbinsections[0].binSectionId,this.selectedbinsections[0].warehouseId,this.selectedbinsections[0].languageId,this.selectedbinsections[0].companyCodeId,this.selectedbinsections[0].plantId);
  ...
  downloadexcel() {
    ...
    this.cs.exportAsExcel(res, "BinSection ID");
  }
  onChange() {
    console.log(this.selectedbinsections.length)
    const choosen= this.selectedbinsections[this.selectedbinsections.length - 1];   
    this.selectedbinsections.length = 0;
    this.selectedbinsections.push(choosen);
  } 
  }
