...
export class QualityMainComponent implements OnInit {
  ...
  onKey(element: any) { // without type info
   ...
   multipleQualityConfirm(){
    ...
    for (let i=0;i<this.selectedquality.length;i++){
    let obj: any = {}
    obj.companyCodeId = [this.auth.companyId];
    obj.plantId = [this.auth.plantId];
    obj.languageId = [this.auth.languageId];
    obj.warehouseId = [this.auth.warehouseId];
    obj.refDocNumber=[this.selectedquality[i].refDocNumber];
    obj.preOutboundNo=[this.selectedquality[i].preOutboundNo];
    this.service.searchv2(obj).subscribe(res => {
      console.log(res);
     filteredData=res;
