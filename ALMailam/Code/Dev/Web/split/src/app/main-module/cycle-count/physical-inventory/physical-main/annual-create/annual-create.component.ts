...
export class AnnualCreateComponent implements OnInit {
  ...
  getAllDropDown() {
    ...
    forkJoin({
      ...
      .subscribe(({ warehouse, storageSection }) => {
        ...
        //for token error
        forkJoin({
          storageBin: this.masterService.getAllStorageBinNew({}).pipe(catchError(err => of(err)))
        })
  ...
  onItemSelect(item: any) {
    this.spin.show();
    forkJoin({
      storageBin: this.masterService.getAllStorageBinNew({storageSectionId: item.value, warehouseId: [this.auth.warehouseId]}).pipe(catchError(err => of(err)))
    })
      .subscribe(({ storageBin }) => {
