...
export class CustomermasterComponent implements OnInit {
  ...
  search(ispageload = false) {
    ...
    this.service.customerMaster(this.form.value).subscribe(
      (res) => {
        this.spin.hide();
  ...
  reload() {
    ...
    this.sub.add(this.service.customerMaster({ companyCode:[this.auth.companyId],branchCode:[this.auth.plantId], }).subscribe(res => {
      res.forEach((x: any) => this.multisalesOrderNo.push({ value: x.customerCode, label: x.customerName }));
  ...
  downloadexcel() {
    ...
    this.salesInvoice.forEach((x) => {
      res.push({
        ...
        'Home Address 1': x.homeAddress1,
        "Home Address 2":x.homeAddress2,
        "Telephone Number":x.homeTelNumber,
        "Mobile Number": x.mobileNumber,
        "Customer Created On": this.cs.dateapiwithTime(x.customerCreationDate),
