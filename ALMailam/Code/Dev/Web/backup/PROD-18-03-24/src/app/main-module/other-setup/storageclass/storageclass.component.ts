...
import { StorageclassNewComponent } from './storageclass-new/storageclass-new.component';
...
export class StorageclassComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'companyCodeId' ,'description','plantId','storageClassId','warehouseId','createdBy','createdOn'];
  ...
  multicountryList:any[]=[];
    ...
    res.forEach((x: { storageClassId: string; description: string, }) => {
      this.multidateformatList.push({
        value: x.storageClassId,
        label: x.storageClassId + '-' + x.description,
      });
  });
...
console.log(this.selectedstorageclass)
  if (data != 'New')
  ...
  const dialogRef = this.dialog.open(StorageclassNewComponent, {
    disableClose: true,
    width: '55%',
    maxWidth: '80%',
    data: { pageflow: data, code: data != 'New' ? this.selectedstorageclass[0].storageClassId : null,warehouseId: data != 'New' ? this.selectedstorageclass[0].warehouseId : null,companyCodeId: data != 'New' ? this.selectedstorageclass[0].companyCodeId : null,plantId: data != 'New' ? this.selectedstorageclass[0].plantId : null,languageId: data != 'New' ? this.selectedstorageclass[0].languageId : null}
  });
...
deleteDialog() {
  ...
  dialogRef.afterClosed().subscribe(result => {
    ...
    if (result) {
      this.deleterecord(this.selectedstorageclass[0].storageClassId,this.selectedstorageclass[0].warehouseId,this.selectedstorageclass[0].companyCodeId,this.selectedstorageclass[0].plantId,this.selectedstorageclass[0].languageId);
...
downloadexcel() {
  ...
  this.cs.exportAsExcel(res, "Storageclass");
}
onChange() {
  console.log(this.selectedstorageclass.length)
