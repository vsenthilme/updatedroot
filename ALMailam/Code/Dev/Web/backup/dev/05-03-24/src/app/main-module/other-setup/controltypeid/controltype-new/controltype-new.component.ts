...
@Component({
  selector: 'app-controltype-new',
  templateUrl: './controltype-new.component.html',
  styleUrls: ['./controltype-new.component.scss']
})
export class ControltypeNewComponent implements OnInit {
 ...
 // dialogRef: any;
  ...
  form = this.fb.group({
    ...
    controlTypeId: [,Validators.required],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.controlTypeId.disable();
 ...
 this.masterService.searchPlant({companyCodeId: [this.form.controls.companyCodeId.value], languageId: [this.form.controls.languageId.value]}).subscribe(res => {
  ...
  if (this.data.code) {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.controlTypeId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
