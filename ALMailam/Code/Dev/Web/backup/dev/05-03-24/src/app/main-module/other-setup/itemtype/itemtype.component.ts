...
import { ItemtypeNewComponent } from './itemtype-new/itemtype-new.component';
...
screenid=3137;
  ...
  console.log(this.selecteditemtype)
    ...
    const dialogRef = this.dialog.open(ItemtypeNewComponent, {
      disableClose: true,
      width: '55%',
      maxWidth: '80%',
      data: { pageflow: data, code: data != 'New' ? this.selecteditemtype[0].itemTypeId : null,warehouseId: data != 'New' ? this.selecteditemtype[0].warehouseId : null,languageId: data != 'New' ? this.selecteditemtype[0].languageId : null,plantId: data != 'New' ? this.selecteditemtype[0].plantId : null,companyCodeId: data != 'New' ? this.selecteditemtype[0].companyCodeId : null}
    });
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.deleterecord(this.selecteditemtype[0].itemTypeId,this.selecteditemtype[0].warehouseId,this.selecteditemtype[0].languageId,this.selecteditemtype[0].plantId,this.selecteditemtype[0].companyCodeId);
  ...
  downloadexcel() {
    ...
    this.itemtype.forEach(x => {
      res.push({
        ...
        "ItemType Id":x.itemTypeId,
       "Description":x.itemType,
       "Created By":x.createdBy,
       "Created On":this.cs.dateapi(x.createdOn),
      });
    })
    this.cs.exportAsExcel(res, "Itemtype");
  }
  onChange() {
