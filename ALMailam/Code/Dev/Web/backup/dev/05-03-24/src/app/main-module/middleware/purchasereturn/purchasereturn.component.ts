...
export class PurchasereturnComponent implements OnInit {
  ...
  multipicklistNo:any[]=[];
  getDropdown() {
    this.sub.add(this.service.purchasereturn({ companyCode:[this.auth.companyId],branchCode:[this.auth.plantId], toOrderReceivedOn: this.cs.todayapi(), fromOrderReceivedOn: this.cs.todayapi(),}).subscribe(res => {
      res.forEach((x: any) => this.multisalesOrderNo.push({ value: x.returnOrderNo, label: x.returnOrderNo }));
    }))
  }
  downloadexcel() {
    ...
    this.cs.exportAsExcel(res, 'Purchase Return Header ');
  }
