...
@Component({
  selector: 'app-variantid-new',
  templateUrl: './variantid-new.component.html',
  styleUrls: ['./variantid-new.component.scss']
})
export class VariantidNewComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    variantSubCode: [,Validators.required],
    variantText: [],
    variantSubType: [,Validators.required],
    deletionIndicator: [],
    createdBy: [],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.variantCode.disable();
     // this.form.controls.palletizationLevelReference.disable();
      this.form.controls.warehouseId.disable();
      this.form.controls.languageId.disable();
      this.form.controls.plantId.disable();
      this.form.controls.variantSubCode.disable();
      this.form.controls.variantSubType.disable();
      this.form.controls.variantType.disable();
      this.form.controls.companyCodeId.disable();
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.variantSubCode,this.data.variantType,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId).subscribe(res => {
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(),this.data.code,this.data.variantSubCode,this.data.variantType,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId).subscribe(res => {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.variantCode + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
