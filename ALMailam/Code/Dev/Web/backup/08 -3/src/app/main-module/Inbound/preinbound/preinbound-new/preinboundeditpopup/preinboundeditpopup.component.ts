...
import {
  PreinboundService
} from '../../preinbound.service';

@Component({
  selector: 'app-preinboundeditpopup',
  templateUrl: './preinboundeditpopup.component.html',
  styleUrls: ['./preinboundeditpopup.component.scss']
})
export class PreinboundeditpopupComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form.controls.manufacturerCode.disable();
      ...
      this.masterService.searchCompany({
        ...
        res.forEach(element => {
          this.companyidList.push({
            value: element.companyCodeId,
            label: element.companyCodeId + '-' + element.description
          });
        });
      });
      this.masterService.searchPlant({
        companyCodeId: [this.data.companyCodeId],
        languageId: [this.data.languageId]
      }).subscribe(res => {
        this.plantidList = [];
        res.forEach(element => {
          this.plantidList.push({
  ...
  fill() {
    ...
    this.form.patchValue(this.data.code[0]);
    console.log(this.form.value);
    console.log(this.data.createdOn);
    console.log(this.data.updatedOn);
    this.form.controls.createdOnFE.patchValue(this.cs.dateTimeApi(this.form.controls.createdOn.value));
    this.form.controls.updatedOnFE.patchValue(this.cs.dateTimeApi(this.form.controls.updatedOn.value));
    console.log(this.form.controls.createdOnFE.value);
    console.log(this.form.controls.updatedOnFE.value);
  }
  submit() {
    this.submitted = true;
    ...
    this.spin.show();
    if (this.data.code) {
      this.sub.add(this.service.Updateline(this.form.getRawValue(), this.form.controls.preInboundNo.value, this.form.controls.warehouseId.value, this.form.controls.companyCode.value, this.form.controls.plantId.value, this.form.controls.languageId.value, this.form.controls.itemCode.value, this.form.controls.lineNo.value, this.form.controls.refDocNumber.value).subscribe(res => {
        this.toastr.success(this.form.controls.preInboundNo.value + " updated successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
