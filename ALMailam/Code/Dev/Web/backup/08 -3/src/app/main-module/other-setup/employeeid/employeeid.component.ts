...
import { EmployeeidNewComponent } from './employeeid-new/employeeid-new.component';
...
screenid=3107;
  ...
  @ViewChild('Setupemployee') Setupemployee: Table | undefined;
  employees: any;
  ...
  constructor(public dialog: MatDialog,
   ...
   // private excel: ExcelService,
    ...
    private service: EmployeeidService ) { }
    toggleFloat() {
    ...
    applyFilterGlobal($event: any, stringVal: any) {
      this.Setupemployee!.filterGlobal(($event.target as HTMLInputElement).value, stringVal);
    }
    RA: any = {};
    ngOnInit(): void {
    ...
    multicountryList:any[]=[];
      ...
      res.forEach((x: { employeeId: string; employeeName: string, }) => {
        this.multidockList.push({
          value: x.employeeId,
          label: x.employeeId + '-' + x.employeeName,
        });
    });
  ...
  console.log(this.selectedemployee)
    ...
    const dialogRef = this.dialog.open(EmployeeidNewComponent, {
      disableClose: true,
      width: '55%',
      maxWidth: '80%',
      data: { pageflow: data, code: data != 'New' ? this.selectedemployee[0].employeeId : null,warehouseId: data != 'New' ? this.selectedemployee[0].warehouseId : null,languageId: data != 'New' ? this.selectedemployee[0].languageId : null,companyCodeId: data != 'New' ? this.selectedemployee[0].companyCodeId : null,plantId: data != 'New' ? this.selectedemployee[0].plantId : null}
    });
...
if(res){
  ...
  deleteDialog() {
    ...
    const dialogRef = this.dialog.open(DeleteComponent, {
      ...
      maxWidth: '80%',
      position: { top: '9%', },
      data: this.selectedemployee[0].code,
    });
  
    dialogRef.afterClosed().subscribe(result => {
  
      if (result) {
        this.deleterecord(this.selectedemployee[0].employeeId,this.selectedemployee[0].warehouseId,this.selectedemployee[0].languageId,this.selectedemployee[0].companyCodeId,this.selectedemployee[0].plantId);
  ...
  downloadexcel() {
    ...
    this.cs.exportAsExcel(res, "Employee");
  }
  onChange() {
    console.log(this.selectedemployee.length)
    const choosen= this.selectedemployee[this.selectedemployee.length - 1];   
    this.selectedemployee.length = 0;
    this.selectedemployee.push(choosen);
  } 
  }
