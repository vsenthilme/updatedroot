...
@Component({
  selector: 'app-perpetual-lines',
  templateUrl: './perpetual-lines.component.html',
  styleUrls: ['./perpetual-lines.component.scss']
})
export class PerpetualLinesComponent implements OnInit {
...
fill(data:any){
  let obj: any = {};
     ...
     obj.cycleCountNo = [data.code.cycleCountNo];
   
  this.service.perpertuallines(obj).subscribe(
    (res) => {
      this.spin.hide();
      this.picking = res;
    },
...
downloadexcel() {
  ...
  const returnOrderNo = this.code.cycleCountNo; 
const fileName = `Perpetual Count Lines_${returnOrderNo}-`;
