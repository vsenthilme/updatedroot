...
import { MasterService } from 'src/app/shared/master.service';
  ...
  export class WarehouseNewComponent implements OnInit {
    ...
    form = this.fb.group({
      ...
      length: [],
      longitude: [],
      modeOfImplementation: [,Validators.required],
      noAisles: [],
      outboundQACheck: [],
      phoneNumber: [],
      ...
      warehouseTypeId: [1,Validators.required],
      width: [],
      zipCode: [],
      zone: [],
    });
 ...
 //this.form.controls.plantId.patchValue(this.form.controls.plantId.value);
      ...
      if (this.js.pageflow != 'New') {
       ...
       this.form.controls.warehouseTypeId.disable();
    ...
    dropdownlist(){
      ...
      this.cas.getalldropdownlist([
       ...
       // this.cas.dropdownlist.setup.warehouseid.url,
        ...
        this.cas.dropdownlist.setup.warehousetypeid.url,
      ]).subscribe((results) => {
     ...
     // this.warehouseidList = this.cas.forLanguageFilter(results[4], this.cas.dropdownlist.setup.warehouseid.key);
      this.languageidList = this.cas.foreachlist2(results[4], this.cas.dropdownlist.setup.languageid.key);
      this.warehousetypeidList=this.cas.forLanguageFilter(results[5],this.cas.dropdownlist.setup.warehousetypeid.key);
       this.form.controls.companyId.patchValue(this.auth.companyId);
    ...
    onItemgroupChange(value){
      ...
      this.masterService.searchWarehousetype({companyCodeId: this.auth.companyId, warehouseId:[value.value], plantId:this.auth.plantId, 
        languageId: [this.auth.languageId], }).subscribe(res => {
          this.warehousetypeidList = [];
          res.forEach(element => {
            this.warehousetypeidList.push({value: element.warehouseTypeId, label: element.warehouseTypeId + '-' + element.warehouseTypeText});
          })
    ...
    if (this.js.code) {
      this.sub.add(this.service.Update(this.form.getRawValue(),this.js.code,this.js.modeOfImplementation,this.js.warehouseTypeId,this.js.companyId,this.js.plantId,this.js.languageId).subscribe(res => {
