...
import { InventorPutawayComponent } from "./inventor-putaway/inventor-putaway.component";
...
export class InventoryReportComponent implements OnInit {
  ...
  filtersearch(excel = false) {
    ...
    if (!excel) {
      this.inventory = [];
      this.nonZeroTotal = []
      this.sub.add(this.service.searchNewSpark(this.searhform.getRawValue()).subscribe((res: any) => {
        this.spin.hide();
          res.forEach(element => {
            this.nonZeroTotal.push(element)
           
            this.totalQty += (element.inventoryQuantity || 0);
  ...
  putaway(res){
    if(res.st_bin=="REC-AL-B1"){
    console.log(res)
    const dialogRef = this.dialog.open(InventorPutawayComponent, {
      width: '72%',
      maxWidth: '90%',
      height:'55%',
      position: { top: '3.5%' },
  ...
  downloadexcel(obj: any) {
    this.spin.show();
    this.sub.add(this.service.searchNewSpark(obj).subscribe((data: any) => {
    console.log(data);
      ...
      if (data.length > 0) {
       data.forEach(x => {
          res.push({
            ...
            'Mfr Name': x.mfr_code,
            'Part No': x.itm_code,
            "Description": x.ref_field_8,
            "Level Id": x.level_id,
            "Storage Bin": x.st_bin,
            'Barcode': x.barcode_id,
            'Inv Qty': x.inv_qty,
            'Alloc Qty': x.alloc_qty,
            'Total': x.ref_field_4,
            "Stock Type": x.stck_typ_text,
            // 'Status': x.statusId,
          
            'Storage Section': x.ref_field_10,
            "Company": x.c_text,
          });
        })
        res.push({
