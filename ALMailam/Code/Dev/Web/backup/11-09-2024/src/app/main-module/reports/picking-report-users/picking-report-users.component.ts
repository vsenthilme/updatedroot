...
this.searhform.controls.toPickConfirmedOnFE.patchValue(currentDate);
    this.searhform.controls.fromPickConfirmedOnFE.patchValue(yesterday);
  }
  ...
  filtersearch(){
    ...
    // }
    this.searhform.controls.fromPickConfirmedOn.patchValue(this.cs.dateNewFormat1(this.searhform.controls.fromPickConfirmedOnFE.value));
        this.searhform.controls.toPickConfirmedOn.patchValue(this.cs.dateNewFormat1(this.searhform.controls.toPickConfirmedOnFE.value));
    ...
    this.sub.add(this.pickingService.pickuplineSparkNew(this.searhform.value).subscribe(res => {
 ...
 //     let result = res.filter((x: any) => x.statusId == 50);
  ...
  downloadexcel() {
    ...
    this.inbound.forEach(x => {
      res.push({
        ...
        'Pickup No': x.pu_no,
        'Line No': x.ob_line_no,
        "Mfr Name":x.mfr_name,
        'Part No': x.itm_code,
        'Description': x.item_text,
        'Barcode Id': x.partner_item_barcode,
        'Bin Location': x.pick_st_bin,
        'Level Id': x.level_id,
        'Picker': x.pickupConfirmedBy,
        "Store": x.partner_code,
        "Allocated Qty":x.alloc_qty,
        "Pick TO Qty": x.pick_cnf_qty,
        "Variance":x.var_qty,
        "Assigned On": this.cs.dateapiwithTime(x.pick_ctd_on),
        "Picked On": this.cs.dateapiwithTime(x.pick_utd_on),
  ...
  getPickingQty(){
    let total = 0;
    this.inbound.forEach(x =>{
      total = total + (x.pa_cnf_qty != null ? x.pa_cnf_qty : 0)
    })
    return Math.round(total *100 / 100)
  }
  getvarianceQty(){
    let total = 0;
    this.inbound.forEach(x =>{
      total = total + (x.var_qty != null ? x.var_qty : 0)
    })
    return Math.round(total *100 / 100)
  }
  ...
  calculateTotals(data?: any[]): void {
   ...
   this.totalQty1=0;
    if (data && data.length > 0) {
      this.totalInventoryQuantity = data.reduce((total, item) => total + (item.alloc_qty != null ? item.alloc_qty : 0), 0);
      this.totalAllocatedQuantity = data.reduce((total, item) => total + (item.pick_cnf_qty != null ? item.pick_cnf_qty : 0), 0);
      this.totalQty1=data.reduce((total, item) => total + (item.var_qty != null ? item.var_qty : 0), 0);
    } else {
      this.inbound.forEach(x => {
        this.totalInventoryQuantity = 0;
        this.totalAllocatedQuantity = 0;
