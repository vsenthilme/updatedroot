   ...
   this.form.controls.manufacturerName.patchValue([this.manufacturingName]);
    ...
    this.form.controls.manufacturerName.patchValue(null);
      ...
      if(this.form.controls.orderType.value == null){
       ...
       this.sub.add(this.reportService.getoutboundlinespark(this.form.getRawValue()).subscribe(res => {
         console.log(res)
         this.inbound = res;
    ...
    calculateTotals(data?: any[]): void {
     ...
     this.totalQty2=0;
      if (data && data.length > 0) {
        ...
        this.totalAllocatedQuantity = data.reduce((total, item) => total + Number(item.pick_CNF_QTY != null ? item.pick_CNF_QTY : 0), 0);
        this.totalQty1=data.reduce((total, item) => total + Number(item.qc_QTY != null ? item.qc_QTY : 0), 0);
        this.totalQty2=data.reduce((total, item) => total + (item.dlv_qty != null ? item.dlv_qty : 0), 0);
      } else {
    ...
    downloadexcel() {
      ...
      this.inbound.forEach(x => {
        res.push({
         ...
         "Sales Invoice Number":x.sales_invoice_number,
         ...
         "Target Branch":x.target_branch_code,
           ...
           "Order Qty":x.ord_qty,
           'Picker Qty': x.pick_CNF_QTY,
           'QA Qty': x.qc_QTY,
          'Delivered Qty': x.dlv_qty,
           'Created Date':this.cs.dateapiwithTime(x.dlv_ctd_on),
