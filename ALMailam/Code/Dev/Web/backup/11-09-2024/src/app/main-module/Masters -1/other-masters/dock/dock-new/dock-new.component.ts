...
@Component({
  selector: 'app-dock-new',
  templateUrl: './dock-new.component.html',
  styleUrls: ['./dock-new.component.scss']
})
export class DockNewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.js.pageflow != 'New') {
      this.form.controls.dockId.disable();
  ...
  fill(){
    this.spin.show();
    this.sub.add(this.service.Get(this.js.code,this.js.warehouseId,this.js.languageId,this.js.plantId,this.js.companyCodeId,this.js.dockType).subscribe(res => {
  ...
  warehouseIdList: any[] = [];
    ...
    dropdownlist(){
      ...
      this.cas.getalldropdownlist([
        ...
        this.cas.dropdownlist.setup.dockid.url,
    ...
    this.warehouseIdList=this.cas.forLanguageFilter(results[0],this.cas.dropdownlist.setup.warehouseid.key);
      this.dockList=this.cas.forLanguageFilter(results[1],this.cas.dropdownlist.setup.dockid.key);
      ...
      this.languageList=this.cas.foreachlist2(results[4],this.cas.dropdownlist.setup.languageid.key);
        ...
        this.masterService.searchdock({companyCodeId: this.auth.companyId, warehouseId:this.auth.warehouseId, plantId:this.auth.plantId, languageId: [this.auth.languageId]}).subscribe(res => {
          this.dockList = [];
          res.forEach(element => {
            this.dockList.push({value: element.dockId, label: element.dockId + '-' + element.dockDescription});
          })
        });
    ...
    onWarehouseChange(value){
      ...
      this.masterService.searchdock({companyCodeId: this.form.controls.companyCodeId.value, warehouseId:value.value, plantId:this.form.controls.plantId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
        this.dockList = [];
        res.forEach(element => {
          this.dockList.push({value: element.dockId, label: element.dockId + '-' + element.dockDescription});
        })
  ...
  if (this.js.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(),this.js.code,this.js.warehouseId,this.js.languageId,this.js.plantId,this.js.companyCodeId,this.js.dockType).subscribe(res => {
      this.toastr.success(res.dockId + " updated successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.router.navigate(['/main/other-masters/dock']);
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.dockId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.router.navigate(['/main/other-masters/dock']);
