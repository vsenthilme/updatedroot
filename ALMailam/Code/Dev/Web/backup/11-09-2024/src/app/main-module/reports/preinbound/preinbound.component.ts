...
this.sourceBranch1 = [
  ...
  ngOnInit(): void {
    ...
    this.form.controls.endCreatedOnFE.patchValue(new Date());
    this.form.controls.startCreatedOnFE.patchValue(currentMonthStartDate);
  }
...
this.form.controls.startCreatedOn.patchValue(this.cs.day_callapiSearch(this.form.controls.startCreatedOnFE.value));
this.form.controls.endCreatedOn.patchValue(this.cs.day_callapiSearch(this.form.controls.endCreatedOnFE.value));
...
obj.endConfirmedOn=this.form.controls.endConfirmedOn.value;
    this.sub.add(this.serviceLine.searchLinesparkNew(this.form.getRawValue()).subscribe(res => {
      console.log(res)
      this.inbound = res;
  ...
  downloadexcel() {
    ...
    this.inbound.forEach(x => {
      res.push({
        ...
        "Branch":x.plant_text,
        "Warehouse":x.wh_text,
        "Source Branch":x.source_branch_code,
        "Order No ": x.ref_doc_no,
        "Line No": x.ib_line_no,
        "Order Type":x.ref_doc_type,
        "Mfr Name":x.mfr_name,
        'Part No': x.item_id,
        'Description': x.text,
        "Order Qty":x.ord_qty,
        'Acc Qty': x.accept_qty,
        'Dam Qty': x.damage_qty,
        'Variance Qty': x.var_qty,
        'Created Date':this.cs.dateapiwithTime(x.ctd_on),
        'Confirmed Date':this.cs.dateapiwithTime(x.ib_cnf_on),
...
totalQty1:number =0;
  calculateTotals(data?: any[]): void {
   ...
   this.totalQty1=0;
    if (data && data.length > 0) {
      this.totalInventoryQuantity = data.reduce((total, item) => total + (item.ord_qty != null ? item.ord_qty : 0), 0);
      this.totalAllocatedQuantity = data.reduce((total, item) => total + (item.accept_qty != null ? item.accept_qty : 0), 0);
      this.totalQty1=data.reduce((total, item) => total + (item.damage_qty != null ? item.damage_qty : 0), 0);
      this.totalvarianceQty=data.reduce((total, item) => total + (item.var_qty != null ? item.var_qty : 0), 0);
    } else {
      this.inbound.forEach(x => {
        this.totalInventoryQuantity = 0;
        this.totalAllocatedQuantity =0;
  ...
  getorderQty(){
      let total = 0;
      this.inbound.forEach(x =>{
        total = total + (x.ord_qty != null ? x.ord_qty : 0)
      })
  ...
  getacceptedQty(){
    let total = 0;
    this.inbound.forEach(x =>{
      total = total + (x.accept_qty != null ? x.accept_qty : 0)
    })
    return Math.round(total *100 / 100)
}
getdamageQty(){
  let total = 0;
  this.inbound.forEach(x =>{
    total = total + (x.damage_qty != null ? x.damage_qty : 0)
  })
  return Math.round(total *100 / 100)
}
getvarianceQty(){
