...
export class InventorymovementComponent implements OnInit {
  ...
  @ViewChild('inventorymovTag') inventorymovTag: Table | any;
    ...
    initialSearch(){
      ...
      if(this.form.controls.packBarcodes.value != null){
        this.form.controls.packBarcodes.patchValue([this.form.controls.packBarcodes.value])
        }
        else{
          this.form.controls.packBarcodes.patchValue(this.form.controls.packBarcodes.value)
        }
...
this.spin.hide();
  ...
  openDialog(data: any = 'New'): void {
    ...
    const dialogRef = this.dialog.open(UpdateinventoryComponent, {
      ...
      maxWidth: '80%',
      position: { top: '9%', },
      data: { pageflow: data, code: this.selectedinventory[0]}
    });

    dialogRef.afterClosed().subscribe(result => {
      console.log(result.movementQty)
      console.log(this.selectedinventory[0])
     if(result){
      this.selectedinventory[0].movementQty=(parseInt(result.movementQty));
      this.filtersearch();
     }
    ...
    onChange() {
      console.log(this.selectedinventory.length)
      const choosen= this.selectedinventory[this.selectedinventory.length - 1];   
      this.selectedinventory.length = 0;
      this.selectedinventory.push(choosen);
    }  
