...
@Component({
  selector: 'app-sublevelid-new',
  templateUrl: './sublevelid-new.component.html',
  styleUrls: ['./sublevelid-new.component.scss']
})
export class SublevelidNewComponent implements OnInit {
  ...
  form = this.fb.group({
    companyCodeId: [,Validators.required],
        ...
        referenceField9: [],
        subLevel: [],
        subLevelId: [,Validators.required],
        updatedBy: [],
        updatedOn: [],
        createdOnFE: [],
        updatedOnFE: [],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.subLevelId.disable();
      this.form.controls.warehouseId.disable();
      this.form.controls.languageId.disable();
      this.form.controls.plantId.disable();
      this.form.controls.companyCodeId.disable();
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.warehouseId,this.data.companyCodeId,this.data.plantId,this.data.languageId,this.data.levelId).subscribe(res => {
 ...
 this.masterService.searchlevel({companyCodeId: this.data.companyCodeId, plantId: this.data.plantId, warehouseId: this.data.warehouseId, languageId: [this.data.languageId]}).subscribe(res => {
  this.levelList = [];
  res.forEach(element => {
    this.levelList.push({value: element.levelId, label: element.levelId + '-' + element.level});
  })
  ...
  onLanguageChange(value){
    ...
    this.masterService.searchlevel({companyCodeId: this.form.controls.companyCodeId.value, plantId: this.form.controls.plantId.value, warehouseId: this.form.controls.warehouseId.value, languageId: [value.value]}).subscribe(res => {
      this.levelList = [];
      res.forEach(element => {
        this.levelList.push({value: element.levelId, label: element.levelId + '-' + element.level});
      });
  ...
  onCompanyChange(value){
    ...
    this.masterService.searchlevel({companyCodeId: value.value, plantId: this.form.controls.plantId.value, warehouseId: this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
      this.levelList = [];
      res.forEach(element => {
        this.levelList.push({value: element.levelId, label: element.levelId + '-' + element.level});
      });
  ...
  onPlantChange(value){
      ...
      this.masterService.searchlevel({companyCodeId: this.form.controls.companyCodeId.value, plantId: value.value, warehouseId: this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
        this.levelList = [];
        res.forEach(element => {
          this.levelList.push({value: element.levelId, label: element.levelId + '-' + element.level});
        });
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(),this.data.code,this.data.warehouseId,this.data.companyCodeId,this.data.plantId,this.data.languageId,this.data.levelId).subscribe(res => {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.subLevelId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
