...
@Component({
  selector: 'app-preinbound-popupedit',
  templateUrl: './preinbound-popupedit.component.html',
  styleUrls: ['./preinbound-popupedit.component.scss']
})
export class PreinboundPopupeditComponent implements OnInit {
  ...
  preInboundLineV2 = this.fb.group({
    brandName: [],
    businessPartnerCode: [],
    companyCode: [],
    companyDescription: [],
  ...
  preInboundLinesV2: FormArray = this.fb.array([this.preInboundLineV2]);
  ...
  form = this.fb.group({
    ...
    warehouseDescription: [],
    warehouseId: [],
    preInboundLineV2: this.preInboundLinesV2,
  });
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code, this.data.warehouseId, this.data.companyCode, this.data.plantId, this.data.languageId).subscribe(res => {
  ...
  submit() {
    ...
    if (this.data.code) {
      this.sub.add(this.service.Update1(this.form.getRawValue(), this.data.code, this.data.warehouseId, this.data.companyCode, this.data.plantId, this.data.languageId).subscribe(res => {
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success(res.approvalProcessId + " Saved Successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
