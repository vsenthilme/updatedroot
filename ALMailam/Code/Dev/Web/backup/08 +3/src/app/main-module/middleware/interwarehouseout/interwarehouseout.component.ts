...
export class InterwarehouseoutComponent implements OnInit {
  ...
  multicustomer:any[]=[];
  getDropdown() {
    this.sub.add(this.service.interwarehouseout({ sourceCompanyCode:[this.auth.companyId],sourceBranchCode:[this.auth.plantId], toOrderReceivedOn: this.cs.todayapi(), fromOrderReceivedOn: this.cs.todayapi(),}).subscribe(res => {
      res.forEach((x: any) => this.multisalesOrderNo.push({ value: x.transferOrderNumber, label: x.transferOrderNumber }));
