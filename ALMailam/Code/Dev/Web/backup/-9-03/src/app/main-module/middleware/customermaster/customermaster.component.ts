...
export class CustomermasterComponent implements OnInit {
  ...
  multistatus:any[]=[];
  getDropdown() {
    this.sub.add(this.service.customerMaster({ companyCode:[this.auth.companyId],branchCode:[this.auth.plantId], toOrderReceivedOn: this.cs.todayapi(), fromOrderReceivedOn: this.cs.todayapi(),}).subscribe(res => {
      res.forEach((x: any) => this.multisalesOrderNo.push({ value: x.customerCode, label: x.customerName }));
      res.forEach((x: any) => this.multistatus.push({ value: x.processedStatusId, label: x.processedStatusId }));
    }))
  }
