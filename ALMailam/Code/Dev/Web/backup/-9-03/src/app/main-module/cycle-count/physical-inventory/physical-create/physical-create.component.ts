...
export class PhysicalCreateComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'no', 'product', 'referenceField8', 'referenceField9', 'referenceField10', 'bin', 'pallet', 'stock', 'referenceField4', 'uom', 'cycleCountTypeId',];
  ...
  ngOnInit(): void {
    ...
    if(this.data.pageflow == 'New'){
      this.displayedColumns = ['no', 'product', 'referenceField8', 'referenceField9', 'referenceField10', 'bin', 'pallet', 'stock', 'referenceField4', 'uom', 'cycleCountTypeId',];
    }
  ...
  //  this.periodicService.runPeriodicHeaderNew(this.data.selectedWareHouse, 'itemCode', this.pageNumber, this.pageSize).subscribe(
    //this.periodicService.runPeriodicHeader(this.data.selectedWareHouse, storageSectionIds).subscribe(
      this.inventoryService.GetInventoryv2(this.data.obj).subscribe(
      result => {
        ...
        result.forEach(element => {
          ...
          if(element.referenceField4 > 0){
            console.log(element.referenceField4)
  ...
  createPeriodicHeader() {
    ...
    this.dataSource.data.forEach(data => {
      ...
      lineObj.inventoryQuantity = data.referenceField4;
      lineObj.inventoryUom = data.inventoryUom;
      lineObj.itemCode = data.itemCode;
      lineObj.referenceField8 = data.referenceField8;
      lineObj.referenceField9 = data.referenceField9;
      lineObj.referenceField10 = data.referenceField10;
