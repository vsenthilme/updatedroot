...
@Component({
  selector: 'app-dateformatid-new',
  templateUrl: './dateformatid-new.component.html',
  styleUrls: ['./dateformatid-new.component.scss']
})
export class DateformatidNewComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    createdOnFE: [],
    dateFormat: [],
    dateFormatId: [,Validators.required],
    deletionIndicator: [],
  ...
  constructor(
    ...
    {
      label: 'Month of the Week,Month Date,Year',
      value: 'Month of the Week,Month Date,Year'
    },
    {
      label: 'M/DD',
      value: 'M/DD'
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.dateFormatId.disable();
      this.form.controls.companyCodeId.disable();
      this.form.controls.plantId.disable();
      this.form.controls.warehouseId.disable();
      this.form.controls.languageId.disable();
 ...
 // this.warehouseidList = this.cas.foreachlist2(results[2], this.cas.dropdownlist.setup.warehouseid.key);
  this.masterService.searchCompany({languageId: [this.form.controls.languageId.value]}).subscribe(res => {
   ...
   this.masterService.searchPlant({companyCodeId: [this.form.controls.companyCodeId.value], languageId: [this.form.controls.languageId.value]}).subscribe(res => {
    ...
    this.masterService.searchWarehouse({companyCodeId: this.form.controls.companyCodeId.value, languageId: [this.form.controls.languageId.value],plantId:this.form.controls.plantId.value}).subscribe(res => {
  ...
  if (this.data.code) {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.dateFormatId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
