...
@Component({
  selector: 'app-specialstockindicator-new',
  templateUrl: './specialstockindicator-new.component.html',
  styleUrls: ['./specialstockindicator-new.component.scss']
})
export class SpecialstockindicatorNewComponent implements OnInit {
 ...
 // dialogRef: any;
  ...
  form = this.fb.group({
    ...
    referenceField8: [],
    referenceField9: [],
    specialStockIndicatorDescription: [],
    specialStockIndicatorId: [,Validators.required],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.specialStockIndicatorId.disable();
      this.form.controls.stockTypeId.disable();
      this.form.controls.warehouseId.disable();
      this.form.controls.languageId.disable();
      this.form.controls.plantId.disable();
      this.form.controls.companyCodeId.disable();
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId,this.data.stockTypeId).subscribe(res => {
  ...
  dropdownlistSuperAdmin(){
    ...
    this.cas.dropdownlist.setup.stocktypeid.url,
    ]).subscribe((results) => {
  ...
  //this.stockList = this.cas.foreachlist2(results[4],this.cas.dropdownlist.setup.stocktypeid.key);
 ...
 this.masterService.searchPlant({companyCodeId: [this.form.controls.companyCodeId.value], languageId: [this.form.controls.languageId.value]}).subscribe(res => {
  ...
  this.masterService.searchstock({companyCodeId: this.form.controls.companyCodeId.value, plantId: this.form.controls.plantId.value, warehouseId: this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
    this.stockList = [];
    res.forEach(element => {
      this.stockList.push({value: element.stockTypeId, label: element.stockTypeId + '-' + element.stockTypeText});
    })
  });
   this.spin.hide();
    }, (err) => {
  ...
  dropdownlist(){
   ...
   this.plantidList = this.cas.foreachlist2(results[3], this.cas.dropdownlist.setup.plantid.key);
   this.masterService.searchstock({companyCodeId: this.auth.companyId, plantId: this.auth.plantId, warehouseId: this.auth.warehouseId, languageId: [this.auth.languageId]}).subscribe(res => {
    this.stockList = [];
    res.forEach(element => {
      this.stockList.push({value: element.stockTypeId, label: element.stockTypeId + '-' + element.stockTypeText});
    })
  ...
  onLanguageChange(value){
    ...
    this.masterService.searchstock({companyCodeId: this.form.controls.companyCodeId.value, plantId: this.form.controls.plantId.value, warehouseId: this.form.controls.warehouseId.value, languageId: [value.value]}).subscribe(res => {
      this.stockList = [];
      res.forEach(element => {
        this.stockList.push({value: element.stockTypeId, label: element.stockTypeId + '-' + element.stockTypeText});
      });
  ...
  onCompanyChange(value){
    ...
    this.masterService.searchstock({companyCodeId: value.value, plantId: this.form.controls.plantId.value, warehouseId: this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
      this.stockList = [];
      res.forEach(element => {
        this.stockList.push({value: element.stockTypeId, label: element.stockTypeId + '-' + element.stockTypeText});
      });
  ...
  onPlantChange(value){
      ...
      this.masterService.searchstock({companyCodeId: this.form.controls.companyCodeId.value, plantId: value.value, warehouseId: this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
        this.stockList = [];
        res.forEach(element => {
          this.stockList.push({value: element.stockTypeId, label: element.stockTypeId + '-' + element.stockTypeText});
        });
  ...
  onWarehouseChange(value){
    this.masterService.searchstock({companyCodeId: this.form.controls.companyCodeId.value, plantId:this.form.controls.plantId.value, warehouseId: value.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
      this.stockList = [];
      res.forEach(element => {
        this.stockList.push({value: element.stockTypeId, label: element.stockTypeId + '-' + element.stockTypeText});
      });
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(),this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId,this.data.stockTypeId).subscribe(res => {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.specialStockIndicatorId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
