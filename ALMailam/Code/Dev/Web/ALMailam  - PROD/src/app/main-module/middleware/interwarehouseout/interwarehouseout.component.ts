...
@Component({
  selector: 'app-interwarehouseout',
  templateUrl: './interwarehouseout.component.html',
  styleUrls: ['./interwarehouseout.component.scss']
})
export class InterwarehouseoutComponent implements OnInit {
  ...
  search(ispageload = false) {
    ...
    this.service.interwarehouseout(this.form.value).subscribe(
      (res) => {
        this.spin.hide();
  ...
  reload() {
    ...
    this.sub.add(this.service.interwarehouseout({ sourceCompanyCode:[this.auth.companyId],sourceBranchCode:[this.auth.plantId], }).subscribe(res => {
  ...
  downloadexcel() {
    ...
    this.cs.exportAsExcel(res, 'OUTBOUND WMS TO WMS ');
  }
  openConfirm(data: any) {
   
    let paramdata = this.cs.encrypt({ code: data, pageflow: 'Edit' });
    this.router.navigate(['/main/middleware/interwarehouseoutlines/' + paramdata]);
