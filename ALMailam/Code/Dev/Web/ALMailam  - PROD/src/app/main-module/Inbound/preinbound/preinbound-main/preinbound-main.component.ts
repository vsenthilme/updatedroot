...
import { CancelSalesReturnComponent } from '../cancel-sales-return/cancel-sales-return.component';
...
screenid=3044;
  ...
  multistatusList:any[]=[];
  getDropdown() {
    this.sub.add(this.service.searchSpark({ warehouseId: [this.auth.warehouseId],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId] }).subscribe(res => {
      res.forEach((x: any) => this.containerNoList.push({ value: x.containerNo, label: x.containerNo }));
    
      res.forEach((x: any) => this.inboundOrderTypeList.push({ value: x.inboundOrderTypeId, label: x.referenceDocumentType }));
      ...
      this.inboundOrderTypeList=this.cs.removeDuplicatesFromArrayNewstatus(this.inboundOrderTypeList);
  ...
  searchfilter(ispageload = false) {
    ...
    this.service.search(this.searhform.value).subscribe(res => {
      ...
      if (ispageload) {
        ...
        res.forEach(x => tempContainerList.push({ item_id: x.containerNo, item_text:x.containerNo}));
        ...
        res.forEach(x => tempinboundOrderTypeList.push({ item_id: x.inboundOrderTypeId, item_text:x.referenceDocumentType}));
        this.inboundOrderTypeList = tempinboundOrderTypeList;
        this.inboundOrderTypeList = this.cs.removeDuplicatesFromArrayNewstatus(this.inboundOrderTypeList);

        let temprefDocNumberList: any[] = [];
        const refDocNumber = [
          ...new Set(res.map((item) => item.refDocNumber)),
        ].filter((x) => x != null);
  ...
  cancel(data: any, module): void {
    const dialogRef = this.dialog.open(CustomdialogComponent, {
      ...
      data: { title: "Confirm", body: "Do you want to replace the Invoice No?" },
    });

    dialogRef.afterClosed().subscribe(result => {
      if (result == 'Yes' && module == 'supplierInvoice') {
      ...
      if (result == 'Yes' && module == 'salesReturn') {
        const dialogRef = this.dialog.open(CancelSalesReturnComponent, {
        disableClose: true,
        width: '55%',
        maxWidth: '80%',
        data: { code: data,}
