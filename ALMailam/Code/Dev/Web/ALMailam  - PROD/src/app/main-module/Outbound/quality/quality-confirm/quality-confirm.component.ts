...
export class QualityConfirmComponent implements OnInit {
  ...
  // start CWMS/IW/2022/018 mugilan 03-10-2022
    ...
    this.spin.show();
    this.sub.add(
      this.service.confirmv2(data).subscribe(
        (res) => {
          this.toastr.success(
            this.code.refDocNumber + ' confirmed Successfully',
            '',
            {
        timeOut: 2000,
        progressBar: false,
        ...
        this.sub.add(
          this.service
            .searchSpark({
              ...
              refDocNumber: [data.code.refDocNumber],
              statusId: [54],
              preOutboundNo: [data.code.preOutboundNo],
            })
            .subscribe(
              (res) => {
                this.sub.add(
                  this.service
                    .getpickinglineSpark({
                      pickupNumber: [res[0].pickupNumber],
                      warehouseId: [this.auth.warehouseId],
                      companyCodeId: [this.auth.companyId],
                      languageId: [this.auth.languageId],
                      plantId: [this.auth.plantId],
                      preOutboundNo: [res[0].preOutboundNo],
                    })
                    .subscribe(
                      (result) => {
