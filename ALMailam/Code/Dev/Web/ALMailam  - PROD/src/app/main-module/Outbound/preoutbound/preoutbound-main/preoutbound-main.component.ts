...
export class PreoutboundMainComponent implements OnInit {
  ...
  reload() {
    ...
    this.searhform.controls.statusId.patchValue([39, 47, 96])
  ...
  cancel(data: any): void {
    const dialogRef = this.dialog.open(CustomdialogComponent, {
      ...
      data: { title: "Cancel Order", body: `Do you want to Cancel this Order No: ${data.refDocNumber}?` },
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result == 'Yes') {
        if (result) {
          ...
          obj.preOutboundNo = data.preOutboundNo;


          this.cancelOutboundOrder(obj);
        }
      }
      else {
        dialogRef.afterClosed().subscribe(result => {
  ...
  cancelOutboundOrder(obj) {
    this.spin.show();
    this.sub.add(this.service.outboundCancellation(obj).subscribe((res) => {
      this.toastr.success(obj.refDocNumber + " Cancelled successfully.", "Notification", {
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
