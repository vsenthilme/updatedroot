...
export class RowComponent implements OnInit {
  ...
  multicountryList:any[]=[];
    ...
    res.forEach((x: { rowId: string; rowNumber: string, }) => {
      this.multidateformatList.push({
        value: x.rowId,
        label: x.rowId + '-' + x.rowNumber,
      });
  });
...
deleteDialog() {
  ...
  dialogRef.afterClosed().subscribe(result => {
    ...
    if (result) {
      this.deleterecord(this.selectedrows[0].rowId,this.selectedrows[0].warehouseId,this.selectedrows[0].companyCodeId,this.selectedrows[0].plantId,this.selectedrows[0].languageId,this.selectedrows[0].floorId,this.selectedrows[0].storageSectionId,);
