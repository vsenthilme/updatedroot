...
@Component({
  selector: 'app-b2btransferorder',
  templateUrl: './b2btransferorder.component.html',
  styleUrls: ['./b2btransferorder.component.scss']
})
export class B2btransferorderComponent implements OnInit {
  ...
  search(ispageload = false) {
    ...
    this.service.b2btransfer(this.form.value).subscribe(
      (res) => {
        this.spin.hide();
  ...
  reload() {
    ...
    this.sub.add(this.service.b2btransfer({ sourceCompanyCode:[this.auth.companyId]}).subscribe(res => {
  ...
  downloadexcel() {
    ...
    this.cs.exportAsExcel(res, 'Inbound NONWMS to WMS');
  }
