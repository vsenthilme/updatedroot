  ...
  intercept(
    ...
    return next.handle(request).pipe(catchError(error => {
      if (error.status === 401 || error.status == 400 || error.status == 415) {
        if (request.url.includes('/auth-token') || (request.url.includes('wms-spark-service')) || (error.error != null && error.error.error.includes("Order is not completely Processed"))) {
