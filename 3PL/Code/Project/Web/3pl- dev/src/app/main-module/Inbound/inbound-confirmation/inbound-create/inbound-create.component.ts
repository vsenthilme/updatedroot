...
export class InboundCreateComponent implements OnInit {
  ...
  fill(data: any) {
    ...
    if (data.pageflow != 'New') {
      ...
      this.spin.show();
      this.sub.add(this.service.Get(data.code.warehouseId, data.code.preInboundNo, data.code.refDocNumber, data.code.companyCode ,data.code.languageId, data.code.plantId).subscribe(res => {
        this.code = res;
