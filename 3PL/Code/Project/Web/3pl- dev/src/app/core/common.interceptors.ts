  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (bearerToken) {
      ...
      if (req.url.split('/')[1] == 'wms-idmaster-service' && req.url.split('/')[2] == 'company' && req.url.split('/').length == 4 && req.method == 'DELETE') {
        ...
        authReq = req.clone({
          headers: req.headers.set("Access-Control-Allow-Origin", "*").set("Access-Control-Allow-Methods", "DELETE, POST, GET, OPTIONS").set("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Requested-With"),
          url: `${url}${req.url}`,
          params: new HttpParams().set("authToken", bearerToken).set('loginUserID', this.authService.userID)
        });
        // if (req.method != 'GET')
