    ...
    private InventoryV2 createInventoryV2(GrLineV2 createdGRLine) {
        ...
        try {
            ...
            if (dbInventory != null) {
                ...
                if (threePLUom != 0.0) {
                    Double threePLCbmPerQty = threePLCbm / threePLUom;
...
//            List<IImbasicData1> imbasicdata1 = imbasicdata1Repository.findByItemCode(inventory.getItemCode());
                ...
                if (threePLUom != 0.0 && threePLCbm != 0.0) {
                    Double threePLCbmPerQty = threePLCbm / threePLUom;
                    inventory.setThreePLCbmPerQty(threePLCbmPerQty);
                } else {
                    inventory.setThreePLCbmPerQty(0.0);
...
//        dbGrLine.setUpdatedOn(DateUtils.getCurrentKWTDateTime());
    ...
    public void deleteGrLineV2(String companyCodeId, String plantId, String languageId,
        ...
        if (grLine != null) {
            ...
            //GrLine Delete, corresponding record in StockMovement
            ...
            // Check how many records in grLine based on refDocNumber
            List<GrLineV2> grLinesToDelete = grLineV2Repository.findByCompanyCodeAndPlantIdAndLanguageIdAndWarehouseIdAndRefDocNumberAndGoodsReceiptNo(
                    companyCodeId, plantId, languageId, warehouseId, refDocNumber, goodsReceiptNo );

            if (grLinesToDelete.size() > 0) {

                // Delete corresponding GrHeader record
                boolean allDeleted = grLinesToDelete.size() == grLineV2Repository.countByCompanyCodeAndPlantIdAndLanguageIdAndWarehouseIdAndRefDocNumberAndGoodsReceiptNoAndDeletionIndicator(
                        companyCodeId, plantId, languageId, warehouseId, refDocNumber, goodsReceiptNo, 1L);

                if (allDeleted) {
                    GrHeader grHeader = grHeaderRepository.findByCompanyCodeAndPlantIdAndLanguageIdAndWarehouseIdAndRefDocNumberAndGoodsReceiptNoAndDeletionIndicator(
                            companyCodeId, plantId, languageId, warehouseId, refDocNumber, goodsReceiptNo, 0L);
