...
@Component({
  selector: 'app-word-order-status',
  templateUrl: './word-order-status.component.html',
  styleUrls: ['./word-order-status.component.scss']
})
export class WordOrderStatusComponent implements OnInit {
  ...
  showFiller = false;
  displayedColumns: string[] = ['workOrderDate', 'workOrderId',  'customerId', 'customerName', 'status', 'created','processedBy','processedTime','leadTime','remarks',];
...
this.currentDate = new Date();
 ...
 dropdownlist() {
   ...
   this.cas.getalldropdownlist([
     this.cas.dropdownlist.trans.workorderNumber.url,
     this.cas.dropdownlist.setup.workorderstatus.url,
    
   ]).subscribe((results) => {
     this.workorderNumberList = this.cas.foreachlist1(results[0], this.cas.dropdownlist.trans.workorderNumber.key);
     this.workOrderSbuList = this.cas.foreachlist1(results[1], this.cas.dropdownlist.setup.workorderstatus.key);
   }, (err) => {
     this.toastr.error(err, "");
     this.spin.hide();
   });
  ...
  filtersearch() {
    ...
    this.spin.show();
    this.sub.add(this.service.getWorkOrderStatusTime(this.form.getRawValue()).subscribe(res => {
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        "W.O Date": this.cs.dateapiutc0(x.workOrderDate),
        "WO No.": x.workOrderId,
    ...
    this.cs.exportAsExcel(res,"WorkOrder-Status");
  }
}
