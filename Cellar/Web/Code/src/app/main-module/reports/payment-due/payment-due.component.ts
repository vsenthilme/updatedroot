...
export class PaymentDueComponent implements OnInit {
 ...
 // endDate: [this.cs.todayapi()],
  ...
  statusList: any[] = [];
  showFiller = false;
  displayedColumns: string[] = ['customerName','civilId','agreementNumber',  'storeNumber', 'size', 'storageType','phase','rentPeriod' ,'dueStatus','lastPaidDate','dueDate','dueDays','lastPaidVoucherAmount','dueAmount','modeOfPayment','agreementStatus','startDate','endDate','phoneNumber'];
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        ...
        "Aggrement No":x.agreementNumber,
        ...
        "Size":x.size,
         ...
         "Status":x.dueStatus,
         "Last Paid Date":this.cs.dateapiutc0(x.lastPaidDate),
         "Due Date":this.cs.dateapiutc0(x.dueDate),
         "Last Paid Voucher Amount":x.lastPaidVoucherAmount,
         "Due Amount":x.dueAmount,
         "Mode Of Payment":x.modeOfPayment,
         "Aggrement Status":x.agreementStatus,
         "Start Date":this.cs.dateapiutc0(x.startDate),
         "End Date":this.cs.dateapiutc0(x.endDate),
         "Secondary No":x.phoneNumber
      });
