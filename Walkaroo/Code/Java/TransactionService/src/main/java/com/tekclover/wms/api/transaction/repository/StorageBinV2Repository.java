...
//            "putaway_block = 0 and pick_block = 0 and status_id = :statusId and \n" +
    ...
    boolean existsByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndStorageBinAndDeletionIndicator(
            String companyCodeId, String plantId, String languageId, String warehouseId, String storageBin, Long deletionIndicator);
    ...
    @Query(value = "SELECT top 1 st_bin FROM tblstoragebin WHERE bin_cl_id = :binclassId and c_id = :companyCode and plant_id = :plantId and \n" +
            "wh_id = :warehouseId and lang_id = :languageId and \n" +
            "st_bin not in (select PROP_ST_BIN from tblputawayheader where status_id = 19 and c_id = :companyCode and plant_id = :plantId and wh_id = :warehouseId and lang_id = :languageId) and \n" +
            "status_id = :statusId and \n" +
            "is_deleted = 0 and st_bin <> 'REC-AL-B2' order by st_bin", nativeQuery = true)
    public String getStorageBinV3(@Param("companyCode") String companyCode,
                                  @Param("plantId") String plantId,
                                  @Param("languageId") String languageId,
                                  @Param("warehouseId") String warehouseId,
                                  @Param("binclassId") Long binclassId,
    ...
    @Query(value = "SELECT top 1 PROPOSED_BIN FROM tblputawaystrategy WHERE ARTICLE = :article and c_id = :companyCode and plant_id = :plantId and \n" +
            "wh_id = :warehouseId and lang_id = :languageId and \n" +
            "(COALESCE(:gender, null) IS NULL OR (GENDER IN (:gender))) and \n" +
            "(COALESCE(:location, null) IS NULL OR (LOCATION IN (:location))) and \n" +
            "is_deleted = 0 ", nativeQuery = true)
    public String getPutAwayStrategyStorageBinV3(@Param("companyCode") String companyCode,
                                                 @Param("plantId") String plantId,
                                                 @Param("languageId") String languageId,
                                                 @Param("warehouseId") String warehouseId,
