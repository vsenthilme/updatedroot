...
public class PickupLineV2Specification implements Specification<PickupLineV2> {
    ...
    public Predicate toPredicate(Root<PickupLineV2> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (searchPickupLine.getAssignedPickerId() != null && !searchPickupLine.getAssignedPickerId().isEmpty()) {
            ...
            predicates.add(group.in(searchPickupLine.getAssignedPickerId()));
        }
        ...
        if (searchPickupLine.getMaterialNo() != null && !searchPickupLine.getMaterialNo().isEmpty()) {
            final Path<Group> group = root.<Group>get("materialNo");
            predicates.add(group.in(searchPickupLine.getMaterialNo()));
        }

        if (searchPickupLine.getPriceSegment() != null && !searchPickupLine.getPriceSegment().isEmpty()) {
            final Path<Group> group = root.<Group>get("priceSegment");
            predicates.add(group.in(searchPickupLine.getPriceSegment()));
        }

        if (searchPickupLine.getArticleNo() != null && !searchPickupLine.getArticleNo().isEmpty()) {
            final Path<Group> group = root.<Group>get("articleNo");
            predicates.add(group.in(searchPickupLine.getArticleNo()));
        }
        if (searchPickupLine.getGender() != null && !searchPickupLine.getGender().isEmpty()) {
            final Path<Group> group = root.<Group>get("gender");
            predicates.add(group.in(searchPickupLine.getGender()));
        }

        if (searchPickupLine.getColor() != null && !searchPickupLine.getColor().isEmpty()) {
            final Path<Group> group = root.<Group>get("color");
            predicates.add(group.in(searchPickupLine.getColor()));
        }

        if (searchPickupLine.getSize() != null && !searchPickupLine.getSize().isEmpty()) {
            final Path<Group> group = root.<Group>get("size");
            predicates.add(group.in(searchPickupLine.getSize()));
        }
        if (searchPickupLine.getNoPairs() != null && !searchPickupLine.getNoPairs().isEmpty()) {
            final Path<Group> group = root.<Group>get("noPairs");
            predicates.add(group.in(searchPickupLine.getNoPairs()));
        }
        if (searchPickupLine.getBarcodeId() != null && !searchPickupLine.getBarcodeId().isEmpty()) {
            final Path<Group> group = root.<Group>get("barcodeId");
            predicates.add(group.in(searchPickupLine.getBarcodeId()));
        }
