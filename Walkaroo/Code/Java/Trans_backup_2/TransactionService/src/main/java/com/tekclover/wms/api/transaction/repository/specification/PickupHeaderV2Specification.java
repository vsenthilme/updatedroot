...
public class PickupHeaderV2Specification implements Specification<PickupHeaderV2> {
    ...
    public Predicate toPredicate(Root<PickupHeaderV2> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (searchPickupHeader.getPriceSegment() != null && !searchPickupHeader.getPriceSegment().isEmpty()) {
            final Path<Group> group = root.<Group>get("priceSegment");
            predicates.add(group.in(searchPickupHeader.getPriceSegment()));
        }

        if (searchPickupHeader.getArticleNo() != null && !searchPickupHeader.getArticleNo().isEmpty()) {
            final Path<Group> group = root.<Group>get("articleNo");
            predicates.add(group.in(searchPickupHeader.getArticleNo()));
        }
        if (searchPickupHeader.getGender() != null && !searchPickupHeader.getGender().isEmpty()) {
            final Path<Group> group = root.<Group>get("gender");
            predicates.add(group.in(searchPickupHeader.getGender()));
        }

        if (searchPickupHeader.getColor() != null && !searchPickupHeader.getColor().isEmpty()) {
            final Path<Group> group = root.<Group>get("color");
            predicates.add(group.in(searchPickupHeader.getColor()));
        }

        if (searchPickupHeader.getSize() != null && !searchPickupHeader.getSize().isEmpty()) {
            final Path<Group> group = root.<Group>get("size");
            predicates.add(group.in(searchPickupHeader.getSize()));
        }
        if (searchPickupHeader.getNoPairs() != null && !searchPickupHeader.getNoPairs().isEmpty()) {
            final Path<Group> group = root.<Group>get("noPairs");
            predicates.add(group.in(searchPickupHeader.getNoPairs()));
        }
