...
//		log.info("storagebinList : " + storagebinList);
     ...
     * @return
     */
    public StorageBin getStorageBinV3 (String referenceField1, String referenceField2, String companyCodeId, 
    		String plantId, String warehouseId, String languageId) {
        Optional<StorageBin> storagebin = storagebinRepository.findTopByCompanyCodeIdAndPlantIdAndWarehouseIdAndLanguageIdAndDeletionIndicatorAndReferenceField1ContainingAndReferenceField2(
                companyCodeId,
                plantId,
                warehouseId,
                languageId,
        ...
        if (storagebin.isEmpty()) {
            // Exception Log
            createStorageBinLog1(referenceField1, languageId, companyCodeId, plantId, warehouseId,
                    "Storage Bin with given values and binClassId - " + referenceField1 + " doesn't exists.");
            throw new BadRequestException("The Given Values:" +
...
//            throw new BadRequestException("The given StorageBin ID : " + warehouseId + ", statusId: " + statusId + " doesn't exist.");
    ...
    public List<StorageBin> findStorageBin(SearchStorageBin searchStorageBin) throws Exception {
        ...
        if(searchStorageBin.getStorageBin() != null && !searchStorageBin.getStorageBin().isEmpty()) {
            List<String> trimmedList = searchStorageBin.getStorageBin().stream().filter(n -> n != null && !n.isBlank()).map(String::trim).distinct().collect(Collectors.toList());
            searchStorageBin.setStorageBin(trimmedList);
        }
        log.info("trimmed searchStorageBin : " + searchStorageBin);
    ...
    public Stream<StorageBin> findStorageBinStream(SearchStorageBin searchStorageBin) throws Exception {
        ...
        log.info("stream searchStorageBin : " + searchStorageBin);
        if(searchStorageBin.getStorageBin() != null && !searchStorageBin.getStorageBin().isEmpty()) {
            List<String> trimmedList = searchStorageBin.getStorageBin().stream().filter(n -> n != null && !n.isBlank()).map(String::trim).distinct().collect(Collectors.toList());
            searchStorageBin.setStorageBin(trimmedList);
        }
        log.info("trimmed stream searchStorageBin : " + searchStorageBin);
