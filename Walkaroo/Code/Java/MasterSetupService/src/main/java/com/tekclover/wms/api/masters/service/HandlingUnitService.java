...
//		log.info("handlingunitList : " + handlingunitList);
	...
	public List<HandlingUnit> findHandlingUnit(SearchHandlingUnit searchHandlingUnit)
		...
		log.info("searchHandlingUnit : " + searchHandlingUnit);
		if(searchHandlingUnit.getHandlingUnit() != null && !searchHandlingUnit.getHandlingUnit().isEmpty()) {
			List<String> trimmedList = searchHandlingUnit.getHandlingUnit().stream().filter(n -> n != null && !n.isBlank()).map(String::trim).distinct().collect(Collectors.toList());
			searchHandlingUnit.setHandlingUnit(trimmedList);
		}
		log.info("trimmed searchHandlingUnit : " + searchHandlingUnit);
