...
public class HandlingEquipmentService {
     ...
     * @throws Exception
     */
    public List<HandlingEquipment> findHandlingEquipment(SearchHandlingEquipment searchHandlingEquipment) throws Exception {

        log.info("searchHandlingEquipment : " + searchHandlingEquipment);
        if(searchHandlingEquipment.getHandlingEquipmentId() != null && !searchHandlingEquipment.getHandlingEquipmentId().isEmpty()) {
            List<String> trimmedList = searchHandlingEquipment.getHandlingEquipmentId().stream().filter(n -> n != null && !n.isBlank()).map(String::trim).distinct().collect(Collectors.toList());
            searchHandlingEquipment.setHandlingEquipmentId(trimmedList);
        }
        log.info("trimmed searchHandlingEquipment : " + searchHandlingEquipment);
