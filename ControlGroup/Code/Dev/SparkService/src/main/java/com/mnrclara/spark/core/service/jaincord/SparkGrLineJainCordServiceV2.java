...
import com.mnrclara.spark.core.model.jaincord.GrLineJainCord;
...
@Service
@Slf4j
public class SparkGrLineJainCordServiceV2 {
    ...
    public SparkGrLineJainCordServiceV2() throws ParseException {
        ...
        //Read from Sql Table
        val df2 = sparkSession.read().option("fetchSize", "10000").jdbc("jdbc:sqlserver://43.230.156.163;databaseName=JAIN_CORD", "tblGrline", connProp)
                .repartition(16);
        df2.createOrReplaceTempView("tblGrlinejain");

    }

    public List<GrLineJainCord> findGrLineV2(FindGrLineV2 findGrLineV2) throws ParseException {
...
//                "TPL_GR_DATE as threePLGrDate, " +
                ...
                "FROM tblGrlinejain WHERE IS_DELETED = 0 ");
...
//        grLineQueryV2.cache();
        ...
        if (findGrLineV2.getPalletId() != null && !findGrLineV2.getPalletId().isEmpty()) {
            grLineQueryV2 = grLineQueryV2.filter(col("PALLET_ID").isin(findGrLineV2.getPalletId().toArray()));
        }

        Encoder<GrLineJainCord> grLineEncoder = Encoders.bean(GrLineJainCord.class);
        Dataset<GrLineJainCord> dataSetControlGroup = grLineQueryV2.as(grLineEncoder);
        List<GrLineJainCord> result = dataSetControlGroup.collectAsList();
