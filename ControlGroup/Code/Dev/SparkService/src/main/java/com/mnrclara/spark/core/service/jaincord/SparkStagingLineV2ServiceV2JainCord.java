...
import com.mnrclara.spark.core.model.jaincord.StagingLineJainCord;
...
@Service
@Slf4j
public class SparkStagingLineV2ServiceV2JainCord {
    ...
    public SparkStagingLineV2ServiceV2JainCord() throws ParseException {
        ...
        //Read from Sql Table
        val df2 = sparkSession.read().option("fetchSize", "10000").jdbc("jdbc:sqlserver://43.230.156.163;databaseName=JAIN_CORD", "tblstagingline", connProp)
                .repartition(16);
        df2.createOrReplaceTempView("tblstaginglineV5jain");
    }

    public List<StagingLineJainCord> findStagingLineV2(FindStagingLineV2 findStagingLineV2) throws ParseException {
        ...
        Dataset<Row>
                ...
                "GRLINE_FLAG as grLineFlag " +
                "FROM tblstaginglineV5jain WHERE IS_DELETED = 0 ");
...
//        stagingLineQueryV2.cache();
        ...
        Encoder<StagingLineJainCord> stagingLineEncoder = Encoders.bean(StagingLineJainCord.class);
        Dataset<StagingLineJainCord> dataSetControlGroup = stagingLineQueryV2.as(stagingLineEncoder);
        List<StagingLineJainCord> result = dataSetControlGroup.collectAsList();
