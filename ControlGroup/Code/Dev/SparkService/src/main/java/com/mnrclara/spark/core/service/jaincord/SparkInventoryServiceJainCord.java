...
import com.mnrclara.spark.core.model.jaincord.InventoryV2JainCord;
...
@Service
@Slf4j
public class SparkInventoryServiceJainCord {
     ...
     * @return
     */
    public List<InventoryV2JainCord> getInventory(FindInventoryV2 findInventory) {
        String sqlQuery = "SELECT INV_ID, LANG_ID, C_ID, PLANT_ID, WH_ID, " +
                ...
                "ORIGIN, BRAND, REF_DOC_NO, C_TEXT, PLANT_TEXT, WH_TEXT, STCK_TYP_TEXT, STATUS_TEXT, PARTNER_CODE, ITM_TYP_ID, " +
                "ITM_TYP_TXT, BATCH_DATE, SORT_NO, METER, LOT_NO, PIECE_ID, GSM, GRADE, COLOR, PALLET_ID FROM tblinventory";
        ...
        addCondition(conditions,  "SORT_NO", findInventory.getSortNo());
        ...
        Dataset<Row> data = spark.read()
                ...
                .jdbc("jdbc:sqlserver://43.230.156.163;databaseName=JAIN_CORD", "(" + sqlQuery + ") as tmp", connProp);
//                .repartition(16);

        Encoder<InventoryV2JainCord> inventoryV2CoreEncoder = Encoders.bean(InventoryV2JainCord.class);
        Dataset<InventoryV2JainCord> inventoryV2CoreDataset = data.as(inventoryV2CoreEncoder);
        List<InventoryV2JainCord> result = inventoryV2CoreDataset.collectAsList();
