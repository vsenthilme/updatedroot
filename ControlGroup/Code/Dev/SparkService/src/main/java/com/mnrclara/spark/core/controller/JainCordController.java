...
import com.mnrclara.spark.core.model.jaincord.*;
...
import com.mnrclara.spark.core.service.jaincord.*;
import com.mnrclara.spark.core.service.jaincord.sparkjoin.IBLineJainCordService;
import com.mnrclara.spark.core.service.jaincord.sparkjoin.OutboundJainCordService;
...
@Slf4j
@RestController
@Api(tags = {"JainCord"}, value = "User Operations related to JainCordController")
@SwaggerDefinition(tags = {@Tag(name = "User", description = "Operations related to JainCord")})
@RequestMapping("wms/spark/jain/cord")
public class JainCordController {

    @Autowired
    SparkPreInboundHeaderServiceV2JainCord sparkPreInboundHeaderService;

    @Autowired
    SparkOrderManagementLineServiceJainCordV2 sparkOrderManagementLineService;

    @Autowired
    SparkContainerReceiptJainCordServiceV2 sparkContainerReceiptService;

    @Autowired
    SparkGrHeaderJainCordServiceV2 sparkGrHeaderService;

    @Autowired
    SparkPutAwayHeaderServiceV2JainCord sparkPutAwayHeaderService;

    @Autowired
    SparkStagingHeaderServiceV2JainCord stagingHeaderService;

    @Autowired
    SparkInboundHeaderJainCordServiceV2 sparkInboundHeaderService;

    @Autowired
    SparkPreOutboundHeaderServiceV2JainCord sparkPreOutboundHeaderService;

    @Autowired
    SparkOutBoundReversalServiceV2JainCord sparkOutBoundReversalService;

    @Autowired
    SparkOutBoundHeaderServiceV2JainCord sparkOutBoundHeaderService;

    @Autowired
    SparkPickupHeaderServiceV2JainCord sparkPickupHeaderService;

    @Autowired
    SparkQualityHeaderServiceV2JainCord sparkQualityHeaderService;

    @Autowired
    SparkStorageBinServiceV2JainCord sparkStorageBinService;

    @Autowired
    SparkPickupLineServiceV2JainCord sparkPickupLineService;

    @Autowired
    SparkInventoryMovementServiceJainCord sparkInventoryMovementService;

    @Autowired
    SparkInhouseTransferLineServiceJainCord sparkInhouseTransferLineService;

    @Autowired
    SparkInventoryServiceJainCordV2 sparkInventoryService;

    @Autowired
    SparkPutAwayLineServiceV2JainCord sparkPutAwayLineService;

    @Autowired
    SparkPeriodicHeaderServiceV2JainCord sparkPeriodicHeaderService;

    @Autowired
    SparkPerpetualHeaderServiceV2JainCord sparkPerpetualHeaderService;

    @Autowired
    SparkInboundLineJainCordServiceV2 sparkInboundLineService;

    @Autowired
    SparkStockReportServiceV2JainCord sparkStockReportService;

    @Autowired
    SparkInhouseTransferHeaderServiceJainCord sparkInhouseTransferHeaderService;

    @Autowired
    SparkOrderStatusReportServiceV2JainCord sparkOrderStatusReportService;

    @Autowired
    SparkPerpetualLineServiceV2JainCord sparkPerpetualLineService;

    @Autowired
    SparkPeriodicLineServiceV2JainCord sparkPeriodicLineService;

    @Autowired
    SparkPreOutBoundLineServiceV2JainCord sparkPreOutBoundLineService;

    @Autowired
    SparkStagingLineV2ServiceV2JainCord sparkStagingLineV2Service;

    @Autowired
    SparkGrLineJainCordServiceV2 sparkGrLineService;

    @Autowired
    SparkQualityLineServiceV2JainCord sparkQualityLineService;

    @Autowired
    SparkImBasicData1JainCordServiceV2 sparkImBasicData1Service;
...
//    ClientGeneralService clientGeneralService;
    ...
    @Autowired
    ImBasicData1V3JainCordService imBasicData1V3CoreService;

    @Autowired
    SparkImBasicData1V5JainCordService sparkImBasicData1V5CoreService;

    @Autowired
    SparkBusinessPartnerJainCordService sparkBusinessPartnerCoreService;

    @Autowired
    IBLineJainCordService ibLineCoreService;

    @Autowired
    OutboundJainCordService outboundCoreService;

    @Autowired
    SparkJainCordPutAwayLineNewService sparkCorePutAwayLineNewService;

    @Autowired
    SparkInventoryServiceJainCord sparkInventoryServiceCore;

    @Autowired
    SparkInboundLineServiceJainCord sparkInboundLineServiceJainCord;
    ...
    public ResponseEntity<?> findPutAwayHeaderV2(@RequestBody SearchPutAwayHeaderV2 searchPutAwayHeaderV2) throws Exception {
        List<PutAwayHeaderJainCord> findPutAwayHeaderV2 = sparkPutAwayHeaderService.findPutAwayHeaderV2(searchPutAwayHeaderV2);
    ...
    // PickupHeaderV2
    @ApiOperation(response = PickupHeaderJainCord.class, value = "Spark Find PickupHeader")
    ...
    public ResponseEntity<?> findPickupHeaderV2(@RequestBody FindPickupHeaderV2 findPickupHeaderV2) throws Exception {
        List<PickupHeaderJainCord> pickupHeaderV2List = sparkPickupHeaderService.findPickupHeaderV2(findPickupHeaderV2);
    ...
    @ApiOperation(response = InventoryV2JainCord.class, value = "Spark Find Inventory New")
    ...
    public ResponseEntity<?> searchInventoryV2Core(@RequestBody FindInventoryV2 findInventoryV2) throws Exception {
        List<InventoryV2JainCord> inventoryList = sparkInventoryServiceCore.getInventory(findInventoryV2);
        return new ResponseEntity<>(inventoryList, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> searchPutAwayLineV2(@RequestBody FindPutAwayLineV2 findPutAwayLineV2) throws Exception {
        List<PutAwayLineJainCord> putAwayLineList = sparkPutAwayLineService.findPutAwayLineV2(findPutAwayLineV2);
    ...
    public ResponseEntity<?> getInboundLineV3(@RequestBody com.mnrclara.spark.core.model.Almailem.FindInboundLineV2 findInboundLineV2) {
        List<com.mnrclara.spark.core.model.Almailem.InboundLineV3> putAwayLines =  sparkInboundLineServiceJainCord.getInboundLine(findInboundLineV2);
    ...
    public ResponseEntity<?> findPeriodicLine(@RequestBody FindPeriodicLineV2 findPeriodicLineV2) throws Exception {
        List<PeriodicLineV2JainCord> findPeriodicLine = sparkPeriodicLineService.findPeriodicLine(findPeriodicLineV2);
    ...
    public ResponseEntity<?> findStagingLineV2(@RequestBody FindStagingLineV2 findStagingLineV2) throws Exception {
        List<StagingLineJainCord> stagingLineV2 = sparkStagingLineV2Service.findStagingLineV2(findStagingLineV2);
        return new ResponseEntity<>(stagingLineV2, HttpStatus.OK);
    }

    //Get GrLine
    @ApiOperation(response = GrLineJainCord.class, value = "Spark FindGrLine")
    @PostMapping("/grline")
    public ResponseEntity<?> findGrLineV2(@RequestBody FindGrLineV2 findGrLineV2) throws Exception {
        List<GrLineJainCord> stagingLineV2 = sparkGrLineService.findGrLineV2(findGrLineV2);
        return new ResponseEntity<>(stagingLineV2, HttpStatus.OK);
    }
...
//    }
    ...
    public ResponseEntity<?> imbasicData1Find(@RequestBody com.mnrclara.spark.core.model.Almailem.SearchImBasicData1 searchImBasicData1) throws Exception {
        List<ImBasicData1V3> imBasicData1V3s = sparkImBasicData1V5CoreService.searchImBasicData1(searchImBasicData1);
        return new ResponseEntity<>(imBasicData1V3s, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> businessPartner(@RequestBody FindBusinessPartner findBusinessPartner) throws Exception {
        List<BusinessPartnerV2> businessPartnerV2s = sparkBusinessPartnerCoreService.findBusinessPartner(findBusinessPartner);
    ...
    public ResponseEntity<?> inboundLine(@RequestBody com.mnrclara.spark.core.model.Almailem.FindInboundLineV2 findInboundLineV2) throws Exception {
        List<InboundLineV3> inboundHeaderV3s = ibLineCoreService.findInboundLine(findInboundLineV2);
    ...
    public ResponseEntity<?> outboundHeaderJoin(@RequestBody FindOutBoundHeader findInboundHeaderV2) throws Exception {
        List<Outbound> inboundHeaderV3s = outboundCoreService.findOutBoundHeader(findInboundHeaderV2);
        return new ResponseEntity<>(inboundHeaderV3s, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?>putAwayLineSpark(@RequestBody FindPutAwayLineV2 findPutAwayLineV2) {
        List<PutAwayLineJainCordNew> putAwayLineCoreList = sparkCorePutAwayLineNewService.getPutAwayLine(findPutAwayLineV2);
