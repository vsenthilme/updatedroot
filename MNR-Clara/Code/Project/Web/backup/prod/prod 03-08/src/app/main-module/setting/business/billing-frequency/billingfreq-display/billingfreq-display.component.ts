...
@Component({
  selector: 'app-billingfreq-display',
  templateUrl: './billingfreq-display.component.html',
  styleUrls: ['./billingfreq-display.component.scss']
})
export class BillingfreqDisplayComponent implements OnInit {
  ...
  form = this.fb.group({
    billingFrequencyId: [],
    billingFrequencyDescription: [, Validators.required],
    billingFrequencyStatus: ["Active",, [Validators.required]],
  ...
  constructor(
    public dialogRef: MatDialogRef<BillingfreqDisplayComponent>,
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      if (this.data.pageflow == 'Display')
        this.form.disable();
      this.fill();
      this.form.controls.billingFrequencyId.disable();
      this.form.controls.classId.disable();
  ...
  submit() {
    ...
    if (this.data.code) {
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success(res.billingFrequencyId + " saved successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
