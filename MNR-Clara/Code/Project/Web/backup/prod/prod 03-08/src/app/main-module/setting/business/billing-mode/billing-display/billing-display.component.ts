...
export class BillingDisplayComponent implements OnInit {
  ...
  form = this.fb.group({
    billingModeId: [],
    billingModeDescription: [, Validators.required],
    billingModeStatus: ["Active", [Validators.required]],
  ...
  constructor(
    public dialogRef: MatDialogRef<BillingDisplayComponent>,
  ...
  submit() {
    ...
    if (this.data.code) {
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success(res.billingModeId + " saved successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
