...
export class QuotationsNewComponent implements OnInit {
  ...
  displayedColumns: string[] = ['sno', 'itemNumber', 'itemDescription', 'quantity', 'rateperHour', 'totalAmount', 'delete'];
  ...
  dropdownlist() {
    ...
    this.cas.dropdownlist.setup.itemNumber.url,
      ...
      this.clientIdCorporationList = this.cas.foreachlist(results[1], this.cas.dropdownlist.client.clientId.key, { clientCategoryId: '1' });
      this.clientIdCorporationList.forEach((x: { key: string; value: string; }) => this.multicorporationList.push({ value: x.key, label: x.value }))
      this.multiselectcorporationList = this.multicorporationList;

      this.glList = this.cas.foreachlist(results[4], this.cas.dropdownlist.setup.itemNumber.key);
  ...
  fill(data: any) {
    ...
    if (data.pageflow != 'New') {
      ...
      this.sub.add(this.service.Get(data.code.quotationNo, data.code.quotationRevisionNo).subscribe(res => {
        ...
        //   }
        // });
this.form.controls.quotationAmount.patchValue(res.quotationAmount)
