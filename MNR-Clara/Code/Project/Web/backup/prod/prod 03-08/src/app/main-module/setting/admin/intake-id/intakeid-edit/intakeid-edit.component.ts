...
@Component({
  selector: 'app-intakeid-edit',
  templateUrl: './intakeid-edit.component.html',
  styleUrls: ['./intakeid-edit.component.scss']
})
export class IntakeidEditComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    intakeFormDescription: [, [Validators.required]],
    createdOn: [],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      ...
      this.form.controls.clientTypeId.disable();
  ...
  dropdownlist() {
    this.spin.show();
    this.cas.getalldropdownlist([this.cas.dropdownlist.setup.languageId.url, this.cas.dropdownlist.setup.screenId.url,
    this.cas.dropdownlist.setup.clientTypeId.url, this.cas.dropdownlist.setup.classId.url]).subscribe((results) => {

      this.languageIdList = this.cas.foreachlist(results[0], this.cas.dropdownlist.setup.languageId.key);
      this.screenIdList = this.cas.foreachlist(results[1], this.cas.dropdownlist.setup.screenId.key, true);
      this.clientTypeList = this.cas.foreachlist(results[2], this.cas.dropdownlist.setup.clientTypeId.key);
  ...
  fill() {
    ...
    this.sub.add(this.service.Get(this.data.code).subscribe(res => {
      ...
      this.form.controls.referenceField1.patchValue(Number(res.referenceField1));
      this.spin.hide();
    }, err => {
  ...
  submit() {
    ...
    if (this.data.code) {
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success(res.intakeFormId + " saved successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
