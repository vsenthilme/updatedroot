...
@Component({
  ...
  styleUrls: ['./usertype-edit.component.scss']
})
export class UsertypeEditComponent implements OnInit {
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code.userTypeId, this.data.code.classId, this.data.code.languageId).subscribe(res => {
  ...
  submit() {
    ...
    if (this.data.code) {
      this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code.userTypeId).subscribe(res => {
        this.toastr.success(this.data.code.userTypeId + " updated successfully!","Notification",{
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
