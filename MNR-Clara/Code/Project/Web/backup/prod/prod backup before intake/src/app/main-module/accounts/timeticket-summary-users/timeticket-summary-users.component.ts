...
export class TimeticketSummaryUsersComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'classId','clientId', 'matterNumber','timeTicketNumber', 'timeTicketDate', 'createdOn', 'billType', 'timeTicketHours', 'activityCode', 'timeTicketDescription',  'taskCode', 'defaultRate', 'timeTicketAmount', 'timeKeeperCode', 'statusIddes'];
...
if(this.searhform.controls.startTimeTicketDate.value){
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      // let date = x.timeTicketDate
      // if(x.timeTicketDate != null && x.timeTicketDate){
      //  let dateStringArray = x.timeTicketDate.split('-')
      ...
      res.push({
        ...
        "Date ": this.cs.dateapiutc0(x.timeTicketDate),
