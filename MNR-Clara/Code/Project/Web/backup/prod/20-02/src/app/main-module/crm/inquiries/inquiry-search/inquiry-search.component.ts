...
export class InquirySearchComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'action', 'inquiryNumber', 'inquiryDate', 'inquiryModeId', 'firstName', 'lastName', 'assignedUserId', 'createdOn', 'classIddes', 'email','referenceField4', 'contactNumber', 'notes', 'sms', 'referenceField2', 'referenceField3', 'statusIddesc',];
...
this.field = true;
      ...
      this.columnsToDisplay = ['select', 'action', 'statusIddesc', 'inquiryNumber', 'inquiryDate', 'inquiryModeId','firstName', 'lastName', 'assignedUserId', 'classIddes', 'email','notes', 'sms','referenceField4'];
      this.Update = 'Assign';
      this.isnew = false;
    }
      ...
      this.columnsToDisplay = ['select', 'action', 'statusIddesc', 'inquiryNumber', 'inquiryDate', 'inquiryModeId', 'firstName', 'lastName', 'email','contactNumber', 'notes', 'sms','referenceField4'];
    } else if (this.pageflow == 'Inquiry Validation') {
      this.field = true;
      this.RA = this.auth.getRoleAccess(1064);
      this.columnsToDisplay = ['select', 'action', 'statusIddesc', 'inquiryNumber', 'inquiryDate', 'inquiryModeId', 'lastName', 'email', 'contactNumber', 'assignedUserId', 'notes', 'sms',];
      this.Update = 'Assign';
      this.columnsToDisplay = ['select', 'action', 'statusIddesc', 'inquiryNumber', 'inquiryDate', 'inquiryModeId', 'firstName', 'lastName', 'email', 'contactNumber', 'assignedUserId', 'notes', 'sms','referenceField4'];
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        ...
        'Inquiry Date ': this.cs.dateapi(x.inquiryDate),
  ...
  search() {
    ...
    this.searhform.controls.inqEndDate.patchValue(this.cs.day_callapiSearch(this.searhform.controls.inqEndDate.value));
    this.searhform.controls.inqStartDate.patchValue(this.cs.day_callapiSearch(this.searhform.controls.inqStartDate.value));
    let classIdList: any[] = []
    this.spin.show();
