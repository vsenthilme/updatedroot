...
export class ClientMattersComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'matterNumber', 'matterDescription', 'clientId', 'caseInformationNo', 'caseCategoryId', 'caseOpenedDate', 'status',];
  ...
  getall(excel: boolean = false) {
    ...
    this.cas.getalldropdownlist([
      ...
      this.sub.add(this.service.Search({clientId : [this.searhform.controls.clientId.value[0]]}).subscribe((res: GeneralMatterElement[]) => {
