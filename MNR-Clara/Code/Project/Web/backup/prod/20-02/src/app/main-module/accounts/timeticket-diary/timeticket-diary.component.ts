...
export class TimeticketDiaryComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'classId','clientId', 'matterNumber','timeTicketNumber', 'stimeTicketDate', 'screatedOn', 'billType', 'timeTicketHours', 'activityCode', 'timeTicketDescription',  'taskCode', 'defaultRate', 'timeTicketAmount', 'timeKeeperCode', 'statusIddes'];
  ...
  getall(excel: boolean = false) {
    ...
    this.cas.getalldropdownlist([
      ...
      this.statuslist = this.cas.foreachlist_searchpage(results[1], this.cas.dropdownlist.setup.statusId.key).filter(s => [33, 34, 35, 46, 56, 51].includes(s.key));
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      ...
      res.push({
        ...
        'Billable Amount ': x.timeTicketAmount,
    ...
    res.push({
      ...
      "Time Ticket hours  ": this.getTotalHours(),
      ...
      'Billable Amount ': this.getBillableAmount(),
      "Time Keeper": '',
      "Status": '',
      //   'Creation Date ': this.cs.dateapi(x.createdOn),     //error throwing
    });
