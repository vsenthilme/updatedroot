...
@Component({
  selector: 'app-activitycode-display',
  templateUrl: './activitycode-display.component.html',
  styleUrls: ['./activitycode-display.component.scss']
})
export class ActivitycodeDisplayComponent implements OnInit {
  screenid: 1026 | undefined;
  ...
  form = this.fb.group({
    ...
    activityCodeDescription: [, [Validators.required]],
    activityCodeStatus: ['Active', [Validators.required]],
    deletionIndicator: [0],
  ...
  constructor(
    public dialogRef: MatDialogRef<ActivitycodeDisplayComponent>,
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      if (this.data.pageflow == 'Display')
        this.form.disable();
      this.fill();
      this.form.controls.activityCode.disable();
      this.form.controls.classId.disable();
      this.form.controls.languageId.disable();
  ...
  submit() {
    ...
    if (this.data.code) {
      this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code).subscribe(res => {
        this.toastr.success(res.activityCode + " updated successfully!","Notification",{
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success(res.activityCode + " saved successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
