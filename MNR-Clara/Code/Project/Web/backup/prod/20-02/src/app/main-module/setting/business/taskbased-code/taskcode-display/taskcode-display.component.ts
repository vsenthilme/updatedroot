...
@Component({
  selector: 'app-taskcode-display',
  templateUrl: './taskcode-display.component.html',
  styleUrls: ['./taskcode-display.component.scss']
})
export class TaskcodeDisplayComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    taskcodeDescription: [, [Validators.required]],
    taskCodeStatus: ['Active', [Validators.required]],
    taskcodeType: [, [Validators.required]],
    summaryLevel: [false, [Validators.required]],
    deletionIndicator: [0],
  ...
  constructor(
    public dialogRef: MatDialogRef<TaskcodeDisplayComponent>,
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      if (this.data.pageflow == 'Display')
        this.form.disable();
      this.fill();
      this.form.controls.taskCode.disable();
      this.form.controls.classId.disable();
  ...
  submit() {
    ...
    if (this.data.code) {
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success(res.taskCode + " saved successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
