...
@Component({
  selector: 'app-casecategory-display',
  templateUrl: './casecategory-display.component.html',
  styleUrls: ['./casecategory-display.component.scss']
})
export class CasecategoryDisplayComponent implements OnInit {
  ...
  dropdownlist() {
    this.spin.show();
    this.cas.getalldropdownlist([this.cas.dropdownlist.setup.classId.url, this.cas.dropdownlist.setup.caseCategoryId.url
    ]).subscribe((results) => {
  ...
  submit() {
    ...
    if (this.data.code) {
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success(res.caseCategory + " saved successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
