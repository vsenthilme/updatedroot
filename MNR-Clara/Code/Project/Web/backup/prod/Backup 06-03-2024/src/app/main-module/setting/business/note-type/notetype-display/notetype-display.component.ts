...
@Component({
  selector: 'app-notetype-display',
  templateUrl: './notetype-display.component.html',
  styleUrls: ['./notetype-display.component.scss']
})
export class NotetypeDisplayComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    noteTypeText: [,  [Validators.required]],
    classId: [3, [Validators.required]],
    languageId: ['EN', [Validators.required]],
    noteTypeStatus: ["Active", [Validators.required]],
    createdOn: [],
    updatedOn: [],
  ...
  formgr: NoteTypeElement | undefined;
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      if (this.data.pageflow == 'Display')
        this.form.disable();
      this.fill();
      this.form.controls.noteTypeId.disable();
      this.form.controls.classId.disable();
    }
  }
  ...
  submit() {
    ...
    if (this.data.code) {
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success(res.noteTypeId + " saved successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
