  ...
  getAllDropdownList(excel: boolean = false) {
   ...
   // this.cas.dropdownlist.matter.matterNumber.url,
     ...
     // this.classIdList = this.cas.foreachlist_searchpage(results[0], this.cas.dropdownlist.setup.classId.key);
      this.statuslist = this.cas.foreachlist_searchpage(results[0], this.cas.dropdownlist.setup.statusId.key).filter(s => [1, 10, 52].includes(s.key));
    ...
    //  this.sub.add(this.service.Getall().subscribe(res => {
      ...
      if (this.ELEMENT_DATA.length > 0) {
        ...
        uniqueQuotationNumberArray.forEach(quotationNumber => {
          ...
          if(quotationNumber == "900412"){
            console.log(groupedQuotationList)
          }
