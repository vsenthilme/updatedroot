...
import { BillingfreqDisplayComponent } from "./billingfreq-display/billingfreq-display.component";
...
export class BillingFrequencyComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'classId', 'billingFrequencyId', 'billingFrequencyDescription', 'billingFrequencyStatus', 'createdBy', 'createdOn'];
  ...
  display(): void {
    ...
    const dialogRef = this.dialog.open(BillingfreqDisplayComponent, {
      disableClose: true,
      width: '50%',
      maxWidth: '80%',
      position: { top: '6.5%' },
...
this.multiselectstatusList = this.multistatusList;
  ...
  deleteDialog() {
    ...
    const dialogRef = this.dialog.open(DeleteComponent, {
      ...
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: this.selection.selected[0].billingFrequencyId,
    });

    dialogRef.afterClosed().subscribe(result => {

      if (result) {
        this.deleterecord(this.selection.selected[0].billingFrequencyId);
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(BillingfreqDisplayComponent, {
      ...
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: { pageflow: data, code: data != 'New' ? this.selection.selected[0].billingFrequencyId : null }
    });
  ...
  getall(excel: boolean = false) {
    ...
    this.cas.getalldropdownlist([this.cas.dropdownlist.setup.classId.url,
      ...
      this.sub.add(this.service.Getall().subscribe((res: any[]) => {
        ...
        res.forEach((x: { billingFrequencyId: string; billingFrequencyDescription:string;}) => this.multibillfreqidList.push({value: x.billingFrequencyId, label: x.billingFrequencyId + '-' + x.billingFrequencyDescription}))
        this.multibillfreqidList = this.cas.removeDuplicatesFromArrayNew(this.multibillfreqidList);

        this.ELEMENT_DATA = res;

        if (excel)
          this.excel.exportAsExcel(res, "Billing Frequency");
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        "Class": x.classId,
        'Billing Frequency': x.billingFrequencyId,
        "Description ": x.billingFrequencyDescription,
        'Status': x.billingFrequencyStatus,
  ...
  checkboxLabel(row?: any): string {
    ...
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.billingFrequencyId + 1}`;
  }
  ...
  searhform = this.fb.group({
    classId: [],
    billingFrequencyId: [],
    billingFrequencyStatus: [],
    billingFrequencyStatusFE: [],
    createdByFE: [],
    createdBy: [],
    createdOn_from: [],
    createdOn_to: [],
  ...
  search() {
    if (this.selectedItems2 && this.selectedItems2.length > 0){
      ...
      this.searhform.patchValue({billingFrequencyStatus: this.selectedItems2 });
    }
