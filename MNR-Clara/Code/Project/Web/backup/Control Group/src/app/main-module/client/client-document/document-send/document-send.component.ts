...
export class DocumentSendComponent implements OnInit {
  ...
  async fill(code: any) {
    ...
    this.sub.add(this.service.Get(code).subscribe(async ress => {
     ...
     // this.locationfile = 'document/' + ress.documentNo + '_' + ress.clientId;//+ '_' + code;
     this.locationfile = 'document/' + ress.clientId;
  ...
  uploadFiles(files: File[]) {
    ...
    this.sub.add(this.service.Get(this.code).subscribe(async ress => {
      ...
      this.spin.hide();
      let url = '/mnr-management-service/clientdocument/' + ress.clientId + '/mailmerge/manual?' +
        'location=' + this.locationfile +
        '&documentNumber=' + ress.documentNo +
        '&classId=' + ress.classId;
      // if (ress.mailMerge)
      //   url = '/mnr-management-service/clientdocument/' + ress.clientId + '/nonMailmerge?' +
      //     'location=' + this.locationfile +
      //     '&documentNumber=' + ress.documentNo +
      //     '&classId=' + ress.classId;
  ...
  post_agreement() {
    ...
    this.sub.add(this.service.Get(this.code).subscribe(res => {
      this.sub.add(this.service.send_document(res.classId, res.clientId, res.documentNo,).subscribe(ress => {
        let response = res;
