...
export class CountryComponent implements OnInit {
  screenid = 1035;
  displayedColumns: string[] = ['select', 'countryId','countryName', 'createdBy',  'createdOn'];
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.deleterecord(this.selection.selected[0].countryId,this.selection.selected[0].languageId,this.selection.selected[0].companyId);
      }
    });
  }
  deleterecord(id: any,languageId:any,companyId) {
    ...
    this.sub.add(this.service.Delete(id,languageId,companyId).subscribe((res) => {
      this.toastr.success(id + " Country Id deleted successfully!", "Notification", {
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(CountryNewComponent, {
      ...
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: { pageflow: data, code: data != 'New' ? this.selection.selected[0].countryId : null,languageId: data != 'New' ? this.selection.selected[0].languageId : null,companyId: data != 'New' ? this.selection.selected[0].companyId : null }
    });
    dialogRef.afterClosed().subscribe(result => {
      this.getAllListData(); 
     this.selection.clear()
  ...
  getall() {
    ...
    this.sub.add(this.service.search(obj).subscribe((res: any[]) => {
      ...
      this.spin.hide();
      res.forEach(
        (x: { languageId: string}) => this.multilanguageList.push({value: x.languageId, label: x.languageId }))
      ...
      res.forEach((x: { countryId: string,countryName: string}) => this.multicountryList.push({value: x.countryId, label: x.countryId+'-' + x.countryName }))
  ...
  downloadexcel() {
    ...
    this.excel.exportAsExcel(res, "Country");
  }
  getAllListData() {
    this.getall();
  }
  ...
  searhform = this.fb.group({
   ...
   search() {
    ...
    this.searhform.controls.createdOn_to.patchValue(this.cs.day_callapiSearch(this.searhform.controls.createdOn_to.value));
    this.searhform.controls.createdOn_from.patchValue(this.cs.day_callapiSearch(this.searhform.controls.createdOn_from.value));
