...
import {
  ClientNewComponent
} from '../client/client-new/client-new.component';
import {
  ControlgroupService
} from './controlgroup.service';
import {
  ControlgroupNewComponent
} from './controlgroup-new/controlgroup-new.component';
import {
  DeleteComponent
...
@Component({
  selector: 'app-controlgroup',
  templateUrl: './controlgroup.component.html',
  styleUrls: ['./controlgroup.component.scss']
})
export class ControlgroupComponent implements OnInit {
  screenid = 1035;
  displayedColumns: string[] = ['select', 'groupId', 'groupName', 'groupTypeName', 'statusId', 'createdBy', 'createdOn'];
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result === 'confirm') {
        ...
        let obj: any = {}
        obj.groupId = [this.selection.selected[0].groupId];
        obj.companyId = ["1000"];
        ...
        this.sub.add(this.service.searchStorePartner(obj).subscribe(res => {
          ...
          if (res.length > 0) {
            const dialogRef1 = this.dialog.open(ConfirmComponent, {
              ...
              data: {
                title: "Delete",
                message: "The Selected Group Cannot be deleted as it contains associated data in Store Partner Listing  ",
                pageflow:"delete"
              }
            });
            ...
            this.deleterecord(
              this.selection.selected[0].groupId,
  ...
  deleterecord(id: any, languageId: any, companyId: any, groupTypeId: any, versionNumber: any) {
    ...
    this.sub.add(this.service.Delete(id, languageId, companyId, groupTypeId, versionNumber).subscribe((res) => {
      this.toastr.success(id + "  Group Type Id deleted successfully!", "Notification", {
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(ControlgroupNewComponent, {
      ...
      data: {
        pageflow: data,
        code: data != 'New' ? this.selection.selected[0].groupId : null,
  ...
  getall() {
    ...
    this.sub.add(this.service.search(obj).subscribe((res: any[]) => {
        ...
        res.forEach((x: {
          groupId: string,
          groupName: string
        }) => this.multigroupidList.push({
          value: x.groupId,
          label: x.groupId + '-' + x.groupName
        }))
  ...
  downloadexcel() {
    ...
    this.excel.exportAsExcel(res, "Group");
  }
  getAllListData() {
    this.getall();
  }
