...
import {
  CompanyNewComponent
} from '../company/company-new/company-new.component';
import {
  SubgroupService
} from './subgroup.service';
import {
  SubgroupNewComponent
} from './subgroup-new/subgroup-new.component';
import {
  DeleteComponent
...
export class SubgroupComponent implements OnInit {
  screenid = 1035;
  displayedColumns: string[] = ['select',  'subGroupTypeId', 'subGroupTypeName', 'groupTypeName', 'statusId', 'createdBy', 'createdOn'];
  ...
  constructor(public dialog: MatDialog,
    private service: SubgroupService,
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result === 'confirm') {
        ...
        let obj: any = {}
        obj.subGroupTypeId = [this.selection.selected[0].subGroupTypeId];
        obj.companyId = ["1000"];
        ...
        this.sub.add(this.service.searchStorePartner(obj).subscribe(res => {
          ...
          if (res.length > 0) {
            const dialogRef1 = this.dialog.open(ConfirmComponent, {
              ...
              data: {
                title: "Delete",
                message: "The Selected Sub Group Cannot be deleted as it contains associated data in Store Partner Listing  ",
                pageflow:"delete"
              }
            });
            ...
            this.deleterecord(
              this.selection.selected[0].subGroupTypeId,
  ...
  deleterecord(id: any, languageId: any, companyId, groupTypeId: any,versionNumber:any) {
    this.spin.show();
  ...
  multisubgroupList: any[] = [];
  getall() {
    this.multisubgroupList=[];
    let obj: any = {};
    obj.statusId = [0];
    this.spin.show();
    this.sub.add(this.service.search(obj).subscribe((res: any[]) => {
        ...
        res.forEach((x: {
          companyId: string,companyIdAndDescription:string
        ...
        res.forEach((x: {
          groupTypeId: string,groupTypeName: string
        }) => this.multigroupTypeList.push({
          value: x.groupTypeId,
          label: x.groupTypeId +'-'+x.groupTypeName
        }))
        this.multigroupTypeList = this.cas.removeDuplicatesFromArrayNew(this.multigroupTypeList);
        res.forEach((x: {
          subGroupTypeId: string,
          subGroupTypeName: string
        }) => this.multisubgroupList.push({
          value: x.subGroupTypeId,
          label: x.subGroupTypeId + '-' + x.subGroupTypeName
        }))
        this.multisubgroupList = this.cas.removeDuplicatesFromArrayNew(this.multisubgroupList);
        res.forEach((x: {
          createdBy: string;
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        ...
        "Sub Group Type ID": x.subGroupTypeId,
    ...
    this.excel.exportAsExcel(res, "Sub Group Type");
  }
  getAllListData() {
    this.getall();
  }
