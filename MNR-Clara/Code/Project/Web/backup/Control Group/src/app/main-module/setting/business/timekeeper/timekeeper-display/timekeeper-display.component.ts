...
export class TimekeeperDisplayComponent implements OnInit {
  ...
  dropdownlist() {
    ...
    this.cas.dropdownlist.setup.userTypeId.url,
      ...
      this.userTypeList.forEach((x: { key: string; value: string; }) => this.multiuserIdList.push({value: x.key, label:  x.value}))
  ...
  classdropdownlist(x: any) {
    ...
    if (x) {
      ...
      this.cas.getalldropdownlist([
        ...
        this.userIdList.forEach((x: { key: string; value: string; }) => this.multitimekeeperList.push({value: x.key, label:  x.key}))
...
console.log(res)
  ...
  onItemSelect(item: any) {
    ...
    if (item.value) this.getTimekeeperdetails(item.value);
