...
import { LanguageService, LanguageElement } from "../language.service";
...
export class LanguageEditComponent implements OnInit, OnDestroy {
  ...
  formgr: LanguageElement | undefined;
  ...
  dropdownlist() {
    ...
    this.cas.getalldropdownlist([this.cas.dropdownlist.setup.classId.url]).subscribe((results) => {
      for (let item of results) {
        this.classidList = this.cas.foreachlist(item, this.cas.dropdownlist.setup.classId.key);
      }
    }, (err) => {
  ...
  submit() {
    ...
    if (this.data.code) {
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success(res.languageId + " saved successfully","Notification",{
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
