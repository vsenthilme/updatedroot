...
import {
  CompanyNewComponent
} from './company-new/company-new.component';
import {
  CompanyService
...
export class CompanyComponent implements OnInit {
  screenid = 1035;
  displayedColumns: string[] = ['select', 'languageId', 'companyId', 'description', 'createdBy', 'createdOn'];
      ...
      this.sub.add(this.service.searchStorePartner(obj).subscribe(res => {
        ...
        if (res.length > 0) {
          const dialogRef1 = this.dialog.open(ConfirmComponent, {
            ...
            data: {
              title: "Delete",
              message: 'The Selected Company Cannot be deleted as it contains associated data in Store Partner Listing',
              pageflow:"delete"
            }
          });
          this.spin.hide();
          ...
          if(res.length == 0){
            ...
            this.deleterecord(this.selection.selected[0].companyId, this.selection.selected[0].languageId);
  ...
  deleterecord(id: any, languageId: any) {
    ...
    this.sub.add(this.service.Delete(id, languageId).subscribe((res) => {
      this.toastr.success(id + " Company Id deleted successfully!", "Notification", {
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(CompanyNewComponent, {
      ...
      data: {
        pageflow: data,
        code: data != 'New' ? this.selection.selected[0].companyId : null,
  ...
  getall() {
    ...
    this.sub.add(this.service.search(obj).subscribe((res: any[]) => {
        ...
        res.forEach((x: {
          ...
          value: x.companyId,
          label: x.companyId + '-' + x.description
        }))
