...
export class StatusComponent implements OnInit {
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.deleterecord(this.selection.selected[0].statusId, this.selection.selected[0].languageId);
      }
    });
  }
  deleterecord(id: any, languageId: any) {
    ...
    this.sub.add(this.service.Delete(id, languageId).subscribe((res) => {
      this.toastr.success(id + " Status Id deleted successfully!", "Notification", {
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(StatusNewComponent, {
      disableClose: true,
      width: '50%',
      maxWidth: '80%',
      position: {
      ...
      data: {
        pageflow: data,
        code: data != 'New' ? this.selection.selected[0].statusId : null,
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        ...
        "Status Description": x.status,
        'Created By': x.createdBy,
        'Created On': this.cs.excel_date(x.createdOn)
      });
    })
    this.excel.exportAsExcel(res, "Status");
  }
  getAllListData() {
    this.getall();
  }
