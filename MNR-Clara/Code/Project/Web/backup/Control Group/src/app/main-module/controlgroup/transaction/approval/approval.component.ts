...
export class ApprovalComponent implements OnInit {
  ...
  displayedColumns  = ['select', 'action', 'requestId', 'storeId', 'createdBy', 'createdOn', 'group', 'template'];
  ...
  ngOnInit(): void {
    ...
    if(this.js.pageflow == 'transfer'){
      this.displayedColumns  = ['select', 'action','languageId', 'companyId', 'requestId', 'storeId', 'createdBy', 'createdOn'];
    }
  }
  deleteDialog() {
  ...
  deleterecord(id: any, languageId: any, companyId: any) {
    ...
    let paramdata = "";
      ...
      if(this.js.pageflow == 'transfer'){
        paramdata = this.cs.encrypt({ line: this.js.line, pageflow: 'transfer'});
      }else{
  ...
  generatePdf(element: any) {
   ...
   // pdfMake.createPdf(dd).download('Store_Group_Report');
    ...
    pdfDocGenerator.getBlob((blob) => {
      var file = new File([blob], this.js.line.requestId + "_Store_Group_Report" + ".pdf"); //  + (new Date().getDate()) +'-'+ (new Date().getMonth() + 1) + '-' + new Date().getFullYear() + '_' + this.cs.timeFormat(new Date()) +  
      if (file) {           
        this.spin.show();
        this.service.uploadfile(file, 'sharepoint/qb').subscribe((resp: any) => {
          this.service.Update({
            referenceField1: resp.file
          }, this.js.line.requestId,this.js.line.languageId , this.js.line.companyId).subscribe(res => {
