...
@Component({
  selector: 'app-admincost-display',
  templateUrl: './admincost-display.component.html',
  styleUrls: ['./admincost-display.component.scss']
})
export class AdmincostDisplayComponent implements OnInit {

  screenid: 1036 | undefined;
  ...
  form = this.fb.group({
    flatFeeRateFrom: [, Validators.required],
    flatFeeRateTo: [, Validators.required],
    adminCost: [, Validators.required],
    adminCostId:[],
    rateUnit: [],
    trafficStatus: ["Active", [Validators.required]],
  ...
  constructor(
    public dialogRef: MatDialogRef<AdmincostDisplayComponent>,
  ...
  submit() {
    ...
    if (this.data.code) {
      this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code).subscribe(res => {
        this.toastr.success("Admin Cost updated successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success("Admin Cost saved successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
