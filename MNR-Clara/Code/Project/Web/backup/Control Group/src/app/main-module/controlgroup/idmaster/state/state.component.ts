...
export class StateComponent implements OnInit {
  screenid = 1035;
  displayedColumns: string[] = ['select', 'stateId','stateName','countryIdAndDescription', 'createdBy',  'createdOn'];
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.deleterecord(this.selection.selected[0].stateId,this.selection.selected[0].languageId,this.selection.selected[0].companyId);
      }
    });
  }
  deleterecord(id: any,languageId:any,companyId) {
    ...
    this.sub.add(this.service.Delete(id,languageId,companyId).subscribe((res) => {
      this.toastr.success(id + " State Id deleted successfully!", "Notification", {
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(StateNewComponent, {
      ...
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: { pageflow: data, code: data != 'New' ? this.selection.selected[0].stateId : null,languageId: data != 'New' ? this.selection.selected[0].languageId : null,companyId: data != 'New' ? this.selection.selected[0].companyId : null,countryId: data != 'New' ? this.selection.selected[0].countryId : null }
    });
    dialogRef.afterClosed().subscribe(result => {
      this.getAllListData(); 
     this.selection.clear()
  ...
  getall() {
    ...
    this.sub.add(this.service.search(obj).subscribe((res: any[]) => {
      ...
      res.forEach((x: { companyId: string,companyIdAndDescription:any}) => this.multicompanyList.push({value: x.companyId, label: x.companyId +'-'+x.companyIdAndDescription}))
      this.multicompanyList = this.cas.removeDuplicatesFromArrayNew(this.multicompanyList);
      res.forEach((x: { countryId: string,countryIdAndDescription:string}) => this.multicountryList.push({value: x.countryId, label: x.countryId+'-'+x.countryIdAndDescription }))
      this.multicountryList = this.cas.removeDuplicatesFromArrayNew(this.multicountryList);
      res.forEach((x: { stateId: string,stateName:string}) => this.multistateList.push({value: x.stateId, label: x.stateId+'-' + x.stateName  }))
  ...
  downloadexcel() {
    ...
    this.excel.exportAsExcel(res, "State");
  }
  getAllListData() {
    this.getall();
  }
