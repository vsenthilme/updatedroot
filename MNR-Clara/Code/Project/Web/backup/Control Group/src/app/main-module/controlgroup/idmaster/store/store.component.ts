...
import {
  StateNewComponent
} from '../state/state-new/state-new.component';
import {
  StoreService
} from './store.service';
import {
  StoreNewComponent
} from './store-new/store-new.component';
import {
  DeleteComponent
...
export class StoreComponent implements OnInit {
  screenid = 1035;
  displayedColumns: string[] = ['select',  'storeId', 'storeName', 'countryIdAndDescription', 'stateIdAndDescription', 'cityIdAndDescription', 'address', 'phoneNo', 'status', 'createdBy', 'createdOn'];
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result === 'confirm') {
        ...
        let obj: any = {};
        let storeId1 = this.selection.selected[0].storeId.toString();
        obj.storeId = [storeId1];
        obj.companyId = ["1000"];
        ...
        this.sub.add(this.service.searchStorePartner(obj).subscribe(res => {
          ...
          if (res.length > 0) {
            const dialogRef1 = this.dialog.open(ConfirmComponent, {
              ...
              data: {
                title: "Delete",
                message: "The Selected Store Cannot be deleted as it contains associated data in Store Partner Listing  ",
                pageflow:"delete"
              }
            });
            ...
            this.deleterecord(
              this.selection.selected[0].storeId,
  ...
  deleterecord(id: any, languageId: any, companyId) {
    ...
    this.sub.add(this.service.Delete(id, languageId, companyId).subscribe((res) => {
      this.toastr.success(id + " Store Id deleted successfully!", "Notification", {
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(StoreNewComponent, {
      ...
      data: {
        pageflow: data,
        code: data != 'New' ? this.selection.selected[0].storeId : null,
  ...
  getall() {
    ...
    this.sub.add(this.service.search(obj).subscribe((res: any[]) => {
      ...
      res.forEach((x: {
        ...
        value: x.country,
        label: x.country + '-' + x.countryIdAndDescription
      }))
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        ...
        "City Name": x.cityIdAndDescription,
        "Address": x.address,
        "Phone ": x.phoneNo,
    ...
    this.excel.exportAsExcel(res, "Store");
  }
  getAllListData() {
    this.getall();
  }
  ...
  checkboxLabel(row ? : any): string {
    ...
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.storeId + 1}`;
  }
