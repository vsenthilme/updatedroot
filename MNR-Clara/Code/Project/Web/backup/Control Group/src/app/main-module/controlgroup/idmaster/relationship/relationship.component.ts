...
import { RelationshipNewComponent } from './relationship-new/relationship-new.component';
...
export class RelationshipComponent implements OnInit {
  ...
  screenid = 1035;
  displayedColumns: string[] = ['select', 'relationShipId','description','statusId','createdBy',  'createdOn'];
  ...
  display(): void {
    ...
    const dialogRef = this.dialog.open(RelationshipNewComponent, {
      disableClose: true,
      width: '50%',
      maxWidth: '80%',
      position: { top: '6.5%' },
  ...
  selection = new SelectionModel<any>(true, []);
   deleteDialog() {
     ...
     dialogRef.afterClosed().subscribe(result => {
       ...
       if (result) {
         this.deleterecord(this.selection.selected[0].relationShipId,this.selection.selected[0].languageId,this.selection.selected[0].companyId);
    ...
    deleterecord(id: any,languageId:any,companyId) {
     ...
     this.sub.add(this.service.Delete(id,languageId,companyId).subscribe((res) => {
       this.toastr.success(id + " Relationship Id deleted successfully!", "Notification", {
         timeOut: 2000,
         progressBar: false,
       });
       this.spin.hide();
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(RelationshipNewComponent, {
      ...
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: { pageflow: data, code: data != 'New' ? this.selection.selected[0].relationShipId: null,languageId: data != 'New' ? this.selection.selected[0].languageId : null,companyId: data != 'New' ? this.selection.selected[0].companyId : null }
    });
    dialogRef.afterClosed().subscribe(result => {
      this.getAllListData(); 
  ...
  dropdownlist(){
   ...
   this.multilanguageList.forEach((x: { key: string; value: string; }) => this.dropdownSelectLanguageID.push({value: x.key, label:  x.value}))
    this.spin.hide();
  ...
  getall() {
   ...
   obj.statusId=[0];
    ...
    this.sub.add(this.service.search(obj).subscribe((res: any[]) => {
       ...
       res.forEach((x: { companyId: string,companyIdDescription:string}) => this.multicompanyList.push({value: x.companyId, label: x.companyId +'-'+x.companyIdDescription }))
       this.multicompanyList = this.cas.removeDuplicatesFromArrayNew(this.multicompanyList);
       res.forEach((x: { relationShipId: string,description: string,}) => this.multigroupList.push({value: x.relationShipId, label: x.relationShipId+'-' + x.description }))
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      ...
      //  "Company Name":x.companyIdDescription,
       "Relationship ID":x.relationShipId,
       "Relationship Name":x.description,
    ...
    this.excel.exportAsExcel(res, "Relationship");
  }
  getAllListData() {
    this.getall();
  }
