...
import { SendEmailApprovalComponent } from './send-email-approval/send-email-approval.component';
...
export class ProposedComponent implements OnInit {
  ...
  deleterecord(id: any, languageId: any, companyId: any) {
    this.spin.show();
    this.js.line.statusId = 2;
    this.js.line.groupId = '';
    this.js.line.groupName = '';
  ...
  edit(){
    ...
    let paramdata = "";
    paramdata = this.cs.encrypt({ pageflow: 'New', line:  this.js.line, });
  ...
  approvalSend(element): void {
    const dialogRef = this.dialog.open(ConfirmComponent, {
      ...
      data: {title: "Confirm", message: "Are you sure you want to send this to approval?"}
    });
    dialogRef.afterClosed().subscribe(result => {
      if(result){
        this.emailSend(element)
      }
    });
  }
  ...
  emailSend(element): void {
    const dialogRef = this.dialog.open(SendEmailApprovalComponent, {
      disableClose: true,
      width: '50%',
      maxWidth: '80%',
      data: {data: element}
    ...
    dialogRef.afterClosed().subscribe(result => {
     if(result){
      ...
      this.service.Update(element, element.requestId, element.languageId, element.companyId).subscribe(res =>{
        this.toastr.success(element.requestId + " sent to approval successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
      ...
      // this.router.navigate(['/main/controlgroup/transaction/approval/' +  paramdata])
