...
import { ClassService, ClassElement } from "../class.service";
...
export class ClassDisplayComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    classStatus: ["Active", [Validators.required]],
  ...
  dropdownlist() {
    this.spin.show();
    this.cas.getalldropdownlist([this.cas.dropdownlist.setup.companyId.url]).subscribe((results) => {
      for (let item of results) {
        this.companyIdList = this.cas.foreachlist(item, this.cas.dropdownlist.setup.companyId.key);
      }
    }, (err) => {
  ...
  submit() {
    ...
    if (this.data.code) {
      ...
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        this.toastr.success(res.classId + " saved successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
