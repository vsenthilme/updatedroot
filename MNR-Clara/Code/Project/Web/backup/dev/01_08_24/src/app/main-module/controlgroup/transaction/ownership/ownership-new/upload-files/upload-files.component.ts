...
export class UploadFilesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow === "summaryView" || this.data.pageflow === "summaryView1") {
      console.log(this.data);
      this.displayedColumns = ['fileName', 'status'];
    }
  
    if (this.data.pageflow === "summaryView") {
      this.service.GetNew(this.data.storeId).subscribe(res => {
        if (res.length > 0) {
          this.rows.clear();
    ...
    } else if (this.data.pageflow === "summaryView1") {
      console.log(1);
      this.service.Get1(this.data.line.requestId).subscribe(res => {
        if (res.length > 0) {
          this.rows.clear();
          res.forEach((d: any) => this.addRow(d, false));
        } else {
  ...
  onFilechange(event: any, element: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      this.spin.show();
      this.service.uploadfile(this.file,  `/sharepoint/CG/${this.form.controls.storeId.value}`).subscribe((resp: any) => {
...
back(){
  ...
  async download(element) {
    ...
    obj.requestId=element.controls.storeId.value.toString();
