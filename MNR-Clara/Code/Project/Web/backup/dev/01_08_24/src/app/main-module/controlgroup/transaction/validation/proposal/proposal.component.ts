...
showSelected=false;
  ...
  findGroupId() {
    ...
    this.validationService.findGroupNew(searchObj).subscribe(res => {   
      console.log(res.exactMatchGroups.length)
      this.result1 = res.exactMatchGroups;
      if(res.exactMatchGroups.length>0 && this.js.pageflow == "New" && this.js.line.groupTypeId == 1003){
        const dialogRef1 = this.dialog.open(ConfirmComponent, {
          ...
          data: {
            title: "Confirm",
            message: 'Similar partnership was found in entity relationship list. Do you want to merge and create a new group?',
            pageflow:"Confirm"
          }
        });
        this.spin.hide();
     ...
     // this.validationService.findGroup(searchObj).subscribe((res: any) => {
        if (res.exactMatchGroups.length > 0) {
          ...
          if(res.exactMatchGroups[0].stores[0].groupTypeId==1003){
            ...
            const dialogRef = this.dialog.open(ControlgroupNewComponent, {
              ...
              data: {
                pageflow: 'TransactionCreate1',
                pageflow1: this.js.line,
                languageId: this.js.line.languageId,
                companyId:  this.js.line.companyId ,
                groupTypeId: this.js.line.groupTypeId,
          ...
          console.log(result);
              ...
              if (result) {
                ...
                this.js.line.groupTypeId=result.groupTypeId;
                this.js.line.groupTypeName=result.groupTypeName;
                for(let i=0;i<res.exactMatchGroups.length;i++){
                  let obj: any = {};
                  obj.storeId = [res.exactMatchGroups[i].stores[i].storeId];
                  obj.languageId = ["EN"];
                  obj.companyId = [1000];
                  this.spin.show();
                  this.sub.add(this.validationService.findstore(obj).subscribe((resultant: any[]) => {
                    this.resultany =resultant[0]
                    this.resultany.groupId=result.groupId;
                    if(result.groupTypeId == 1001){
                      result.groupTypeName ="1001 - Combined";
                    }
                    if(result.groupTypeId == 1002){
                     result.groupTypeName="1002 - Brothers & Sisters";
                    }
                    if(result.groupTypeId == 1003){
                      result.groupTypeName="1003 - Independent";
                    }
                    this.resultany.groupName= result.groupName;
                    this.resultany.groupTypeId=result.groupTypeId
                    this.resultany.groupTypeName= result.groupTypeName;
                    console.log(this.resultany)
                    this.sub.add(this.validationService.Update(this.resultany, res.exactMatchGroups[i].stores[i].storeId, "EN", 1000,resultant[0].versionNumber).subscribe(res => {
                      this.toastr.success(this.resultany.storeId + " Group Type Id updated successfully!", "Notification", {
                        timeOut: 2000,
                        progressBar: false,
    ...
    if(this.js.line.groupTypeId == 1001){
      this.js.line.groupTypeName ="1001 - Combined";
    }
    if(this.js.line.groupTypeId == 1002){
      this.js.line.groupTypeName="1002 - Brothers & Sisters";
    }
    if(this.js.line.groupTypeId == 1003){
      this.js.line.groupTypeName="1003 - Independent";
    }
          ...
          dialogRef.afterClosed().subscribe(result => {
            if (result == "Yes") {
              ...
              this.service.searchControlGroup({ groupTypeId: ["1003"] }).subscribe(res => {
                ...
                this.js.line.groupTypeName = '1003-Independent';//res[0].groupTypeName;
