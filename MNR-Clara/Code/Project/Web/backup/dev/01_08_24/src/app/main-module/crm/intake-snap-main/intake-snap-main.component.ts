...
import { IntakeUploadesComponent } from "./intake-uploades/intake-uploades.component";
...
export class IntakeSnapMainComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'action', 'statusId_des', 'intakeFormNumber', 'intakeFormId_des', 'inquiryNumber', 'uploadDocument', 'document', 'clientDocument', 'feedback', 'name', 'createdOn', 'email', 'notes', 'sentOnString', 'receivedOnString', 'approvedOnString', 'createdOn'];
  ...
  uploadDocument(data){
    const dialogRef = this.dialog.open(IntakeUploadesComponent, {
      disableClose: true,
      width: '55%',
      maxWidth: '80%',
      data: { code: data }
