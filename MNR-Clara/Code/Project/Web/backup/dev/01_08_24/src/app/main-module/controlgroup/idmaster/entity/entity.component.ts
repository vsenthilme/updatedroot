...
export class EntityComponent implements OnInit {
  screenid = 1212;
  displayedColumns: string[] = ['select', 'entityId','entityName', 'statusId', 'createdBy', 'createdOn','clientId'];
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.deleterecord(this.selection.selected[0].entityId, this.selection.selected[0].languageId, this.selection.selected[0].companyId,this.selection.selected[0].clientId);

      }
    });
  }
  deleterecord(id: any, languageId: any, companyId:any,clientId:any) {
    this.spin.show();
    this.sub.add(this.service.Delete(id, languageId, companyId,clientId).subscribe((res) => {
      this.toastr.success(id + " Entity Id deleted successfully!", "Notification", {
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(EntityNewComponent, {
      disableClose: true,
      width: '50%',
      maxWidth: '80%',
      position: {
  ...
  getall() {
    ...
    this.sub.add(this.service.search(obj).subscribe((res: any[]) => {
        console.log(res)
        res = this.cs.removeDuplicatesFromArrayList(res , 'entityId')
        ...
        res.forEach((x: {
          entityId: string,
          entityName: string
        }) => this.multientityList.push({
          value: x.entityId,
          label: x.entityId + '-' + x.entityName
        }))
        this.multientityList = this.cas.removeDuplicatesFromArrayNew(this.multientityList);
        res.forEach((x: {
          createdBy: string;
