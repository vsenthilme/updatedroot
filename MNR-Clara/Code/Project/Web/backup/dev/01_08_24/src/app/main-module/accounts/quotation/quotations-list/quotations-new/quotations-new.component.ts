...
export class QuotationsNewComponent implements OnInit {
  ...
  caseCategoryIdListNumber:any[]=[];
  caseSubCategoryIdListNumber:any[]=[];
  dropdownlist() {
    this.spin.show();
    this.cas
      .getalldropdownlist([
      ...
      results[2].matterList.forEach((x: any) => {
        this.multiselectmatterList.push({ value: x.key, label: x.key + '-' + x.value });
      })

      results[2].caseCategoryList.forEach((x: any) => {
        this.caseCategoryIdList.push({ value: Number(x.key), label: x.key + '-' + x.value  });
      });
      this.caseCategoryIdListNumber=this.caseCategoryIdList;
      results[2].subCaseCategoryList.forEach((x: any) => {
        this.caseSubCategoryIdList.push({ value: Number(x.key), label: x.key + '-' + x.value });
      })
      this.caseSubCategoryIdListNumber=this.caseSubCategoryIdList;
          ...
          if(this.pageflow != 'New'){
            console.log(this.QuotationLine)
