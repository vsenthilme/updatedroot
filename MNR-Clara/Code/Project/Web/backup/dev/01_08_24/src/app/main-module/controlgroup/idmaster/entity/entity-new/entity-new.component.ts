...
export class EntityNewComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.data.code) {
      this.data1.entityName=(this.form.controls.entityName.value);
      this.data1.statusId=(this.form.controls.statusId.value);
      this.sub.add(this.service.UpdateBulk([this.data1]).subscribe(res => {
      ...
      let arrayValue: any[] = [];
      this.form.controls.clientId.value.forEach(x => {
        let obj: any = {};
        obj.clientId = x,
          obj.companyId = "1000",
          obj.companyIdAndDescription = "Monty & Ramirez LLP",
          obj.entityName = this.form.controls.entityName.value,
      ...
      this.sub.add(this.service.Create(arrayValue).subscribe(res => {
        ...
        this.clientService.Create(this.clientForm.getRawValue()).subscribe(clientcreatedRes => {
         ...
         arrayValue.forEach(x => {x.referenceField1 = clientcreatedRes.clientId; x.entityId = res[0].entityId})
          this.service.UpdateBulk(arrayValue).subscribe(updateResult => {
            this.toastr.success(" Entity Id saved successfully!", "Notification", {
              timeOut: 2000,
              progressBar: false,
            });
