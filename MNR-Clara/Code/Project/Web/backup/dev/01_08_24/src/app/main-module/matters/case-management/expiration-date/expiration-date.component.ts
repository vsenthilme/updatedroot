...
export class ExpirationDateComponent implements OnInit {
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(ExpirationNewComponent, {
      ...
      maxWidth: '80%',
      //position: { top: '6.5%' },
      data: { pageflow: data, matter: this.cs.decrypt(this.route.snapshot.params.code).code, matterdesc: this.matterdesc, code: data != 'New' ? this.selection.selected[0] : null,languageId:data.languageId }
    });
