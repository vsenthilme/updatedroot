...
//public mask = [ '+','(', /[1-9]/,')', ' ', /\d/, /\d/, /\d/, '-', /\d/, /\d/, /\d/, /\d/]
  ...
  validateNo(e: { which: any; keyCode: any; }): boolean {
  ...
  constructor(private fb: FormBuilder, private auth: AuthService, private cs: CommonService, private route: ActivatedRoute, private location: Location,
    ...
    private service: EnglishService,
  ...
  ngOnInit(): void {
    ...
    //for all itform
    //  this.form.controls.doHaveImmigrationAttorney.valueChanges.subscribe(val => {
    ...
    this.form.get('spouseOrFiancePersonalInfo.fullName')?.disable();
    this.form.get('clientReceivedBenefits.maritalStatus')?.valueChanges.subscribe(val => {
      this.form.get('clientReceivedBenefits.spouseOrFiancePersonalInfo.fullName')?.disable();

      this.form.controls.spouseOrFiancePersonalInfo
      if (val == 'Married') {
        this.form.get('clientReceivedBenefits.spouseOrFiancePersonalInfo.fullName')?.enable();

        this.form.controls.spouseOrFiancePersonalInfo.enable();
      }
    });
  ...
  fill(data: any) {
    ...
    if (data.intakeFormNumber) {
      ...
      //   this.multiselectreferralList = this.multireferralList;
        ...
        if(res.statusId != 9){
          this.sub.add(this.service.GetAttorney(
  ...
  getErrorNumber() {
    ...
    return 'Please fill only Numeric Values';
...
submit1() {
    ...
    //athis.form.patchValue({listOfMediumAboutFirm: this.selectedItems[0].id});
    // if (this.selectedItems && this.selectedItems.length > 0){
    //   let referralList: any[]=[]
    //   this.selectedItems.forEach((a: any)=> referralList.push(a.id))
    //   this.form.get('referenceMedium')?.get('listOfMediumAboutFirm')?.patchValue(referralList);
    // }
    ...
    if (this.pageflow == 'New')
      ...
      this.sub.add(this.service.CreateAttorney(this.form.getRawValue()).subscribe(res => {
