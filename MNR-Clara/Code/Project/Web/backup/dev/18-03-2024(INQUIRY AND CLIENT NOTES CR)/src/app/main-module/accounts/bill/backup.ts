...
export class BillComponent implements OnInit {
 ...
 this.search();
  ...
  findUserRole(element: any){
    let multiUserType: any[] = [];
    let multiRole: any[] = [];
    let finalRole: any[] = [];
    this.sub.add(this.service.getInvoicePdfData(element.invoiceNumber).subscribe((res: any) => {
      ...
      this.timekeeperService.Get(timeKeeper.timeTicketCode).subscribe(timekeeperres => {
        multiUserType.push({code: timekeeperres.timekeeperCode, type: timekeeperres.userTypeId})
        console.log(multiUserType)
        this.userType.Get(timekeeperres.userTypeId, timekeeperres.classId, timekeeperres.languageId).subscribe(userTypeRes => {
          multiRole.push({type: userTypeRes.userTypeId, desc: userTypeRes.userTypeDescription})
          console.log(multiRole)
        })
        console.log(Array.from(new Set(multiRole.concat(multiUserType))));
      })
