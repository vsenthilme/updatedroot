...
export class PaymentplanNewComponent implements OnInit {
  ...
  displayedColumns: string[] = ['itemNumber', 'dueDate', 'dueAmount', 'remainingDueNow','paidAmount', 'balanceAmount', 'paymentReminderDays','reminderDate'];
  ...
  paymentPlanLines = this.fb.group({
    statusId: [] ,
  });
  ...
  fill(data: any) {
    ...
    if (data.pageflow != 'New') {
      ...
      if(data.pageflow!='New'){
        this.displayedColumns= ['itemNumber', 'dueDate', 'dueAmount', 'remainingDueNow','paidAmount', 'balanceAmount', 'paymentReminderDays','reminderDate','statusId'];
       }
      ...
      this.sub.add(this.service.Get(data.code.paymentPlanNumber, data.code.paymentPlanRevisionNo).subscribe(res => {
       ...
       this.form.controls.paymentPlanLines.patchValue(res.paymentPlanLines);
        this.dataSource = new MatTableDataSource<any>(res.paymentPlanLines);
        console.log(res.paymentPlanLines);
        let total = 0;
        this.dataSource.data.forEach(element => {
          total = total + (element.paidAmount != null ? parseInt(element.paidAmount) : 0);
        })
       // return (Math.round(total * 100) / 100);
       let balanceAmount = Number(this.form.controls.paymentPlanTotalAmount.value) - Number(total); 
       this.form.controls.balanceAmount.patchValue(balanceAmount)
...
console.log(this.paymentPlanLines.controls.statusId.value);
  ...
  calculatePaidAmount(element){
    if (element.paidAmount != 0) {
      let balacnce = (Number(element.dueAmount)) - Number(element.paidAmount);
      element.balanceAmount = balacnce;
    this.dataSource._updateChangeSubscription();
    }else{
      element.balanceAmount = element.dueAmount
    }
  ...
  getInstallementAmount(){
    let total = 0;
    this.dataSource.data.forEach(element => {
      total = total + Number(element.dueAmount!= null ? element.dueAmount: 0);
    })
    return (Math.round(total * 100) / 100);
  }
  getBalanceAmount(){
    let total = 0;
    this.dataSource.data.forEach(element => {
      total = total + (element.remainingDueNow!= null ? element.remainingDueNow: 0);
    })
    return (Math.round(total * 100) / 100);
  }
  getPaidAmount(){
    let total = 0;
    this.dataSource.data.forEach(element => {
      total = total + Number(element.paidAmount!= null ? element.paidAmount: 0);
    })
    return (Math.round(total * 100) / 100);
  }
  getActualBalanceAmount(){
    let total = 0;
    this.dataSource.data.forEach(element => {
      total = total + Number(element.balanceAmount!= null ? element.balanceAmount: 0);
    })
    return (Math.round(total * 100) / 100);
  }
}
