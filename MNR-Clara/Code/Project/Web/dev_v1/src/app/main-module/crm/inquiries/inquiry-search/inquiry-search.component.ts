...
export class InquirySearchComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        ...
        'Inquiry Date ': this.cs.excel_date(x.sinquiryDate),
        "Inquiry Mode": x.inquiryModeId,
        'First Name': x.firstName,
        'Last Name ': x.lastName,
        'Assigned To ': x.assignedUserId,
        // 'Assigned On': this.cs.excel_date(x.screatedOn),
        'Class': x.classIddes,
        'Email': x.email,
  ...
  getstatusDropdown() {
    this.sub.add(this.statusservice.Getall().subscribe(res => {
      ...
      if (filteredRecords.length > 0) {
        this.statuslist = filteredRecords
          .map((item: any) => ({
            value: item.statusId,
            description:item.status,
            label:item.statusId+'-'+ item.status,
  ...
  getallationslist() {
    ...
    this.cas.dropdownlist.crm.inquiryNumber.url]).subscribe((results) => {
      ...
      this.inquiryMode = this.cas.foreachlist_searchpage(results[0], this.cas.dropdownlist.setup.inquiryModeId.key);
      ...
      classIdList = this.cas.foreachlist_searchpage(results[1], this.cas.dropdownlist.setup.classId.key);
      ...
      this.InquiryNoList = this.cas.foreachlist_searchpage(results[2], this.cas.dropdownlist.crm.inquiryNumber.key);
    ...
    const statusIds = this.statuslist.map(status => status.value);

    // Set statusId field in the search form
    this.searhform.patchValue({ statusId: statusIds });
    console.log(this.statuslist);
this.multiselectstatusList=this.statuslist;
      this.sub.add(this.service.Search(this.searhform.getRawValue()).subscribe((res: inquiryElement[]) => {
        ...
        this.ELEMENT_DATA.forEach((x) => {
          x.statusIddesc = this.statuslist.find(y => y.value == x.statusId)?.description;
