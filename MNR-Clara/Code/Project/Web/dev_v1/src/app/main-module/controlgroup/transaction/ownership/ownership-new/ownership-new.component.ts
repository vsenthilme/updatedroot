...
import { ClientcontrolgroupService } from '../../../master/clientcontrolgroup/clientcontrolgroup.service';
...
fileUpload:any[]=[];
  ...
  onFilechange(event: any, element: any) {
    ...
    console.log(this.file);
      this.spin.show();
       this.service.uploadfile(this.file, `sharepoint/CG/document/${this.form.controls.storeId.value}`).subscribe((resp: any) => {
       ...
       progressBar: false,
        ...
        element.controls.fileName.patchValue(resp.file)
         ...
         element.controls.storeId.patchValue(this.form.controls.storeId.value);   
  ...
  delete(element, index) {
    if (this.js.pageflow != "New") {
      this.service.Delete1(element.controls.storeId.value, element.controls.id.value).subscribe(res => {
  ...
  constructor(private fb: FormBuilder,
    ...
    private CGservice: ClientcontrolgroupService,
  ...
  onClientChange1(value) {
    ...
    if (this.form.controls.groupTypeId.value == 1001) {
      obj.groupTypeId = [1001];
    } else {
  ...
  dropdownlist() {
    ...
    this.cas.getalldropdownlist([
      ...
      this.setupService.searchStoredropdown().subscribe(res => {
        ...
        if (this.js.pageflow == 'TransEdit') {
          console.log(this.js)
          this.form.controls.storeId.patchValue(this.js.code);
          this.setupService.searchClientNew({
            languageId: [this.form.controls.languageId.value],
            companyId: [this.form.controls.companyId.value],
            storeId: [this.form.controls.storeId.value],
    ...
    if (this.js.code && this.js.pageflow != "New") {
      ...
      if (this.js.pageflow != "New") {
        ...
        this.sub.add(this.service.Update(this.form.getRawValue(), this.js.code, this.js.languageId, this.js.companyId).subscribe(res => {
          ...
          //   this.spin.show();
          //   this.service.uploadfile(this.fileUpload[i], `sharepoint/CG/${this.form.controls.storeId.value}/${res.requestId}`).subscribe((resp: any) => {
          //    this.toastr.success("File uploaded successfully.", "Notification", {
          //       timeOut: 2000,
          //       progressBar: false,
          //     });
          ...
          this.sub.add(this.service.search1({
            ...
            if (res.length > 0) {
              this.service.Update2(this.rows.value, this.js.requestId).subscribe(resp => {
                let paramdata = '';
      ...
      this.ValidationService.findGroupNew(searchObj).subscribe(result => {
        console.log(result)
           if(result.exactMatchGroups.length >0){
            this.form.controls.groupTypeId.patchValue(result.exactMatchGroups[0].stores[0].groupTypeId);
            ...
            for (let i = 1; i <= 10; i++) {
              const controlName = `coOwnerId${i}`;
              
              if (this.form.controls[controlName].value != null) {
                obj.clientId.push(this.form.controls[controlName].value);
                
              }
            }
            this.sub.add(this.CGservice.search({clientId :obj.clientId}).subscribe((res: any[]) => {
              console.log(res.length)
              if(res.length == 0){
                this.groupType='1002';
                this.form.controls.groupTypeId.patchValue(Number(this.groupType))
                console.log(this.form.getRawValue())
              }
              else{
                if(res[0].groupTypeId != null){
                this.form.controls.groupTypeId.patchValue(res[0].groupTypeId)
                }else{
                  this.groupType='1002';
                  this.form.controls.groupTypeId.patchValue(Number(this.groupType))
                }
