...
import { UserProfileService } from "../../setting/admin/user-profile/user-profile.service";
...
pdfMake.fonts = fonts;
@Component({
  selector: 'app-timeticket-diary',
  templateUrl: './timeticket-diary.component.html',
  styleUrls: ['./timeticket-diary.component.scss']
})
export class TimeticketDiaryComponent implements OnInit {
  ...
  searhform = this.fb.group({
    ...
    sendTimeTicketDate: [],
    sstartTimeTicketDateFE: [],
    sendTimeTicketDateFE: []
    , matterNumberFE: [],
    notesNumber: [],
    startCreatedOn: [],
    statusId: [33,],
  ...
  displayedColumns: string[] = ['select', 'clientId', 'matterNumber', 'stimeTicketDate', 'timeTicketDescription', 'approvedBillableTimeInHours', 'approvedBillableAmount', 'timeTicketHours', 'timeTicketAmount', 'defaultRate', 'timeKeeperCode', 'taskCode', 'activityCode', 'billType', 'timeTicketNumber', 'statusIddes', 'portalCreation', 'screatedOn', 'classId',];
  ...
  js: any = {}
  ngOnInit(): void {
    this.RA = this.auth.getLoginRoleAccess(this.screenid);
    this.timeTicketPoup()
    ...
    if (code != undefined) {
      ...
      // if(this.js.code){   
      //     this.openNewTabDialog(this.js.pageflow)
      // }
    }
    this.getall();
  ...
  getall(excel: boolean = false) {
    ...
    this.cas.getalldropdownlist([
      ...
      this.cas.dropdownlist.matter.dropdownNew.url,
      ...
      this.multiSelectMatterNoList = this.cs.removeDuplicateInArray(this.multiSelectMatterNoList)
      this.spin.hide();
    }, (err) => {
      this.spin.hide();
  ...
  filtersearch() {
    ...
    this.sub.add(this.service.Search(this.searhform.getRawValue()).subscribe((res: TimeTicketsElement[]) => {
      res.forEach((x) => {
        ...
        x['timeKeeperCodeShort'] = x.timeKeeperCode;
        ...
        //   x['stimeTicketDate1'] = this.cs.day_callapiSearch(x.stimeTicketDate)
      })
      // this.ELEMENT_DATA = res
      this.ELEMENT_DATA = res.filter(x => x.deletionIndicator == 0);

      this.ELEMENT_DATA.sort((a, b) => (a.screatedOn > b.screatedOn) ? -1 : 1)
      if (this.searhform.controls.sstartTimeTicketDate.value) {
        this.finalCreatedOn = true
        this.filterStartDate = this.searhform.controls.sstartTimeTicketDate.value;
        this.filterEndDate = this.searhform.controls.sendTimeTicketDate.value
      }
  ...
  openDialog(data: any = 'new'): void {
    if (data != 'New') {
      ...
      if (this.selection.selected[0].timeKeeperCodeShort != this.auth.userId && this.auth.userTypeId != 7) {
  ...
  downloadexcel() {
    ...
    res.push({
      ...
      "Task Code  ": this.getbilledTotalHours(),
      "Activity Code  ": 'Total Non-Billed',
      "Rate per Hour  ": this.getnonbilledTotalHours(),
      "Time Keeper": '',
      "Status": '',
      //   'Creation Date ': this.cs.dateapi(x.createdOn),     //error throwing
    });
    ...
    if (this.dataSource.data.length > 0) {
      ...
      this.dataSource.data.forEach((data, i) => {
        ...
        bodyArray.push([
          ...
          { text: data.stimeTicketDate != null ? this.datePipe.transform(data.stimeTicketDate, 'MM-dd-yyyy') : '', fontSize: 7, border: [false, false, false, false], lineHeight: 1.3 },
      ...
      dd.content.push(
        '\n',
        {
          ...
          table: {
            ...
            body: [
              [{ text: 'Date Range  :', fontSize: 8, alignment: 'left', bold: true }, { text: this.searhform.controls.sstartTimeTicketDate.value != null ? this.datePipe.transform(this.searhform.controls.sstartTimeTicketDate.value, 'MM-dd-yyyy') + " - " + this.datePipe.transform(this.searhform.controls.sendTimeTicketDate.value, 'MM-dd-yyyy') : 'All', fontSize: 8 }, { text: '', fontSize: 8 }, { text: '', fontSize: 8 }, { text: '', fontSize: 8 }],
