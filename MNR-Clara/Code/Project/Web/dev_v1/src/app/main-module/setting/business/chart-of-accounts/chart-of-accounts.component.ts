...
export class ChartOfAccountsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'classId', 'accountNumber', 'accountDescription', 'accountType', 'accountStatus', 'createdBy', 'createdOn'];
  ...
  deleteDialog() {
    ...
    const dialogRef = this.dialog.open(DeleteComponent, {
      ...
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: this.selection.selected[0].accountNumber,
    });

    dialogRef.afterClosed().subscribe(result => {

      if (result) {
        this.deleterecord(this.selection.selected[0].accountNumber);
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(ChartDisplayComponent, {
      ...
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: { pageflow: data, code: data != 'New' ? this.selection.selected[0].accountNumber : null }
    });
...
multiselectaccountNumberList: any[] = [];   
  ...
  getall(excel: boolean = false) {
    ...
    this.cas.getalldropdownlist([this.cas.dropdownlist.setup.classId.url,
      ...
      this.sub.add(this.service.Getall().subscribe((res: any[]) => {
        ...
        res.forEach((x: { accountNumber: any; accountDescription: string;}) => this.multiselectaccountNumberList.push({value: x.accountNumber, label: x.accountNumber + '-' + x.accountDescription}))
        this.multiselectaccountNumberList = this.cas.removeDuplicatesFromArrayNew(this.multiselectaccountNumberList);

        
        this.ELEMENT_DATA = res;

        if (excel)
          this.excel.exportAsExcel(res, "Chart of Account");
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        "Class": x.classId,
        'Charts of Accounts': x.accountNumber,
...
if (this.selectedItems2 && this.selectedItems2.length > 0){
  ...
  this.searhform.patchValue({accountStatus: this.selectedItems2 });
}
