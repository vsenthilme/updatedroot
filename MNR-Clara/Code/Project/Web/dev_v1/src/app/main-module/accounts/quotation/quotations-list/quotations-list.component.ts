...
export class QuotationsListComponent implements OnInit {
  ...
  getAllDropdownList(excel: boolean = false) {
   ...
   this.cas.dropdownlist.matter.dropdown.url,
      ...
      this.spin.hide();
      this.statuslist = this.cas.foreachlist_searchpage(results[0], this.cas.dropdownlist.setup.statusId.key).filter(s => [1, 10, 52,54].includes(s.key));
      this.statuslist.forEach((x: { key: string; value: string; }) => this.multistatusList.push({ value: x.key, label: x.key + '-' + x.value }))
      this.multiselectstatusList = this.multistatusList;
      this.clientcatlist = this.cas.foreachlist_searchpage(results[1], this.cas.dropdownlist.setup.clientCategoryId.key);
      ...
      // this.quotationlist = this.cas.foreachlist_searchpage(results[2], this.cas.dropdownlist.accounting.quotationNo.key);
     ...
     // this.quotationHeaderList =  results[2].sort((a, b) => (a.quotationRevisionNo > b.quotationRevisionNo) ? -1 : 1);
    
      results[3].classList.forEach((x: any) => {
        this.classIdList.push({ value: x.key, label: x.key + '-' + x.value });
      })
      results[3].clientNameList.forEach((x: any) => {
        this.multiselectclientList.push({ value: x.key, label: x.key + '-' + x.value });
      })
      results[3].matterList.forEach((x: any) => {
        this.multiselectmatterListList.push({ value: x.key, label: x.key + '-' + x.value });
      })
      this.service.Search({}).subscribe(res => {
        res.forEach(element => {
          this.multiselectquotationList.push({quotationNo: element.quotationNo, label: element.quotationNo,classId:element.classId,deletionIndicator:element.deletionIndicator,clientId:element.clientId,statusId:element.statusId,matterNumber:element.matterNumber,quotationRevisionNo:element.quotationRevisionNo,quotationAmount:element.quotationAmount,quotationDate:element.quotationDate,});
        });
        this.quotationHeaderList = this.multiselectquotationList;
        this.getAllListData();
        })
  ...
  getAllListData(excel: boolean = false){
    ...
    //  this.sub.add(this.service.Getall().subscribe(res => {
      ...
      if (this.auth.classId != '3'){
        this.ELEMENT_DATA = this.quotationHeaderList.filter(x => x.classId === Number(this.auth.classId) && x.deletionIndicator == 0);
      }
      else
        this.ELEMENT_DATA = this.quotationHeaderList.filter(x => x.deletionIndicator == 0);
      ...
      this.ELEMENT_DATA.forEach((x) => {
        ...
        x['clientName'] = this.multiselectclientList.find(y => y.value == x.clientId)?.label;
      })
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        ...
        'Quote Date': this.cs.excel_date(x.quotationDate),
      });
    })
  ...
  search() {
    ...
    console.log(typeof(this.searhform.controls.quotationNo))
    if (typeof this.searhform.controls.quotationNo === "object" && Array.isArray(this.searhform.controls.quotationNo.value)) {
      console.log(1); // Log to indicate that quotationNo is an object
  
      // Assuming quotationNo is an array, patch its value to the first item's quotationNo
      if (this.searhform.controls.quotationNo.value.length > 0) {
          this.searhform.controls.quotationNo.patchValue(
              [this.searhform.controls.quotationNo.value[0].quotationNo]
          );
      }
  }
  ...
  this.sub.add(this.service.Searchrev(this.searhform.getRawValue()).subscribe((res: any[]) => {
        this.spin.hide();
        if (this.auth.classId != '3')
  ...
  generatePdf(element: any, type: any) {
    this.spin.show();
    console.log(element)
