...
@Component({
  ...
  templateUrl: './subgroup-new.component.html',
  styleUrls: ['./subgroup-new.component.scss']
})
export class SubgroupNewComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    statusId: [],
    subGroupTypeId: [],
    subGroupTypeName: [,[Validators.required]],
    updatedBy:[],
    updatedOn: [],
    createdOn_to:[],
    createdOn_from:[],
  ...
  ngOnInit(): void {
    ...
    this.form.controls.versionNumber.disable();
    }
  }
  countryList:any[]=[];
  controltypeList: any[]=[];
  ...
  dropdownSelectversionListID: any[]=[];
  dropdownlist(){
    this.spin.show();
    this.cas.getalldropdownlist([
      ...
      // this.cas.dropdownlist.cgsetup.versionno.url,
    ]).subscribe((results) => {
  ...
  if (this.data.pageflow1 == 'TransactionCreate') {
   ...
   this.setupService.searchControlType({languageId: [this.auth.languageId],companyId:["1000"],statusId:[0]}).subscribe(res => {
    ...
    this.dropdownSelectcontroltypeID=this.cas.removeDuplicatesFromArrayNew(this.dropdownSelectcontroltypeID);
    })
  ...
  //   this.setupService.searchControlType({companyId: [this.form.controls.companyId.value],  
  //     languageId: [this.form.controls.languageId.value], groupTypeId: [value.value],statusId:[0]}).subscribe(res => {
         ...
         this.sub.add(this.service.search(obj).subscribe(res => {
          if(res.length == 1){
            const dialogRef1 = this.dialog.open(ConfirmComponent,{
              ...
              maxWidth: '80%',
              position: { top: '6.5%' },
              data: {title: "Update", message: "This Entry already Exist!,Do you wish to Continue?"}
            ...
            dialogRef1.afterClosed().subscribe(result => {
              if (result == "Yes") {
                ...
                this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
                  this.toastr.success(res.subGroupTypeId + " Updated Successfully!","Notification",{
                      timeOut: 2000,
                      progressBar: false, 
                },
                ); 
            ...
            this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
              this.toastr.success(res.subGroupTypeId + " Saved Successfully!","Notification",{
                  timeOut: 2000,
                  progressBar: false, 
            },
            ); 
