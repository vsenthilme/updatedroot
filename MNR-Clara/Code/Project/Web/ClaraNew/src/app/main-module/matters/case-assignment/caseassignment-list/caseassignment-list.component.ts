...
export class CaseassignmentListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'clientId', 'clientName', 'matterNumber','matterDescription', 'partner', 'originatingTimeKeeper', 'assignedTimeKeeper', 'responsibleTimeKeeper',  'referenceField1','referenceField2','legalAssistant','classId', 'statusIddes',];
...
multiproginattornerList: any[] = [];
  ...
  multiselectLegalAssistantList: any [] = [];
  multiLegalAssistantList: any [] = [];
  ...
  getall(excel: boolean = false) {
    ...
    this.statuslist = this.cas.foreachlist_searchpage(results[0], this.cas.dropdownlist.setup.statusId.key).filter(s => [39].includes(s.key));
      ...
      results[1].clientNameList.forEach((x: any) => {
        this.multiselectclientList.push({ value: x.key, label: x.key + '-' + x.value });
        })
        results[1].matterList.forEach((x: any) => {
          this.multiselectmatternoList.push({ value: x.key, label: x.key + '-' + x.value });
        })
        results[1].caseCategoryList.forEach((x: any) => {
      ...
      this.spin.show();
      this.sub.add(this.service.searchStream({}).subscribe((res: any[]) => {
        res.forEach((x: { responsibleTimeKeeper: string;}) => {if(x.responsibleTimeKeeper != null){this.multirespattornerList.push({value: x.responsibleTimeKeeper, label:  x.responsibleTimeKeeper})}})
        ...
        res.forEach((x: { originatingTimeKeeper: string;}) => {if(x.originatingTimeKeeper != null){this.multiproginattornerList.push({value: x.originatingTimeKeeper, label:  x.originatingTimeKeeper})}})
        ...
        res.forEach((x: { assignedTimeKeeper: string;}) => {if(x.assignedTimeKeeper != null){this.multiassignattornerList.push({value: x.assignedTimeKeeper, label:  x.assignedTimeKeeper})}})
        ...
        res.forEach((x: { legalAssistant: string;}) => {if(x.legalAssistant != null){this.multiLegalAssistantList.push({value: x.legalAssistant, label:  x.legalAssistant})}})
        this.multiselectLegalAssistantList = this.multiLegalAssistantList;
        this.multiselectLegalAssistantList = this.cas.removeDuplicatesFromArrayNew(this.multiselectLegalAssistantList);
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        ...
        "Legal Assistant1":x.referenceField2,
        'Legal Assistant2': x.legalAssistant,
        "Class ID": x.classId == 1 ? 'L&E' : x.classId == 2 ? 'Immigration' : '',
