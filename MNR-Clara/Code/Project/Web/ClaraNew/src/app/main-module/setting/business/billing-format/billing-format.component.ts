...
import { AdminService } from "../admin-cost/admin.service";
import { BillingFormatService } from "./billing-format.service";
import { BillingformatDisplayComponent } from "./billingformat-display/billingformat-display.component";
...
export class BillingFormatComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'classId', 'billingFormatId', 'billingFormatDescription', 'billingFormatStatus', 'createdBy', 'createdOn'];
  ...
  display(): void {
    ...
    const dialogRef = this.dialog.open(BillingformatDisplayComponent, {
      disableClose: true,
      width: '50%',
      maxWidth: '80%',
      position: { top: '5.7%', },
  ...
  deleteDialog() {
    ...
    const dialogRef = this.dialog.open(DeleteComponent, {
      ...
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: this.selection.selected[0].billingFormatId,
    });

    dialogRef.afterClosed().subscribe(result => {

      if (result) {
        this.deleterecord(this.selection.selected[0].billingFormatId);
  ...
  openDialog(data: any = 'new'): void {
    ...
    const dialogRef = this.dialog.open(BillingformatDisplayComponent, {
      ...
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: { pageflow: data, code: data != 'New' ? this.selection.selected[0].billingFormatId : null }
    });
  ...
  getall(excel: boolean = false) {
    ...
    this.cas.getalldropdownlist([this.cas.dropdownlist.setup.classId.url,
      ...
      this.sub.add(this.service.Getall().subscribe((res: any[]) => {
        // res.forEach((x: { createdBy: string;}) => this.multiyseridList.push({id: x.createdBy, itemName: x.createdBy}))
      ...
      this.multiselectyseridList = this.multiyseridList;
        ...
        res.forEach((x: { billingFormatId: string; billingFormatDescription:string;}) => this.multiselectbillformatList.push({value: x.billingFormatId, label: x.billingFormatId + '-' + x.billingFormatDescription}))
        this.multiselectbillformatList = this.cas.removeDuplicatesFromArrayNew(this.multiselectbillformatList);

        this.ELEMENT_DATA = res;

        if (excel)
          this.excel.exportAsExcel(res, "Billing Format");
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        "Class": x.classId,
        'Billing Format': x.billingFormatId,
        "Description ": x.billingFormatDescription,
        'Status': x.billingFormatStatus,
  ...
  checkboxLabel(row?: any): string {
    ...
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.billingFormatId + 1}`;
  }
  ...
  search() {
    ...
    if (this.selectedItems2 && this.selectedItems2.length > 0){
      ...
      this.searhform.patchValue({billingFormatStatus: this.selectedItems2 });
    }
