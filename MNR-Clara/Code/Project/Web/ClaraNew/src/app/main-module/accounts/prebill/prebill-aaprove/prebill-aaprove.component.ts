...
import { TransferTimeTicketComponent } from "./transfer-time-ticket/transfer-time-ticket.component";
...
export class PrebillAaproveComponent implements OnInit {
  ...
  ttbSelected = false;
  getMatterDetails() {
    if (this.paymentApprovalData.code.matterNumber) {
      this.sub.add(this.serviceMatter.Get(this.paymentApprovalData.code.matterNumber).subscribe(res => {
  ...
  displayedColumns1: string[] = ['select', 'sno', 'expenseCode', 'expenseno', 'expenseDate', 'expenseDescription', 'expenseAmount', 'expenseType'];
      ...
      this.sub.add(this.service.gettimeticketpreBillNo(this.paymentPlan.preBillNumber).subscribe((res: any[]) => {
        if (res) {
          res.forEach((x) => {
            ...
            if (x.billType == "non-billable") {
              x.approvedAmount = 0;
              x.approvedHours = 0;
            }
            if (x.billType == "nocharge") {
              x.approvedAmount = 0;
            }
  ...
  clearselection1(row: any) {
    if (!this.selection1.isSelected(row)) {
      this.selection1.clear();
    }
  ...
  approve() {
    ...
    if (this.dataSource.data.length > 0) {
      ...
      let obj: any = {};
      obj.preBillNumber = this.paymentPlan.preBillNumber;
      obj.matterNumber = this.paymentPlan.matterNumber
      objArray.push(obj);
  ...
  save() {
    ...
    // if (this.dataSource.data.length === 0) {
    //   this.toastr.error("There are no time tickets", "Notification", {
    //     timeOut: 2000,
    //     progressBar: false,
    //   });
    //   return;
  ...
  approvedhrscalculation(element) {
    let result
    //this.dataSource.data.forEach(element => {
    console.log(element)
      ...
      if (element.billType == "non-billable") {
        ...
        element.approvedHours = 0;
      }
      if (element.billType == "nocharge") {
        element.approvedAmount = 0;
      }
  ...
  billtypeChange(element) {
   // this.dataSource.data.forEach(element => {
      ...
      if (element.billType == "billable" && element.timeTicketHours != 0 && element.timeTicketAmount != 0) {
        ...
        element.approvedHours = element.timeTicketHours;
        element.approvedAmount = ((element.assignedRatePerHour != null ? element.assignedRatePerHour : 0) * element.approvedHours);
      }
      if (element.billType == "billable" && element.timeTicketHours != 0 && element.timeTicketAmount == 0) {
        element.approvedHours = element.timeTicketHours;
        element.approvedAmount = ((element.assignedRatePerHour != null ? element.assignedRatePerHour : 0) * element.approvedHours);
  ...
  createTimeTicket() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        obj.timeTicketHours = result.approvedBillableTimeInHours;
        obj.timeTicketAmount = result.approvedBillableAmount;
  ...
  creaetexpense() {
    const dialogRef = this.dialog.open(ExpenseNewComponent, {
      width: '70%',
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: {  pageflow: 'New',approval: this.paymentApprovalData, lines: this.dataSource.data.length > 0 ? this.dataSource.data[0] : null }
    });
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
      if (result) {
        result.sreferenceField2=(this.cs.dateNewFormat1(result.referenceField2))
        result.positiveExpenseAmount=result.expenseAmount;
        this.dataSource1.data.push(result);
  ...
  transferTimeticket(){
    ...
    const dialogRef = this.dialog.open(TransferTimeTicketComponent, {
      width: '50%',
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: { lines: this.selection.selected }
    });
    dialogRef.afterClosed().subscribe(result => {
      console.log(result);
  ...
  deleteDialog() {
    ...
    if((this.selection.selected.length !=0)&&(this.selection1.selected.length =0)){
        ...
        dialogRef.afterClosed().subscribe(result => {
          if (result) {
            this.deleteTimeTicket(this.selection.selected);
        ...
        const dialogRef = this.dialog.open(DeleteComponent, {
          ...
          maxWidth: '80%',
          position: { top: '6.5%' },
          data: this.selection1.selected,
        });
        dialogRef.afterClosed().subscribe(result => {
          if (result) {
            this.deleteexpense(this.selection1.selected);
      ...
      deleteTimeTicket(id: any) {
        console.log(id)
       this.spin.show();
        let obj: any = {};
        obj.fromMatterNumber = id[0].matterNumber;
        obj.preBillNumber = id[0].referenceField1;
        obj.timeTickets = id;
        this.service.DeleteTimeTicket(obj).subscribe(res => {
          this.toastr.success("Matter Time Tickers deleted successfully","Notification",{
            timeOut: 2000,
            progressBar: false,
          });
      ...
      deleteexpense(id: any) {
       ...
       this.spin.show();
        ...
        this.service.Deleteexpense(id[0].matterExpenseId).subscribe(res => {
          this.toastr.success("Expense deleted successfully","Notification",{
            timeOut: 2000,
            progressBar: false,
          });
          window.location.reload();
  ...
  editExpenseDescription(data: any = 'new'): void {
    if (data != 'New'){
     ...
     if(this.selection1.selected[0].statusId == 46 || this.selection1.selected[0].statusId == 45 || this.selection1.selected[0].statusId == 51 ||this.selection1.selected[0].statusId == 56) {
    ...
    const dialogRef = this.dialog.open(ExpenseNewComponent, {
      ...
      maxWidth: '80%',
      position: { top: '6.5%' },
      data: { pageflow: data, matter:this.selection1.selected[0].matterNumber, matterdesc: this.matterdescription, code: data != 'New' ? this.selection1.selected[0].matterExpenseId : null }
    });
