...
export class GeneralListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'classId', 'matterNumber', 'matterDescription', 'clientId', 'caseCategoryId', 'caseSubCategoryId','petitionerName', 'originatingTimeKeeper', 'responsibleTimeKeeper', 'scaseOpenedDate', 'statusId', ];
...
        }
  ...
  openDialog(data: any = 'new'): void {
    ...
    this.sub.add(this.matterRate.getMatterRateByMatterNumber(this.selection.selected[0].matterNumber).subscribe((res) => {
      ...
      console.log(this.searhform.controls.classId1.value);
   ...
   //  if(timeKeeperCodeList.length == unMmatchedList.length){
     ...
     if(2 > 4){
      ...
      if (this.selection.selected.length > 0) {
        ...
        obj.matterNumber=this.selection.selected[0].matterNumber;
        obj.clientId=this.searhform.controls.clientId1.value;
        obj.classIdDescription=this.selection.selected[0].classId;
        obj.clientName=this.selection.selected[0].clientId;
        obj.statusDesc=this.selection.selected[0].statusId;
        obj.languageId=this.auth.languageId;
        obj.createdBy=this.auth.userId;
        obj.updatedBy=this.auth.userId;
        obj.matterDescription=this.selection.selected[0].matterDescription;
        this.spin.show();
        this.sub.add(this.service.CreateFavorite(obj).subscribe(res => {
          this.spin.hide();
        }, err => {
          this.cs.commonerror(err);
          this.spin.hide();
      ...
      this.spin.hide();
      this.toastr.error("Matter Rate has not been assigned", "Notification", {
        timeOut: 2000,
        progressBar: false,
      });
