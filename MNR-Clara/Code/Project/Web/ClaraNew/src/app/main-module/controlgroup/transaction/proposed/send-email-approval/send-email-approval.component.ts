...
@Component({
  selector: 'app-send-email-approval',
  templateUrl: './send-email-approval.component.html',
  styleUrls: ['./send-email-approval.component.scss']
})
export class SendEmailApprovalComponent implements OnInit {

  subjectenglish = this.data.data.requestId + " Request ID is ready for Approval"
 ...
 englishBodyText = this.data.data.requestId + " Request ID is ready for Approval";
  englishHeader =  this.data.data.requestId + " Request ID is ready for Approval";
  ...
  form = this.fb.group(
    {
      ...
      subject: ['Request is ready for approval'],
      toAddress: ['rwillars@montyramirezlaw.com', [Validators.required, Validators.pattern("[a-zA-Z0-9.-_]{1,}@[a-zA-Z.-]{2,}[.]{1}[a-zA-Z]{2,}")]],
    });
  //
  ...
  fill() {
    ...
    this.form.controls.toAddress.patchValue('rwillars@montyramirezlaw.com');
    if (this.data.alternateEmail)
      this.form.controls.ccAddress.patchValue(this.data.alternateEmail);

    this.form.controls.bodyText.patchValue(this.form.controls.bodyText.value.replace('Field_1', 'Ruth Willars')) //this.data.firstName    //.replace('linkurlform', this.data.formurl))
