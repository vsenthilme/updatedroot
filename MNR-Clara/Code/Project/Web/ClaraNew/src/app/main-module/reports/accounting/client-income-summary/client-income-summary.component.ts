...
import { ReportServiceService } from "../../report-service.service";
  ...
  @Component({
    selector: 'app-client-income-summary',
    templateUrl: './client-income-summary.component.html',
    styleUrls: ['./client-income-summary.component.scss']
  })
  export class ClientIncomeSummaryComponent implements OnInit {
  ...
  thisDocumentType: any[] = [];
   ...
   @ViewChild(MatSort) sort: MatSort;
    ...
    filtersearch() {
      ...
      this.spin.show();
      this.sub.add(this.service.getClientIncome(this.form.getRawValue()).subscribe(res => {
    ...
    downloadexcel() {
      ...
      this.dataSource.data.forEach(x => {
        res.push({
          ...
          'Total Receipts': x.totalReceipts,
          'Total Refunds': x.totalRefund,
          'Net Receipts' : x.netReceipts,
      ...
      res.push({
        'Client ID': '',
        'Client Name': '',
        'Total Receipts': this.totalrec(),
        'Total Refunds':this.totalrefund(),
        'Net Receipts' :this.netreceipt(),

      });
      this.excel.exportAsExcel(res, "Client Income Summary");
    }
    totalrec() {
      let total = 0;
     this.dataSource.data.forEach(element => {
       total = total + (element.totalReceipts != null ? element.totalReceipts : 0);
     })
     return total;
      }
    totalrefund() {
      let total = 0;
     this.dataSource.data.forEach(element => {
       total = total + (element.totalRefund != null ? element.totalRefund : 0);
     })
     return total;
      }
    netreceipt() {
      let total = 0;
     this.dataSource.data.forEach(element => {
       total = total + (element.netReceipts != null ? element.netReceipts : 0);
     })
     return total;
      }
