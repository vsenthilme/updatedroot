...
export class OwnershipNewComponent implements OnInit {
  ...
  onFilechange(event: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      this.spin.show(); 
      this.service.uploadfile(this.file, `sharepoint/qb/${this.form.controls.storeId.value}`).subscribe((resp: any) => {
       ...
       let emptyArray: any[] = [];
        for(var i = 0; i < this.file.length; i ++){
          let obj: any = {};
          obj.fileName = this.file[i].name;
          obj.statusId = 1;
          obj.storeId = this.form.controls.storeId.value;

          this.addRow(obj, false)
        }
