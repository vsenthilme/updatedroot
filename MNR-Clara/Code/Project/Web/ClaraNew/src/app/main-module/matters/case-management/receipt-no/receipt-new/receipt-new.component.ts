...
export class ReceiptNewComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   console.log(this.form.controls.receiptNote.value);
    ...
    this.form.controls.statusId.valueChanges.subscribe(x => {
      if (x == 10){
      this.approved = true;
      if(this.form.controls.languageId.value == "EN"){
        this.form.controls.receiptNote.patchValue("Our records indicate an upcoming expiration or eligibility on your immigration status. Please call 713-289-4546. Visit us at http://montyramirezlaw.com")
       }
       else{
        this.form.controls.receiptNote.patchValue("Nuestros registros indican un proximo vencimiento o elegibilidad en su estatus migratorio. Llame al 713-289-4546. Visitanos en http://montyramirezlaw.com ")
       }
      }
      else{
        this.approved=false;
  ...
  submit() {
    ...
    if(this.form.controls.toggleNotification.value == true){
      this.form.controls['receiptNote'].setValidators([Validators.required, Validators.maxLength(160),]);
      this.form.controls['receiptNote'].updateValueAndValidity();
