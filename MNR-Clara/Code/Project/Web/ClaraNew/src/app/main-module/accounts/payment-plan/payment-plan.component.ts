...
export class PaymentPlanComponent implements OnInit {
  ...
  deleteDialog() {
    ...
    if ((this.selection.selected[0].statusId == 43 || this.selection.selected[0].statusId == 54)) {
  ...
  openDialog(data: any = 'new'): void {
    ...
    let paramdata = "";
    if (this.selection.selected.length > 0) {
      if ((this.selection.selected[0].statusId == 43 || this.selection.selected[0].statusId == 54)&& data == 'Edit') {
  ...
  getall(excel: boolean = false) {
   ...
   // this.cas.dropdownlist.matter.matterNumber.url,
      ...
      this.statuslist = this.cas.foreachlist_searchpage(results[1], this.cas.dropdownlist.setup.statusId.key).filter(s => [1, 10, 12, 13, 43,53,54].includes(s.key));
  ...
  searchStatusList = { statusId: [1, 12, 43,53,54] };
