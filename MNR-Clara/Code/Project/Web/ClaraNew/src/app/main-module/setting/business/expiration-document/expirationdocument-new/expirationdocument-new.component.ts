...
@Component({
  selector: 'app-expirationdocument-new',
  templateUrl: './expirationdocument-new.component.html',
  styleUrls: ['./expirationdocument-new.component.scss']
})
export class ExpirationdocumentNewComponent implements OnInit {
  screenid: 1129 | undefined;
  ...
  form = this.fb.group({
    ...
    documentTypeDescription: [, [Validators.required, Validators.maxLength(200),]],
  ...
  constructor(
    public dialogRef: MatDialogRef<ExpirationdocumentNewComponent>,
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code.documentType, this.data.code.classId, this.data.code.languageId).subscribe(res => {
  ...
  submit() {
    ...
    if (this.data.code) {
      this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code.documentType).subscribe(res => {
        this.toastr.success(this.data.code.documentType + " updated successfully!","Notification",{
          timeOut: 2000,
          progressBar: false,
        });
        this.spin.hide();
