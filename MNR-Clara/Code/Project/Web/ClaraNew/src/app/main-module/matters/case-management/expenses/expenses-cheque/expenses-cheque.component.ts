...
export class ExpensesChequeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form.controls.referenceField2.patchValue(this.cs.dateNewFormat1(this.form.controls.sreferenceField2.value));
...
myFormData!: FormData;
onFilechange(event){
  this.form.controls.checkRequestFileName.patchValue(event.target.files[0].name);
}
uploadFiles() {

  if (this.form.controls.checkRequestFileName) {
    this.spin.show();
      ...
      if (file) {
        ...
        this.service.uploadfile(file, 'expensecheckrequest/' + this.js.data.matterExpenseId + '/').subscribe((resp: any) => {
          ...
          this.service.Update({
            ...
            sreferenceField2:(this.cs.dateNewFormat1(this.form.controls.referenceField2.value)),
