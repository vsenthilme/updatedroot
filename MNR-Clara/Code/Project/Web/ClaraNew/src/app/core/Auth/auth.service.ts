...
//  /   private email: LoginComponent,
  ...
  verifyOtp(contactNumber: string, otp: any) {
    // sessionStorage.clear();
     ...
     return new Promise((resolve, reject) => {
       this.sub.add(
         this.http.get<any>(`/mnr-setup-service/login/verifyEmailOTP?userId=${contactNumber}&otp=${otp}&typeOfLoginotp=${1}`).subscribe(
           (res) => {
             // if (res == true) {
              if(res){
               ...
               this.sub.add(this.service.Get(this.userRoleId).subscribe(resuser => {
                 ...
                 if(res.classId == 2){
                  let paramdata = "";
                  paramdata = this.cs.encrypt('login');
                   this.router.navigate(["/main/matters/case-management/general/" + paramdata])
                 }else{
                  let paramdata = "";
                    paramdata = this.cs.encrypt('login');
                    this.router.navigate(['/main/accounts/timeticket/' + paramdata])
                   //this.router.navigate(["/main/accounts/timeticketLogin"]);
                 }
                 this.spin.hide();
               }, err => {
  ...
  get timeNotification() {
    return JSON.parse(sessionStorage.getItem("user") as '{}').referenceField9;
  }

  get complaintNotification() {
    return JSON.parse(sessionStorage.getItem("user") as '{}').referenceField10;
  }
  get shown() {
    return JSON.parse(sessionStorage.getItem("user") as '{}').referenceField7;
  }  
  get actualHours() {
    return JSON.parse(sessionStorage.getItem("user") as '{}').referenceField8;
  }
