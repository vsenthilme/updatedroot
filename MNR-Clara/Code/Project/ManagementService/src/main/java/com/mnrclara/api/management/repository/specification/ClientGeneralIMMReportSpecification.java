...
public class ClientGeneralIMMReportSpecification implements Specification<ClientGeneral> {
	
	SearchClientGeneralIMMReport searchClientGeneral;
	
	public ClientGeneralIMMReportSpecification (SearchClientGeneralIMMReport inputSearchParams) {
	...
	public Predicate toPredicate(Root<ClientGeneral> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
	     ...
	     if (searchClientGeneral.getDocketwiseId() != null && !searchClientGeneral.getDocketwiseId().isEmpty()) {
        	 final Path<Group> group = root.<Group> get("referenceField9");
        	 predicates.add(group.in(searchClientGeneral.getDocketwiseId()));
         }
