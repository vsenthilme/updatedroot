...
password = false;
   ...
   // let phoneNumber = this.phoneNumberConvert();
    this.sub.add(
      this.auth.sendOtp(this.lgForm.controls.userName.value).subscribe((result: any) => {
        ...
        this.toastr.error(err.error.error, "Notification");
      }));
  }
  phoneNumberConvert() {
    ...
    // let phoneNumber = this.phoneNumberConvert();
     this.sub.add(
       this.auth.sendEmailOtp(this.lgForm.controls.email.value).subscribe((result: any) => {
         ...
         if(result == false){
          this.toastr.error("The given Email Id doesn't exist.", "Notification");
          this.spin.hide();
        }
       }, (err: any) => {
        this.toastr.error(err.error.error, "Notification");
  ...
  radioChange(e){
    console.log(e);
    this.lgForm.controls.userName.reset();
    this.lgForm.controls.email.reset();
  }
}