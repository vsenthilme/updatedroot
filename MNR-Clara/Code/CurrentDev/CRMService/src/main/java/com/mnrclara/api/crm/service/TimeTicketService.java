...
//                        "Time Ticket",
//                        new Date(), "System");
                String message = "Hi " + (timeTicket.getTimeKeeperName() != null ? timeTicket.getTimeKeeperName() : timeTicket.getTkCode()) +
                        ", Billable hours for the last week was " + timeTicketHours + "(Target 35 Hours) ";
                NotificationSave notificationInput = new NotificationSave();
                notificationInput.setUserId(Arrays.asList(timeTicket.getTkCode()));
                notificationInput.setUserType(null);
                notificationInput.setMessage(message);
                notificationInput.setTopic("Time Ticket");
                notificationInput.setCreatedOn(new Date());
                notificationInput.setCreatedBy("System");
