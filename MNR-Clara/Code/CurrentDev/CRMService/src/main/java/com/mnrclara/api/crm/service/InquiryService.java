...
//		Long status = 7L; //STATUS_ID - Hard Coded Value "7"
	...
	public boolean deleteInquiry (String inquiryCode, String loginUserID) {
		...
		if ( inquiry != null) {
			/*
			 *  Validate STATUS_ID = 03,04,05,06,63,64,65,66,75  if yes then update ISDELETED value from 0 to 1
			 *  for the selected INQ_NO and flag field "REF_FIELD_10"
			 */
			List<Long> statusIds = Arrays.asList(3L,4L,5L,6L,63L,64L,65L,66L,75L);
			if (inquiry.getStatusId() != null) {
				boolean pass = statusIds.stream().anyMatch(n -> n.equals(inquiry.getStatusId()));
