...
import com.mnrclara.api.crm.model.dto.ClientNote;
...
public class ManagementService {
	...
	//CR - 04_04_2024 - V.Senthil
	public ClientNote createClientNote (ClientNote addClientNote, String loginUserID, String authToken) {
		try {
			HttpHeaders headers = new HttpHeaders();
			headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
			headers.add("User-Agent", "MNRClara's RestTemplate");
			headers.add("Authorization", "Bearer " + authToken);
			HttpEntity<?> entity = new HttpEntity<>(addClientNote, headers);
			UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(getManagementServiceUrl() + "clientnote")
					.queryParam("loginUserID", loginUserID);
			ResponseEntity<ClientNote> result = getRestTemplate().exchange(builder.toUriString(), HttpMethod.POST, entity,
					ClientNote.class);
			log.info("result : " + result.getStatusCode());
			return result.getBody();
		} catch (Exception e) {
