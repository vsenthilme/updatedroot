...
public class PotentialClientReportSpecification implements Specification<PotentialClient> {
	...
	public Predicate toPredicate(Root<PotentialClient> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		if (searchPotentialClient.getFollowUpBy() != null && !searchPotentialClient.getFollowUpBy().isEmpty()) {
			final Path<Group> group = root.<Group> get("referenceField4");
			predicates.add(group.in(searchPotentialClient.getFollowUpBy()));
		}
		...
		if (searchPotentialClient.getStartProspectiveFileDate() != null && searchPotentialClient.getEndProspectiveFileDate() != null) {
			predicates.add(cb.between(root.get("createdOn"),
					searchPotentialClient.getStartProspectiveFileDate(), searchPotentialClient.getEndProspectiveFileDate()));
		}
