...
public interface MatterGenAccRepository extends PagingAndSortingRepository<MatterGenAcc, Long>,
    ...
    public List<MatterGenAcc> findByClassIdAndBillingModeIdInAndDeletionIndicator(
            Long classId, List<String> billingModeIds, Long deleteFlag);
    ...
    public List<MatterGenAcc> findByClassIdAndBillingModeIdAndStatusIdAndDeletionIndicator(
            Long classId, String billingModeId, Long statusId, long deleteFlag);
...
//            + "	else x1.imPetitionerName end) as petitionerName,\n"
            ...
            + "tcg.petitioner_nm petitionerName,\n"
            ...
            + "(select note_text from tblmatternoteid where is_deleted=0 and matter_no=tmg.matter_no order by ctd_on DESC limit 1) matterNotes\n"
            + "from tblmattergenaccid tmg \n"
            + "left join tblcasecategoryid tcc on tcc.case_category_id=tmg.case_category_id \n"
            + "left join tblcasesubcategoryid tcs on tcs.case_sub_category_id=tmg.case_sub_category_id \n"
            + "left join tblclientgeneralid tcg on tcg.client_id=tmg.client_id \n"
            ...
            + "(COALESCE(:classId,null) IS NULL OR (tmg.class_id IN (:classId))) and \n"
            + "(COALESCE(:paralegal,null) IS NULL OR (tma.ref_field_2 IN (:paralegal))) and \n"
            + "(COALESCE(:startDate) IS NULL OR (tmg.case_open_date BETWEEN :startDate AND :endDate)) and \n"
...
//            + "tn.note_text noteDescription, \n"
            + "(SELECT note_text From tblmatternoteid where is_deleted = 0 and matter_no = tmg.matter_no ORDER BY ctd_on DESC LIMIT 1) noteDescription, \n"
            + "tma.originating_tk originatingTimeKeeper, \n"
            + "tma.assigned_tk assignedTimeKeeper,  \n"
            + "tma.responsible_tk responsibleTimeKeeper, \n"
            + "tma.legal_assist legalAssistant, \n"
...
//            + "	from( \n"
    ...
    @Query(value = "select tc.class as classId,\n"
            ...
            + "left join tblclientgeneralid tcg on tcg.client_id=mg.client_id \n"
            + "left join tblstatusid ts on ts.status_id=mg.status_id \n"
            + "left join tblmatterassignmentid tma on tma.matter_no=mg.matter_no \n"
            + "WHERE \n"
            + "(COALESCE(:matterNumber) IS NULL OR (mg.matter_no IN (:matterNumber))) and \n"
            + "(COALESCE(:classId) IS NULL OR (tc.class_id IN (:classId))) and \n"
            + "(COALESCE(:clientId) IS NULL OR (tcg.client_id IN (:clientId))) and \n"
    ...
    @Query(value = "SELECT matter.MATTER_NO AS matterNumber, \r\n"
            + "matter.MATTER_TEXT AS matterDescription \r\n"
            + "FROM tblmattergenaccid matter \r\n"
            + "WHERE matter.IS_DELETED = 0", nativeQuery = true)
    public List<IMatterDropDown> getMatterDropDown();
    ...
    @Query(value = "select\n"
            ...
            + "tm.STATUS_ID statusId,\n"
            + "ts.STATUS_TEXT caseStatus,\n"
//            + "(case when trn.DOC_TYPE = 'I-140' then (DATE_FORMAT(tm.PRIORITY_DATE,'%m-%d-%Y')) else null end) priorityDate,\n"
            + "(DATE_FORMAT(tm.PRIORITY_DATE,'%m-%d-%Y')) priorityDate,\n"
//            + "(case when trn.DOC_TYPE = 'EAD' then DATE_FORMAT(ADDDATE(trn.ELIGIBILITY_DATE, INTERVAL 5 YEAR),'%m-%d-%Y') else null end) h1BEndDate,\n"
            + "(select (DATE_FORMAT(ADDDATE(trn.ELIGIBILITY_DATE, INTERVAL 5 YEAR),'%m-%d-%Y')) from tblreceiptappnotice trn where trn.DOC_TYPE = 'EAD' and trn.is_deleted = 0 and trn.matter_no = tm.matter_no order by trn.ctd_on desc limit 1) h1BEndDate,\n"
            + "ma.PARTNER partner,\n"
            ...
            + "(select (DATE_FORMAT(trn.ELIGIBILITY_DATE,'%m-%d-%Y')) from tblreceiptappnotice trn where trn.DOC_TYPE = 'I94' and trn.is_deleted = 0 and trn.matter_no = tm.matter_no order by trn.ctd_on desc limit 1) eligibilityDate,\n"
...
//            + "(case when trn.DOC_TYPE = 'I94' then (DATE_FORMAT(trn.EXPIRATION_DATE,'%m-%d-%Y')) else null end) expirationDate,\n"
            + "(select (DATE_FORMAT(ted.EXPIRATION_DATE,'%m-%d-%Y')) from tblexpirationdate ted where ted.DOC_TYPE = 'I94' and ted.is_deleted = 0 and ted.matter_no = tm.matter_no order by ted.ctd_on desc limit 1) expirationDate,\n"
            + "(select tmn.NOTE_TEXT from tblnotetypeid tnt left join tblmatternoteid tmn on tnt.NOTE_TYP_ID = tmn.NOTE_TYP_ID where tmn.is_deleted = 0 and tnt.NOTE_TYP_TEXT = 'Job Description' and tmn.matter_no = tm.matter_no order by tmn.ctd_on desc limit 1) mRJobDescriptionNote,\n"
            + "(select tmn.NOTE_TEXT from tblnotetypeid tnt left join tblmatternoteid tmn on tnt.NOTE_TYP_ID = tmn.NOTE_TYP_ID where tmn.is_deleted = 0 and tnt.NOTE_TYP_TEXT = 'IV' and tmn.matter_no = tm.matter_no order by tmn.ctd_on desc limit 1) mRIVNote,\n"
            + "(select tmn.NOTE_TEXT from tblnotetypeid tnt left join tblmatternoteid tmn on tnt.NOTE_TYP_ID = tmn.NOTE_TYP_ID where tmn.is_deleted = 0 and tnt.NOTE_TYP_TEXT = 'NIV' and tmn.matter_no = tm.matter_no order by tmn.ctd_on desc limit 1) mRNIVNote,\n"
            + "(select note_text from tblmatternoteid where is_deleted=0 and NOTE_TYP_ID = 62 and matter_no=tm.matter_no order by CTD_ON DESC limit 1) academyNotes,\n"
            + "DATE_FORMAT(tm.RECEIPT_DATE,'%m-%d-%Y') receiptDate\n"
//            + "trn.RECEIPT_TYPE receiptType,\n"
//            + "trn.DOC_TYPE documentType\n"
            + "from tblmattergenaccid tm\n"
...
//            + "left join tblreceiptappnotice trn on trn.MATTER_NO=tm.MATTER_NO\n"
            ...
            + "(COALESCE(:statusId,null) IS NULL OR (tm.STATUS_ID IN (:statusId))) and \n"
...
//			@Param(value = "documentType") List<String> documentType,
	...
	public List<MatterGenAcc> findByStatusIdNotAndDeletionIndicatorAndReferenceField9IsNotNull(long l, long m);
	
	public List<MatterGenAcc> findByStatusIdInOrderByCreatedOnDesc(List<Long> statusId);
}