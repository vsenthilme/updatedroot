...
public interface MatterDocListHeaderRepository extends JpaRepository<MatterDocListHeader,Long>, JpaSpecificationExecutor<MatterDocListHeader> {
	...
	@Query(value = "select MATTER_NO as matterNo, class_id as classId, client_id as clientId, matter_text as matterText from tblmatterdoclistheader \n" +
			" WHERE noti_status = 0 AND is_deleted = 0 ", nativeQuery = true)
	List<IKeyValuePair> findByCheckListNoAndClassIdAndClientId();

	@Modifying
	@Query(value = "update tblmatterdoclistheader set noti_status = 1 where MATTER_NO = :matterNo \n" +
