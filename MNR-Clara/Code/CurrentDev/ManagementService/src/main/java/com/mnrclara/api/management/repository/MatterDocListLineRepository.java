...
public interface MatterDocListLineRepository extends JpaRepository<MatterDocListLine,Long>, JpaSpecificationExecutor<MatterDocListLine> {
	...
	@Modifying
	@Query(value = "DELETE FROM tblmatterdoclistline WHERE MATTER_LINE_ID = :matterLineId", nativeQuery = true)
	void deleteMatterDocListLine(@Param("matterLineId") long matterLineId);
}