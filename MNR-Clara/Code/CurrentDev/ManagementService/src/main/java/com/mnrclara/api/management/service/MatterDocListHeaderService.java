...
public class MatterDocListHeaderService {
	...
	public MatterDocListHeader createMatterDocListHeader (AddMatterDocListHeader newMatterDocListHeader, String loginUserID)
			...
			MatterDocListLine dbMatterDocListLine = new MatterDocListLine();
				...
				if(newMatterDocListLine.isUploadDateIndicator()){
					dbMatterDocListLine.setUploadDate(new Date());
				}
			...
			if(savedMatterDocListHeader != null) {
				sendNotification(savedMatterDocListHeader.getMatterHeaderId(), savedMatterDocListHeader.getMatterNumber(),
						savedMatterDocListHeader.getClassId(), savedMatterDocListHeader.getClientId());
			}
...
//			savedMatterDocListHeader.setMatterDocLists(new HashSet<>());
		if(updateMatterDocListHeader.getMatterDocLists()!=null){
			for (AddMatterDocListLine newAddMatterDocList : updateMatterDocListHeader.getMatterDocLists()) {
				...
				if (dbMatterDocList != null) {
					if(newAddMatterDocList.isUploadDateIndicator() && !dbMatterDocList.isUploadDateIndicator()){
						dbMatterDocList.setUploadDate(new Date());
					}
	...
	public MatterDocListHeader updateMatterDocListHeader (UpdateMatterDocListHeader updateMatterDocListHeader,Long matterHeaderId, String loginUserID)
		...
		BeanUtils.copyProperties(updateMatterDocListHeader, dbMatterDocListHeader, CommonUtils.getNullPropertyNames(updateMatterDocListHeader));
		//dbMatterDocListHeader.setStatusId(23L);
		dbMatterDocListHeader.setReceivedOn(new Date());
		dbMatterDocListHeader.setUpdatedBy(loginUserID);
		dbMatterDocListHeader.setUpdatedOn(new Date());
...
//			savedMatterDocListHeader.setMatterDocLists(new HashSet<>());
		...
		if(savedMatterDocListHeader.getMatterDocLists() != null && !savedMatterDocListHeader.getMatterDocLists().isEmpty()) {
			Set<MatterDocListLine> dbMatterDocListLines = dbMatterDocListHeader.getMatterDocLists();
			for(MatterDocListLine deleteline : dbMatterDocListLines) {
				matterDocListLineRepository.deleteMatterDocListLine(deleteline.getMatterLineId());
				log.info("delete line : " + deleteline.getMatterLineId() + ", " + deleteline.getMatterHeaderId());
			}
			log.info("all lines deleted successfully---> " + dbMatterDocListLines.size());
		}

		if(updateMatterDocListHeader.getMatterDocLists()!=null){
			for (AddMatterDocListLine newAddMatterDocList : updateMatterDocListHeader.getMatterDocLists()) {
				MatterDocListLine dbMatterDocListLine = new MatterDocListLine();
				BeanUtils.copyProperties(newAddMatterDocList, dbMatterDocListLine, CommonUtils.getNullPropertyNames(newAddMatterDocList));
				if (dbMatterDocListLine.getUploadDate() == null && newAddMatterDocList.isUploadDateIndicator()) {
					dbMatterDocListLine.setUploadDate(new Date());
				}
				dbMatterDocListLine.setDeletionIndicator(0L);
				dbMatterDocListLine.setCreatedBy(loginUserID);
				...
				dbMatterDocListLine.setMatterHeaderId(savedMatterDocListHeader.getMatterHeaderId());
				savedMatterDocListHeader.getMatterDocLists().add(matterDocListLineRepository.save(dbMatterDocListLine));
				log.info("updated [created] successfully");
			}
		}
	...
	// Send Notification
	public void sendNotification(Long matterHeaderId, String matterNo, Long classId, String clientId) {
		try {
			String matterText = matterDocumentRepository.findByMatterText(matterNo);
			List<String> deviceToken = hhtNotificationRepository.getDeviceToken(classId, clientId);
			...
			if (deviceToken != null && !deviceToken.isEmpty()) {
				...
				String response = pushNotificationService.sendPushNotification(matterHeaderId, classId, clientId,
						deviceToken, title, message, orderType);
				log.info("status update successfully");
				if (response.equals("OK")) {
					matterDocListHeaderRepository.updateNotificationStatus(matterNo, classId, clientId);
				}
			}
		} catch (Exception e) {
