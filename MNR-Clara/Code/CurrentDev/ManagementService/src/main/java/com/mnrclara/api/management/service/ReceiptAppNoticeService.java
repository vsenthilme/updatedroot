...
public class ReceiptAppNoticeService {
	...
	public ReceiptAppNotice createReceiptAppNotice (AddReceiptAppNotice newReceiptAppNotice, String loginUserID) 
		...
		ReceiptAppNotice receiptAppNotice = receiptAppNoticeRepository.save(dbReceiptAppNotice);
		if(receiptAppNotice != null) {
			sendNotification(receiptAppNotice.getReceiptNo(),  receiptAppNotice.getClassId(), receiptAppNotice.getClientId());
		}
		return receiptAppNotice;
	}

	// Send Notification
	public void sendNotification(String receiptNo, Long classId, String clientId) {
...
//				String message = " A NEW RECEIPT NOTICE - " + receiptNo + " HAS BEEN CREATED ";
				String message = " A new document has been uploaded by M&R.";
				String response = pushNotificationService.sendNotificationForReceipt(classId, clientId,
						deviceToken, title, message, orderType, receiptNo);
				log.info("status update successfully");
				if (response.equals("OK")) {
					receiptAppNoticeRepository.updateNotificationStatus(receiptNo, classId, clientId);
				}
			}
		} catch (Exception e) {
