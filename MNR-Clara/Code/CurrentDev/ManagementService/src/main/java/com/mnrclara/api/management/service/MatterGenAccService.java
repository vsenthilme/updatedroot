...
import com.mnrclara.api.management.repository.specification.MatterDropDownSpecification;
...
public class MatterGenAccService {
    ...
    @Autowired
    MatterDropdownRepository matterDropdownRepository;

    @Autowired
    DocketWiseIdRepository docketWiseIdRepository;
    ...
    public List<IMatterDropDown> getMatterDropdown() {
        ...
        List<IMatterDropDown> matterDropDownList = matterGenAccRepository.getMatterDropDown();

        return matterDropDownList;
    }
     ...
     * @return
     */
    public Stream<MatterNumberDropDown> getMatterDropdownNew() {
        MatterDropDownSpecification spec = new MatterDropDownSpecification();
        Stream<MatterNumberDropDown> matterDropDownList = matterDropdownRepository.stream(spec,MatterNumberDropDown.class);
    ...
    public MatterGenAcc getMatterGenAccFromDocketwise(String matterNumber, String matterId) throws ParseException {
        ...
        if (dbMatterGenAcc != null && dbMatterGenAcc.getDeletionIndicator() != null
            ...
            if(matter.getPreference_category_id() != null) {
                String docketWiseId = docketWiseIdRepository.getDescription(matter.getPreference_category_id());
                if(docketWiseId != null && !docketWiseId.isEmpty()) {
                    dbMatterGenAcc.setReferenceField32(docketWiseId);
                }}
    ...
    public MatterGenAcc updateMatterGenAcc(String matterNumber, MatterGenAcc updateMatterGenAcc,
        ...
        if (!loginUserID.equalsIgnoreCase("QBUpdate")) {
        	...
        	dbMatterGenAcc.setUpdatedBy(dbMatterGenAcc.getUpdatedBy());
        }
...
//	}
    public List<ImmigrationReportImpl> getMatterImmigrationReport(ImmigrationMatter immigrationMatter) throws ParseException {
        ...
        if (searchMatterIMMReport.getFromCaseClosedDate() != null && searchMatterIMMReport.getToCaseClosedDate() != null) {
            ...
            log.info("-------FromCaseClosedDate--------> : " + dates[0]);  
            log.info("-------ToCaseClosedDate--------> : " + dates[1]);  
        }
        List<ImmigrationReportImpl> iMatterImmigrationReport = matterGenAccRepository.getImmigrationReport(
                ...
                searchMatterAssignmentIMMReport.getParalegal(),
...
//			}
    ...
    public List<AcademicReportImpl> getAcademicReport(AcademicReportInput academicReportInput) throws ParseException {
        ...
        List<AcademicReportImpl> iAcademicReport = matterGenAccRepository.getAcademicReport(
                ...
                academicReportInput.getStatusId(),
...
//				academicReportInput.getDocumentType(),
    ...
    //---------------------------DOCKETWISE-SCHEDUKER------------------------------------------------------------
    /*
     * MatterGenAcc table -> StatusID <> 30 and is_deleted=0 and ref_field_9 != null
     */
    public void scheduleUpdatePriorityDateForAllMatters () {
    	List<MatterGenAcc> matterGenAccList = matterGenAccRepository.findByStatusIdNotAndDeletionIndicatorAndReferenceField9IsNotNull(30L, 0L);
    	log.info("Docketwise scheduler started-------> " + new Date());  
    	log.info("----------matterGenAccList------> " + matterGenAccList.size());  
    	matterGenAccList.stream().forEach(m -> {
    		try {
				getMatterGenAccFromDocketwise (m.getMatterNumber(), m.getReferenceField9());
				Thread.sleep(2000);
			} catch (Exception e) {
				log.error("scheduleUpdatePriorityDateForAllMatters Error : " + e.toString());
				e.printStackTrace();
			}
    	});
    	log.info("Docketwise scheduler ended-------> " + new Date()); 
//    	try {
//			getMatterGenAccFromDocketwise ("10019-19", "3836669");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
    } 
