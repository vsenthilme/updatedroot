package com.mnrclara.api.management.model.mattergeneral;

public interface AcademicReportImpl {
    String getClassId();
    String getClassDescription();
    String getCorporationClientId();
    String getCorporationClientName();
    String getClientId();
    String getClientName();
    String getDateOfHire();
    String getHrPartners();
    String getLeadership();
    String getPosition();
    String getCity();
    String getState();
    String getEmailId();
    String getContactNumber();
    String getMatterNumber();
    String getMatterDescription();
    String getPriorityDate();
    String getPartner();
    String getAssignedTimekeeper();
    String getParalegal();
    String getLegalAssistant();
    String getCaseSubCategoryId();
    String getCaseSubCategoryDescription();
    String getCaseCategoryId();
    String getCaseCategoryDescription();
    String getEligibilityDate();
    String getExpirationDate();
    String getReceiptDate();
    String getReceiptType();
    String getDocumentType();
    String getAcademyComments();
    String getAuditEmail();
    String getH1BEndDate();
    String getMRJobDescriptionNote();
    String getMRNIVNote();
    String getMRIVNote();
    Long getStatusId();
    String getCaseStatus();
    String getAcademyNotes();
}
