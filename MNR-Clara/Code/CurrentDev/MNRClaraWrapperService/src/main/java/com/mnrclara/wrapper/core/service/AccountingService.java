...
public class AccountingService {
	...
	public QuotationHeader[] findQuotationHeaders (SearchQuotationHeader searchQuotationHeader, String authToken) {
		try {
			...
			UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(getAccountingServiceUrl() + "quotationheader/v2/findQuotationHeader");
	...
	// SEARCH
	public QuotationHeader[] findAllRevisionQuotationHeaders (SearchQuotationHeader searchQuotationHeader, String authToken) {
		try {
			HttpHeaders headers = new HttpHeaders();
			headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
			headers.add("User-Agent", "MNRClara RestTemplate");
