		...
		if (billingFormat != null) {
			...
			try {
				...
				for (IMatterTimeTicket iMatterTimeTicket : matterTimeTicket) {
					...
					if (iMatterTimeTicket.getTimeTicketAmount() == null) {
						timeTicketAmount = iMatterTimeTicket.getTtAmount();
					} else {
						timeTicketAmount = iMatterTimeTicket.getTimeTicketAmount();
					}
					
					if (iMatterTimeTicket.getTimeTicketHours() == null) {
						timeTicketHours = iMatterTimeTicket.getTtHours();
					} else {
						timeTicketHours = iMatterTimeTicket.getTimeTicketHours();
					}
					
					timeKeeperSummary.setTimeTicketAmount(timeTicketAmount);
					timeKeeperSummary.setTimeTicketHours(timeTicketHours);
					timeKeeperSummary.setTimeTicketAssignedRate(rate);
					
					log.info("------timeTicketAmount * timeTicketHours-----> : " + timeTicketAmount + "," + timeTicketHours);					
					double timeTicketTotal = timeTicketAmount * timeTicketHours;
...
//			paymentReceivedStartDate = DateUtils.addTimeToDate(prebillEndDate, 1);
		...
		if (matterGenAcc.getBillingModeId().equalsIgnoreCase("1")) {
			ICurrentFees currentFees = matterTimeTicketRepository.getCurrentFees(preBillNumber);
			if (currentFees != null && currentFees.getAppBillAmount() == null) {
				finalSummary.setCurrentFees(currentFees.getTimeTicketAmount());
			} else if (currentFees != null && currentFees.getAppBillAmount() != null) {
				finalSummary.setCurrentFees(currentFees.getAppBillAmount());
			}
		...
		try {
			...
			if (billingFormat != null && billingFormat.getReferenceField9() != null && 
				...
				for (IMatterTimeTicketInvoice iMatterTimeTicket : matterTimeTicket) {
					...
					//Query written by V.Senthil - 29-03-2024 - get assigned rate from matter rate changed to default rate from matter timeticket
					// since getting assigned rate from matter rate will result in output form changes - including all previously generated invoices
//					Double assignedRate = matterTimeTicketRepository.getAssignedRateFromMatterRate(matterGenAcc.getMatterNumber(), iMatterTimeTicket.getTimeKeeperCode());
					Double assignedRate = matterTimeTicketRepository.getAssignedRateFromMatterTimeTicket(matterGenAcc.getMatterNumber(),
							iMatterTimeTicket.getTimeKeeperCode(), invoiceHeader.getPreBillNumber());
...
//					paymentReceivedStartDate = DateUtils.addTimeToDate(prebillEndDate, 1);
	...
	private TimeTicketDetail buildPreBillTimeTickets(String matterNumber, Long preBillStatusId, String preBillNumber,
		...
		for (com.mnrclara.api.accounting.model.prebill.MatterTimeTicket ticket : matterTimeTickets) {
			...
			// App_Bill_Amount
			Double timeTicketAmount = 0D;
			if (ticket.getApprovedBillableAmount() == null) {
				timeTicketAmount = ticket.getTimeTicketAmount();
			} else {
				timeTicketAmount = ticket.getApprovedBillableAmount();
			}
			log.info("-------timeTicketAmount----> : " + timeTicketAmount);		
			
			timeTicket.setBillableAmount(timeTicketAmount);
			totalAmount = totalAmount + timeTicketAmount;
			if(ticket.getBillType().equalsIgnoreCase("Billable")){
				totalBillableAmount = totalBillableAmount + timeTicketAmount;
			}
			
			Double timeTicketHours = 0D;
			if (ticket.getApprovedBillableTimeInHours() == null) {
				timeTicketHours = ticket.getTimeTicketHours();
			} else {
				timeTicketHours = ticket.getApprovedBillableTimeInHours();
			}
			
			log.info("-------timeTicketHours----> : " + timeTicketHours);	
			timeTicket.setBillableTimeInHours(timeTicketHours);
			totalHours = totalHours + timeTicketHours;
			if(ticket.getBillType().equalsIgnoreCase("Billable")){
				totalBillableHours = totalBillableHours + timeTicketHours;
			}
	...
	private TimeTicketDetail buildInvoiceTimeTickets(String matterNumber, Long preBillStatusId, String preBillNumber,
		...
		// Pass Matter_No in MatterTimeTicket table to fetch the below details
		List<com.mnrclara.api.accounting.model.prebill.MatterTimeTicket> matterTimeTickets =
				matterTimeTicketRepository.findByMatterNumberAndReferenceField1AndBillTypeNotAndApprovedBillableAmountGreaterThanOrderByTimeTicketDateAscTimeTicketNumberAsc(matterNumber, 
						preBillNumber, "Non-Billable", 0D);
		List<TimeTicket> timeTickets = new ArrayList<>();
		...
		for (com.mnrclara.api.accounting.model.prebill.MatterTimeTicket ticket : matterTimeTickets) {
			...
			// App_Bill_Amount
			if (ticket.getApprovedBillableAmount() != null && ticket.getApprovedBillableAmount() > 0D) {
				...
				timeTicket.setBillableAmount(0D);
			}
