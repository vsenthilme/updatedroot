...
public interface MatterTimeTicketRepository extends JpaRepository<MatterTimeTicket,Long>, JpaSpecificationExecutor<MatterTimeTicket>,DynamicNativeQuery {
	...
	@Query(value="SELECT DEF_RATE AS assignedRate \r\n"
			+ "	FROM tblmattertimeticketid \r\n"
			+ "	WHERE MATTER_NO = :matterNumber and TK_CODE = :tkCode and REF_FIELD_1 = :preBillNo and IS_DELETED = 0 LIMIT 1", nativeQuery=true)
	public Double getAssignedRateFromMatterTimeTicket(@Param ("matterNumber") String matterNumber, @Param ("tkCode") String tkCode, @Param ("preBillNo") String preBillNo);
