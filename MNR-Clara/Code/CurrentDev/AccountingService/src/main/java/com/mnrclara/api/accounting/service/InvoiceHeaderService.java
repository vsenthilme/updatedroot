...
//			log.info("results: " + invPreBillDetails);
	...
	public AddInvoiceHeader updateInvoiceHeader (String invoiceNumber, UpdateInvoiceHeader updateInvoiceHeader, String loginUserID)
		...
		if (dbInvoiceHeader != null) {
			...
			// Send Notification
			if(dbInvoiceHeader != null && dbInvoiceHeader.getReferenceField8() != null && dbInvoiceHeader.getReferenceField8().equalsIgnoreCase("sent")) {
				sendNotification(dbInvoiceHeader.getInvoiceNumber(), dbInvoiceHeader.getClassId(), dbInvoiceHeader.getClientId());          // send notification
			}
...
public List<PartnerBillingReportImpl> createPartnerBillingReport(@Valid BillingReportInput billingReportInput) throws Exception {
	...
	if (billingReportInput.getReferredBy() == null || billingReportInput.getReferredBy().isEmpty()) {
		billingReportInput.setReferredBy(null);
	}
...
//	invoiceHeaderRepository.getARInvoiceList(billingReportInput.getClassId(),billingReportInput.getFromPostingDate(), billingReportInput.getToPostingDate());
	...
	List<PartnerBillingReportImpl> billingReportList = invoiceHeaderRepository.partnerBillingReport(billingReportInput.getClassId(),
			...
			billingReportInput.getReferredBy(),
...
//					billingReport.setAdminCost(iMatterGenAcc.getAdminCost());
					// ADMIN COST - code by V.Senthil on 21_06_2024
					Double iAdminCost = invoiceHeaderRepository.getAdminCost(invoiceHeader.getMatterNumber(),
							invoiceHeader.getPreBillNumber(), "ADM");
					billingReport.setAdminCost(iAdminCost);
//				log.info("--------AdminCost------> : " + iMatterGenAcc.getAdminCost());
	...
	public List<MatterPLReport> createMatterPLReport(@Valid MatterPLReportInput matterPLReportInput) throws Exception {
		try {
			...
			for(MatterPLReportImpl newMatterPLReport : dbMatterPLReportImpl){
				...
				dbMatterPLReport.setTimeTicketHrsCaptured(newMatterPLReport.getTimeTicketHrsCaptured());
				dbMatterPLReport.setTimeTicketBookedHrs(newMatterPLReport.getTimeTicketBookedHrs());
...
//				Double totalBilledValue = dbMatterPLReport.getTotalBilled();
				...
				// Total billed - (Time captured + cost captured)
				//Client confirmed with Mail on 05_04_2024 to change the formula as follows
				Double matterPL = (dbMatterPLReport.getTotalBilled() != null ? dbMatterPLReport.getTotalBilled() : 0) -
									((dbMatterPLReport.getTimeticketCaptured() != null ? dbMatterPLReport.getTimeticketCaptured() : 0) +
									(dbMatterPLReport.getCostCaptured() != null ? dbMatterPLReport.getCostCaptured() : 0));
...
//				dbImmigrationPaymentPlanReport.setLines(dbImmigrationPaymentPlanLine);
	 ...
	 * @param clientId
	 */
	public void sendNotification(String invoiceNumber, Long classId, String clientId) {
		try {
			List<Long> notificationStatus = invoiceHeaderRepository.notificationStatus(invoiceNumber, classId, clientId);
				...
				if(status == 0L) {
					...
					if (deviceToken != null && !deviceToken.isEmpty()) {
						...
						if (response.equalsIgnoreCase("OK")) {
							invoiceHeaderRepository.updateNotificationStatus(invoiceNumber, classId, clientId);
						}
					}
				}
			}
