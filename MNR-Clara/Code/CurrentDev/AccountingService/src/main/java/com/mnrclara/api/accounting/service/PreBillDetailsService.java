...
//				log.info("----executeBill---group----matterTimeTickets------> : " + matterTimeTickets );
				...
				List<String> ttMatterNumbers = matterTimeTickets.stream()
								.map(com.mnrclara.api.accounting.model.prebill.MatterTimeTicket::getMatterNumber)
								.collect(Collectors.toList());
				
				List<com.mnrclara.api.accounting.model.prebill.MatterExpense> matterExpenses = 
						matterExpenseRepository.findByReferenceField2Between(dates[0], dates[1]);
				List<String> expenseMatterNumbers = matterExpenses.stream()
						.map(com.mnrclara.api.accounting.model.prebill.MatterExpense::getMatterNumber)
						.collect(Collectors.toList());
				
				Set<String> setMatterNumbers = new HashSet<>();
				setMatterNumbers.addAll(ttMatterNumbers);
				setMatterNumbers.addAll(expenseMatterNumbers);
				log.info("--1--executeBill---group----matterNumbers------> : " + setMatterNumbers );
				
				matterNumbers = new ArrayList<>(setMatterNumbers);
				log.info("--2--executeBill---group----matterNumbers------> : " + setMatterNumbers );
				
				// This code is not needed
//				matterNumbers = matterGenAccRepository.findMatterNumbersForBillGroup (newBillByGroup);
			}
...
//				}
				...
				if (preBillApproveData.getMatterNumber() != null) {
					...
					for (PreBillDetails preBillDetails : dbPreBillDetailsList) {
						...
						// BillMode ID Logic
						com.mnrclara.api.accounting.model.prebill.MatterGenAcc matterGenAcc = 
								matterGenAccRepository.findByMatterNumber(preBillDetails.getMatterNumber());
						if (matterGenAcc.getBillingModeId() != null & matterGenAcc.getBillingModeId().equalsIgnoreCase("2")) {
							Double total = Double.valueOf(referenceField5) + preBillDetails.getTotalAmount();
							log.info("----------1.0.3--------total-----" + total);
							preBillDetails.setReferenceField5(String.valueOf(total));
						}
	...
	public void sendNotification(AddPreBillDetails preBillDetails,String topic){
		...
		String documentNumber = preBillDetails.getPreBillNumber();
