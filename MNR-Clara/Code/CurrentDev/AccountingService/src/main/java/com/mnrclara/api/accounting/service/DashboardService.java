...
import com.mnrclara.api.accounting.model.reports.*;
import com.mnrclara.api.accounting.repository.*;
...
public class DashboardService {
    ...
    @Autowired
    BilledPaidFeesReportRepository billedPaidFeesReportRepository;
    ...
    public List<MatterListingReportImpl> getMatterListingData(SearchMatterGeneral searchMatterGeneral) {
        try {
            ...
            if (searchMatterGeneral.getFromCaseOpenDate() != null && searchMatterGeneral.getToCaseOpenDate() != null) {
                Date[] dates = DateUtils.addTimeToDatesForSearch(searchMatterGeneral.getFromCaseOpenDate(), searchMatterGeneral.getToCaseOpenDate());
                searchMatterGeneral.setFromCaseOpenDate(dates[0]);
                searchMatterGeneral.setToCaseOpenDate(dates[1]);
            }
            if (searchMatterGeneral.getFromCaseFiledDate() != null && searchMatterGeneral.getToCaseFiledDate() != null) {
                Date[] dates = DateUtils.addTimeToDatesForSearch(searchMatterGeneral.getFromCaseFiledDate(), searchMatterGeneral.getToCaseFiledDate());
                searchMatterGeneral.setFromCaseFiledDate(dates[0]);
                searchMatterGeneral.setToCaseFiledDate(dates[1]);
            }
			List<MatterListingReportImpl> data = matterGenAccRepository.getAllMatterListing(searchMatterGeneral.getClassId(),
                    ...
                    searchMatterGeneral.getFromCaseOpenDate(),
                    searchMatterGeneral.getToCaseOpenDate(),
                    searchMatterGeneral.getFromCaseFiledDate(),
                    searchMatterGeneral.getToCaseFiledDate());
			return data;
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//    public List<ARReportImpl> getArReport(SearchAR searchAr) throws Exception {
//        try {
//            Date[] dates = DateUtils.addTimeToDatesForSQL(searchAr.getFromDate(), searchAr.getToDate());
//            searchAr.setFromDate(dates[0]);
//            searchAr.setToDate(dates[1]);
//            List<ARReportImpl> searchResults = null;
//            // When Closed Matters Check box is ticked - consider all the matters with all STATUS_ID
//            if (!searchAr.getIncludeClosedMatter()) {    // Include specific StatusID
//                searchResults = arReportRepository.getARReportWithoutClosedMatter(
//                        searchAr.getClassId(),
//                        searchAr.getClientId(),
//                        searchAr.getMatterNumber(),
//                        searchAr.getCaseCategory(),
//                        searchAr.getCaseSubCategory(),
//                        searchAr.getTimeKeeper(),
//                        searchAr.getFromDate(),
//                        searchAr.getToDate()
//                );
//            } else {
//                searchResults = arReportRepository.getARReport(
//                        searchAr.getClassId(),
//                        searchAr.getClientId(),
//                        searchAr.getMatterNumber(),
//                        searchAr.getCaseCategory(),
...
//    }
    ...
    public List<ARReportImpl> getArReport(SearchAR searchAr) throws Exception {
        try {
            ...
            String today = null;
            if (searchAr.getFromDate() != null && searchAr.getToDate() != null) {
                fromDate = DateUtils.dateToString(searchAr.getFromDate());
                toDate = DateUtils.dateToString(searchAr.getToDate());
                today = DateUtils.dateToString(new Date());
            }
            List<ARReportImpl> searchResults = null;
            if (fromDate.equalsIgnoreCase("2000-01-01") && toDate.equalsIgnoreCase(today)) {
                searchResults = getArReportOld(searchAr);
            } else {
                ...
                arReportRepository.truncatePaymentTable();
                arReportRepository.truncateInvoiceTable();

                //fill the temporary data for ar report calculation
//                arReportRepository.getARPaymentList(searchAr.getClassId(),searchAr.getFromDate(), searchAr.getToDate());
//                arReportRepository.getARInvoiceList(searchAr.getClassId(),searchAr.getFromDate(), searchAr.getToDate());
                arReportRepository.getARPaymentList(searchAr.getFromDate(), searchAr.getToDate());
                arReportRepository.getARInvoiceList(searchAr.getFromDate(), searchAr.getToDate());
                arReportRepository.updateInvoiceAmount(searchAr.getFromDate(), searchAr.getToDate());

                // When Closed Matters Check box is ticked - consider all the matters with all STATUS_ID
                if (!searchAr.getIncludeClosedMatter()) {    // Exclude specific StatusID 30
                    searchResults = arReportRepository.findARReportWithoutClosedMatter(
                            searchAr.getClassId(),
                            searchAr.getClientId(),
                            searchAr.getMatterNumber(),
                            searchAr.getCaseCategory(),
                    ...
                    searchResults = arReportRepository.findARReport(
                            searchAr.getClassId(),
                            searchAr.getClientId(),
                            searchAr.getMatterNumber(),
                            searchAr.getCaseCategory(),
...
//			log.info("ARReport------searchResults----> : " + searchResults);
    ...
    public List<ARReportImpl> getArReportOld(SearchAR searchAr) throws Exception {
        try {
            Date[] dates = DateUtils.addTimeToDatesForSQL(searchAr.getFromDate(), searchAr.getToDate());
            searchAr.setFromDate(dates[0]);
            searchAr.setToDate(dates[1]);
...
//	        	log.info("matterNumber NO-------->: " + matterNumber);
    ...
    private ARReport prepARReport(String matterNumber, IARReport iarReport, Date[] dates) {
        ...
        if (timeKeeper != null) {
            arReport.setAssignedTimeKeeper(timeKeeper.getAssignedTimeKeeper());
            arReport.setOriginatingTimeKeeper(timeKeeper.getOriginatingTimeKeeper());
            arReport.setResponsibleTimeKeeper(timeKeeper.getResponsibleTimeKeeper());
            arReport.setPartner(timeKeeper.getPartner());
            arReport.setLegalAssistant(timeKeeper.getLegalAssistant());
            arReport.setParalegal(timeKeeper.getReferenceField2());
            arReport.setLawClerk(timeKeeper.getReferenceField1());
        }
...
//    }
    ...
    public List<BilledPaidFeesReportImpl> getBilledPaidFeesReport(BilledPaidFees requestData) throws ParseException {
        ...
        //truncate report temporary table
        billedPaidFeesReportRepository.truncateReportTable();
        //populate report table with unique timekeeper along with case category and case sub category
        billedPaidFeesReportRepository.getTimeKeeperCodeTable(requestData.getCaseCategoryId(), requestData.getTimeKeeperCode());
        //update monthly billed amount for respective timekeeper in the report table
        billedPaidFeesReportRepository.updateMonthlyBilledAmount(dates[0], dates[1]);
        //update monthly timeticket amount for respective timekeeper in the report table
        billedPaidFeesReportRepository.updateYearlyBilledAmount(yearStartDate[0], dates[1]);
        //update yearly billed amount for respective timekeeper in the report table
        billedPaidFeesReportRepository.updateMonthlyApprovedAmount(dates[0], dates[1]);
        //update yearly timeticket amount for respective timekeeper in the report table
        billedPaidFeesReportRepository.updateYearlyApprovedAmount(yearStartDate[0], dates[1]);

        //update monthly Invoice amount for respective timekeeper in the report table
        billedPaidFeesReportRepository.truncateIntermediateReportTableMIA();
        billedPaidFeesReportRepository.getMonthlyInvoiceAmount(dates[0], dates[1], requestData.getCaseCategoryId());
        billedPaidFeesReportRepository.updateMonthlyInvoiceAmount();
        //update yearly Invoice amount for respective timekeeper in the report table
        billedPaidFeesReportRepository.truncateIntermediateReportTableYIA();
        billedPaidFeesReportRepository.getYearlyInvoiceAmount(yearStartDate[0], dates[1], requestData.getCaseCategoryId());
        billedPaidFeesReportRepository.updateYearlyInvoiceAmount();
        //update monthly Payment amount for respective timekeeper in the report table
        billedPaidFeesReportRepository.truncateIntermediateReportTableMPA();
        billedPaidFeesReportRepository.getMonthlyPaymentAmount(dates[0], dates[1], requestData.getCaseCategoryId());
        billedPaidFeesReportRepository.updateMonthlyPaymentAmount();
        //update yearly payment amount for respective timekeeper in the report table
        billedPaidFeesReportRepository.truncateIntermediateReportTableYPA();
        billedPaidFeesReportRepository.getYearlyPaymentAmount(yearStartDate[0], dates[1], requestData.getCaseCategoryId());
        billedPaidFeesReportRepository.updateYearlyPaymentAmount();

        //get invoice amount for respective date range(monthly) in invoice header table
//        Double mInvoiceAmount = billedPaidFeesReportRepository.getInvoiceAmount(dates[0], dates[1], requestData.getCaseCategoryId());
        //get payment amount for respective date range(monthly) in payment table
//        Double mPaymentAmount = billedPaidFeesReportRepository.getPaymentAmount(dates[0], dates[1], requestData.getCaseCategoryId());

        //get invoice amount for respective date range(yearly) in invoice header table
//        Double yInvoiceAmount = billedPaidFeesReportRepository.getInvoiceAmount(yearStartDate[0], dates[1], requestData.getCaseCategoryId());
        //get payment amount for respective date range(yearly) in payment table
//        Double yPaymentAmount = billedPaidFeesReportRepository.getPaymentAmount(yearStartDate[0], dates[1], requestData.getCaseCategoryId());

//        mInvoiceAmount = mInvoiceAmount != null ? mInvoiceAmount : 0D;
//        mPaymentAmount = mPaymentAmount != null ? mPaymentAmount : 0D;

//        yInvoiceAmount = yInvoiceAmount != null ? yInvoiceAmount : 0D;
//        yPaymentAmount = yPaymentAmount != null ? yPaymentAmount : 0D;

//        Double mPaidAmount = mPaymentAmount / mInvoiceAmount;
//        Double yPaidAmount = yPaymentAmount / yInvoiceAmount;

//        List<BilledPaidFeesReportImpl> results = billedPaidFeesReportRepository.getBilledPaidFeesReport(mPaidAmount, yPaidAmount);
        List<BilledPaidFeesReportImpl> results = billedPaidFeesReportRepository.getBilledPaidFeesReport();
        return results;
    }
