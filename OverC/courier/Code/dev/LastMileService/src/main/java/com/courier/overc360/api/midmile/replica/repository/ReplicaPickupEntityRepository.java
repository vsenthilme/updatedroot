...
//            "CONCAT (ts.type_id, ' - ', ts.type_text) As statusText \n" +
    ...
    @Query(value = "select count(status_id) statusCount " +
            "from tblpickup_entity " +
            "where IS_DELETED = 0 and status_id IN (49, 52, 53) " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    Object[] getInProgress(@Param("fromDate") Date fromDate,
                         @Param("toDate") Date toDate);

    @Query(value = "select count(status_id) statusCount " +
            "from tblpickup_entity " +
            "where IS_DELETED = 0 and status_id IN (49, 53) " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    Object[] getAttempted(@Param("fromDate") Date fromDate,
                           @Param("toDate") Date toDate);

    @Query(value = "select count(status_id) statusCount " +
            "from tblpickup_entity " +
            "where IS_DELETED = 0 and status_id = 3 " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    Object[] getPicked(@Param("fromDate") Date fromDate,
                          @Param("toDate") Date toDate);

    @Query(value = "select count(status_id) statusCount " +
            "from tblpickup_entity " +
            "where IS_DELETED = 0 and status_id = 52 " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
