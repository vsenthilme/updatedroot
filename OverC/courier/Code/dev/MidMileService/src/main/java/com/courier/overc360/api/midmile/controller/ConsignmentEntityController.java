...
//    }
    ...
    // Update Mobile Delivery courierId
    @ApiOperation(response = Delivery.class, value = "Delivery Mobile Update CourierId")
    @PatchMapping("/mobileUpdate")
    public ResponseEntity<?> deliveryMobUpdate(@Valid @RequestBody List<DeliveryMobUpdateInput> input, @RequestParam String loginUserID) {
        List<Delivery> response = consignmentService.deliveryMobUpdate(input, loginUserID);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    // Get Customer consignment Status Count
    @ApiOperation(response = CustomerConsignmentStatusCount.class, value = "Customer Pickup Status Count")
    @PostMapping("/customerConsignment/count")
    public ResponseEntity<?>getStatusCustomerConsignmentStatusCount(@Valid @RequestBody StatusCountInput input) throws ParseException {
        List<CustomerConsignmentStatusCount> response = consignmentService.statusCustomerConsignmentCount(input);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> createConsignmentFromB2B(@Valid @RequestBody List<Consignment> consignmentEntityList,
                                                      @RequestParam String loginUserID) throws Exception {
        List<Consignment> dbConsignment = b2BService.createConsignmentFromB2b(consignmentEntityList, loginUserID);
        return new ResponseEntity<>(dbConsignment, HttpStatus.OK);
    }
}
