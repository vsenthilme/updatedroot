...
public interface ReplicaConsignmentEntityRepository extends JpaRepository<ReplicaConsignmentEntity, Long>, JpaSpecificationExecutor<ReplicaConsignmentEntity> {
    ...
    ReplicaConsignmentEntity findByLanguageIdAndCompanyIdAndPartnerIdAndPartnerHouseAirwayBillAndDeletionIndicator(
...
//                                                            @Param(value = "houseAirwayBill") List<String> houseAirwayBill);
    ...
    @Query(value = "SELECT top 1 PRODUCT_TEXT from tblcustomer where customer_id = :partnerId and is_deleted = 0", nativeQuery = true)
    String getCustomerNm(@Param("partnerId") String partnerId);
    ...
    @Query(value = "select count(STATUS_ID) statusCount " +
            ...
            "where IS_DELETED = 0 and STATUS_ID in (2) " +
    ...
    Object[] getPickupAwait(@Param("fromDate") Date fromDate,
                           @Param("toDate") Date toDate);

    @Query(value = "select count(STATUS_ID) statusCount " +
            "from tblpickup_entity " +
            "where IS_DELETED = 0 and STATUS_ID in (49) " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    Object[] getPickupSchedule(@Param("fromDate") Date fromDate,
                            @Param("toDate") Date toDate);

    @Query(value = "select count(STATUS_ID) statusCount " +
            "from tblpickup_entity " +
            "where IS_DELETED = 0 and STATUS_ID in (3) " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    Object[] getPickupCompleted(@Param("fromDate") Date fromDate,
                               @Param("toDate") Date toDate);

    @Query(value = "SELECT count(HAWB_TYP_ID) statusCount " +
            "FROM tblconsignment_entity " +
            "WHERE IS_DELETED = 0 and HAWB_TYP_ID in (10) " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    Object[] getInTransit(@Param("fromDate") Date fromDate,
                          @Param("toDate") Date toDate);

    @Query(value = "select count(status_id) statusCount \n" +
            "from tbldelivery \n" +
            "where IS_DELETED = 0 and status_id in (50) " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    Object[] getOutforDelivery(@Param("fromDate") Date fromDate,
                               @Param("toDate") Date toDate);

    @Query(value = "select count(status_id) statusCount \n" +
            "from tbldelivery \n" +
            "where IS_DELETED = 0 and status_id in (18) " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    ...
    @Query(value = "select count(status_id) statusCount \n" +
            "from tbldelivery \n" +
            "where IS_DELETED = 0 and status_id in (55, 54, 42) " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    ...
    @Query(value = "SELECT count(HAWB_TYP_ID) statusCount " +
            "FROM tblconsignment_entity " +
            "WHERE IS_DELETED = 0 and HAWB_TYP_ID in (22, 23, 24, 41) " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    Object[] getRto(@Param("fromDate") Date fromDate,
                          @Param("toDate") Date toDate);

    @Query(value = "select count(status_id) statusCount \n" +
            "from tbldelivery \n" +
            "where IS_DELETED = 0 and status_id in (54) " +
            "and (COALESCE(CONVERT(VARCHAR(255), :fromDate), NULL) IS NULL OR (CTD_ON between COALESCE(CONVERT(VARCHAR(255), \n" +
            ":fromDate), NULL) And COALESCE(CONVERT(VARCHAR(255), :toDate), NULL))) ", nativeQuery = true)
    Object[] getCancelled(@Param("fromDate") Date fromDate,
                            @Param("toDate") Date toDate);
