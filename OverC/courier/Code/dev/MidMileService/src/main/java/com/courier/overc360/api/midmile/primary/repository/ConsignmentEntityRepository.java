...
//    );
    ...
    @Query(value = "SELECT * FROM tblconsignment_entity WHERE (COALESCE(:houseAirwayBill, NULL) IS NULL OR HOUSE_AIRWAY_BILL IN (:houseAirwayBill)) " +
            "AND (COALESCE(:languageId, NULL) IS NULL OR LANG_ID IN (:languageId)) AND (COALESCE(:companyId, NULL) IS NULL OR C_ID IN (:companyId)) " +
            "AND IS_DELETED = 0", nativeQuery = true)
    ConsignmentEntity getConsignment(@Param("houseAirwayBill") String houseAirwayBill,
                                     @Param("languageId") String languageId,
                                     @Param("companyId") String companyId);
