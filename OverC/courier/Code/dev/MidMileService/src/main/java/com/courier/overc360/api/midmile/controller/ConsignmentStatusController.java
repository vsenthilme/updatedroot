...
public class ConsignmentStatusController {
    ...
    @GetMapping("/consignment-status")
    public Page<ReplicaConsignmentStatus> getConsignmentStatus(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "100000") int size) {
        // Call the service method and return paginated results
        return consignmentStatusService.findStatus(page, size);
    }

//    @GetMapping(value = "/api/consignment-status", produces = MediaType.APPLICATION_JSON_VALUE)
//    public void streamConsignmentStatus(HttpServletResponse response) {
//        response.setContentType(MediaType.APPLICATION_JSON_VALUE);
//
//        try (PrintWriter writer = response.getWriter()) {
//            writer.write("["); // Start of JSON array
//            consignmentStatusService.streamConsignmentStatus(writer); // Delegates streaming logic
//            writer.write("]"); // End of JSON array
//        } catch (Exception e) {
//            throw new RuntimeException("Error streaming consignment status", e);
//        }
//    }
