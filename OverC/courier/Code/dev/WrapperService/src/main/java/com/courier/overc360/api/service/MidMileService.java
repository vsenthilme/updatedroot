...
//    }
    ...
    public Delivery[] updateDeliveryMob(List<DeliveryMobUpdateInput> input, String loginUserID, String authToken) {
        try {
            HttpHeaders headers = new HttpHeaders();
            headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
            headers.add("User-Agent", "MNRClara's RestTemplate");
            ...
            RestTemplate restTemplate = getRestTemplate();
            restTemplate.setRequestFactory(new HttpComponentsClientHttpRequestFactory(client));
            UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(getMidMileServiceUrl() + "consignment/mobileUpdate")
                    .queryParam("loginUserID", loginUserID);
    ...
    public CustomerConsignmentStatusCount[] findCustomerConsignmentStatusCount(StatusCountInput input, String authToken) {
        try {
            HttpHeaders headers = new HttpHeaders();
            headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
            headers.add("User-Agent", "RestTemplate");
            headers.add("Authorization", "Bearer " + authToken);
            UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(getMidMileServiceUrl() + "consignment/customerConsignment/count");
            HttpEntity<?> entity = new HttpEntity<>(input, headers);
            ResponseEntity<CustomerConsignmentStatusCount[]> result = getRestTemplate().exchange(builder.toUriString(), HttpMethod.POST, entity, CustomerConsignmentStatusCount[].class);
            log.info("result : " + result.getStatusCode());
            return result.getBody();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    // Create new Consignment
    public Consignment[] createConsignmentFromB2b(Consignment[] addConsignment, String loginUserID, String authToken) {
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
        headers.add("User-Agent", "RestTemplate");
        headers.add("Authorization", "Bearer " + authToken);
        UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(getMidMileServiceUrl() + "consignment/create/b2b")
                .queryParam("loginUserID", loginUserID);
        HttpEntity<?> entity = new HttpEntity<>(addConsignment, headers);
        ResponseEntity<Consignment[]> result = getRestTemplate().exchange(builder.toUriString(), HttpMethod.POST, entity, Consignment[].class);
        return result.getBody();
    }
}
