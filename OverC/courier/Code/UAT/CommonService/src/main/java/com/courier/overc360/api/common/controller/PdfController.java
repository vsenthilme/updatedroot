...
public class PdfController {
    ...
    public ResponseEntity<?> extractPdfDetails(@RequestParam String fileName, @RequestParam String loginUserId) {
        try {
            // Extract details from the PDF
            List<UpdateCCR> details = pdfApacheExtractionService.bayanPdfExtractDetails(fileName.trim(), loginUserId);
    ...
    @PostMapping("/extract/v3")
    public ResponseEntity<?> extractPdfDetailsWithConsoleId(@RequestParam String fileName, @RequestParam String loginUserId, @RequestParam String consoleId) {
        try {
            // Extract details from the PDF
            List<UpdateCCR> details = pdfApacheExtractionService.pdfExtractDetails(fileName.trim(), loginUserId, consoleId);
