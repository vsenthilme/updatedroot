...
//			localDateTime = localDateTime.plusHours(numberOfDays);
    ...
    // Extract Date Only in UTC
    public static Date extractDateOnly(Date dateTime) {
        LocalDate localDate = dateTime.toInstant()
                .atZone(ZoneId.of("UTC")) // Explicitly use UTC
                .toLocalDate();
        return Date.from(localDate.atStartOfDay(ZoneId.of("UTC")).toInstant());
    }

    // Extract Time Only in UTC
    public static LocalTime extractTimeOnly(Date dateTime) {
