  ...
  template: `
    <ul *ngIf="collapsed && data.items && data.items.length > 0 "
      ...
      <li *ngFor="let item of data.items" class="sublevel-nav-item" [class.expanded]="item.expanded" (click)="handleClick(item)"  [routerLink]="item.routerLink">
          <div class="listSelector" [ngClass]="{'active': isActive(item.routerLink), 'd-none': checkSubMenu(item.routerId)}" ></div>
          <div class="sublevel-nav-box">
            <a class="sublevel-nav-link" [ngClass]="checkMenu(item.routerId) ? 'd-none' : ''"
          ...
          <a class="sublevel-nav-link"  [ngClass]="checkSubMenu(item.routerId) ? 'd-none' : ''"
