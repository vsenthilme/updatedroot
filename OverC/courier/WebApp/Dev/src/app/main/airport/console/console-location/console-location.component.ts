...
export class ConsoleLocationComponent {
  ...
  downloadCCR(res: any) {
    ...
    Object.keys(groupedByConsoleId).forEach(consoleId => {
      ...
      const worksheetConsole = workbook.addWorksheet(`${1}`);
      ...
      const newRow = {
        '#': '',
        'AWB': consoleData[0].consoleGroupName || '',
        'Origin': consoleData[0].consoleName || '',
        'Origin Code': '',
        'Shipper': '',
        'WT KG': '',
        'PCS': '',
      ...
      const headerRowConsole = worksheetConsole.addRow(Object.values(consoleManifestColumns.map(col => col.header)));
      headerRowConsole.eachCell((cell) => {
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
      ...
      consoleData.forEach((item: any, index: number) => {
        ...
        consoleManifestColumns.forEach(col => {
          if (col.format === 'number') {
            ...
            exportItem[col.header] = item[col.field];
            // Calculate totals
            if (['grossWeight', 'noOfPieces', 'customsValue', 'iata'].includes(col.field)) {
              const value = parseFloat(item[col.field]) || 0;
              (totals as any)[col.header] += value;
            }
          }
        });
        const cellRow = worksheetConsole.addRow(Object.values(exportItem));
        cellRow.eachCell({ includeEmpty: true }, (cell) => {
          cell.border = {
            top: { style: 'thin' },
            left: { style: 'thin' },
            bottom: { style: 'thin' },
      ...
      // Add totals row only for console worksheet
      const totalsRow = {
        '#': 'Total',
        'AWB': '',
        'Origin': '',
        ...
        'Currency': '',
        'Value': '',
        'Customs KD': totals['Customs KD'],
        'IATA KD': totals['IATA KD'],
        'HS Code': '',
      };
      const totalsRowExcel = worksheetConsole.addRow(Object.values(totalsRow));
      ...
      const headerRowInvoices = worksheetInvoices.addRow(Object.values(invoicesColumns.map(col => col.header)));
  
      headerRowInvoices.eachCell((cell) => {
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: '8EA9DB' } // Light Ice Blue color
      ...
      const headerRowInvoiceItems = worksheetInvoiceItems.addRow(Object.values(invoiceItemsColumns.map(col => col.header)));
  
      headerRowInvoiceItems.eachCell((cell) => {
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: '8EA9DB' } // Light Ice Blue color
