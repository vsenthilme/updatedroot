...
export class ConsoleComponent {
  ...
  callTableHeader() {
    this.cols = [
      ...
      { field: 'countConsole', header: 'Console Count' },
      ...
      next: (result) => {
        ...
        for (const consoleId in groupedByConsoleId) {
          if (groupedByConsoleId.hasOwnProperty(consoleId)) {
            ...
            const base64Image = iwExpressLogo.headerLogo;
            ...
            // Add 4 empty rows to create space before adding headers
            for (let i = 0; i < 4; i++) {
              worksheet.addRow([]);
            }
  
            // Add new row before the console data
            const newRow = {
              '#': '',
            ...
            const headerRowFirst = worksheet.addRow(newRowValues);
            ...
            headerRow.eachCell((cell) => {
              cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: '8EA9DB' } }; // Background color
              cell.font = { bold: true, color: { argb: 'FFFFFF' } }; // White text color
              cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
            });
  
            // Initialize totals for specific columns
            ...
            // Map console data to rows and calculate totals
            consoleData.forEach((item: any, index: number) => {
              const exportItem: any = {};
              cols.forEach(col => {
                if (col.format === 'number') {
                  // For numeric columns, use index + 1
                  exportItem[col.header] = index + 1;
                } else if (['grossWeight', 'noOfPieces', 'customsValue', 'iata'].includes(col.field)) {
                  const value = parseFloat(item[col.field]) || 0;
                  exportItem[col.header] = value;
                  totals[col.header] += value;
                } else {
                  exportItem[col.header] = item[col.field];
                }
              });
              worksheet.addRow(Object.values(exportItem)).eachCell((cell) => {
                cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
              });
            });
            ...
            const totalsRow = {
              ...
              'Consignee Name': '',
              'Currency': '',
              'Value': '', // Keep Value column empty or use it if required
              'Customs KD': totals['Customs KD'] || '',
              'IATA KD': totals['IATA KD'] || '',
              'HS Code': '',
            };
  
            worksheet.addRow(Object.values(totalsRow)).eachCell((cell) => {
              cell.font = { bold: true };
              cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
            });
          }
