...
public interface OrderManagementLineV2Repository extends JpaRepository<OrderManagementLineV2, Long>,
    ...
    @Query(value =
            "WITH Table1_Count AS ( \n" +
                    ...
                    "FROM tblpreoutboundline where is_deleted = 0 and \n" +
            ...
            "Table2_Count AS ( \n" +
                    "SELECT pre_ob_no, \n" +
                    "(select count(*) from (select COUNT(ob_line_no) cnt from tblordermangementline oml where oml.pre_ob_no = ol.pre_ob_no and \n" +
                    "(COALESCE(:companyCodeId, null) IS NULL OR (oml.c_id IN (:companyCodeId))) and \n" +
                    "(COALESCE(:plantId, null) IS NULL OR (oml.plant_id IN (:plantId))) and \n" +
                    "(COALESCE(:languageId, null) IS NULL OR (oml.lang_id IN (:languageId))) and \n" +
                    "(COALESCE(:warehouseId, null) IS NULL OR (oml.wh_id IN (:warehouseId))) \n" +
                    "group by oml.ob_line_no) x) AS line_count \n" +

                    "FROM tblordermangementline ol where ol.is_deleted = 0 and \n" +
                    "(COALESCE(:companyCodeId, null) IS NULL OR (ol.c_id IN (:companyCodeId))) and \n" +
                    "(COALESCE(:plantId, null) IS NULL OR (ol.plant_id IN (:plantId))) and \n" +
                    "(COALESCE(:languageId, null) IS NULL OR (ol.lang_id IN (:languageId))) and \n" +
                    "(COALESCE(:warehouseId, null) IS NULL OR (ol.wh_id IN (:warehouseId))) and \n" +
            ...
            "JOIN Table2_Count t2c ON t1c.pre_ob_no = t2c.pre_ob_no AND t1c.line_count = t2c.line_count \n" +
                    ...
                    "(COALESCE(:companyCodeId, null) IS NULL OR (t1.c_id IN (:companyCodeId))) and \n" +
                    "(COALESCE(:plantId, null) IS NULL OR (t1.plant_id IN (:plantId))) and \n" +
                    "(COALESCE(:languageId, null) IS NULL OR (t1.lang_id IN (:languageId))) and \n" +
                    "(COALESCE(:warehouseId, null) IS NULL OR (t1.wh_id IN (:warehouseId))) and \n" +
