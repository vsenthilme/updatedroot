...
//            + "im.WH_ID in (:warehouseId) AND "
    ...
    @Query(value = "SELECT CASE WHEN SUM(CASE WHEN status_id = :statusId AND ref_field_2 = 'true' THEN 1 ELSE 0 END) = COUNT(ref_doc_no) \n" +
            "THEN 1 ELSE 0 END AS are_counts_equal \n" +
            "and c_id = :companyCode and plant_id = :plantId and lang_id = :languageId and wh_id = :warehouseId \n" +
            "and is_deleted = 0 and ref_doc_no = :refDocNo and pre_ib_no = :preInboundNo", nativeQuery = true)
    public Long getInboundConfirmStatus(@Param("companyCode") String companyCode,
                                        @Param("plantId") String plantId,
                                        @Param("languageId") String languageId,
                                        @Param("warehouseId") String warehouseId,
                                        @Param("refDocNo") String refDocNo,
