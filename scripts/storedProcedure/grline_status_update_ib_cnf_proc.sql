...
CREATE OR ALTER PROCEDURE grline_status_update_ib_cnf_proc 
...
BEGIN
	...
	IBL.GR_CNF_BY = @updatedBy, IBL.GR_CNF_ON = @updatedOn
	FROM tblgrline IBL INNER JOIN
	...
	IBL.MFR_NAME = X.MFR_NAME AND IBL.IB_LINE_NO = X.IB_LINE_NO AND IBL.IS_DELETED = 0 AND IBL.STATUS_ID != @statusId
