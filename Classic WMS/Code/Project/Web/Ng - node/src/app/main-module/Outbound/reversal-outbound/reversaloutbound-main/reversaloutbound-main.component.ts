...
export class ReversaloutboundMainComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select','no','lineno','partner','description', 'product','refdocno', 'variant', 'type','preoutboundno', 'order','req', 'actions', ];
  dataSource = new MatTableDataSource< reversal>(ELEMENT_DATA);
  selection = new SelectionModel< reversal>(true, []);
