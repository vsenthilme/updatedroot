...
//    }
    ...
    public GrHeaderV2 createGrHeaderV2(GrHeaderV2 newGrHeader, String loginUserID)
        ...
        GrHeaderV2 saveGrHeader = grHeaderV2Repository.save(newGrHeader);
        if (saveGrHeader != null) {
            sendNotification(saveGrHeader.getLanguageId(), saveGrHeader.getCompanyCode(),
                    saveGrHeader.getPlantId(), saveGrHeader.getWarehouseId(), saveGrHeader.getRefDocNumber());
        }
        return saveGrHeader;
    }
    ...
    public void sendNotification(String languageId, String companyId, String plantId, String warehouseId, String processId) throws FirebaseMessagingException {
        List<IKeyValuePair> notification = grHeaderV2Repository.findByStatusIdAndNotificationStatusAndDeletionIndicatorDistinctRefDocNo();

        if (notification != null) {
            for (IKeyValuePair grHeader : notification) {
                // Get NotificationId
                IKeyValuePair notifyId = grHeaderV2Repository.getNotificationId(companyId, languageId, "1");
                ...
                List<String> userIds = grHeaderV2Repository.getUserId(companyId, languageId, notifyId.getUserRole());
                if (userIds.isEmpty()) {
                    log.warn("No Users found for the specified role: {}, companyId: {}", notifyId.getUserRole(), companyId);
                    return;
                }

                List<String> deviceToken = grHeaderV2Repository.getDeviceToken(languageId, companyId, plantId, warehouseId, userIds);

                if (deviceToken != null && !deviceToken.isEmpty()) {
                    String title = "GRHEADER";
                    String message = grHeader.getRefDocType() + " ORDER - " + grHeader.getRefDocNumber() + " - IS RECEIVED";
                    ...
                    if (response.equals("OK")) {
                        grHeaderV2Repository.updateNotificationStatus(grHeader.getRefDocNumber(), grHeader.getWarehouseId());
