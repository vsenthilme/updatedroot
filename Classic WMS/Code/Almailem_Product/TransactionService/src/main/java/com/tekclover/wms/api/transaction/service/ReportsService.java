...
import com.tekclover.wms.api.transaction.repository.specification.BillingTransactionReportSpecification;
...
public class ReportsService extends BaseService {
    @Autowired
    private BillingTransactionReportRepository billingTransactionReportRepository;
    @Autowired
...
//                shipmentDelivery.setOrderType(getOutboundOrderTypeDesc(outboundLine.getOutboundOrderTypeId()));
                shipmentDelivery.setOrderType(getOutboundOrderTypeDesc(
                        outboundLine.getCompanyCodeId(), outboundLine.getPlantId(),
                        outboundLine.getLanguageId(), outboundLine.getWarehouseId(),
                        outboundLine.getOutboundOrderTypeId()));
                shipmentDelivery.setCustomerRef(outboundLine.getRefDocNumber()); // REF_DOC_NO
                shipmentDelivery.setCommodity(outboundLine.getItemCode());
                shipmentDelivery.setDescription(outboundLine.getDescription());
...
//        AuthToken authTokenForMastersService = authTokenService.getMastersServiceAuthToken();
        ...
        try {
            ...
            for (OutboundLineV2 outboundLine : outboundLineSearchResults) {
                ...
                shipmentDelivery.setDeliveryTo(outboundLine.getPartnerCode());
//                shipmentDelivery.setOrderType(getOutboundOrderTypeDesc(outboundLine.getOutboundOrderTypeId()));
                shipmentDelivery.setOrderType(getOutboundOrderTypeDesc(outboundLine.getCompanyCodeId(), outboundLine.getPlantId(),
                        outboundLine.getLanguageId(), outboundLine.getWarehouseId(),
                        outboundLine.getOutboundOrderTypeId()));
                shipmentDelivery.setCustomerRef(outboundLine.getRefDocNumber()); // REF_DOC_NO
                shipmentDelivery.setCommodity(outboundLine.getItemCode());
...
//                shipmentDeliverySummary.setBranchCode(outboundHeader.getPartnerCode());                    // PARTNER_CODE/PARTNER_NM
                ...
                shipmentDeliverySummary.setBranchDesc(getOutboundOrderTypeDesc(outboundHeader.getCompanyCodeId(), outboundHeader.getPlantId(),
                        outboundHeader.getLanguageId(), outboundHeader.getWarehouseId(),
                        outboundHeader.getOutboundOrderTypeId()));
...
//                    partnerCode, authTokenForMastersService.getAccess_token());
            ...
            summaryMetrics.setPartnerCode(outboundOrderTypeId + "-" + getOutboundOrderTypeDesc(companyCode, plantId, languageId, warehouseId, outboundOrderTypeId));
...
//	}
        ...
        try {
            ...
            if (searchImBasicData1 != null) {
                ...
                if (searchImBasicData1.getItemCode() != null && searchImBasicData1.getItemCode().size() > 1) {
                    throw new BadRequestException("Item Code can be null or Single");
                }
            }
    ...
    /**
     * 
     * @param findBillingTransactionReport
     * @return
     */
    public Stream<BillingTransactionReport> getBillingTransactionReport(FindBillingTransactionReport findBillingTransactionReport) {
        try {
//            if (findBillingTransactionReport.getStartCreatedOn() == null && findBillingTransactionReport.getEndCreatedOn() == null) {
//                throw new BadRequestException("Dates are mandatory");
//            }
            if (findBillingTransactionReport.getStartCreatedOn() != null && findBillingTransactionReport.getEndCreatedOn() != null) {
                Date[] dates = DateUtils.addTimeToDatesForSearch(findBillingTransactionReport.getStartCreatedOn(),
                        findBillingTransactionReport.getEndCreatedOn());
                findBillingTransactionReport.setStartCreatedOn(dates[0]);
                findBillingTransactionReport.setEndCreatedOn(dates[1]);
            }

            if (findBillingTransactionReport != null) {
                if (findBillingTransactionReport.getCompanyCodeId() == null) {
                    findBillingTransactionReport.setCompanyCodeId("0");
                }
                if (findBillingTransactionReport.getPlantId() == null) {
                    findBillingTransactionReport.setPlantId("0");
                }
                if (findBillingTransactionReport.getLanguageId() == null) {
                    findBillingTransactionReport.setLanguageId("0");
                }
                if (findBillingTransactionReport.getWarehouseId() == null) {
                    findBillingTransactionReport.setWarehouseId("0");
                }
                Date emptyDate = new Date();
                if (findBillingTransactionReport.getStartCreatedOn() == null) {
                    findBillingTransactionReport.setStartCreatedOn(DateUtils.emptyDateTime(emptyDate));
                }
                if (findBillingTransactionReport.getEndCreatedOn() == null) {
                    findBillingTransactionReport.setEndCreatedOn(DateUtils.emptyDateTime(emptyDate));
                }
            }

            billingTransactionReportRepository.billingTransactionReport(
                    findBillingTransactionReport.getCompanyCodeId(),
                    findBillingTransactionReport.getPlantId(),
                    findBillingTransactionReport.getLanguageId(),
                    findBillingTransactionReport.getWarehouseId(),
                    findBillingTransactionReport.getStartCreatedOn(),
                    findBillingTransactionReport.getEndCreatedOn());
            log.info("Report Generated successfully through Stored Procedure");

            BillingTransactionReportSpecification specification = new BillingTransactionReportSpecification(findBillingTransactionReport);
            Stream<BillingTransactionReport> inventoryStockList = billingTransactionReportRepository.stream(specification, BillingTransactionReport.class);
            log.info("BillingTransactionReport -----> Output");
...
//		log.info("FastSlowMovingDashboard itemData : " + itemData);
    ...
    /**
     *
     * @param searchPickingProductivityReport
     * @return
     * @throws java.text.ParseException
     */
    public List<PickingProductivityReport> pickingProductivityReport(SearchPickingProductivityReport searchPickingProductivityReport) throws java.text.ParseException {

        if(searchPickingProductivityReport.getCompanyCodeId() == null || searchPickingProductivityReport.getCompanyCodeId().isEmpty()) {
            searchPickingProductivityReport.setCompanyCodeId(null);
        }
        if(searchPickingProductivityReport.getPlantId() == null || searchPickingProductivityReport.getPlantId().isEmpty()) {
            searchPickingProductivityReport.setPlantId(null);
        }
        if(searchPickingProductivityReport.getLanguageId() == null || searchPickingProductivityReport.getLanguageId().isEmpty()) {
            searchPickingProductivityReport.setLanguageId(null);
        }
        if(searchPickingProductivityReport.getWarehouseId() == null || searchPickingProductivityReport.getWarehouseId().isEmpty()) {
            searchPickingProductivityReport.setWarehouseId(null);
        }
        if(searchPickingProductivityReport.getRefDocNo() == null || searchPickingProductivityReport.getRefDocNo().isEmpty()) {
            searchPickingProductivityReport.setRefDocNo(null);
        }
        if(searchPickingProductivityReport.getPreOutboundNo() == null || searchPickingProductivityReport.getPreOutboundNo().isEmpty()) {
            searchPickingProductivityReport.setPreOutboundNo(null);
        }
        if(searchPickingProductivityReport.getAssignedPickerId() == null || searchPickingProductivityReport.getAssignedPickerId().isEmpty()) {
            searchPickingProductivityReport.setAssignedPickerId(null);
        }
        if(searchPickingProductivityReport.getLevelId() == null || searchPickingProductivityReport.getLevelId().isEmpty()) {
            searchPickingProductivityReport.setLevelId(null);
        }
        if(searchPickingProductivityReport.getOutboundOrderTypeId() == null || searchPickingProductivityReport.getOutboundOrderTypeId().isEmpty()) {
            searchPickingProductivityReport.setOutboundOrderTypeId(null);
        }
        if(searchPickingProductivityReport.getStatusId() == null || searchPickingProductivityReport.getStatusId().isEmpty()) {
            searchPickingProductivityReport.setStatusId(null);
        }
        if(searchPickingProductivityReport.getStartDate() != null && searchPickingProductivityReport.getEndDate() != null) {
            Date[] dates = DateUtils.convertStringToDate(searchPickingProductivityReport.getStartDate(), searchPickingProductivityReport.getEndDate());
            searchPickingProductivityReport.setStartConfirmedOn(dates[0]);
            searchPickingProductivityReport.setEndConfirmedOn(dates[1]);
        }
       log.info("SearchPickingProductivityReport Input: " + searchPickingProductivityReport );

       List<PickingProductivityImpl> results = pickupLineV2Repository.findPickingProductivityReport(
         searchPickingProductivityReport.getCompanyCodeId(),
         searchPickingProductivityReport.getPlantId(),
         searchPickingProductivityReport.getLanguageId(),
         searchPickingProductivityReport.getWarehouseId(),
         searchPickingProductivityReport.getRefDocNo(),
         searchPickingProductivityReport.getPreOutboundNo(),
         searchPickingProductivityReport.getAssignedPickerId(),
         searchPickingProductivityReport.getLevelId(),
         searchPickingProductivityReport.getOutboundOrderTypeId(),
         searchPickingProductivityReport.getStatusId(),
         searchPickingProductivityReport.getStartConfirmedOn(),
         searchPickingProductivityReport.getEndConfirmedOn());

        List<PickingProductivityReport> reportResult = new ArrayList<>();
        if(results != null && !results.isEmpty()) {
            for(PickingProductivityImpl ipicking : results) {

                SearchPickupLineV2 searchPickupLine = new SearchPickupLineV2();
                searchPickupLine.setCompanyCodeId(searchPickingProductivityReport.getCompanyCodeId());
                searchPickupLine.setPlantId(searchPickingProductivityReport.getPlantId());
                searchPickupLine.setLanguageId(searchPickingProductivityReport.getLanguageId());
                searchPickupLine.setWarehouseId(searchPickingProductivityReport.getWarehouseId());
                searchPickupLine.setOutboundOrderTypeId(searchPickingProductivityReport.getOutboundOrderTypeId());
                searchPickupLine.setRefDocNumber(searchPickingProductivityReport.getRefDocNo());
                searchPickupLine.setPreOutboundNo(searchPickingProductivityReport.getPreOutboundNo());
                searchPickupLine.setStatusId(searchPickingProductivityReport.getStatusId());
                searchPickupLine.setFromPickConfirmedOn(searchPickingProductivityReport.getStartConfirmedOn());
                searchPickupLine.setToPickConfirmedOn(searchPickingProductivityReport.getEndConfirmedOn());
                searchPickupLine.setLevelId(searchPickingProductivityReport.getLevelId());
                searchPickupLine.setAssignedPickerId(Collections.singletonList(ipicking.getAssignedPickerId()));

                List<PickupLineV2> pickupLineList = pickupLineService.findPickupLineV2(searchPickupLine).collect(Collectors.toList());

                if(pickupLineList != null && !pickupLineList.isEmpty()) {
                    for (PickupLineV2 pickupLine : pickupLineList) {
                        PickingProductivityReport pickingProductivity = new PickingProductivityReport();
                        pickingProductivity.setCompanyCodeId(ipicking.getCompanyCodeId());
                        pickingProductivity.setPlantId(ipicking.getPlantId());
                        pickingProductivity.setLanguageId(ipicking.getLanguageId());
                        pickingProductivity.setWarehouseId(ipicking.getWarehouseId());
                        pickingProductivity.setCompanyDescription(ipicking.getCompanyDescription());
                        pickingProductivity.setPlantDescription(ipicking.getPlantDescription());
                        pickingProductivity.setWarehouseDescription(ipicking.getWarehouseDescription());
                        pickingProductivity.setAssignedPickerId(ipicking.getAssignedPickerId());
                        pickingProductivity.setOrders(ipicking.getOrders());
                        pickingProductivity.setLeadTime(ipicking.getLeadTime());
                        pickingProductivity.setAvgLeadTime(ipicking.getAvgLeadTime());
                        pickingProductivity.setParts(ipicking.getParts());
                        pickingProductivity.setPartsPerHr(ipicking.getPartsPerHr());
                        pickingProductivity.setTotalLeadTime(ipicking.getTotalLeadTime());
                        pickingProductivity.setTotalPartsPerHr(ipicking.getTotalPartsPerHr());
                        pickingProductivity.setAvgTotalLeadTime(ipicking.getAvgTotalLeadTime());
                        pickingProductivity.setTotalParts(ipicking.getTotalParts());
                        pickingProductivity.setTotalOrders(ipicking.getTotalOrders());
                        BeanUtils.copyProperties(pickupLine, pickingProductivity, CommonUtils.getNullPropertyNames(pickupLine));
                        reportResult.add(pickingProductivity);
                    }
                } else {
                PickingProductivityReport pickingProductivity = new PickingProductivityReport();
                pickingProductivity.setCompanyCodeId(ipicking.getCompanyCodeId());
        ...
        log.info("pickingProductivityReport results: " + reportResult.size());
       return reportResult;
    }
     ...
     * @throws Exception
     */
    public List<CharData> findPickupData(FindReport findReport) throws Exception {
        ...
        if (findReport.getLeadTime() == 1 && findReport.getQuantity() == 0) {
            results = pickupLineV2Repository.getLeadTime(findReport.getStartConfirmedOn(), findReport.getEndConfirmedOn());
        } else if (findReport.getQuantity() == 1 && findReport.getLeadTime() == 0) {
            results = pickupLineV2Repository.getQuantity(findReport.getStartConfirmedOn(), findReport.getEndConfirmedOn());
        } else if (findReport.getAverageLeadTime() == 1 && findReport.getLeadTime() == 0 && findReport.getQuantity() == 0) {
            results = pickupLineV2Repository.getAverageLeadTime(findReport.getStartConfirmedOn(), findReport.getEndConfirmedOn());
        } else if (findReport.getProductivity() == 1 && findReport.getLeadTime() == 0 && findReport.getQuantity() == 0 && findReport.getAverageLeadTime() == 0) {
            results = pickupLineV2Repository.getProductivity(findReport.getStartConfirmedOn(), findReport.getEndConfirmedOn());
        } else {
     ...
     * @throws Exception
     */
    public List<CharData> findPutAwayReport(FindReport findReport) throws Exception {
