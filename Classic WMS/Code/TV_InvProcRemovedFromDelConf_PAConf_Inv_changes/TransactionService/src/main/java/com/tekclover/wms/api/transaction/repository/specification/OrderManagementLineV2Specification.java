...
public class OrderManagementLineV2Specification implements Specification<OrderManagementLineV2> {
    ...
    public Predicate toPredicate(Root<OrderManagementLineV2> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		 ...
		 if (searchOrderMangementLine.getLevelId() != null && !searchOrderMangementLine.getLevelId().isEmpty()) {
        	 final Path<Group> group = root.<Group> get("referenceField2");
        	 predicates.add(group.in(searchOrderMangementLine.getLevelId()));
         }

		 if (searchOrderMangementLine.getFloorId() != null && !searchOrderMangementLine.getFloorId().isEmpty()) {
        	 final Path<Group> group = root.<Group> get("referenceField3");
        	 predicates.add(group.in(searchOrderMangementLine.getFloorId()));
         }
