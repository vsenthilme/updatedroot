...
@RestController
public class PreInboundHeaderController extends BaseController {
...
//       	log.info("PreInboundHeader : " + preinboundheader);
    ...
    //=====================================================Base==================================================================//
    @ApiOperation(response = PreInboundHeaderV2.class, value = "Get a PreInboundHeader V2 for duplicate checking") // label for swagger
    @GetMapping("/v2/duplicateCheck/{refDocNumber}")
    public ResponseEntity<?> getPreInboundHeaderForDuplicateCheckV2(@PathVariable String refDocNumber, @RequestParam Long orderTypeId,
                                                                    @RequestParam String companyCode, @RequestParam String plantId,
                                                                    @RequestParam String languageId, @RequestParam String warehouseId) {
        //set dataBaseConnectivity
        setDataSourceContextHolder(companyCode, plantId, languageId, warehouseId);

        PreInboundHeaderEntityV2 preinboundheader = preinboundheaderService.getPreInboundHeaderForDuplicateCheckV2(
                companyCode, plantId, languageId, warehouseId, refDocNumber, orderTypeId);
        return new ResponseEntity<>(preinboundheader, HttpStatus.OK);
    }

    @ApiOperation(response = Optional.class, value = "Get company, plant, warehouse Description") // label for swagger
    @GetMapping("/v2/description")
    public ResponseEntity<?> getDescription(@RequestParam String companyCode, @RequestParam String plantId,
                                            @RequestParam String languageId, @RequestParam String warehouseId) {
        //set dataBaseConnectivity
        setDataSourceContextHolder(companyCode, plantId, languageId, warehouseId);
        IKeyValuePair description = preinboundheaderService.getDescription(companyCode, plantId, languageId, warehouseId);
        log.info("description : " + description);
        return new ResponseEntity<>(description, HttpStatus.OK);
    }

    @ApiOperation(response = Optional.class, value = "Get Status Description") // label for swagger
    @GetMapping("/v2/stockTypeDescription")
    public ResponseEntity<?> getStockTypeDescription(@RequestParam String companyCode, @RequestParam String plantId,
                                                     @RequestParam String languageId, @RequestParam String warehouseId,
                                                     @RequestParam Long stockTypeId) {
        //set dataBaseConnectivity
        setDataSourceContextHolder(companyCode, plantId, languageId, warehouseId);
        String description = preinboundheaderService.getStockTypeDesc(companyCode, plantId, languageId, warehouseId, stockTypeId);
        return new ResponseEntity<>(description, HttpStatus.OK);
    }

    @ApiOperation(response = Optional.class, value = "Get Status Description") // label for swagger
    @GetMapping("/v2/itemTypeDescription")
    public ResponseEntity<?> getItemTypeDescription(@RequestParam String companyCode, @RequestParam String plantId,
                                                    @RequestParam String languageId, @RequestParam String warehouseId,
                                                    @RequestParam Long itemTypeId) {
        //set dataBaseConnectivity
        setDataSourceContextHolder(companyCode, plantId, languageId, warehouseId);
        String description = preinboundheaderService.getItemTypeDesc(companyCode, plantId, languageId, warehouseId, itemTypeId);
        return new ResponseEntity<>(description, HttpStatus.OK);
    }
}