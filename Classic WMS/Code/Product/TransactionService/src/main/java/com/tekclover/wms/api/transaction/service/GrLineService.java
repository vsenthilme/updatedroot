...
public class GrLineService extends BaseService {
     ...
     * @return
     */
    public GrLine getGrLine(String warehouseId, String companyCode, String plantId, String languageId,
     ...
     * @return
     */
    public List<GrLine> getGrLine(String preInboundNo, String companyCode, String plantId, String languageId, String refDocNumber, String packBarcodes, Long lineNo, String itemCode) {
        List<GrLine> grLine =
     ...
     * @return
     */
    public List<GrLine> getGrLine(String refDocNumber, String companyCode, String plantId, String languageId,
                                  String packBarcodes, String warehouseId, String preInboundNo, String caseCode) {
        List<GrLine> grLine =
     ...
     * @return
     */
    public List<GrLine> getGrLine(String companyCode, String languageId, String plantId,
                                  String warehouseId, String refDocNumber, String packBarcodes) {
        List<GrLine> grLine =
                grLineRepository.findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndRefDocNumberAndPackBarcodesAndDeletionIndicator(
                        languageId,
                        companyCode,
                        plantId,
                        warehouseId,
     ...
     * @return
     */
    public List<GrLine> getGrLineForUpdate(String warehouseId, String companyCode, String plantId, String languageId, String preInboundNo, String refDocNumber, Long lineNo, String itemCode) {
        List<GrLine> grLine =
     ...
     * @return
     */
    public List<PackBarcode> generatePackBarcode(String companyCode, String languageId, String plantId,
        ...
        // Accept Qty
        if (acceptQty != 0) {
            String nextRangeNumber = getNextRangeNumber(NUM_RAN_ID, warehouseId, companyCode, plantId, languageId, authTokenForIDMasterService.getAccess_token());
...
//                warehouseId = newGrLine.getWarehouseId();
                ...
                for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
                    ...
                    IkeyValuePair ikeyValuePair = containerReceiptRepository.getDescription(
                            newGrLine.getLanguageId(), newGrLine.getCompanyCode(),
                            newGrLine.getPlantId(), newGrLine.getWarehouseId());

                    if(ikeyValuePair != null){
                        dbGrLine.setCompanyDescription(ikeyValuePair.getCompanyDesc());
                        dbGrLine.setPlantDescription(ikeyValuePair.getPlantDesc());
                        dbGrLine.setWarehouseDescription(ikeyValuePair.getWarehouseDesc());
                    }
                    ...
                    String statusDescription = containerReceiptRepository.getStatusDescription(dbGrLine.getStatusId(), newGrLine.getLanguageId());
                    ...
                    List<GrLine> oldGrLine =
                            grLineRepository.findByGoodsReceiptNoAndItemCodeAndLineNoAndLanguageIdAndCompanyCodeAndPlantIdAndRefDocNumberAndPackBarcodesAndWarehouseIdAndPreInboundNoAndCaseCodeAndDeletionIndicator(
                            dbGrLine.getGoodsReceiptNo(),
                            dbGrLine.getItemCode(),
                            dbGrLine.getLineNo(),
            ...
            for (GrLine grLine : createdGRLines) {
                 ...
                 */
                List<GrHeader> grHeaders = grHeaderService.getGrHeader(
                ...
                StatusId idStatus = idmasterService.getStatus(17L,grLine.getLanguageId(), authTokenForIDService.getAccess_token());
                ...
                for (GrHeader grHeader : grHeaders) {
                    ...
                    String statusDescription = containerReceiptRepository.getStatusDescription(grHeader.getStatusId(), grHeader.getLanguageId());
                    if(statusDescription != null) {
                ...
                List<StagingLineEntity> stagingLineEntityList =
                        stagingLineService.getStagingLine(
                                grLine.getCompanyCode(),
                                grLine.getPlantId(),
                                grLine.getLanguageId(),
                                grLine.getWarehouseId(),
                 ...
                 */
                InboundLine inboundLine = inboundLineService.getInboundLine(
                        grLine.getWarehouseId(),
                        grLine.getCompanyCode(),
                        grLine.getPlantId(),
                        grLine.getLanguageId(),
    ...
    private void createPutAwayHeader(GrLine createdGRLine, String loginUserID) {
        ...
        if (createdGRLine != null && createdGRLine.getAssignedUserId() != null) {
            ...
            AuthToken authTokenForIDMasterService = authTokenService.getIDMasterServiceAuthToken();
            long NUM_RAN_CODE = 7;
            String nextPANumber = getNextRangeNumber(NUM_RAN_CODE, warehouseId, companyCode, plantId,
                    languageId, authTokenForIDMasterService.getAccess_token());
...
//							createdGRLine.getItemCode(), 1L, 0L);
            List<Inventory> stBinInventoryList =
                    inventoryRepository.findByWarehouseIdAndCompanyCodeIdAndPlantIdAndLanguageIdAndItemCodeAndBinClassIdAndReferenceField10InAndDeletionIndicator(
                            warehouseId,companyCode,plantId,languageId,
                            itemCode, 1L, storageSectionIds, 0L);
                ...
                if (createdGRLine.getQuantityType().equalsIgnoreCase("A")) {
                    ...
                    if (stBins != null && stBins.size() > 2000) {
                        ...
                        if (storageBin != null && storageBin.length > 0) {
                            ...
                            Long binClassId = 2L;
                            StorageBin stBin = mastersService.getStorageBin(companyCode, plantId, languageId, warehouseId, binClassId, authTokenForMastersService.getAccess_token());
                ...
                if (createdGRLine.getQuantityType().equalsIgnoreCase("D")) {
                    ...
                    if (stBins != null && stBins.size() > 2000) {
                        ...
                        if (storageBin != null && storageBin.length > 0) {
                            ...
                            Long binClassId = 2L;
                            StorageBin stBin = mastersService.getStorageBin(
    ...
    private Inventory createInventory(GrLine createdGRLine) {
        ...
        AuthToken authTokenForMastersService = authTokenService.getMastersServiceAuthToken();
        StorageBin storageBin = mastersService.getStorageBin(createdGRLine.getCompanyCode(),
     ...
     * @throws InvocationTargetException
     */
    public GrLine updateGrLine(String warehouseId, String companyCode, String plantId, String languageId, String preInboundNo, String refDocNumber, String goodsReceiptNo, String palletCode, String caseCode, String packBarcodes, Long lineNo, String itemCode,
                               String loginUserID, UpdateGrLine updateGrLine)
            throws IllegalAccessException, InvocationTargetException {
        GrLine dbGrLine = getGrLine(warehouseId, companyCode, plantId, languageId, preInboundNo, refDocNumber, goodsReceiptNo, palletCode, caseCode,
     ...
     * @throws InvocationTargetException
     */
    public void deleteGrLine(String warehouseId, String preInboundNo, String refDocNumber,
                             String goodsReceiptNo, String palletCode, String caseCode,
                             String companyCode, String plantId, String languageId, String packBarcodes,
                             Long lineNo, String itemCode, String loginUserID)
            throws IllegalAccessException, InvocationTargetException {

        GrLine grLine = getGrLine(warehouseId, companyCode, plantId, languageId,
