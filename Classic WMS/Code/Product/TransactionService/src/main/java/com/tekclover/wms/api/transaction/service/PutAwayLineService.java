...
public class PutAwayLineService extends BaseService {
	 ...
	 * @return
	 */
	public long getPutAwayLineByStatusId (String companyId, String plantId, String languageId, String warehouseId,
										  String preInboundNo, String refDocNumber) {
		long putAwayLineStatusIdCount =
				putAwayLineRepository.getPutawayLineCountByStatusId(companyId, plantId,
						warehouseId, languageId, preInboundNo, refDocNumber);
	 ...
	 * @return
	 */
	public long getPutAwayLineByStatusId (String companyCode, String plantId, String languageId,
										  String warehouseId, String putAwayNumber, String refDocNumber, Long statusId) {
		long putAwayLineStatusIdCount = 
				putAwayLineRepository.getPutawayLineCountByStatusId(companyCode, plantId, warehouseId,languageId, putAwayNumber, refDocNumber, statusId);
		return putAwayLineStatusIdCount;
	}
	 ...
	 * @return
	 */
	public List<PutAwayLine> getPutAwayLine (String companyCode, String plantId, String languageId,
	...
	public PutAwayLine createPutAwayLine (AddPutAwayLine newPutAwayLine, String loginUserID) 
		...
		IkeyValuePair ikeyValuePair = containerReceiptRepository.getDescription(newPutAwayLine.getLanguageId(),
				newPutAwayLine.getCompanyCode(), newPutAwayLine.getPlantId(), newPutAwayLine.getWarehouseId());
		if(ikeyValuePair != null){
			dbPutAwayLine.setCompanyDescription(ikeyValuePair.getCompanyDesc());
			dbPutAwayLine.setPlantDescription(ikeyValuePair.getPlantDesc());
			dbPutAwayLine.setWarehouseDescription(ikeyValuePair.getWarehouseDesc());
		}
		String statusDescription = containerReceiptRepository.getStatusDescription(newPutAwayLine.getStatusId(), newPutAwayLine.getLanguageId());
...
//				Warehouse warehouse = getWarehouse(newPutAwayLine.getWarehouseId());
				...
				Optional<PutAwayLine> existingPutAwayLine = putAwayLineRepository.findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndGoodsReceiptNoAndPreInboundNoAndRefDocNumberAndPutAwayNumberAndLineNoAndItemCodeAndProposedStorageBinAndConfirmedStorageBinInAndDeletionIndicator(
						newPutAwayLine.getLanguageId(), newPutAwayLine.getCompanyCode(), newPutAwayLine.getPlantId(), newPutAwayLine.getWarehouseId(), newPutAwayLine.getGoodsReceiptNo(),
					...
					if (createdPutAwayLine != null && createdPutAwayLine.getPutawayConfirmedQty() > 0L ) {
						...
						StorageBin dbStorageBin = 
								mastersService.getStorageBin(
										dbPutAwayLine.getConfirmedStorageBin(),
						...
						mastersService.updateStorageBin(dbPutAwayLine.getConfirmedStorageBin(), dbStorageBin, dbPutAwayLine.getCompanyCode(), dbPutAwayLine.getPlantId(),
								dbPutAwayLine.getLanguageId(), dbPutAwayLine.getWarehouseId(), loginUserID, authTokenForMastersService.getAccess_token());
						
						if (isInventoryCreated && isInventoryMovemoentCreated) {
							List<PutAwayHeader> headers = putAwayHeaderService.getPutAwayHeader(
									createdPutAwayLine.getWarehouseId(),
	...
	private InventoryMovement createInventoryMovement(PutAwayLine dbPutAwayLine) {
		...
		List<Inventory> inventoryList = 
				inventoryService.getInventory (dbPutAwayLine.getWarehouseId(), dbPutAwayLine.getItemCode(), 1L,
						dbPutAwayLine.getCompanyCode(), dbPutAwayLine.getPlantId(), dbPutAwayLine.getLanguageId());
