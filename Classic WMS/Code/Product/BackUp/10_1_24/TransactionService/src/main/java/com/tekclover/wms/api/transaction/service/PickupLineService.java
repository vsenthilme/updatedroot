...
public class PickupLineService extends BaseService {
     ...
     * @return
     */
    public List<PickupLine> getPickupLineForReversal(String warehouseId, String companyCodeId, String plantId, String languageId, String preOutboundNo, String refDocNumber,
                                                     String partnerCode, Long lineNumber, String itemCode) {
        List<PickupLine> pickupLine = pickupLineRepository
                .findAllByWarehouseIdAndCompanyCodeIdAndPlantIdAndLanguageIdAndPreOutboundNoAndRefDocNumberAndPartnerCodeAndLineNumberAndItemCodeAndDeletionIndicator(
                        warehouseId, companyCodeId, plantId, languageId, preOutboundNo, refDocNumber, partnerCode, lineNumber, itemCode, 0L);
        if (pickupLine != null && !pickupLine.isEmpty()) {
            return pickupLine;
        }
        throw new BadRequestException("The given PickupLine ID : " + "warehouseId:" + warehouseId +
                "companyCodeId: " + companyCodeId + " plantId: " + plantId + " languageId: " + languageId + ",preOutboundNo:"
     ...
     * @return
     */
    public List<PickupLine> getPickupLineForUpdateConfirmation(String warehouseId, String companyCodeId, String plantId, String languageId, String preOutboundNo,
     ...
     * @return
     */
    public List<PickupLine> getPickupLine(String warehouseId, String companyCodeId, String plantId, String languageId, String preOutboundNo,
                                          String refDocNumber, String partnerCode, List<Long> lineNumbers, List<String> itemCodes) {
        List<PickupLine> pickupLine = pickupLineRepository
                .findAllByWarehouseIdAndCompanyCodeIdAndPlantIdAndLanguageIdAndPreOutboundNoAndRefDocNumberAndPartnerCodeAndLineNumberInAndItemCodeInAndDeletionIndicator(
                        warehouseId, companyCodeId, plantId, languageId, preOutboundNo, refDocNumber, partnerCode, lineNumbers, itemCodes, 0L);
        if (pickupLine != null && !pickupLine.isEmpty()) {
            return pickupLine;
        }
        return null;
...
//				+ ",preOutboundNo:" + preOutboundNo + ",refDocNumber:" + refDocNumber + ",partnerCode:" + partnerCode + " doesn't exist.");
     ...
     * @return
     */
    public List<Inventory> getAdditionalBins(String companyCodeId, String plantId, String languageId, String warehouseId,
                                             String itemCode, Long OB_ORD_TYP_ID, String proposedPackBarCode, String proposedStorageBin) {
        ...
        if (OB_ORD_TYP_ID == 0L || OB_ORD_TYP_ID == 1L || OB_ORD_TYP_ID == 3L) {
            ...
            List<Inventory> inventoryAdditionalBins =
                    fetchAdditionalBins(companyCodeId, plantId, languageId, storageSectionIds, warehouseId, itemCode,
        ...
        if (OB_ORD_TYP_ID == 2L) {
            ...
            List<Inventory> inventoryAdditionalBins = fetchAdditionalBinsForOB2(companyCodeId, plantId, languageId, storageSectionIds, warehouseId,
    ...
    public List<PickupLine> createPickupLine(@Valid List<AddPickupLine> newPickupLines, String loginUserID)
        ...
        for (AddPickupLine newPickupLine : newPickupLines) {
            ...
            dbPickupLine.setCompanyCodeId(warehouse.getCompanyCodeId());
        ...
        for (PickupLine dbPickupLine : createdPickupLineList) {
            //------------------------UpdateLock-Applied------------------------------------------------------------
            Inventory inventory = inventoryService.getInventory(dbPickupLine.getWarehouseId(), dbPickupLine.getCompanyCodeId(), dbPickupLine.getPlantId(), dbPickupLine.getLanguageId(),
                            ...
                            try {
                                ...
                                if (inventoryByStBin == null) {
                                    StorageBin dbStorageBin = mastersService.getStorageBin(inventory.getStorageBin(),
                                            dbPickupLine.getWarehouseId(), dbPickupLine.getCompanyCodeId(),
                                            dbPickupLine.getPlantId(), dbPickupLine.getLanguageId(), authTokenForMastersService.getAccess_token());
                                    dbStorageBin.setStatusId(0L);
                                    dbStorageBin.setWarehouseId(dbPickupLine.getWarehouseId());
                                    mastersService.updateStorageBin(inventory.getStorageBin(), dbStorageBin, inventory.getCompanyCodeId(),
                                            inventory.getPlantId() , inventoryByStBin.getLanguageId(), inventory.getWarehouseId(), loginUserID,
                        ...
                        if (inventoryBySTBIN != null && (inventoryBySTBIN.getAllocatedQuantity() == null || inventoryBySTBIN.getAllocatedQuantity() == 0D)
                            ...
                            try {
                                ...
                                StorageBin dbStorageBin = mastersService.getStorageBin(inventory.getStorageBin(),
                                        ...
                                        dbPickupLine.getPlantId(), dbPickupLine.getLanguageId(), authTokenForMastersService.getAccess_token());
                                dbStorageBin.setStatusId(0L);
                                mastersService.updateStorageBin(inventory.getStorageBin(), dbStorageBin,inventory.getCompanyCodeId(), inventory.getPlantId(),
                                        inventory.getLanguageId(), inventory.getWarehouseId(), loginUserID,
                                        authTokenForMastersService.getAccess_token());
                            } catch (Exception e) {
                                log.error("updateStorageBin Error :" + e.toString());
                                e.printStackTrace();
            ...
            StorageBin storageBin = mastersService.getStorageBin(dbPickupLine.getWarehouseId(), BIN_CLASS_ID,
                    authTokenForMastersService.getAccess_token());
            Inventory existingInventory = inventoryService.getInventory(dbPickupLine.getWarehouseId(), dbPickupLine.getCompanyCodeId(), dbPickupLine.getPlantId(), dbPickupLine.getLanguageId(),
            ...
            if (existingInventory != null) {
                try {
                    ...
                    Inventory updatedInventory = inventoryService.updateInventory(dbPickupLine.getWarehouseId(), dbPickupLine.getCompanyCodeId(),
...
//				log.info("outboundLine updated : " + outboundLine);
            ...
            if (dbPickupLine.getStatusId() == 50L) {
                ...
                try {
                    ...
                    QC_NO = getNextRangeNumber(NUM_RAN_CODE, dbPickupLine.getWarehouseId(), dbPickupLine.getCompanyCodeId(), dbPickupLine.getPlantId(), dbPickupLine.getLanguageId());
                    ...
                    StatusId idStatus = idmasterService.getStatus(54L, dbPickupLine.getLanguageId(), authTokenForIDService.getAccess_token());
         ...
         */
        List<OutboundLine> outboundLineList = outboundLineService.getOutboundLine(warehouseId, preOutboundNo, refDocNumber, partnerCode);
        ...
        try {
            ...
            StatusId idStatus = idmasterService.getStatus(STATUS_ID, languageId, authTokenForIDService.getAccess_token());
    ...
    public List<PickupLine> updatePickupLineForConfirmation(String warehouseId, String companyCodeId, String plantId, String languageId, String preOutboundNo,
        ...
        List<PickupLine> dbPickupLine = getPickupLineForUpdateConfirmation(warehouseId, companyCodeId, plantId, languageId, preOutboundNo, refDocNumber,
    ...
    public List<PickupLine> deletePickupLineForReversal(String warehouseId, String companyCodeId, String plantId, String languageId, String preOutboundNo, String refDocNumber,
                                                        String partnerCode, Long lineNumber, String itemCode, String loginUserID)
            throws IllegalAccessException, InvocationTargetException {
        List<PickupLine> dbPickupLine = getPickupLineForReversal(warehouseId, companyCodeId, plantId, languageId, preOutboundNo, refDocNumber, partnerCode,
                lineNumber, itemCode);
        if (dbPickupLine != null && !dbPickupLine.isEmpty()) {
     ...
     * @return
     */
    private List<Inventory> fetchAdditionalBins(String companyCodeId, String plantId, String languageId, List<String> storageSectionIds, String warehouseId,
        ...
        List<Inventory> listInventory =
                inventoryService.getInventoryForAdditionalBins(companyCodeId, plantId, languageId,
                        warehouseId, itemCode, storageSectionIds);
     ...
     * @return
     */
    private List<Inventory> fetchAdditionalBinsForOB2(String companyCodeId, String plantId, String languageId, List<String> storageSectionIds, String warehouseId,
                                                      String itemCode, String proposedPackBarCode, String proposedStorageBin) {
        List<Inventory> listInventory = inventoryService.getInventoryForAdditionalBinsForOB2(companyCodeId, plantId, languageId, warehouseId,
                itemCode, storageSectionIds, 1L /* STCK_TYP_ID */);
