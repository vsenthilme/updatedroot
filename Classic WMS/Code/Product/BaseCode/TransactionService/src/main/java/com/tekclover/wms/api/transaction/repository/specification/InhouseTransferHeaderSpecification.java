...
public class InhouseTransferHeaderSpecification implements Specification<InhouseTransferHeader> {
    ...
    public Predicate toPredicate(Root<InhouseTransferHeader> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (searchInHouseTransferHeader.getCompanyCode() != null && !searchInHouseTransferHeader.getCompanyCode().isEmpty()) {
            final Path<Group> group = root.<Group>get("companyCode");
            predicates.add(group.in(searchInHouseTransferHeader.getCompanyCode()));
        }
        if (searchInHouseTransferHeader.getLanguageId() != null && !searchInHouseTransferHeader.getLanguageId().isEmpty()) {
            final Path<Group> group = root.<Group>get("languageId");
            predicates.add(group.in(searchInHouseTransferHeader.getLanguageId()));
        }
        if (searchInHouseTransferHeader.getPlantId() != null && !searchInHouseTransferHeader.getPlantId().isEmpty()) {
            final Path<Group> group = root.<Group>get("plantId");
            predicates.add(group.in(searchInHouseTransferHeader.getPlantId()));
        }
