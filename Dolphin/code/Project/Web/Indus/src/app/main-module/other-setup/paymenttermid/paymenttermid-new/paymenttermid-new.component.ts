...
@Component({
  selector: 'app-paymenttermid-new',
  templateUrl: './paymenttermid-new.component.html',
  styleUrls: ['./paymenttermid-new.component.scss']
})
export class PaymenttermidNewComponent implements OnInit {
  ...
  constructor(
    ...
    private service: PaymenttermidService,
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.paymentTermId.disable();
  ...
  onwarehouseChange(value){
    this.masterService.searchpaymentterm({companyCodeId: this.form.controls.companyCodeId.value, plantId:this.form.controls.plantId.value, warehouseId:this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
      console.log(res.length);
      if(res.length >0){
      this.paymentterm=res.length +1;
      this.form.controls.paymentTermId.patchValue(this.paymentterm);
      }else{
        this.paymentterm=1;
        this.form.controls.paymentTermId.patchValue(this.paymentterm);
      }
 ...
 if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId).subscribe(res => {
     ...
     this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.paymentTermId + " Saved Successfully!","Notification",{
       timeOut: 2000,
        progressBar: false,
     });
      this.spin.hide();
