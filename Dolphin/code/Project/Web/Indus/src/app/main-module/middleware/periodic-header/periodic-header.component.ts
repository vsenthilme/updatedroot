...
@Component({
  selector: 'app-periodic-header',
  templateUrl: './periodic-header.component.html',
  styleUrls: ['./periodic-header.component.scss']
})
export class PeriodicHeaderComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    fromOrderProcessedOnFE: [],
    fromOrderReceivedOn: [],
    fromOrderReceivedOnFE: [],
    periodicHeaderId: [],
    processedStatusId: [],
    toOrderProcessedOn: [],
    toOrderProcessedOnFE: [],
    toOrderReceivedOn: [],
    toOrderReceivedOnFE: [],
  });
  ...
  search(ispageload = false) {
    this.spin.show();
    this.form.controls.fromOrderProcessedOn.patchValue(this.cs.day_callapi(this.form.controls.fromOrderProcessedOnFE.value));
    this.form.controls.toOrderProcessedOn.patchValue(this.cs.day_callapi(this.form.controls.toOrderProcessedOnFE.value));
    ...
    this.service.periodicHeader(this.form.value).subscribe(
      (res) => {
        this.spin.hide();
  ...
  multicustomer:any[]=[];
  getDropdown() {
    this.sub.add(this.service.periodicHeader({ companyCode:[this.auth.companyId],branchCode:[this.auth.plantId] }).subscribe(res => {
  ...
  downloadexcel() {
    ...
    this.cs.exportAsExcel(res, 'PERIODIC COUNT');
  }
  openConfirm(data: any) {
    console.log(data);   
    let paramdata = this.cs.encrypt({ code: data, pageflow: 'Edit' });
    this.router.navigate(['/main/middleware/periodicHeaderlines/' + paramdata]);
