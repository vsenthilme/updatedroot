...
getDropdown() {
  this.sub.add(this.service.searchSpark({ warehouseId: [this.auth.warehouseId],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId] }).subscribe(res => {
    res.forEach((x: any) => this.businessPartnerTypeList.push({ value: x.businessPartnerType, label: (x.businessPartnerType == 1 ?'Supplier':'Consumer') }));
    res.forEach((x: any) => this.statusIdList.push({ value: x.statusId, label: (x.statusId== 1?'Active':'Inactive')}));
    res.forEach((x: any) => this.partnerCodeList.push({ value: x.partnerCode, label: x.partnerName}));
    ...
    this.partnerCodeList=this.cs.removeDuplicatesFromArrayNewstatus(this.partnerCodeList);
  }))
