...
export class BillingmodeNewComponent implements OnInit {
 ...
 // dialogRef: any;
  ...
  onwarehouseChange(value){
    this.masterService.searchbillmode({companyCodeId: this.form.controls.companyCodeId.value, plantId:this.form.controls.plantId.value, warehouseId:this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
      console.log(res.length);
      if(res.length >0){
      this.billingmode=res.length +1;
      this.form.controls.billModeId.patchValue(this.billingmode);
      }else{
        this.billingmode=1;
        this.form.controls.billModeId.patchValue(this.billingmode);
      }
