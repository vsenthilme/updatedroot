...
@Component({
  selector: 'app-refdoctypeid-new',
  templateUrl: './refdoctypeid-new.component.html',
  styleUrls: ['./refdoctypeid-new.component.scss']
})
export class RefdoctypeidNewComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    referenceDocumentTypeId: [,Validators.required],
    referenceField1: [],
    referenceField10: [],
    referenceField2: [],
    referenceField3: [],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.referenceDocumentTypeId.disable();
 ...
 this.masterService.searchPlant({companyCodeId: [this.form.controls.companyCodeId.value], languageId: [this.form.controls.languageId.value]}).subscribe(res => {
  ...
  if (this.data.code) {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.referenceDocumentTypeId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
