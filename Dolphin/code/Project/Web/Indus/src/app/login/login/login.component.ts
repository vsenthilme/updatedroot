...
export class LoginComponent implements OnInit {
  ...
  loginNew(){
    ...
    let obj: any = {};
    obj.userId = this.lgForm.controls.userName.value;
    obj.loginPassword = this.lgForm.controls.password.value;
    
  this.http.post<any>('/wms-idmaster-service/login/v2', obj).subscribe((res) => {
      if(res.users.length  == 1){
        if(res.users[0].resetPassword == true){
          this.resetTrue = true;
          this.loginTrue = false;
          this.warehouseSelection = false;
          this.userResult = null;
          this.userResult = res.users[0];
          this.spin.hide();
        }
        else if(res.users[0].statusId == 2){
          this.toastr.error(
            "User " + res.users[0].userName + " is currently inactive",
            "Notification"
          );
          this.spin.hide();
            setTimeout(() => {
        ...
        else if(res.users[0].userTypeId == 5){
          this.toastr.error(
        ...
        else if(res.users[0].userTypeId == 7){
          this.toastr.error(
            "User not authorized to login",
            "Notification"
          );
          ...
          this.auth.requestPermission(res.users[0]);
        }
      }
