...
export class CommonService {
  ...
  inboundorderType = [
    ...
    { id: 5, text: 'Direct Stock Receipt' },
  ]
  ...
  public noAccess(screen){
    this.toastr.warning("Don't have " + screen + " access", "Warning", {
      timeOut: 2000,
      progressBar: false,
    });
  }
  ...
  dateapi(date: any) {
    const del_date = this.datepipe.transform(date, "dd-MM-yyyy", "GMT +3");//GMT-06:00
    return del_date;
  }
  dateapiwithTime(date: any) {
    const del_date = this.datepipe.transform(date, "dd-MM-yyyy HH:mm:ss", "GMT +3"); // UTC 0
    return del_date;
  }
  ...
  day_callapigmt(date: any) {
    const del_date = this.datepipe.transform(date, 'yyyy-MM-ddT00:00:00.000', "UTC+06:00");
    return del_date;
  }
  day_callapigmt1(date: any) {
    const del_date = this.datepipe.transform(date, 'yyyy-MM-ddT23:59:00.000', "UTC+06:00");
    return del_date;
  }
  ...
  dateNewFormat111(date: any) {
    const del_date = this.datepipe.transform(date, 'yyyy-MM-dd 00:00:00');
    return del_date;
  }

  dateNewFormat111122(date: any) {
    const del_date = this.datepipe.transform(date, 'yyyy-MM-dd 11:59:00');
    return del_date;
  }
 ...
 timeFormat(date: any) {
  ...
  dateMMYY(date: any) {
    const del_date = this.datepipe.transform(date, "dd-MM-yyyy","GMT +3");
    return del_date;
  }
  dateddMMYY(date: any) {
    const del_date = this.datepipe.transform(date, "MM-dd-yyyy");
  ...
  getDataDiffInDays(startDate: any, endDate: any) {
    var diff = this.date(endDate).getTime() - this.date(startDate).getTime();
    var days = Math.floor(diff / (60 * 60 * 24 * 1000));
    var hours = Math.floor(diff / (60 * 60 * 1000)) - (days * 24);
    var minutes = Math.floor(diff / (60 * 1000)) - ((days * 24 * 60) + (hours * 60));
  ...
  getTotalTime(time1: string, time2: string, time3: string) {
  ...
  exportAsCSV(data: any, filename: string = "Downloaded_Excel") {
    // const asnConf = data;
  ...
  removeDuplicates(data) {
    ...
    data.forEach(item => {
      // Generate a unique key based on the two columns you want to compare
      const key = item.companyCodeId + '-' + item.plantId + '-' + item.warehouseId + '-' + item.moduleId;
console.log(key)
    ...
    // Update your data array with uniqueData
    return uniqueData;
  }
 ...
 removeDuplicatesFromArrayList1(list: any = []) {
    let dataObj: any = {};
    for (let i = 0; i < list.length; i++) {
      
      list[i]['key'] = list[i].companyCodeId + '-' + list[i].plantId + '-' + list[i].warehouseId + '-' + list[i].moduleId;
...
removeDuplicateObjBOM(array1, array2){
  const array1IDs = new Set(array1.map(({ bomItem }) => bomItem));
  const combined = [
    ...array1,
    ...array2.filter(({ bomItem }) => !array1IDs.has(bomItem))
  ];
  return combined
}
