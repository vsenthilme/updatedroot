...
import { ControltypeidService } from '../main-module/other-setup/controltypeid/controltypeid.service';
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (bearerToken) {
      ...
      if (req.url === 'firebase-messaging-sw.js') {
        console.log(2)
        newUrl = 'https://d.classicwms.com/wmscore/firebase-messaging-sw.js';

        authReq = req.clone({ url: newUrl});
      }

    if (req.url.includes('upload') &&  req.method == 'POST') {
      authReq = req.clone({ url: `${url}${req.url}` });
    }
    ...
    else if (req.url.split('/')[1] == 'batch-upload' &&  req.method == 'POST') {
