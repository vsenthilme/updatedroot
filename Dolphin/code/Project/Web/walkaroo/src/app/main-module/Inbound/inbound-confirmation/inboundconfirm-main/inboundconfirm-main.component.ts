...
//const inboundOrderTypeId = [...new Set(res.map(item => item.inboundOrderTypeId))].filter(x => x != null)
  ...
  openDialog(data: any = 'Display',type?: any): void {
    ...
    if (this.selectedready.length > 0) {
      paramdata = this.cs.encrypt({ code: this.selectedready[0], pageflow: 'Display' ,pageflow1:'Inbound Summary'});
      this.router.navigate(['/main/inbound/inbound-create/' + paramdata]);
    }
    else {
      paramdata = this.cs.encrypt({ pageflow: 'Display',pageflow1:'Inbound Summary' });
