...
import { StratergiesService } from 'src/app/main-module/Masters -1/masternew/stratergies/stratergies.service';
...
@Component({
  selector: 'app-create-stock-adjustment',
...
export class CreateStockAdjustmentComponent implements OnInit {
  ...
  form = this.fb.group({
    adjustmentQty: [, Validators.required],
  afterAdjustment: [],
  allocatedQuantity:[],
  amsReferenceNo: [],
  barcodeId: [],
  ...
  ngOnInit(): void {
    ...
    console.log(this.form.controls.isCycleCount.value)
  ...
  changevalue(value){
    console.log(value)
    if(value =='Y'){
      this.form.controls.isDamage.patchValue("N")
    }
    else{
      this.form.controls.isDamage.patchValue("Y")
    }
  }
  ...
  submit() {
   ...
   this.form.controls.dateOfAdjustment.patchValue(this.cs.day_callapiSearch(this.form.controls.dateOfAdjustment.value))
      this.sub.add(this.service.createstockadjustmentV2(this.form.getRawValue()).subscribe(res => {
        this.toastr.success( "Stock Adjustment Created Successfully!", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
