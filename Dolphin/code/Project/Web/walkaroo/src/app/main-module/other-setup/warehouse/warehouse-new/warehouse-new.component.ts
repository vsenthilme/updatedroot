...
export class WarehouseNewComponent implements OnInit {
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.plantId,this.data.companyCodeId,this.data.languageId).subscribe(res => {
  ...
  dropdownlist(){
   ...
   // this.plantidList = this.cas.forLanguageFilter(results[2], this.cas.dropdownlist.setup.plantid.key);
...
if (this.data.code) {
  this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code,this.data.plantId,this.data.companyCodeId,this.data.languageId).subscribe(res => {
