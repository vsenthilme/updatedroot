...
console.log(this.selectedgrNew1)
  ...
  submitInterim2(){
    //this.finalValue=[];
    this.spin.show();
    console.log(this.grNew); // Check the structure of this.grNew

    for (let i = 0; i < this.grNew.length; i++) {
      const grItem = this.grNew[i];
        const newBarcode = {
          quantityType:'A',
         
        };
      
        grItem.packBarcodes.push(newBarcode);
      
       
    }
  console.log(this.grNew)
   this.finalValue=this.grNew;
    this.service.CreateLine(this.finalValue).subscribe(res => {
      this.toastr.success("GR created successfully", "Notification",
       {
        timeOut: 2000,
