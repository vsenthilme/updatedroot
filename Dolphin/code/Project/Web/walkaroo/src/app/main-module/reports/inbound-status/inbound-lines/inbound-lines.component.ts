...
export class InboundLinesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.warehouseId, this.data.preInboundNo, this.data.refDocNumber, this.auth.companyId ,this.data.languageId, this.data.plantId).subscribe(res => {
  ...
  ELEMENT_DATA: any[] = [];
  displayedColumns: string[] = [  'lineNo', 'manufacturerName', 'itemCode','articleNo', 'materialNo','gender','size','priceSegment','color',
