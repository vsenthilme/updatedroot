...
export class PrepetualConfirmComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (js.pageflow == 'New') {
      ...
      console.log(JSON.parse(sessionStorage.getItem('RunPerpetualResponse') as '[]'));
      ...
      this.prepetualCountService.get(this.code).subscribe(result=>{
        ...
        if(js.pageflow == "Variance Analysis"){
          ...
          this.code.perpetualLine.forEach(element => {
            console.log(element.countedQty - element.inventoryQuantity)
       ...
       if(js.pageflow == "Assign User"){
        ...
        this.pageTitle = this.pageflow;
        this.displayedColumns = ['select', 'no', 'itemCode', 'itemDesc', 'manufacturerPartNo', 'storageSectionId', 'storageBin', 'packBarcodes', 'stockTypeId', 'inventoryQuantity', 'cycleCounterId','countedQty', 'statusId'];
      }
        this.dataSource.paginator = this.paginator;
        this.dataSource.sort = this.sort;
      });
