...
@Component({
  selector: 'app-adhocmoduleid-new',
  templateUrl: './adhocmoduleid-new.component.html',
  styleUrls: ['./adhocmoduleid-new.component.scss']
})
export class AdhocmoduleidNewComponent implements OnInit {
  ...
  form = this.fb.group({
    adhocModuleDescription: [],
    adhocModuleId: [,Validators.required],
...
moduleIdAndDescription:[],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.adhocModuleId.disable();
  ...
  dropdownlist(){
   ...
   this.masterService.searchModule({companyCodeId:  this.auth.companyId, plantId:this.auth.plantId, warehouseId: this.auth.warehouseId, languageId: [this.auth.languageId]}).subscribe(res => {
    ...
    this.moduleList = this.cas.removeDuplicatesFromArrayNew(this.moduleList);
  });
  ...
  dropdownlistSuperAdmin(){
    ...
    // this.companyidList = this.cas.foreachlist2(results[1], this.cas.dropdownlist.setup.companyid.key);
    // this.warehouseidList = this.cas.foreachlist2(results[2], this.cas.dropdownlist.setup.warehouseid.key);
    //this.moduleList = this.cas.foreachlist2(results[3], this.cas.dropdownlist.setup.moduleid.key);
 ...
 this.masterService.searchModule({languageId: [this.form.controls.languageId.value],companyCodeId:this.form.controls.companyCodeId.value,plantId:this.form.controls.plantId.value}).subscribe(res => {
  this.moduleList = [];
   res.forEach(element => {
  this.moduleList.push({value: element.moduleId, label: element.moduleId + '-' + element.moduleDescription});
   });
...
onPlantChange(value){
  ...
  if (this.data.code) {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.adhocModuleId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
