...
export class ProcessNewComponent implements OnInit {
  ...
  form = this.fb.group({
    companyCodeId: [,Validators.required],
       ...
       deletionIndicator: [],
        ...
        processDescription: [],
        processSequenceId:[,Validators.required],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.processSequenceId.disable();
     //this.form.controls.processId.disable();
      this.form.controls.warehouseId.disable();
      this.form.controls.languageId.disable();
      this.form.controls.plantId.disable();
      this.form.controls.companyCodeId.disable();
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId,this.data.processId).subscribe(res => {
  ...
  dropdownlistSuperAdmin(){
    ...
    this.cas.dropdownlist.setup.processid.url,
    ]).subscribe((results) => {
      ...
      //  this.process = this.cas.foreachlist2(results[4], this.cas.dropdownlist.setup.storageclassid.key);
 ...
 }); this.masterService.searchprocess({companyCodeId: this.data.companyCodeId, languageId: [this.data.languageId],plantId:this.data.plantId,warehouseId:this.data.warehouseId}).subscribe(res => {
  this.processList = [];
  res.forEach(element => {
    this.processList.push({value: element.processId, label: element.processId + '-' + element.processText});
  });
});
   this.spin.hide();
    }, (err) => {
...
this.plantidList = this.cas.forLanguageFilter(results[3], this.cas.dropdownlist.setup.plantid.key);
this.masterService.searchprocess({companyCodeId: this.auth.companyId, plantId: this.auth.plantId, warehouseId: this.auth.warehouseId, languageId: [this.auth.languageId]}).subscribe(res => {
  this.processList = [];
  res.forEach(element => {
    this.processList.push({value: element.processId, label: element.processId + '-' + element.processText});
  })
  ...
  onLanguageChange(value){
    ...
    this.masterService.searchprocess({companyCodeId: this.form.controls.companyCodeId.value, plantId: this.form.controls.plantId.value, warehouseId: this.form.controls.warehouseId.value, languageId: [value.value]}).subscribe(res => {
      this.processList = [];
      res.forEach(element => {
        this.processList.push({value: element.processId, label: element.processId + '-' + element.processText});
      });
  ...
  onCompanyChange(value){
    ...
    this.masterService.searchprocess({companyCodeId: value.value, plantId: this.form.controls.plantId.value, warehouseId: this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
      this.processList = [];
      res.forEach(element => {
        this.processList.push({value: element.processId, label: element.processId + '-' + element.processText});
      });
  ...
  onPlantChange(value){
    ...
    this.masterService.searchprocess({companyCodeId: this.form.controls.companyCodeId.value, plantId: value.value, warehouseId: this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
      this.processList = [];
      res.forEach(element => {
        this.processList.push({value: element.processId, label: element.processId + '-' + element.processText});
      });
...
onWarehouseChange(value){
  this.masterService.searchprocess({companyCodeId: this.form.controls.companyCodeId.value, plantId:this.form.controls.plantId.value, warehouseId: value.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
    this.processList = [];
    res.forEach(element => {
      this.processList.push({value: element.processId, label: element.processId + '-' + element.processText});
    });
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId,this.data.processId).subscribe(res => {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.processSequenceId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
