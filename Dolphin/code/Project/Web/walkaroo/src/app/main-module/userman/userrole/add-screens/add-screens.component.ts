...
@Component({
  ...
  styleUrls: ['./add-screens.component.scss']
})
export class AddScreensComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.js.pageflow != "New") {
      this.sub.add(this.service.Update(data, this.form.controls.roleId.value, this.auth.warehouseId, this.auth.companyId, this.auth.plantId, this.auth.languageId).subscribe(res => {
