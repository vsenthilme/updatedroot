...
calculateTotals(data?: any[]): void {
  ...
  this.totalQty1=0;
   if (data && data.length > 0) {
     ...
     this.totalQty1= data.reduce((total, item) => total + (item.referenceField4 != null ? item.referenceField4 : 0), 0);
   } else {
     this.inventory.forEach(x => {
       this.totalInventoryQuantity = 0;
       this.totalAllocatedQuantity = 0;
