...
import { ReportsModule } from 'src/app/main-module/reports/reports.module';
...
export class BarChartComponent implements OnInit {
    ...
    transformData(result: any) {
        ...
        result.forEach(item => {
            ...
            if (!item.pickupConfirmedOn) return;
    
            const date = new Date(item.pickupConfirmedOn).toISOString().split('T')[0]; // YYYY-MM-DD
            ...
            const confirmedQty = Number(item.pickConfirmQty ) || 0;
            dateMap[formattedDate].total += confirmedQty;
    
            // Store each item's details for drilldown
            dateMap[formattedDate].items.push({
                name: item.refDocNumber, // Assuming you have an item name field
                //y: confirmedQty
                y: item.leadTime
            });
        });
        ...
        this.drilldownData = Object.keys(dateMap).map(date => ({
            ...
            data: dateMap[date].items.map(item => [item.name, item.y]) // Convert items to array format for drilldown
        }));
    ...
    pickerChoosed(event:any){
        ...
        obj.warehouseId = [this.auth.warehouseId];
        if(event != null){
            obj.assignedPickerId = [event.value];
        }else{
            obj.assignedPickerId = [this.userList[0].value];
        }
    
        this.spin.show();
       this.report.pickingProducitivy(obj).subscribe((res: any[]) => {
          if (res) {
