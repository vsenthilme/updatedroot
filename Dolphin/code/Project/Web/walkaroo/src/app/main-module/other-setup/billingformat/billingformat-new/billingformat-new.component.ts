...
@Component({
  selector: 'app-billingformat-new',
  templateUrl: './billingformat-new.component.html',
  styleUrls: ['./billingformat-new.component.scss']
})
export class BillingformatNewComponent implements OnInit {
 ...
 // dialogRef: any;
  ...
  form = this.fb.group({
    billFormatId: [,Validators.required],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.billFormatId.disable();
  ...
  onwarehouseChange(value){
    this.masterService.searchbillformat({companyCodeId: this.form.controls.companyCodeId.value, plantId:this.form.controls.plantId.value, warehouseId:this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
      console.log(res.length);
      if(res.length >0){
      this.billingformat=res.length +1;
      this.form.controls.billFormatId.patchValue(this.billingformat);
      }else{
        this.billingformat=1;
        this.form.controls.billFormatId.patchValue(this.billingformat);
      }
    });
  }
 ...
 if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId).subscribe(res => {
     ...
     this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.billFormatId + " Saved Successfully!","Notification",{
       timeOut: 2000,
        progressBar: false,
     });
      this.spin.hide();
