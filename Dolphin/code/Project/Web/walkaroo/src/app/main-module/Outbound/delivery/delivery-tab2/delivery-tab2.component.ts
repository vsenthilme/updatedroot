...
export class DeliveryTab2Component implements OnInit {
  ...
  location:any;
    ...
    async download(element) {
      this.spin.show()
    if((element == 'deliveryConfirmation')) {
      this.fileName='Delivery_confirmation_template.xlsx'
      this.location='/outbound/DeliveryConfirmation'
    }
    ...
    uploadDocument(event){
      this.service.DeliveryUpload(this.file, this.selectedContext).subscribe(   //[this.selectedContext]
        (resp: any) => {
    ...
    showSuccess() {
      this.toastr.success('Delivery Processed successfully',);
    }
    preInboundUpload(){
