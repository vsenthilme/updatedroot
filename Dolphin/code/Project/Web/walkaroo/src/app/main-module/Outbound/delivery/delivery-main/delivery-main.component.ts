...
export class DeliveryMainComponent implements OnInit {
  ...
  search(ispageload = false) {
    //if (!ispageload) {
      ...
      //dateconvertion
      // this.searhform.controls.endDeliveryConfirmedOn.patchValue(this.cs.dateNewFormat111122(this.searhform.controls.endDeliveryConfirmedOnFE.value));
      // this.searhform.controls.startDeliveryConfirmedOn.patchValue(this.cs.dateNewFormat111(this.searhform.controls.startDeliveryConfirmedOnFE.value));
  ...
  downloadexcel() {
    ...
    this.delivery.forEach(x => {
      res.push({
        ...
        "HU Serial No": x.referenceField2,
        "Order Date":this.cs.dateapiwithTime(x.refDocDate),
        "Delivery Date ":this.cs.dateapiwithTime(x.deliveryConfirmedOn),
        "Lead Time": x.refDocDate ? this.cs.getDataDiff(x.refDocDate,x.deliveryConfirmedOn) : '',
        " Order Category ": x.referenceField1,
      });

    })
    this.cs.exportAsExcel(res, "Delivered");
  }
