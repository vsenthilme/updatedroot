...
StatusList: any[] = [];
  ...
  getDropdown(){
    this.sub.add(this.reportService.pickupSpark({warehouseId : [this.auth.warehouseId],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId]}).subscribe(res => {
      ...
      res.forEach((x: any) =>
        this.multisalesorder.push({ value: x.salesOrderNumber, label: x.salesOrderNumber })
        );
        res.forEach((x: any) => {
          if (x.levelId !== null) {
              this.multilevel.push({ value: x.levelId, label: x.levelId });
          }
      });
      ...
      this.multisalesorder=this.cs.removeDuplicatesFromArrayNewstatus(this.multisalesorder);
