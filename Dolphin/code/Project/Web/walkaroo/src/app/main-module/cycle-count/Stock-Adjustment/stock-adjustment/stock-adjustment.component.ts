...
export class StockAdjustmentComponent implements OnInit {
  ...
  create(): void {
    this.router.navigate(['/main/cycle-count/stockAdjust'])
  ...
  handleSearch1(filterInput){
    console.log(filterInput);
     if (filterInput) {
       this.calculateTotals(this.stockAdjustmentTag.filteredValue);
     }
     else {
       this.calculateTotals(this.stockAdjustment);
   
     }
   }
 
   filteredInventory: any[] = []; // Filtered inventory data array, if applicable
   totaladjustQuantity: number = 0; // Total inventory quantity
   totalbeforeadjustQuantity: number = 0; // Total allocated quantity
   totalAfteradjustQuantity: number = 0; 
 totalQty1:number =0;
 calculateTotals(data?: any[]): void {
  console.log(data);
  this.totaladjustQuantity = 0;
  this.totalbeforeadjustQuantity = 0;
  this.totalAfteradjustQuantity = 0;

  if (data && data.length > 0) {
    this.totaladjustQuantity = data.reduce((total, item) => total + (item.adjustmentQty != null ? item.adjustmentQty : 0), 0);
    this.totalbeforeadjustQuantity = data.reduce((total, item) => total + (item.beforeAdjustment != null ? item.beforeAdjustment : 0), 0);
    this.totalAfteradjustQuantity = data.reduce((total, item) => total + (item.afterAdjustment != null ? item.afterAdjustment : 0), 0);
  }
else{
  // Calculate the sum of all total elements
  this.totaladjustQuantity = this.stockAdjustment.reduce((total, item) => total + (item.adjustmentQty != null ? item.adjustmentQty : 0), this.totaladjustQuantity);
  this.totalbeforeadjustQuantity = this.stockAdjustment.reduce((total, item) => total + (item.beforeAdjustment != null ? item.beforeAdjustment : 0), this.totalbeforeadjustQuantity);
  this.totalAfteradjustQuantity = this.stockAdjustment.reduce((total, item) => total + (item.afterAdjustment != null ? item.afterAdjustment : 0), this.totalAfteradjustQuantity);
}
 }
