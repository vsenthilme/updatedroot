...
export class PricelistassignmentNewComponent implements OnInit {
  ...
  fill(){
    ...
    this.sub.add(this.service.Get(this.js.code,this.js.warehouseId,this.js.languageId,this.js.plantId,this.js.companyCodeId,this.js.partnerCode).subscribe(res => {
      ...
      this.masterService.searchbuisnesspartner({ warehouseId: [res.warehouseId],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId]}).subscribe(res1 => {
     ...
     this.masterService.searchbuisnesspartner({ warehouseId:[this.auth.warehouseId], partnerCode:[res.partnerCode],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId] }).subscribe(res => {
      this.businesspartner2List = [];
     res.forEach(element => {
       this.businesspartner2List.push({value: element.businessPartnerType, label: element.businessPartnerType == 1 ?'Supplier ': 'Customer'});
    })
    });
    ...
    dropdownlist(){
     ...
     this.cas.dropdownlist.master.businesspartnertype.url,
        ...
        this.priceDropdown.forEach(element => {
          this.priceList.push({value: element.priceListId, label: element.priceListId+'-'+element.description+'-'+element.moduleIdAndDescription+'-'+element.serviceTypeIdAndDescription, companyCodeId: element.companyCodeId, plantId: element.plantId, languageId: element.languageId,warehouseId: element.warehouseId,moduleId: element.moduleId,serviceTypeId:element.serviceTypeId,chargeRangeId:element.chargeRangeId});
       this.priceList1=this.cs.removeDuplicatesFromArrayNewstatus(this.priceList);
        });
        
 // this.businesspartnerList=this.cas.forLanguageFilter(results[0],this.cas.dropdownlist.master.businesspartner.key);
 this.masterService.searchbuisnesspartner({ warehouseId:[this.form.controls.warehouseId.value],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId] }).subscribe(res => {
  this.businesspartnerList = [];
 res.forEach(element => {
   this.businesspartnerList.push({value: element.partnerCode, label: element.partnerCode+'-'+element.partnerName});
})
  ...
  this.businesspartner2List=this.cas.forLanguageFilter(results[2],this.cas.dropdownlist.master.businesspartnertype.key);
    ...
    priceListsWithDetails:any[]=[];
     onpricelistChange(value){
      console.log(value);
      const selectedValue = value.value;
      console.log(this.priceList)
      this.priceListsWithDetails = this.priceList.filter(item => item.value === selectedValue);
    console.log(this.priceListsWithDetails);
    ...
    //   });
     ...
     onbusinesspartnerChange(value){
      this.masterService.searchbuisnesspartner({  warehouseId:[this.form.controls.warehouseId.value],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId] ,partnerCode:[value.value] }).subscribe(res => {
        this.businesspartner2List = [];
       res.forEach(element => {
         this.businesspartner2List.push({value: element.businessPartnerType, label: element.businessPartnerType == 1 ?'Supplier ': 'Customer'});
      })
  ...
  if (this.js.code) {
    ...
    console.log(this.priceListsWithDetails)
    this.spin.show();
    for (let i=0;i<this.priceListsWithDetails.length;i++){
      this.form.controls.moduleId.patchValue(this.priceListsWithDetails[i].moduleId);
      this.form.controls.chargeRangeId.patchValue(this.priceListsWithDetails[i].chargeRangeId);
      this.form.controls.priceListId.patchValue(this.priceListsWithDetails[i].value);
      this.form.controls.serviceTypeId.patchValue(this.priceListsWithDetails[i].serviceTypeId);
      this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
        ;
      })
      )
    }
    this.toastr.success(this.priceListsWithDetails[0].value + " Assigned successfully.","Notification",{
      timeOut: 2000,
      progressBar: false,
    });
      this.router.navigate(['/main/threePLmaster/pricelistassign']);
