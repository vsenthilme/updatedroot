...
requiredDeliveryDate : [],
 ...
 panelOpenState = false;
  ...
  ngOnInit(): void {
    ...
    this.business.searchSpark(obj1).subscribe((res: any[]) => {
      res.forEach(element => {
        this.multiPartnerList.push({value: element.partnerCode, label: element.partnerCode + '-' + element.partnerName});
         });
         this.spin.hide();
    })
    this.soHeader.get('wareHouseId')?.disable();
  this.soHeader.get('branchCode')?.disable();
  this.soHeader.get('companyCode')?.disable();
  this.soHeader.get('languageId')?.disable();
  }
 ...
 soHeader.transferOrderNumber = this.soHeader.controls.transferOrderNumber.value;
    ...
    for(let i=0;i<this.inventory.length;i++){
      this.inventory[i].supplierCode=this.soHeader.controls.storeID.value;
      this.inventory[i].storeID=this.soHeader.controls.storeID.value;
     }
   obj.soLine= this.inventory;
  

this.soHeader.controls.requiredDeliveryDate.patchValue(soHeader.requiredDeliveryDate);
console.log(obj)
    this.sub.add(this.service.createOUTBOUNDSO(obj).subscribe(res => {
  if(res){
