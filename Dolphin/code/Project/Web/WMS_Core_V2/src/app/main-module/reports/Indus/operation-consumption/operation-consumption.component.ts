...
import { ContainerReceiptService } from 'src/app/main-module/Inbound/Container-receipt/container-receipt.service';
...
@Component({
  selector: 'app-operation-consumption',
  templateUrl: './operation-consumption.component.html',
  styleUrls: ['./operation-consumption.component.scss']
})
export class OperationConsumptionComponent implements OnInit {
  ...
  callTableHeader() {
    this.cols = [
      { field: 'productionOrderNo', header: 'Order No' },
      { field: 'itemCode', header: 'Part No' },
      { field: 'itemDescription', header: 'Part Name' },
      { field: 'phaseNumber', header: 'Phase No' },
      { field: 'phaseDescription', header: 'Phase Name' },
      { field: 'receipeQuantity', header: 'Order Qty' },
      ...
      { field: 'consumedQuantity', header: 'Consumpted Qty' },
  ...
  filtersearch() {
    ...
    this.sub.add(this.reportService.operationConsumption(this.searhform.getRawValue()).subscribe(res => {
  ...
  downloadexcel() {
    ...
    this.storageselection.forEach(x => {
      res.push({
        ...
        "From Date - To Date": this.cs.dateapi(x.Date),
