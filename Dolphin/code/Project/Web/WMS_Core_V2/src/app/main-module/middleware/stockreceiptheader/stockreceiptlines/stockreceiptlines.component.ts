...
@Component({
  selector: 'app-stockreceiptlines',
  templateUrl: './stockreceiptlines.component.html',
  styleUrls: ['./stockreceiptlines.component.scss']
})
export class StockreceiptlinesComponent implements OnInit {
...
this.picking=data.code.stockReceiptLines;
}
...
downloadexcel() {
  ...
  this.picking.forEach((x, index) => {
    res.push({
      ...
      "UOM": x.unitOfMeasure,
      "Receipt Qty": x.receiptQty,
      "Receipt Date": this.cs.dateapiwithTime(x.receiptDate),
      // 'Created By': x.createdBy,
      // 'Date': this.cs.dateapi(x.createdOn),
    });
  });
  const receiptNo = this.code.receiptNo; 
const fileName = `DIRECT RECEIPT -_${receiptNo}-`;
