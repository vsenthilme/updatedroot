...
@Component({
  selector: 'app-serviceid-new',
  templateUrl: './serviceid-new.component.html',
  styleUrls: ['./serviceid-new.component.scss']
})
export class ServiceidNewComponent implements OnInit {
  disabled = false;
  step = 0;
  //dialogRef: any;
  ...
  fill() {
    this.spin.show();
    this.sub.add(this.service.Get(this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId,this.data.moduleId).subscribe(res => {
      this.form.patchValue(res, { emitEvent: false });
      this.form.controls.moduleId.patchValue(res.moduleId as string)
  ...
  dropdownlist(){
   ...
   // this.moduleList = this.cas.forLanguageFilter(results[3], this.cas.dropdownlist.setup.moduleid.key);
    this.masterService.searchModule({companyCodeId: [this.form.controls.companyCodeId.value], languageId: [this.form.controls.languageId.value],plantId:[this.form.controls.plantId.value],warehouseId:[this.form.controls.warehouseId.value]}).subscribe(res => {
      this.moduleList = [];
      res.forEach(element => {
  ...
  this.moduleList=this.cs.removeDuplicateInArray(this.moduleList);
   this.spin.hide();
    }, (err) => {
      this.toastr.error(err, "");
      this.spin.hide();
...
onmoduleChange(value){
  this.masterService.searchserviceType({companyCodeId: this.form.controls.companyCodeId.value, plantId:this.form.controls.plantId.value, warehouseId:this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value],moduleId:[this.form.controls.moduleId.value]}).subscribe(res => {
console.log(res.length);
if(res.length >0){
this.servicetype=res.length +1;
this.form.controls.serviceTypeId.patchValue(this.servicetype);
}else{
  this.servicetype=1;
  this.form.controls.serviceTypeId.patchValue(this.servicetype);
}
...
onLanguageChange(value){
  ...
  this.masterService.searchModule({companyCodeId:  this.form.controls.companyCodeId.value, plantId:this.form.controls.plantId.value, warehouseId: this.form.controls.warehouseId.value, languageId: [value.value]}).subscribe(res => {
    this.moduleList = [];
    res.forEach(element => {
      this.moduleList.push({value: element.moduleId, label: element.moduleId + '-' + element.moduleDescription});
    });
...
onCompanyChange(value){
  ...
  this.masterService.searchModule({companyCodeId: value.value, plantId: this.form.controls.plantId.value, warehouseId: this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
    this.moduleList = [];
    res.forEach(element => {
      this.moduleList.push({value: element.moduleId, label: element.moduleId + '-' + element.moduleDescription});
    });
...
onPlantChange(value){
    ...
    this.masterService.searchModule({companyCodeId:this.form.controls.companyCodeId.value, plantId: value.value, warehouseId: value.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
      this.moduleList = [];
      res.forEach(element => {
        this.moduleList.push({value: element.moduleId, label: element.moduleId + '-' + element.moduleDescription});
      });
  ...
  if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(),this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId,this.data.moduleId).subscribe(res => {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.serviceTypeId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
