...
export class StoragebintypeNewComponent implements OnInit {
  ...
  dropdownlist(){
   ...
   this.companyList = this.cas.forLanguageFilter(results[1], this.cas.dropdownlist.setup.companyid.key);
    ...
    this.masterService.searchstoragetypeenter({companyId: this.auth.companyId, plantId: this.form.controls.plantId.value, warehouseId: this.form.controls.warehouseId.value, languageId: this.auth.languageId}).subscribe(res => {
    ...
    this.masterService.searchstoragebintype({companyCodeId: this.auth.companyId, plantId: this.auth.plantId, warehouseId: this.auth.warehouseId, languageId: [this.auth.languageId]}).subscribe(res => {
