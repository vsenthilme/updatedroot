...
export class PreoutboundService {
  ...
  searchOutboundHeader(obj: any) {
    ...
    public outboundUpload(file: File, apiName:any) {
      ...
      return this.http.post(`/wms-transaction-service/warehouse/outbound/${apiName}/upload`+ '?languageId='+this.auth.languageId+'&companyCodeId='+this.auth.companyId+'&plantId='+this.auth.plantId+'&warehouseId='+this.auth.warehouseId+'&loginUserID='+this.auth.userID , formParams)
    }
}
