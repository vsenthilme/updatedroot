...
@Component({
  selector: 'app-billingfrequency-new',
  templateUrl: './billingfrequency-new.component.html',
  styleUrls: ['./billingfrequency-new.component.scss']
})
export class BillingfrequencyNewComponent implements OnInit {
 ...
 // dialogRef: any;
  ...
  form = this.fb.group({
    billFrequencyId: [,Validators.required],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.billFrequencyId.disable();
  ...
  onwarehouseChange(value){
    this.masterService.searchbillfrequency({companyCodeId: this.form.controls.companyCodeId.value, plantId:this.form.controls.plantId.value, warehouseId:this.form.controls.warehouseId.value, languageId: [this.form.controls.languageId.value]}).subscribe(res => {
      console.log(res.length);
      if(res.length >0){
      this.billingfrequency=res.length +1;
      this.form.controls.billFrequencyId.patchValue(this.billingfrequency);
      }else{
 ...
 if (this.data.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code,this.data.warehouseId,this.data.languageId,this.data.companyCodeId,this.data.plantId).subscribe(res => {
     ...
     this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.billFrequencyId + " Saved Successfully!","Notification",{
       timeOut: 2000,
        progressBar: false,
     });
      this.spin.hide();
