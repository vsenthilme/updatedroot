...
export class PreoutboundMainComponent implements OnInit {
  ...
  @ViewChild('preoutboundTag') preoutboundTag: Table | undefined;
  ...
  callTableHeader() {
    this.cols = [
      { field: 'referenceField10', header: 'Status' },
      { field: 'partnerCode', header: 'Store Code' },
      { field: 'outboundOrderTypeId', header: 'Order Type', format: 'extra' },
      { field: 'referenceField1', header: 'Order Category' },
      { field: 'preOutboundNo', header: 'Preoutbound No', format:'extra' },
      { field: 'refDocNumber', header: 'Order No' },
      { field: 'refDocDate', header: 'Ordered Date', format: 'date' },
  ...
  search(ispageload = false) {
    ...
    this.spin.show();
    this.service.searchNew(this.searhform.value).subscribe(res => {
      ...
      if (ispageload) {
        ...
        res.forEach(x => this.statusIdList.push({ value: x.statusId, label: x.referenceField10 }));
        this.statusIdList = this.cs.removeDuplicatesFromArrayNewstatus(this.statusIdList)

      }
      this.preoutbound = res;
  ...
  openDialog(data: any = 'new', type?: any): void {
    if (type) {
      this.selectedPreoutbound.push(type);
    }
    if (data != 'New')
      if (this.selectedPreoutbound.length === 0) {
    ...
    debugger
    if (this.selectedPreoutbound.length > 0) {
      paramdata = this.cs.encrypt({ code: this.selectedPreoutbound[0], pageflow: data });
  ...
  downloadexcel() {
    const exportData = this.preoutbound.map(item => {
  ...
  selectedPreoutbound: any[] = [];
  onChange() {
    const choosen = this.selectedPreoutbound[this.selectedPreoutbound.length - 1];
    this.selectedPreoutbound.length = 0;
    this.selectedPreoutbound.push(choosen);
  }
}
