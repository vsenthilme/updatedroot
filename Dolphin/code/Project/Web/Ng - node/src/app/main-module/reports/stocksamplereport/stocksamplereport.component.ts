...
export class StocksamplereportComponent implements OnInit {
  ...
  showFiller = false;
  displayedColumns: string[] = ['itemCode','referenceField8','referenceField9', 'storageBin', 'referenceField10', 'packBarcodes', 'inventoryQuantity', 'allocatedQuantity', 'total', 'stockTypeId', 'inventoryUom', 'createdBy', 'createdOn', 'asnNumber']; //20-06-23 streaming
  ...
  applyFilter(event: Event) {
    ...
    if(this.dataSource.filter.length == 0){
      this.dataSource.data.forEach(element => {
        this.totalonhandqty = this.totalonhandqty + (element.inventoryQuantity != null ? element.inventoryQuantity : 0);
        this.totaldamqty = this.totaldamqty + (element.allocatedQuantity != null ? element.allocatedQuantity : 0);
        this.totalholdqty = this.totalholdqty + (element.total != null ? element.total : 0);
      })
    }
  else{
    for (let i=0;i<this.dataSource.filteredData.length;i++){
      this.totalonhandqty = this.totalonhandqty + (this.dataSource.filteredData[i].inventoryQuantity != null ? this.dataSource.filteredData[i].inventoryQuantity : 0);
      this.totaldamqty = this.totaldamqty + (this.dataSource.filteredData[i].allocatedQuantity != null ? this.dataSource.filteredData[i].allocatedQuantity : 0);
      this.totalholdqty = this.totalholdqty + (this.dataSource.filteredData[i].total != null ? this.dataSource.filteredData[i].total : 0);
  ...
  onStorageType(searchKey) {
    ...
    if (searchVal !== '' && searchVal !== null) {
      forkJoin({
        storageList: this.reportService.getStorageDropDown2(searchVal.trim(),this.auth.companyId,this.auth.plantId,this.selectedItems[0],this.auth.languageId).pipe(catchError(err => of(err))),
      })
