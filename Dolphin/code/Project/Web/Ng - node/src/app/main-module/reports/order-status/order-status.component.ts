  ...
  applyFilter(event: Event) {
    ...
    if(this.dataSource.filter.length == 0){
      this.dataSource.data.forEach(element => {
        this.totalonhandqty = this.totalonhandqty + (element.orderedQty != null ? element.orderedQty : 0);
        this.totaldamqty = this.totaldamqty + (element.deliveredQty != null ? element.deliveredQty : 0);
        
      })
    }
  else{
    for (let i=0;i<this.dataSource.filteredData.length;i++){
      this.totalonhandqty = this.totalonhandqty + (this.dataSource.filteredData[i].orderedQty != null ? this.dataSource.filteredData[i].orderedQty : 0);
      this.totaldamqty = this.totaldamqty + (this.dataSource.filteredData[i].deliveredQty != null ? this.dataSource.filteredData[i].deliveredQty : 0);
  ...
  dropdownfill() {
    ...
    forkJoin({
      ...
      .subscribe(({ warehouse }) => {
        ...
        this.multiOrderTypeList.push({ value: 'N', label: 'Normal' }, { value: 'S', label: 'Special' });
        this.multiselectOrderTypeList = this.multiOrderTypeList;

        this.multiStatusList.push({ value: 'Delivered', label: 'Delivered' }, { value: 'Partial', label: 'Partial Deliveries' }, { value: 'Unfulfilled', label: 'Not fulfilled' });
  ...
  onWarehouseSelect(items: any) {
    ...
    this.deliveryService.searchBusinessPartner(obj).subscribe(
      result => {
        ...
        result.forEach(element => {
          if (this.multiCustomerCodeList.filter(x => x.value == element.partnerCode).length == 0)
            this.multiCustomerCodeList.push({ value: element.partnerCode, label: element.partnerCode })
        });
