...
export class InboundCreateComponent implements OnInit {
  ...
  confirm() {
    ...
    this.dataSource.data.forEach(data => {
      if (data.statusId != 12 && data.statusId != 13 && data.statusId != 14 && data.statusId != 16
        && data.statusId != 20 && data.statusId != 22) {  // PRODUCTION ORDER ISSUE ADDED 22 STATUS ID
        dataErrorArray.push(data.lineNo);
      }
    });
    if (dataErrorArray.length > 0) {
      this.toastr.error(
        "Line no: " + dataErrorArray + " not completely processed",
        "Notification", {
        timeOut: 2000,
        progressBar: false,
      }
