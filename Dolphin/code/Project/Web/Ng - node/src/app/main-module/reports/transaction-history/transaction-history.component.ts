...
import { TransactionHistorylinesComponent } from './transaction-historylines/transaction-historylines.component';
...
export class TransactionHistoryComponent implements OnInit {
  ...
  displayedColumns: string[] = ['warehouseId', 'itemCode','itemDescription',  'openingStock', 'inboundQty', 'outboundQty','stockAdjustmentQty','closingStock','systemInventory','variance','docLines'];
  ...
  onItemType(searchKey) {
    ...
    if (searchVal !== '' && searchVal !== null) {
      forkJoin({
        itemList: this.service.getItemCodeDropDown2(searchVal.trim(),this.auth.companyId,this.auth.plantId,this.searhform.controls.warehouseId.value,this.auth.languageId).pipe(catchError(err => of(err))),
      })
        .subscribe(({ itemList }) => {
  ...
  lines(res){
    console.log(res)
    const dialogRef = this.dialog.open(TransactionHistorylinesComponent, {
      //width: '55%',
      maxWidth: '83%',
      position: { top: '8.5%' },
      data: res 
