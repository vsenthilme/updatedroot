...
export class CommonService {
  ...
  public getstatus_text(id: any) {
    ...
    if (result != null && result != undefined && result != '[]') {
      result = result.filter(x => x.statusId === id && x.languageId ==  this.auth.languageId);
    }
  ...
  constructor(private toastr: ToastrService, private datepipe: DatePipe, private http: HttpClient, private auth: AuthService, ) { 
  ...
  public commonerrorNew(msg: any) {
    if(msg.status != 0){
      this.toastr.error(msg.error.message, msg.error.error, {
        timeOut: 2000,
        progressBar: false,
      });
    }else{
