...
export class StratergiesNewComponent implements OnInit {
  ...
  this.masterService.searchstratergytype({companyCodeId: this.auth.companyId, languageId: [this.auth.languageId],plantId:this.auth.plantId,warehouseId:this.auth.warehouseId}).subscribe(res => {
    this.strategyList = [];
    res.forEach(element => {
