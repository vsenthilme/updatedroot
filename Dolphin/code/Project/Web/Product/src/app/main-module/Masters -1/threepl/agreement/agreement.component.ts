...
export class AgreementComponent implements OnInit {
  ...
  @ViewChild('Setupagreement') Setupagreement: Table | undefined;
  agreement: any;
  ...
  ngOnInit(): void {
    ...
    applyFilterGlobal($event: any, stringVal: any) {
      this.Setupagreement!.filterGlobal(($event.target as HTMLInputElement).value, stringVal);
    }
    // ngOnInit(): void {
    //   //this.getAll(
  ...
  console.log(this.selectedagreement)
    ...
    const dialogRef = this.dialog.open(AgreementNewComponent, {
      ...
      width: '85%',
      maxWidth: '65%',
      data: { pageflow: data, code: data != 'New' ? this.selectedagreement[0].code : null,}
    });
...
//   }
  ...
  // }
  onChange() {
    console.log(this.selectedagreement.length)
    const choosen= this.selectedagreement[this.selectedagreement.length - 1];   
    this.selectedagreement.length = 0;
    this.selectedagreement.push(choosen);
  }  
  }
