...
export class ReversalOutboundPopupComponent implements OnInit {
  ...
  multipalletsinglelist: SelectItem[] = [];
  ...
  ngOnInit(): void {
    ...
    this.sub.add(this.service.searchsoLine({ warehouseId: [this.auth.warehouseId], statusId: [50, 57, 51] }).subscribe(res => {
      ...
      this.refDocNumberList.forEach(x => this.multiorderList.push({ item_id: x.refDocNumber, item_text: x.refDocNumber }))
  ...
  getpalletid() {
    ...
    this.sub.add(this.service.searchsoLine({ refDocNumber: [this.refDocNumber], statusId: [50, 57, 51] }).subscribe(res => {
      ...
      this.packBarcodesList.forEach(x => this.multipalletsinglelist.push({ item_id: x.itemCode, item_text: x.itemCode }))
  ...
  onItemSelect(item: any) {
    this.refDocNumber = this.selectedItems[0].item_id;
    this.getpalletid();
    console.log(item);
  }
