...
import { InhouseMainPopupComponent } from "./inhouse-main-popup/inhouse-main-popup.component";
...
export class InhouseMainComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select','status', 'transferNumber','inhouseTransferLine', 'createdOn', 'transferTypeId', 'createdBy',];
  ...
  search() {
    this.sub.add(this.service.search({warehouseId : [this.auth.warehouseId]}).subscribe((res: any) => {
  ...
  openDialog1(element): void {
    const dialogRef = this.dialog.open(InhouseMainPopupComponent, {
      disableClose: true,
      width: '90%',
      maxWidth: '100%',
     data: element,
