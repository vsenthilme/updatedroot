...
import { D3CollapsibleComponent } from "src/app/main-module/dashboard/dashboard/d3-collapsible/d3-collapsible.component";
...
export class InboundconfirmMainComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'statusId', 'refDocNumber', 'Details', 'inboundOrderTypeId', 'containerNo', 'doc', 'createdOn', 'confirmedOn', 'refdocno', 'confirmedBy',];
  ...
  search(ispageload = false) {
    ...
    this.service.search(this.searhform.value).subscribe(res => {
      ...
      if (ispageload) {
        ...
        this.statusIdList = this.statusList;
      }
  ...
  d3Tree(): void {
    const dialogRef = this.dialog.open(D3CollapsibleComponent, {
      width: '18rem',
