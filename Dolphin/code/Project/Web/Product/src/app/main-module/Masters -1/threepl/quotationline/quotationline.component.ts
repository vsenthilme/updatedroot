...
import { QuotationheaderService } from '../quotationheader/quotationheader.service';
import { QuotationlineNewComponent } from './quotationline-new/quotationline-new.component';

@Component({
  selector: 'app-quotationline',
  templateUrl: './quotationline.component.html',
  styleUrls: ['./quotationline.component.scss']
})
export class QuotationlineComponent implements OnInit {

  advanceFilterShow: boolean;
  @ViewChild('Setupquotation') Setupquotation: Table | undefined;
  quotationheader: any;
  ...
  constructor(public dialog: MatDialog,
   ...
   // private excel: ExcelService,
    ...
    private service:QuotationheaderService ) { }
  ngOnInit(): void {
    ...
    applyFilterGlobal($event: any, stringVal: any) {
      this.Setupquotation!.filterGlobal(($event.target as HTMLInputElement).value, stringVal);
    }
    // ngOnInit(): void {
    //   //this.getAll(
  ...
  console.log(this.selectedquotation)
    ...
    const dialogRef = this.dialog.open(QuotationlineNewComponent, {
      disableClose: true,
      width: '85%',
      maxWidth: '65%',
      data: { pageflow: data, code: data != 'New' ? this.selectedquotation[0].code : null,}
    });
...
//   }
  ...
  // }
  onChange() {
    console.log(this.selectedquotation.length)
    const choosen= this.selectedquotation[this.selectedquotation.length - 1];   
    this.selectedquotation.length = 0;
    this.selectedquotation.push(choosen);
  }  
  }
