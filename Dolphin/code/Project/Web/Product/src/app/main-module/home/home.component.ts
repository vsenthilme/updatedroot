...
import { CompanySetupService } from '../setup-organisation/company/company-setup.service';
import { VerticalService } from '../other-setup/vertical/vertical.service';
import { DooridService } from '../other-setup/doorid/doorid.service';
...
export class HomeComponent implements OnInit {
  ...
  constructor(private router: Router, public dialog: MatDialog, private ms: MenuService, private auth: AuthService, private service: PreoutboundService,
    private idmastser: DooridService,    private http: HttpClient,) { 
    this.currentEnv = environment.name; 
      ...
      // })
    this.cronJob = new CronJob('03 17 * * *', async () => {
      try {
        await this.bar();
      } catch (e) {
        console.error(e);
  ...
  logout() {
    this.http.patch<any>(`/wms-idmaster-service/usermanagement/${this.auth.userID}?warehouseId=${this.auth.warehouseId}&companyCode=${this.auth.companyId}&languageId=${this.auth.languageId}&plantId=${this.auth.plantId}&userRoleId=${this.auth.userRoleId}`, {isLoggedIn: false, companyCode: this.auth.companyId, plantId: this.auth.plantId, languageId: this.auth.languageId, userRoleId: this.auth.userRoleId}).subscribe(userres =>{
    if(userres){
      this.auth.logout();
      setTimeout(() => {
