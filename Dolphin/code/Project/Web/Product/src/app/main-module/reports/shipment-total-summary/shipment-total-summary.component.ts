...
this.dropdownfill();
  ...
  generatePdf(element: any) {
    ...
    element.shipmentDeliverySummary.forEach(x => this.branchList.push({ value: x.branchCode, label: x.branchCode }));
  ...
  let summaryShippedQty = 0;
    this.branchList.forEach(x =>{
      dd.content.push({
        stack: [
          { text: x.value, alignment: 'center', bold: true, fontSize: 12 }
        ]
      }, '\n\n')
      let bodyArray: any[] = [];
      bodyArray.push([
     ...
     //   totalOrders = totalOrders + (partner.shipmentDispatchList.length);
        ...
        totalLinesShipped = totalLinesShipped + (data.lineOrdered != null ? data.lineOrdered : 0);
        ...
        totalPercent = totalPercent + (data.percentageShipped != null ? data.percentageShipped : 0);
          ...
          //     { text: 8, fontSize: 9, alignment: 'left', bold: true, border: [false, false, false, false], lineHeight: 2 },
          //   ])
          // }
      })  
      dd.content.push(
    ...
    let bodyArray: any[] = [];
        bodyArray.push([
          ...
          { text: '# Lines Shipped', bold: true, fontSize: 8, border: [true, true, true, true] },
          { text: 'SKUs Picked', bold: true, fontSize: 8, border: [true, true, true, true] },
          ...
          percentageShipped = percentageShipped + (summary.metricsSummary.percentageShipped != null ? summary.metricsSummary.percentageShipped : 0);
            ...
            if((i+1) == element.summaryMetrics.length){
              bodyArray.push([
                ...
                { text:  Math.round((percentageShipped / element.summaryMetrics.length)),  bold: true,  fontSize: 8, border: [true, true, true, true], lineHeight: 2 },
              ])
            }
        });
        dd.content.push(
            ...
            columns: [
              { width: '*', text: '' },
              {
                ...
                table: {
                  ...
                  widths: [120, 50,  45, 45, 45, 45, 45, 45],
                  body: bodyArray,
                  alignment: "center"
                },
              },
