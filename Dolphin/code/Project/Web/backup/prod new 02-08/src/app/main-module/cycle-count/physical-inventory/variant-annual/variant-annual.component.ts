...
export class VariantAnnualComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'warehouseId', 'cycleCountNo', 'by', 'countedOn', 'statusId',];
  ...
  goTo(pageFlow: any = 'Edit',) {
    ...
    if (this.selection.selected.length == 0) {
      ...
      let paramdata = "";
      paramdata = this.cs.encrypt({type: pageFlow, periodicHeaderData: this.selection.selected[0]});
      this.router.navigate(['/main/cycle-count/variant-annual-edit/' + paramdata]);
    }
  }
}
