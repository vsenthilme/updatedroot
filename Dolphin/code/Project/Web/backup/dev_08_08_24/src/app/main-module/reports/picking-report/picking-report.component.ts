...
export class PickingReportComponent implements OnInit {
   ...
   // displayedColumns: string[] = [ 'statusId', 'refDocNumber','referenceField1', 'lineNumber', 'itemCode', 'description', 'pickedPackCode', 'proposedStorageBin','assignedPickerId','partnerCode', 'pickConfirmQty', 'pickupConfirmedOn', 'pickupCreatedOn', 'leadTime'];
    ...
    //wms demo
    displayedColumns: string[] = ['select', 'statusId', 'refDocNumber','referenceField1', 'lineNumber', 'itemCode', 'description', 'pickedPackCode', 'proposedStorageBin','assignedPickerId','partnerCode', 'pickConfirmQty', 'pickupConfirmedOn'];
    ...
    applyFilter(event: Event) {
      ...
      if(this.dataSource.filter.length == 0){
        this.dataSource.data.forEach(element => {
          this.totalonhandqty = this.totalonhandqty + (element.pickConfirmQty != null ? element.pickConfirmQty : 0);
          
        })
      }
    else{
      for (let i=0;i<this.dataSource.filteredData.length;i++){
        this.totalonhandqty = this.totalonhandqty + (this.dataSource.filteredData[i].pickConfirmQty != null ? this.dataSource.filteredData[i].pickConfirmQty : 0);
      }
    }
    }
    ngOnInit(): void {
