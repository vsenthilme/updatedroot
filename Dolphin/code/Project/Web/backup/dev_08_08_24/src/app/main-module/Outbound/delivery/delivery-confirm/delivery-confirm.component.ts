...
export class DeliveryConfirmComponent implements OnInit {
    ...
    if (data.pageflow != 'New') {
      ...
      this.sub.add(this.service.searchLinenew({ preOutboundNo: [this.code.preOutboundNo], refDocNumber: [this.code.refDocNumber], warehouseId: [this.code.warehouseId] }).subscribe(res => {
        ...
        res.forEach((x: any) => {
          ...
          if((parseInt(x.referenceField10) > 0) && (data.pageflow1 == 'readyForDelivery')){
            x.statusId = 57;
          }
  ...
  displayedColumns: string[] = [ 'statusId', 'lineNumber', 'itemCode', 'one', 'orderQty', 'referenceField9', 'referenceField10', 'deliveryQty', 'referenceField1', 'outboundOrderTypeId',];
  ...
  update(){
    this.spin.show();
    let statusError: any = [];
    let dataErrorArray1: any = [];
    this.dataSource.data.forEach(element => {
      if ((element.statusId != 47) && (element.statusId != 57) && (element.statusId != 51)) {
        statusError.push(element.lineNumber);
      }

      if((this.pageflow1 == 'readyForDelivery')){
        element.deliveryQty = element.referenceField10;
      }
      
      if ((parseInt(element.deliveryQty) > parseInt(element.orderQty)) || (parseInt(element.referenceField10) > parseInt(element.deliveryQty)) || (parseInt(element.deliveryQty) > parseInt(element.referenceField9)) ) {
        dataErrorArray1.push(element.lineNumber);
      }
    ...
    if (statusError.length > 0) {
      this.toastr.error(
        "SKU is not completely processed in line no : " + statusError,
        "Notification", {
        timeOut: 2000,
        progressBar: false,
      }
    ...
    if (dataErrorArray1.length > 0) {
      this.toastr.error(
        "Quantities mismatched in line no : " + dataErrorArray1,
        "Notification", {
        timeOut: 2000,
        progressBar: false,
      }
    ...
    this.sub.add(this.service.updateOutboundLineBatch(this.dataSource.data).subscribe(res => {
      // this.toastr.success(
  ...
  confirm(res) {
    ...
    res.forEach(data => {
      if ((parseInt(data.deliveryQty) > parseInt(data.orderQty)) || (parseInt(data.referenceField10) > parseInt(data.deliveryQty)) || (parseInt(data.deliveryQty) > parseInt(data.referenceField9)) ) {
  ...
  downloadexcel() {
    ...
    this.dataSource.data.forEach(x => {
      res.push({
        ...
        "Status ": x.statusId != null ? this.cs.getstatus_text(x.statusId) : '',
...
back(){
  ...
  generatePdf(element: any) {
    ...
    this.sub.add(this.service.getOtBoundDeliveryFormPdf(element.refDocNumber, element.warehouseId).subscribe((res: any) => {
      ...
      //Receipt List
      if (res.length > 0) {
        let date = this.datePipe.transform(element.requiredDeliveryDate, 'dd-MMM-yyyy', "GMT00:00");
        ...
        let totalQuantity = 0;
        res.sort((a, b) => ((a.commodity as number) > (b.commodity as number)) ? 1 : -1).forEach((delivery, i) => {
