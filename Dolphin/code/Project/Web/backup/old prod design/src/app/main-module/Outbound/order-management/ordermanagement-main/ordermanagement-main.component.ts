 ...
 reload(){
  ...
  allocateBulk() { 
    ...
    if (checkAnyUnallocated.length > 0) {
      ...
      this.spin.show();
      this.sub.add(this.service.allocateBatch(this.selection.selected).subscribe(res => {
        this.spin.hide();
  ...
  unallocateBulk() {
    ...
    if (checkAnyAllocated.length > 0) {
      ...
      this.spin.show();
      this.sub.add(this.service.unallocateBatch(this.selection.selected).subscribe(res => {
        this.spin.hide();
        if (res) {
          this.toastr.success("Unallocated successfully", "Notification", {
            timeOut: 2000,
