...
export class GetAllStockMovementComponent implements OnInit {
  ...
  showFiller = false;
  displayedColumns: string[] = ['warehouseId', 'manufacturerSKU', 'itemCode', 'itemText', 'documentType', 'documentNumber', 'openingStock', 'newmMovementQty', 'balanceOHQty', 'customerCode', 'confirmedOn', 'confirmedOn1'];
  ...
  searchBarcodeScan(){
    this.spin.show();
      this.reportService.getallStockMovement().subscribe(res => {
        ...
        obj.itemCode = [res[res.length - 1].itemCode];
        obj.warehouseId = [res[res.length - 1].warehouseId];
        obj.binClassId = [1];
        this.reportService.findInventory(obj).subscribe(inventoryResult => {
        ...
        if (res != null && res.length > 0) {
          ...
          res.forEach(element => {
            if (i >= 0) { 
              if (i == res.length) {
                res[i - 1]['balanceOHQty'] = inventoryQuantity;
              }
              else {
              res[i - 1]['balanceOHQty'] = (res[i]['openingStock']);
              }
              if (res[i - 1]['documentType'] == 'OutBound') {
                res[i - 1]['openingStock'] = res[i - 1]['balanceOHQty'] + res[i - 1]['movementQty'];
              }
              else {
                res[i - 1]['openingStock'] = res[i - 1]['balanceOHQty'] - res[i - 1]['movementQty'];
              }
