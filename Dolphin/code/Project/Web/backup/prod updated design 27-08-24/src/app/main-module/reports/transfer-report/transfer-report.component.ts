...
export class TransferReportComponent implements OnInit {
    ...
    applyFilter(event: Event) {
      ...
      if(this.dataSource.filter.length == 0){
        this.dataSource.data.forEach(element => {
          this.totalonhandqty = this.totalonhandqty + (element.transferConfirmedQty != null ? element.transferConfirmedQty : 0);
       
        })
      }
    else{
      for (let i=0;i<this.dataSource.filteredData.length;i++){
        this.totalonhandqty = this.totalonhandqty + (this.dataSource.filteredData[i].transferConfirmedQty != null ? this.dataSource.filteredData[i].transferConfirmedQty : 0);
    ...
    reset(){
      ...
      this.startConfirmedOn = null;
      this.endConfirmedOn = null;
    }
