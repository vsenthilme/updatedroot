...
export class ItemMainComponent implements OnInit {
  ...
  @ViewChild('itemReceiptTag') itemReceiptTag: Table | undefined;
  screenid: 1049 | undefined;

  displayedColumns: string[] = ['select', 'referenceField10', 'refDocNumber', 'preInboundNo', 'caseCode', 'Received', 'goodsReceiptDate',];
  ...
  callTableHeader() {
    this.cols = [
      ...
      { field: 'caseCode', header: 'Case No' },
      { field: 'createdBy', header: 'Case Received By' },
      { field: 'createdOn', header: 'Case Received On', format: 'date' },
    ];
  }
  ...
  downloadexcel() {
    ...
    this.cs.exportAsExcel(exportData, 'Goods Receipt');
  }
