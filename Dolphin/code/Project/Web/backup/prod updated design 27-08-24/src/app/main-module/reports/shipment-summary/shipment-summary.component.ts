...
this.dropdownfill();
  ...
  generatePdf(element: any) {
    ...
    let fromDate = this.datePipe.transform(this.form.controls.fromDeliveryDate.value, 'MM-dd-yyyy : hh:mm');
    let toDate = this.datePipe.transform(this.form.controls.toDeliveryDate.value, 'MM-dd-yyyy : hh:mm');
    var dd: any;
    dd = {
      pageSize: "A4",
      ...
      dd.content.push(
        {
          table: {
            ...
            headerRows: 1,
            widths: [50, 60, 35, 35, 60, 30, 30, 30, 30, 30, 30],
            body: bodyArray
          },
        }, '\n'
      )
      ...
      if (element.summaryMetrics.length > 0) {
        ...
        bottomTable.forEach((table, index) => {
          ...
          bodyArray.push([
            ...
            { text: index == 0 ? 'N-Normal' : 'S-Special', colSpan: 4, bold: true, alignment: 'center', fontSize: 8, border: [true, true, true, true] },
            {},
            {},
            {},
          ], [
