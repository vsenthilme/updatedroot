...
export class PhysicalInventoryService {
  ...
  findPeriodicDataListNew(data: any) {
    return this.apiService.post(this.configService.findPeriodicHeaderListNew, data);      //20-06-23 streaming
  }
  ...
  updatePeriodicHeader(data: any) {
    return this.apiService.patchWithBody(this.configService.periodicHeaderUpdate + `/${data.cycleCountNo}?cycleCountTypeId=${data.cycleCountTypeId}&warehouseId=${data.warehouseId}` + '&companyCodeId='+ this.auth.companyId +'&plantId='+ this.auth.plantId +'&languageId='+ this.auth.languageId, data);
  }
  ...
  findPeriodicDLine(data: any) {
    return this.apiService.post('/wms-transaction-service/periodicline/findPeriodicLine', data);
  }
