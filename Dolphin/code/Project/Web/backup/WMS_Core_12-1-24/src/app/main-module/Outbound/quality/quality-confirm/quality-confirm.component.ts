  ...
  fill(data: any) {
    if (data.pageflow != 'New') {
      ...
      if (data.pageflow == 'Display') {
        ...
        obj.qualityinspectionNo = [data.code.qualityInspectionNo];

        this.sub.add(this.service.getqualityline(obj).subscribe(res => {
          let data: any[] = [];
          // res.forEach((x: any) => {
...
// start of  CWMS/IW/2022/018 mugilan 02-10-2022
        ...
        this.sub.add(this.service.search(obj).subscribe(res => {
          ...
          obj.plantId = [this.auth.plantId];
          obj.warehouseId = [this.auth.warehouseId];
          obj.pickupNumber = [res[0].pickupNumber], 

          this.sub.add(this.service.getpickinglineSpark(obj).subscribe(result => {
          console.log(res)
          console.log(result)
// res.forEach(element => {
