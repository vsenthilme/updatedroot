...
export class PreinboundPopupeditComponent implements OnInit {
  ...
  preInboundLines: FormArray = this.fb.array([this.preInboundLine]);
  ...
  form = this.fb.group({
    ...
    warehouseDescription: [],
    warehouseId: [],
    preInboundLine: this.preInboundLines,
  });
  ...
  submit() {
    ...
    if (this.data.code) {
      this.sub.add(this.service.Update(this.form.getRawValue(), this.data.code, this.data.warehouseId, this.data.companyCode, this.data.plantId, this.data.languageId).subscribe(res => {
