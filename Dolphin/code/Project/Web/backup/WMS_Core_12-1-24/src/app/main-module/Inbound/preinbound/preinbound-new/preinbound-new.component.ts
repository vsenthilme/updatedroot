...
export class PreinboundNewComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'statusId', 'invoiceNo', 'lineno', 'itemCode', 'itemDescription', 'manufacturerCode', 'businessPartnerCode', 'orderQty', 'orderUom', 'stockTypeId', 'expectedArrivalDate',];
  isShowDiv = false;
...
js: any;
  ...
  fill(data: any) {
    ...
    if (data.pageflow != 'New') {
      ...
      this.spin.show();
      this.sub.add(this.service.Get(data.code.preInboundNo, data.code.warehouseId, data.code.companyCode, data.code.plantId, data.code.languageId).subscribe(res => {
      ...
      this.sub.add(this.sservice.Getall().subscribe(res => {
        ...
        this.packBarcodesList.forEach(x => this.multiitemlistList.push({value: x.containerNo, label: x.containerNo}))
  ...
  submit() {
    ...
    this.sub.add(this.service.processASN(this.preInboundNew).subscribe(res => {
      ...
      this.sub.add(this.sservice.search({ containerNo: [this.form.controls.containerNo.value] }).subscribe(res => {
        ...
        this.spin.hide();
        this.spin.show();
        this.sub.add(this.sservice.Update({ refDocNumber: this.form.controls.refDocNumber.value, }, res[0].containerReceiptN,res[0].refDocNumber,res[0].preInboundNo,res[0].companyCodeId, res[0].plantId, res[0].languageId, res[0].warehouseId,).subscribe(res => {
          //   this.toastr.success(this.form.controls.preInboundNo.value + " Processed successfully!");
          this.spin.hide();
          this.router.navigate(["/main/inbound/preinbound"])
        }, err => {
    ...
    if (this.code) {
      this.sub.add(this.service.Update(this.form.getRawValue(), this.code.preInboundNo, this.code.warehouseId, this.code.companyCode, this.code.plantId, this.code.languageId).subscribe(res => {
  ...
  bom() {
    ...
    this.spin.show();
    this.sub.add(this.service.createbom(data.itemCode, data.lineNo, data.preInboundNo, data.refDocNumber, data.warehouseId, data.companyCode, data.plantId, data.languageId).subscribe(res => {
