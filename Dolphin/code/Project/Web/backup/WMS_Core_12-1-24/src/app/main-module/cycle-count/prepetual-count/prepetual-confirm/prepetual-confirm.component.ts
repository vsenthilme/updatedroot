...
export class PrepetualConfirmComponent implements OnInit {
  ...
  assign(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.selection.selected.forEach((x: any) => { x.cycleCounterId = result; x.companyCode = x.companyCodeId });
        this.toastr.success("User Assign successfully", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
  ...
  ngOnInit(): void {
    ...
    if (js.pageflow == 'New') {
      ...
      this.displayedColumns = ['no', 'warehouseId', 'itemCode', 'itemDesc', 'manufacturerPartNo', 'storageSectionId', 'storageBin', 'packBarcodes', 'stockTypeId', 'inventoryQuantity'];
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
    }
    else {
      ...
      this.prepetualCountService.get(this.code).subscribe(result => {
        ...
        if (js.pageflow == "Variance Analysis") {
          ...
          this.displayedColumns = ['action2s', 'no', 'itemCode', 'itemDesc', 'manufacturerPartNo', 'storageSectionId', 'storageBin', 'packBarcodes', 'stockTypeId', 'inventoryQuantity', 'countedQty', 'varianceQty', 'cycleCounterId', 'statusId'];
        }
        if (js.pageflow == "Count") {
          ...
          this.displayedColumns = ['select', 'no', 'warehouseId', 'itemCode', 'itemDesc', 'manufacturerPartNo', 'storageSectionId', 'storageBin', 'packBarcodes', 'stockTypeId', 'inventoryQuantity', 'countedQty', 'varianceQty', 'cycleCounterId', 'statusId'];
        }
        if (js.pageflow == "Edit") {
          console.log("Edit")
          this.dataSource = new MatTableDataSource(this.code.perpetualLine);
          this.displayedColumns = ['no', 'warehouseId', 'itemCode', 'itemDesc', 'manufacturerPartNo', 'storageSectionId', 'storageBin', 'packBarcodes', 'stockTypeId', 'inventoryQuantity', 'cycleCounterId', 'countedQty', 'statusId'];
        }
        if (js.pageflow == "Assign User") {
          ...
          this.pageTitle = this.pageflow;
          this.displayedColumns = ['select', 'no', 'warehouseId', 'itemCode', 'itemDesc', 'manufacturerPartNo', 'storageSectionId', 'storageBin', 'packBarcodes', 'stockTypeId', 'inventoryQuantity', 'cycleCounterId', 'statusId'];
        }
        this.dataSource.paginator = this.paginator;
        this.dataSource.sort = this.sort;
      });
  ...
  displayedColumns: string[] = ['select', 'no', 'warehouseId', 'itemCode', 'itemDesc', 'manufacturerPartNo', 'storageSectionId', 'storageBin', 'packBarcodes', 'stockTypeId', 'inventoryQuantity', 'cycleCounterId', 'countedQty', 'statusId'];
