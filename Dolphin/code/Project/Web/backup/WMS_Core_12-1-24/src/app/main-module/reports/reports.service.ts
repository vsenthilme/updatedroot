...
export class ReportsService {
  ...
  constructor(private apiService: ApiService, private configService: ConfigService, private http: HttpClient, private auth: AuthService) { }
  ...
  getShipmentDeliverySummarynew(data: any) {
    return this.apiService.get(`/wms-transaction-service/reports/shipmentDeliverySummary1?fromDeliveryDate=${data.fromDeliveryDate}&toDeliveryDate=${data.toDeliveryDate}&warehouseId=${data.warehouseId}` + '&companyCode='+ this.auth.companyId +'&plantId='+ this.auth.plantId +'&languageId='+ this.auth.languageId);
  }
...
findInventory2(obj:any){
  ...
  GetallDataStockReport(itemCode: string[], itemText: string, stockTypeText: string, warehouseId: string[]) {
    return this.apiService.get(this.configService.newstockReportAllData_url + '?itemCode=' + itemCode + '&itemText=' + itemText + '&stockTypeText=' + stockTypeText + '&warehouseId=' + warehouseId + '&companyCodeId='+ this.auth.companyId +'&plantId='+ this.auth.plantId +'&languageId='+ this.auth.languageId);
  }
  ...
  getBinnerReportSpark(obj: any) {
    return this.http.post<any>('/wms-spark-service/putawayline', obj);
  }
  ...
  getpickupLine(obj: any) {
    return this.http.post<any>('/wms-transaction-service/pickupline/findPickupLine' , obj);
  }
}
