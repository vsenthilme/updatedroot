...
export class Pallet1Component implements OnInit {
  ...
  generate() {
    ...
    this.spin.show();
    this.sub.add(this.service.GetBarcode(this.noOfCase, this.data[0].warehouseId, this.data[0].companyCode, this.data[0].languageId, this.data[0].plantId).subscribe(res => {
      this.spin.hide();
      res.forEach((y: any) => {
