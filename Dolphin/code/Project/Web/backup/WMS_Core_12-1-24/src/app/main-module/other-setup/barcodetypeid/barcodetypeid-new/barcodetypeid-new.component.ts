...
@Component({
  selector: 'app-barcodetypeid-new',
  templateUrl: './barcodetypeid-new.component.html',
  styleUrls: ['./barcodetypeid-new.component.scss']
})
export class BarcodetypeidNewComponent implements OnInit {
 ...
 this.masterService.searchPlant({companyCodeId: [this.form.controls.companyCodeId.value], languageId: [this.form.controls.languageId.value]}).subscribe(res => {
  ...
  if (this.data.code) {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.barcodeTypeId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
