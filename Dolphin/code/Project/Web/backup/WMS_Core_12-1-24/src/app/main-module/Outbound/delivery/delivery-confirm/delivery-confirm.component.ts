...
export class DeliveryConfirmComponent implements OnInit {
  ...
  fill(data: any) {
    ...
    if (data.pageflow != 'New') {
      ...
      obj.preOutboundNo = [this.code.preOutboundNo];
        obj.refDocNumber = [this.code.refDocNumber];

      this.sub.add(this.service.searchLinenew(obj).subscribe(res => {
  ...
  update(){
    ...
    this.deliveryconfirm.forEach(element => {
    ...
    this.sub.add(this.service.updateOutboundLineBatch(this.deliveryconfirm).subscribe(res => {
      // this.toastr.success(
