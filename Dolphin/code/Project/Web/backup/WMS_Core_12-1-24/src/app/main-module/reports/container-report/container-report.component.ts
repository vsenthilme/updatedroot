...
export class ContainerReportComponent implements OnInit {
  ...
  containerget() {
    ...
    this.sub.add(this.containerservice.searchSpark(obj).subscribe(res => {
  ...
  downloadexcel() {
    ...
    this.container.forEach(x => {
      res.push({
        ...
        " Container Unloaded On":this.cs.dateTimeApi(x.referenceField2),
        'Actual Receipt Date':this.cs.dateTimeApi(x.referenceField5),
        // 'Created By': x.createdBy,
