...
StatusList: any[] = [];
  ...
  multiOrderNo: any[] = [];
  getDropdown(){
    this.sub.add(this.reportService.getpickupLine({warehouseId : [this.auth.warehouseId],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId]}).subscribe(res => {
  ...
  filtersearch(){
    ...
    if(this.searhform.controls.fromPickConfirmedOn.value == null ||this.searhform.controls.fromPickConfirmedOn.value == undefined || !this.searhform.controls.fromPickConfirmedOn){
    ...
    this.sub.add(this.pickingService.getpickinglineSpark(this.searhform.value).subscribe(res => {
