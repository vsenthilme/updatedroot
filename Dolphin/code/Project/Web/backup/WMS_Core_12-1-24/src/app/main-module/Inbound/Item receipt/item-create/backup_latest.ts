import {
  ...
  export class ItemCreateComponent implements OnInit {
    ...
    delete() {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          this.spin.show();
          this.sub.add(this.grservice.DeleteLinewithoutCase(this.selectedgrNew[0]).subscribe(res => {
            this.spin.hide();
            this.ngOnInit();
          }, err => {
    ...
    lablelGenerate(element) {
      console.log(this.selectedgrNew)
      if (this.selectedgrNew.length === 0) {
        this.toastr.error("Kindly select one row", "", {
          timeOut: 2000,
          progressBar: false,
  ...
  console.log(this.selectedgrNew1)
    ...
    onChange() {
      ...
      this.lablelGenerate(this.selectedgrNew[0]);
    }
  ...
  // }
    ...
    //   if (element.packBarcodes.length == 0) {
    //     this.spin.show();
    //     this.sub.add(this.service.packBarcodeV2(element.acceptedQty, element.damageQty, element.warehouseId, element.companyCode, element.plantId, element.languageId).subscribe(res => {
