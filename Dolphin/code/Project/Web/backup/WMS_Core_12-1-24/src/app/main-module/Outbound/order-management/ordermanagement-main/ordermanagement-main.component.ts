...
statusId=48;
  ...
  this.spin.show();
   this.service.searchLineSpark(this.searhform.value).subscribe(res => {
     ...
     let deletionResult = res.filter((x: any) => x.deletionIndicator == 0);
     if (ispageload) {
 ...
 reload(){
   ...
   this.searhform.controls.companyCode.patchValue([this.auth.companyId]);
  ...
  Allocate(data: any) {
    this.spin.show();
    this.sub.add(this.service.Allocate(data.itemCode, data.lineNumber, data.partnerCode, data.preOutboundNo, data.refDocNumber, data.warehouseId,this.auth.companyId,this.auth.plantId,this.auth.languageId).subscribe(res => {
      console.log(res)
      this.spin.hide();
