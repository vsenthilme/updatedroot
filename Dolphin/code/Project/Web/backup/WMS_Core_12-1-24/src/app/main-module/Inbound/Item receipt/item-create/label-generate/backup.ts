...
export class LabelGenerateComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.data.packBarcodes.length == 0) {
      this.spin.show();
      this.sub.add(this.service.packBarcode(this.data.acceptedQty, this.data.damageQty, this.data.warehouseId, this.data.companyCode, this.data.plantId, this.data.languageId).subscribe(res => {
