...
export class PalletComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.data.statusId == 13) {
      ...
      obj.companyCode = [this.data.companyCode];
      obj.plantId = [this.data.plantId];
      obj.warehouseId = [this.data.warehouseId];
      obj.preInboundNo = [this.data.preInboundNo];
  ...
  Delete() {
    ...
    this.spin.show();
    this.sub.add(this.service.deleteLine(this.data.lineNo, this.selectedAssignedCase[0].caseCode, this.data.itemCode, this.data.preInboundNo, this.selectedAssignedCase[0].stagingNo, this.selectedAssignedCase[0].warehouseId, this.selectedAssignedCase[0].companyCode, this.selectedAssignedCase[0].plantId, this.selectedAssignedCase[0].warehouseId).subscribe(res => {
  ...
  Confirm() {
    ...
    this.sub.add(this.service.caseConfirmation(dataaa, dataaa[0].caseCode,dataaa[0].languageId,dataaa[0].companyCode,dataaa[0].plantId).subscribe(res => {
