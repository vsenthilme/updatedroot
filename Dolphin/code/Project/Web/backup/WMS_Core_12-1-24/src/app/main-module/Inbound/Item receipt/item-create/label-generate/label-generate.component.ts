...
export class LabelGenerateComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.data.line.packBarcodes.length > 0) { 
      ...
      this.spin.show();
      this.sub.add(this.service.packBarcode(this.data.line.acceptedQty, this.data.line.damageQty, this.data.line.warehouseId, this.data.line.companyCode, this.data.line.plantId, this.data.line.languageId).subscribe(res => {
...
findDimensions(){
  ...
  obj.itemCode = [this.data.line.itemCode];
  obj.manufacturerPartNo = [this.data.line.manufacturerPartNo];
