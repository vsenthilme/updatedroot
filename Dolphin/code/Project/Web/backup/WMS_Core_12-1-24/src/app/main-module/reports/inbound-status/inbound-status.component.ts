...
export class InboundStatusComponent implements OnInit {
  ...
  form = this.fb.group({
    containerNo: [],
    endConfirmedOn: [],
    endCreatedOn: [[, Validators.required]],
    inboundOrderTypeId: [],
    refDocNumber: [],
    startConfirmedOn: [],
  ...
  multiOrderNo: any[] = [];
    getDropdown(){
      this.sub.add(this.reportService.getInboundStatusReport({warehouseId : [this.auth.warehouseId],companyCode:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId]}).subscribe(res => {
  ...
  filtersearch() {
    ...
    if (this.form.controls.startCreatedOn.value == "" || this.form.controls.endCreatedOn.value == "") {
      this.toastr.error(
        "Please fill the required fields to continue",
        "Notification", {
        timeOut: 2000,
    ...
    this.sub.add(this.service.getInboundStatusReport(obj).subscribe(res => {
      this.inbound = res;
      this.spin.hide()
