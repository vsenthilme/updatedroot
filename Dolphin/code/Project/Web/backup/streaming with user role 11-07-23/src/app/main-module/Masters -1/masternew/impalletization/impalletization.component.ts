...
@Component({
  selector: 'app-impalletization',
  templateUrl: './impalletization.component.html',
  styleUrls: ['./impalletization.component.scss']
})
export class ImpalletizationComponent implements OnInit {
  screenid: 3027 | undefined;
  ...
  openDialog(data: any = 'New'): void {
    ...
    let paramdata = "";
    paramdata = this.cs.encrypt({ pageflow: data, code: data != 'New' ? this.selecteditemtype[0].palletizationLevel : null,warehouseId: data != 'New' ? this.selecteditemtype[0].warehouseId : null,languageId: data != 'New' ? this.selecteditemtype[0].languageId : null, plantId: data != 'New' ? this.selecteditemtype[0].plantId : null,companyCodeId: data != 'New' ? this.selecteditemtype[0].companyCodeId : null,itemCode: data != 'New' ? this.selecteditemtype[0].itemCode : null});
 ...
 this.itemtype = res;
  ...
  deleteDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.deleterecord(this.selecteditemtype[0].palletizationLevel,this.selecteditemtype[0].warehouseId,this.selecteditemtype[0].languageId,this.selecteditemtype[0].plantId,this.selecteditemtype[0].companyCodeId,this.selecteditemtype[0].itemCode);
