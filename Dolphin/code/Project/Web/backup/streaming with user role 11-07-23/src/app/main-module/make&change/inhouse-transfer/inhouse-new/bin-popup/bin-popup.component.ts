...
export class BinPopupComponent implements OnInit {
  ...
  load(sourceStorageBin: any) {
    ...
    this.sub.add(this.service.GetInventory({ warehouseId: [this.auth.warehouseId], storageBin: [sourceStorageBin], itemCode: this.sourceItemCode != null ? [this.sourceItemCode] : [] }).subscribe(res => {
      this.spin.hide();
...
itemCodeList1: any[] = [];
  ...
  save() {
    ...
    if(!this.sourceStorageBin || this.sourceStorageBin == null || this.sourceStorageBin == undefined){
      this.toastr.error("Please select source item code to continue", "Notification", {
        timeOut: 2000,
        progressBar: false,
      });
    ...
    this.sub.add(this.service.GetAllBinNew({ warehouseId: [this.auth.warehouseId], storageBin: [this.targetStorageBin] }).subscribe(res => {
      if(res[0]){
       this.spin.show();
  ...
  displayedColumns: string[] = ['select',  '1no', '1no2', '1lineno', '1supcode',];
  displayedColumns2: string[] = ['no', 'no2', 'lineno', 'supcode',];
  ...
  binSelected(e){
    ...
    this.sub.add(this.service.GetInventory({storageBin: [e.value]}).subscribe(res => {
  ...
  selectedRow(e){
    ...
    this.destinationDataSource.data = this.selection.selected;
    console.log(this.destinationDataSource)
  }

  checkQty(e, element, i){
    if(e > element.inventoryQuantity){
      this.toastr.error(
        "Target Qty cannot be greater than source qty",
        "Notification",{
          timeOut: 2000,
          progressBar: false,
        }
