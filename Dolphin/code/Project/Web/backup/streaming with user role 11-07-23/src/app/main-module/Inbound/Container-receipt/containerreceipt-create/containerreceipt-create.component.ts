...
export class ContainerreceiptCreateComponent implements OnInit {
  ...
  fill(data: any) {
    ...
    if (data.pageflow != 'New') {
      ...
      this.spin.show();
      this.sub.add(this.service.Get(data.code.containerReceiptNo).subscribe(res => {
  ...
  submit() {
    ...
    if (this.code) {
      this.sub.add(this.service.Update(this.form.getRawValue(), this.code.containerReceiptNo).subscribe(res => {
