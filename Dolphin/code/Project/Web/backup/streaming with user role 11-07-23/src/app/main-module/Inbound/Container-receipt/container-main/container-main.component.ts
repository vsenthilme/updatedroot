...
export class ContainerMainComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'referenceField9',  'containerNo', 'containerReceiptNo', 'refDocNumber', 'partnerCode', 'createdBy', 'referenceField2','referenceField5'];
  ...
  search(ispageload = false) {
    ...
    this.service.searchStreaming(this.searhform.value).subscribe(res => {
      ...
      if (ispageload) {
        ...
        // this.statusIdList = tempstatusIdList;
        res.forEach(x => this.statusIdList.push({ value: x.statusId, label: x.referenceField9}));
