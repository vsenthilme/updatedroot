...
export class ReversalPopupComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.spin.show();
    this.sub.add(this.service.searchStreaming({}).subscribe(res => {
      this.spin.hide();
  ...
  getpalletid() {
    this.spin.show();
    this.sub.add(this.service.searchStreaming({ refDocNumber: [this.refDocNumber[0].id] }).subscribe(res => {
      this.spin.hide();
