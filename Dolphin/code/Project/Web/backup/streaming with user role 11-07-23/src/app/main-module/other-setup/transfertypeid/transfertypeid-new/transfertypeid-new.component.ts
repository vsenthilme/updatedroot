...
@Component({
  selector: 'app-transfertypeid-new',
  templateUrl: './transfertypeid-new.component.html',
  styleUrls: ['./transfertypeid-new.component.scss']
})
export class TransfertypeidNewComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    transferTypeText: [,Validators.required ],
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      this.form.controls.transferTypeId.disable();
  ...
  if (this.data.code) {
    ...
    this.sub.add(this.service.Create(this.form.getRawValue()).subscribe(res => {
      this.toastr.success(res.transferTypeId + " Saved Successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
