...
import  { stockElement, StocksampleService }  from "../stocksamplereport/stocksample.service";
  ...
  screenid=3165;
    ...
    displayedColumns: string[] = ['select', 'warehouseId', 'referenceField9',  'containerNo', 'containerReceiptNo', 'refDocNumber','containerType','origin','partnerCode', 'createdBy', 'referenceField2','referenceField5'];
   ...
   res.forEach(element => {
    ...
    filtersearch(excel = false) {
      ...
      this.containerservice.searchStreaming(this.searhform.value).subscribe(res => {
        console.log(res)
        this.spin.hide();
