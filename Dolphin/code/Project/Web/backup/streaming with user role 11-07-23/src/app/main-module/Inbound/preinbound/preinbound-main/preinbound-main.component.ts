...
export class PreinboundMainComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'referenceField10', 'inboundOrderTypeId', 'refDocNumber', 'containerNo', 'preInboundNo', 'createdBy', 'refDocDate',];
  ...
  filtersearch() {
    this.spin.show();
    this.sub.add(this.service.searchstreaming(this.searhform.getRawValue()).subscribe((res: any[]) => {
      this.spin.hide();
  ...
  search() {
    ...
    this.spin.show();
    this.sub.add(this.service.searchstreaming(obj).subscribe(res => {
  ...
  searchfilter(ispageload = false) {
    ...
    this.service.searchstreaming(this.searhform.value).subscribe(res => {
      // let result = res.filter((x: any) => x.warehouseId == this.warehouseId);
      this.spin.hide();
      if (ispageload) {
