...
export class VarianteditAnnualComponent implements OnInit {
  ...
  displayedColumns: string[] = ['actions', 'no', 'itemCode', 'referenceField8', 'manufacturerPartNo', 'storageSectionId', 'storageBin', 'pallet', 'packBarcodes', 'stockTypeId', 'spl', 'inventoryQty', 'countedQty', 'variance', 'by', 'countedOn', 'status',];
  ...
  ngOnInit(): void {
    ...
    if (this.data && this.data.periodicLine) {
      ...
      this.displayedColumns = ['actions', 'no', 'itemCode', 'referenceField8', 'manufacturerPartNo', 'storageSectionId', 'storageBin', 'packBarcodes', 'stockTypeId', 'inventoryQty', 'countedQty', 'variance', 'cycleCounterId', 'status'];
