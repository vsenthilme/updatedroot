...
export class PhysicalMainComponent implements OnInit {
  ...
  toggle = true;
  displayedColumns: string[] = ['select', 'actions', 'warehouseId', 'cycleCountNo', 'by', 'countedOn', 'statusId',];
  ...
  goTo(type: any, element: any = null) {
    ...
    if(type == "count"){
      this.spin.show();
      this.periodicService.findPeriodicDLineNew({cycleCountNo: element.cycleCountNo, lineStatusId: [72]}).subscribe(res => {
        ...
        if (this.selection.selected.length == 0) {
          ...
          let paramdata = "";
          paramdata = this.cs.encrypt({type: type, periodicHeaderData: this.selection.selected[0] });
          this.router.navigate(['/main/cycle-count/periodic-count-confirm/' + paramdata]);
        }
        this.spin.hide();
      })
    }
    if(type == "Assign"){
      this.spin.show();
    this.periodicService.findPeriodicDLineNew({cycleCountNo: element.cycleCountNo, lineStatusId: [70, 72, 73]}).subscribe(res => {
    ...
    if(type == "Display"){
      this.spin.show();
      this.periodicService.findPeriodicDLineNew({cycleCountNo: this.selection.selected[0].cycleCountNo}).subscribe(res => {
        if (this.selection.selected.length == 0) {
          this.toastr.error(
            "Please choose line to edit",
            "Notification"
