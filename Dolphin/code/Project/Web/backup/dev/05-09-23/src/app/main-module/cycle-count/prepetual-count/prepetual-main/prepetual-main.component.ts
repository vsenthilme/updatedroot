...
export class PrepetualMainComponent implements OnInit {
  ...
  openDialog(data: any = 'New', linedata: any = null): void {
    ...
    if(data == "Assign User"){
      this.spin.show();
      this.prepetualCountService.findPerpetualLine({cycleCountNo: [linedata.cycleCountNo], lineStatusId: [70, 72, 73]}).subscribe(res => {
        if(res.length == 0){
    ...
    if(data == "Count"){
      this.spin.show();
      this.prepetualCountService.findPerpetualLine({cycleCountNo: [linedata.cycleCountNo], lineStatusId: [72]}).subscribe(res => {
        if(res.length == 0){
          this.toastr.error("Please assign counter first", "Notification");
          this.spin.hide();
          return;
      } else{
        let paramdata = this.cs.encrypt({ code: linedata == null ? this.selection.selected[0] : linedata, pageflow: this.pageflow = data });
    ...
    if(data == "Display"){
      this.spin.show();
      this.prepetualCountService.findPerpetualLine({cycleCountNo: [this.selection.selected[0].cycleCountNo]}).subscribe(res => {
        if(res.length == 0){
          this.toastr.error(  "No lines found", "Notification");
          this.spin.hide();
          return;
