...
import {
  InhouseTransferService
} from '../../make&change/inhouse-transfer/inhouse-transfer.service';
import {
  MasterService
...
export class UpdateInventoryComponent implements OnInit {
  ...
  findItemCode() {
    ...
    this.sub.add(this.inventory.GetInventory({
      ...
      res.forEach((element: {
        ...
        if (res[0].referenceField9) {
          this.mfrPartNo = res[0].referenceField9;
        }
        //   if(res[0].referenceField10){this.storageSection = String(res[0].referenceField10);}
        if (res[0].referenceField8) {
          this.description = res[0].referenceField8;
        }
        ...
        this.multiPackBarcodes.push({
          value: element.packBarcodes,
          label: element.packBarcodes
        });
        ...
        this.multistorageBin.push({
          value: element.storageBin,
          label: element.storageBin
        });
        this.multistorageBin = this.cs.removeDuplicatesFromArrayNewstatus(this.multistorageBin);

        this.multiSelectStorageSectionID.push({
  ...
  dropdownfill() {
    ...
    forkJoin({
      ...
      .subscribe(({
        ...
        this.multiSelectStorageSectionID1.forEach(x => this.multiSelectStorageSectionID.push({
  ...
  binLocationChange(e) {
    ...
    this.spin.show();
    this.sub.add(this.inventory.GetInventory(obj).subscribe(res => {
      this.multiPackBarcodes = [];
      this.multispecialStockIndicatorId = [];
      this.multistockTypeId = [];
      this.multiSelectStorageSectionID = [];
      ...
      res.forEach((element: {
        ...
        this.multiSelectStorageSectionID = this.cs.removeDuplicatesFromArrayNewstatus(this.multiSelectStorageSectionID);
        if (this.multiSelectStorageSectionID.length == 1) {
