...
import { PreoutboundLinesComponent } from "./preoutbound-lines/preoutbound-lines.component";
...
export class PreoutboundComponent implements OnInit {
 ...
 // displayedColumns: string[] = [ 'statusId','itemCode','proposedStorageBin', 'confirmedStorageBin', 'packBarcodes', 'putAwayQuantity', 'putawayConfirmedQty', 'confirmedBy','confirmedOn','createdOn', 'leadTime'];
      //wms demo
      displayedColumns: string[] = ['select', 'referenceField10', 'lines','partnerCode', 'outboundOrderTypeId', 'referenceField1', 'preOutboundNo', 'refDocNumber', 'refDocDate', 'requiredDeliveryDate',];
  ...
  multipreOutboundNo: any[] = [];
    getDropdown(){
      this.sub.add(this.service.searchSpark({warehouseId : [this.auth.warehouseId]}).subscribe(res => {
...
this.form.controls.startOrderDate.patchValue(this.cs.day_callapiSearch(this.form.controls.startOrderDate.value));
    ...
    this.sub.add(this.service.searchSpark(this.form.getRawValue()).subscribe(res => {
      console.log(res)
  ...
  openDialog(element): void {
    const dialogRef = this.dialog.open(PreoutboundLinesComponent, {
      disableClose: true,
      width: '80%',
      maxWidth: '80%',
     data: element,
