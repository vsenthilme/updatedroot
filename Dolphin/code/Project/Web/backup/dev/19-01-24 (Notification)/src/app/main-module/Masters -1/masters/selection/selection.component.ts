...
import { SelectionElement, MasterproductselectionService } from "./masterproductselection.service";
...
export class SelectionComponent implements OnInit {
  ...
  getAll() {
    this.spin.show();
    //this.sub.add(this.service.searchSpark(this.searhform.getRawValue()).subscribe((res) => {
  ...
  getAllSpark(){
    this.spin.show()
    this.service.searchSpark(this.searhform.getRawValue()).subscribe(res => {
...
this.spin.hide();
  ...
  search1(event) {
    forkJoin({
      itemList: this.ReportsService.getItemCodeDropDown2(event.query,this.auth.companyId,this.auth.plantId,this.auth.warehouseId,this.auth.languageId).pipe(catchError(err => of(err))),
    })
    .subscribe(({ itemList }) => {
      this.multiselectItemCodeList = [];
      itemList.forEach(x => this.multiselectItemCodeList.push({ value: x.itemCode, label: x.description }));
      itemList.forEach(x => this.multiselectItemCodeList1.push(x.itemCode));
      this.results = this.multiselectItemCodeList.filter(c => c.value.includes(event.query));
    });
