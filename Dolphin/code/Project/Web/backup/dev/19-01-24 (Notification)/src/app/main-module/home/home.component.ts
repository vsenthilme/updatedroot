...
export class HomeComponent implements OnInit {
  ...
  constructor(private router: Router, public dialog: MatDialog, private ms: MenuService, private auth: AuthService, private service: PreoutboundService,
    private uploadService: ShipmentSummaryComponent, private notificationService: WmsIdmasterService,     private overlayContainer: OverlayContainer,    private webSocketAPIService: WebSocketAPIService,) { 
  ...
  getNotificationCount(){
    this.notificationService.getNotification(this.auth.userID).subscribe((res: any)=>{
      console.log(res);
  ...
  readAllNotifications(event){
    console.log(event)
      ...
      if(event == true){
        this.notificationService.markNotificationsAsRead().subscribe(res=>{
          this.notificationCount = 0;
          this.notificationDataFromDB.next([]);
        });
      }
