...
export class ErrorInterceptor implements HttpInterceptor {
  ...
  private addTokenHeader(request: HttpRequest<any>, token: string) {
    ...
    if (request.url.includes('/wms-idmaster-service/login'))
      ...
      if (request.url.includes('/wms-transaction-service/reports/shipmentDeliverySummary1'))
      return request.clone({ params: new HttpParams().append("authToken", token) });
      
      if (request.url.includes('/wms-transaction-service/shipmentDispatchSummary'))
      return request.clone({ params: new HttpParams().append("authToken", token) });
