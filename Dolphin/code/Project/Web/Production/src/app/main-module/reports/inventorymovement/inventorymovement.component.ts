...
import { UpdateInventorymovementComponent } from "./update-inventorymovement/update-inventorymovement.component";
...
export class InventorymovementComponent implements OnInit {
    ...
    applyFilter(event: Event) {
      ...
      if(this.dataSource.filter.length == 0){
        this.dataSource.data.forEach(element => {
          this.totalonhandqty = this.totalonhandqty + (element.movementQty != null ? element.movementQty : 0);
       
        })
      }
    else{
      for (let i=0;i<this.dataSource.filteredData.length;i++){
        this.totalonhandqty = this.totalonhandqty + (this.dataSource.filteredData[i].movementQty != null ? this.dataSource.filteredData[i].movementQty : 0);
    ...
    onItemType(searchKey) {
      ...
      if (searchVal !== '' && searchVal !== null) {
        forkJoin({
          itemList: this.ReportsService.getItemCodeDropDown2(searchVal.trim(),this.auth.companyId,this.auth.plantId,this.selectedItems[0],this.auth.languageId).pipe(catchError(err => of(err))),
        })
          .subscribe(({ itemList }) => {
    ...
    filtersearch(){
     ...
     obj.fromCreatedOn = this.cs.day_callapiSearch(this.fromCreatedOn);
      ...
      this.spin.show();
      this.sub.add(this.ReportsService.getInventoryMovement(obj).subscribe(res => {
        console.log(res)
        res.forEach(element => {
        if(element.movementType == 1 && element.submovementType == 1){
          element['subMovementText'] = 'Staging'
    ...
    openDialog(data: any = 'New'): void {
      ...
      const dialogRef = this.dialog.open(UpdateInventorymovementComponent, {
        disableClose: true,
        width: '50%',
        maxWidth: '80%',
        position: { top: '9%', },
        data: { pageflow: data, code: this.selection.selected[0]}
      });
