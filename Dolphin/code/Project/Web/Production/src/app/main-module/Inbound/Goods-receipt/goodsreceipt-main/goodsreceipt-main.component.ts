...
export class GoodsreceiptMainComponent implements OnInit {
  ...
  @ViewChild('goodsReceiptTag') goodsReceiptTag: Table | undefined;
  ...
  callTableHeader() {
    this.cols = [
      ...
      { field: 'stagingNo', header: 'Staging No' },
      { field: 'createdBy', header: 'Created By' },
      { field: 'createdOn', header: 'Staging Date', format: 'date' },
    ];
  }
  ...
  downloadexcel() {
    const exportData = this.goodsReceipt.map(item => {
    ...
    this.cs.exportAsExcel(exportData, 'Case Receipt');
  }
  ...
  openDialog(data: any = 'new', type?: any): void {
    if (type) {
      this.selectedGoodsReceipt.push(type);
    }
    if (data != 'New')
      if (this.selectedGoodsReceipt.length === 0) {
    ...
    let paramdata = "";
    if (this.selectedGoodsReceipt[0].statusId === 24) {
    ...
    if (this.selectedGoodsReceipt.length > 0) {
      paramdata = this.cs.encrypt({ code: this.selectedGoodsReceipt[0], pageflow: data });
  ...
  selectedGoodsReceipt: any[] = [];
  onChange() {
    const choosen = this.selectedGoodsReceipt[this.selectedGoodsReceipt.length - 1];
    this.selectedGoodsReceipt.length = 0;
    this.selectedGoodsReceipt.push(choosen);
  }
}
