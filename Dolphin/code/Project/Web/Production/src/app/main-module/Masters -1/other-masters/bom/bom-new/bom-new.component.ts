...
export class BomNewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.data.pageflow != 'New') {
      ...
      let count = 0;
       let result = this.data.code.bomLines.filter((x: any) => x.deletionIndicator == 0);
  ...
  dropdownlist() {
    this.spin.show();
    this.sub.add(this.service.GetitemcodeNew().subscribe(res => {
      ...
      forkJoin({
        ...
        .subscribe(({ itemList }) => {
          if (itemList != null && itemList.length > 0) {
            ...
            this.itemCodeList1 = itemList;
            this.itemCodeList1.forEach(x => this.multiselectItemCodeList1.push({value: x.itemCode, label: x.itemCode + ' - ' + x.manufacturerName + ' - ' + x.description, manufacturingName: x.manufacturerName, description: x.description }))
          }
        });
   }
 }
 
 onItemCodeChange(e, index){
   this.BomLine.controls.referenceField6.patchValue(e.description);
   console.log(this.BomLine)
  ...
  fill() {
    ...
    this.sub.add(this.service.Get(this.data.code.parentItemCode, this.data.code.warehouseId,).subscribe(res => {
      ...
      this.form.controls.parentItemCode.patchValue(res.parentItemCode)
  ...
  deleteLine(element){
    this.spin.show();
  this.service.DeleteLine(element.bomNumber, element.childItemCode, element.companyCode, element.languageId, element.plantId, element.warehouseId).subscribe(res => {
    this.toastr.success(element.bomNumber + " deleted successfully!", "Notification", {
      timeOut: 2000,
      progressBar: false,
    });
//    this.refreshTable();
