...
export class PreinboundComponent implements OnInit {
 ...
 // displayedColumns: string[] = [ 'statusId','itemCode','proposedStorageBin', 'confirmedStorageBin', 'packBarcodes', 'putAwayQuantity', 'putawayConfirmedQty', 'confirmedBy','confirmedOn','createdOn', 'leadTime'];
  ...
  applyFilter(event: Event) {
    ...
    if(this.dataSource.filter.length == 0){
      this.dataSource.data.forEach(element => {
        this.totalonhandqty = this.totalonhandqty + (element.acceptedQty != null ? element.acceptedQty : 0);
        this.totaldamqty = this.totaldamqty + (element.damageQty != null ? element.damageQty : 0);
        this.totalholdqty = this.totalholdqty + (element.variance != null ? element.variance : 0);
      })
    }
  else{
    for (let i=0;i<this.dataSource.filteredData.length;i++){
      this.totalonhandqty = this.totalonhandqty + (this.dataSource.filteredData[i].acceptedQty != null ? this.dataSource.filteredData[i].acceptedQty : 0);
      ...
      this.totalholdqty = this.totalholdqty + (this.dataSource.filteredData[i].variance != null ? this.dataSource.filteredData[i].variance : 0);
  ...
  filtersearch(){
    ...
    this.sub.add(this.serviceLine.search(this.form.getRawValue()).subscribe(res => {
      res.forEach(x=> {
