...
export class PickingReportComponent implements OnInit {
   ...
   // displayedColumns: string[] = [ 'statusId', 'refDocNumber','referenceField1', 'lineNumber', 'itemCode', 'description', 'pickedPackCode', 'proposedStorageBin','assignedPickerId','partnerCode', 'pickConfirmQty', 'pickupConfirmedOn', 'pickupCreatedOn', 'leadTime'];
    ...
    //wms demo
    displayedColumns: string[] = ['select', 'statusId', 'refDocNumber','referenceField1', 'lineNumber', 'itemCode', 'description', 'pickedPackCode', 'proposedStorageBin','assignedPickerId','partnerCode', 'pickConfirmQty', 'pickupCreatedOn'];
  ...
  StatusList: any[] = [];
    ...
    applyFilter(event: Event) {
      ...
      if(this.dataSource.filter.length == 0){
        this.dataSource.data.forEach(element => {
          ...
          this.totaldamqty = this.totaldamqty + (element.pickConfirmQty != null ? element.pickConfirmQty : 0);
      ...
      for (let i=0;i<this.dataSource.filteredData.length;i++){
        ...
        this.totaldamqty = this.totaldamqty + (this.dataSource.filteredData[i].pickConfirmQty != null ? this.dataSource.filteredData[i].pickConfirmQty : 0);
    ...
    toPickConfirmedOn: any;
    fromPickConfirmedOn: any;
    statusId= [];

    reset(){
      this.refDocNumber = null
      this.pickedPackCode = null
      this.itemCode = null
      this.pickedStorageBin= null;
      this.toPickConfirmedOn = null
      this.fromPickConfirmedOn = null
      this.statusId= [];
    }
   ...
   //     let result = res.filter((x: any) => x.statusId == 50);
    ...
    downloadexcel() {
      ...
      this.dataSource.data.forEach(x => {
        res.push({
          ...
          "Pickup Date": this.cs.dateapi(x.pickupCreatedOn),
    ...
    deleterecord(obj: any) {
      this.spin.show();
      this.sub.add(this.pickingService.deletePickupLine(obj).subscribe((ress) => {
