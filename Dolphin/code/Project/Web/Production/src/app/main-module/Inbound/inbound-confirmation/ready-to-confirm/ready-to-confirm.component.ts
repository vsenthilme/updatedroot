...
export class ReadyToConfirmComponent implements OnInit {
  ...
  @ViewChild('readytoConfirmTag') readytoConfirmTag: Table | undefined;
  ...
  displayedColumns: string[] = ['select', 'statusId', 'refDocNumber', 'inboundOrderTypeId', 'containerNo', 'doc', 'createdOn'];
  ...
  downloadexcel() {
    const exportData = this.readytoConfirm.map(item => {
    ...
    this.cs.exportAsExcel(exportData, 'Ready To Confirm');
  }
  ...
  openDialog(data: any = 'new', type?: any): void {
    if (type) {
      this.selectedReadytoConfirm.push(type);
    }
    if (data != 'New')
      if (this.selectedReadytoConfirm.length === 0) {
    ...
    if (data != 'Display')
      if (this.selectedReadytoConfirm[0].statusId == 24) {
        this.toastr.error("Order can't be edited as it's already confirmed.", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.selectedReadytoConfirm = [];
        return;
      }
    let paramdata = "";

    if (this.selectedReadytoConfirm.length > 0) {
      paramdata = this.cs.encrypt({ code: this.selectedReadytoConfirm[0], pageflow: data });
  ...
  replace() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.spin.show();
        this.sub.add(this.service.replaceASN(this.selectedReadytoConfirm[0].warehouseId, this.selectedReadytoConfirm[0].preInboundNo, result, this.selectedReadytoConfirm[0].refDocNumber).subscribe((res) => {
  ...
  selectedReadytoConfirm: any[] = [];
  onChange() {
    const choosen = this.selectedReadytoConfirm[this.selectedReadytoConfirm.length - 1];
    this.selectedReadytoConfirm.length = 0;
    this.selectedReadytoConfirm.push(choosen);
  }
