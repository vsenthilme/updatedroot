...
export class InboundconfirmMainComponent implements OnInit {
  ...
  @ViewChild('confirmedTag') confirmedTag: Table | undefined;
  ...
  downloadexcel() {
    const exportData = this.confirmed.map(item => {
      ...
      this.cols.forEach(col => {
        if (col.format !== 'date' && col.format !== 'extra') {
          ...
          if (col.field == 'inboundOrderTypeId') {
            exportItem[col.header] = this.cs.getinboundorderType_text(item[col.field]);
          }
          if (col.field == 'leadTime') {
            exportItem[col.header] = item.confirmedOn != null ? this.cs.getDataDiff(item.createdOn, item.confirmedOn) : '';
          }
        }
      });
      return exportItem;
    });
    this.cs.exportAsExcel(exportData, 'Confirmed');
  }
  ...
  callTableHeader() {
    this.cols = [
      ...
      { field: 'inboundOrderTypeId', header: 'Order Type', format: 'extra' },
      { field: 'containerNo', header: 'Container No' },
      { field: 'doc', header: 'Doc', format: 'extra' },
      { field: 'createdOn', header: 'Order Date', format: 'date' },
      { field: 'confirmedOn', header: 'Receipt Confirmation Date', format: 'date' },
      { field: 'leadTime', header: 'Lead Time', format: 'extra' },
  ...
  openDialog(data: any = 'new', type?: any): void {
    if (type) {
      this.selectedConfirmed.push(type);
    }
    if (data != 'New')
      if (this.selectedConfirmed.length === 0) {
    ...
    if (data != 'Display')
      if (this.selectedConfirmed[0].statusId == 24) {
        this.toastr.error("Order can't be edited as it's already confirmed.", "Notification", {
          timeOut: 2000,
          progressBar: false,
        });
        this.selectedConfirmed=[];
        return;
      }
    let paramdata = "";

    if (this.selectedConfirmed.length > 0) {
      paramdata = this.cs.encrypt({ code: this.selectedConfirmed[0], pageflow: data });
  ...
  replace() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.spin.show();
        this.sub.add(this.service.replaceASN(this.selectedConfirmed[0].warehouseId, this.selectedConfirmed[0].preInboundNo, result, this.selectedConfirmed[0].refDocNumber).subscribe((res) => {
  ...
  selectedConfirmed: any[] = [];
  onChange() {
    const choosen = this.selectedConfirmed[this.selectedConfirmed.length - 1];
    this.selectedConfirmed.length = 0;
    this.selectedConfirmed.push(choosen);
  }
