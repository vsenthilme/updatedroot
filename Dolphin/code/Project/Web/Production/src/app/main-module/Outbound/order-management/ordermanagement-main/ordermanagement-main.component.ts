...
export class OrdermanagementMainComponent implements OnInit {
  ...
  @ViewChild('ordermanagementTag') ordermanagementTag: Table | undefined;
  ...
  callTableHeader() {
    this.cols = [
      ...
      { field: 'inventoryQty', header: 'Inv Qty' },
      { field: 'allocatedQty', header: 'Allocated Qty' },
      { field: 'requiredDeliveryDate', header: 'Required Date', format: 'date' },
      { field: 'preOutboundNo', header: 'Preoutbound No' },
      { field: 'proposedPackBarCode', header: 'Pallet ID' },
  ...
  Unallocate(data: any) {
    this.spin.show();
    this.sub.add(this.service.Unallocate(data.itemCode, data.lineNumber, data.partnerCode, data.preOutboundNo, data.refDocNumber, this.auth.warehouseId, data.proposedPackBarCode, data.proposedStorageBin).subscribe(res => {
      this.spin.hide();
  ...
  Allocate(data: any) {
    this.spin.show();
    this.sub.add(this.service.Allocate(data.itemCode, data.lineNumber, data.partnerCode, data.preOutboundNo, data.refDocNumber, this.auth.warehouseId).subscribe(res => {
      console.log(res)
      this.spin.hide();
  ...
  displayedColumns: string[] = ['select', 'actions', 'referenceField7', 'refDocNumber', 'partnerCode', 'outboundOrderTypeId', 'lineNumber', 'itemCode', 'orderQty', 'inventoryQty', 'allocatedQty', 'requiredDeliveryDate', 'preOutboundNo', 'proposedPackBarCode', 'proposedStorageBin', 'description',];
  ...
  selectRow(row) {
    this.selection.toggle(row);
    console.log(this.selectedOrderManagement);
  }
  allocateBulk() {
    ...
    let checkAnyUnallocated: any = [];
    this.selectedOrderManagement.forEach(x => {
      x['warehouseId'] = this.auth.warehouseId;
      if (x.referenceField7 == 'ALLOCATED') {
        checkAnyUnallocated.push(x.itemCode);
      }
    });
    if (checkAnyUnallocated.length > 0) {
      this.toastr.error(
        "Item Codes: " + checkAnyUnallocated + " is already allocated",
        "Notification", {
        timeOut: 2000,
        progressBar: false,
      }
    ...
    this.sub.add(this.service.allocateBatch(this.selectedOrderManagement).subscribe(res => {
      ...
      if (res) {
        let checkAvailableAllocatedQty: any = [];
        res.forEach(x => {
          if (x.allocatedQty <= 0) {
            checkAvailableAllocatedQty.push(x.itemCode);
          }
        });
        if (checkAvailableAllocatedQty.length > 0) {
          this.toastr.warning(
            "Item Codes: " + checkAvailableAllocatedQty + " Stock not available for allocation",
            "Notification", {
            timeOut: 2000,
            progressBar: false,
          }
  ...
  unallocateBulk() {
    ...
    let checkAnyAllocated: any = [];
    this.selectedOrderManagement.forEach(x => {
      x['warehouseId'] = this.auth.warehouseId;
      if (x.referenceField7 == 'Unallocated') {
        checkAnyAllocated.push(x.itemCode);
      }
    });
    if (checkAnyAllocated.length > 0) {
      this.toastr.error(
        "Item Codes: " + checkAnyAllocated + " is already Unallocated",
        "Notification", {
        timeOut: 2000,
        progressBar: false,
      }
  ...
  isSelected(item: any): boolean {
    return this.selectedOrderManagement.includes(item);
  }
}
