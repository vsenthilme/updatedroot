...
export class PickingMainComponent implements OnInit {
  ...
  callTableHeader() {
    this.cols = [
      ...
      { field: 'assignedPickerId', header: 'Picker' },
      { field: 'partnerCode', header: 'Store' },
      { field: 'pickToQty', header: 'To Qty' },
      { field: 'pickupCreatedOn', header: 'Created Date', format: 'date' },
    ];
  }
  ...
  deleteDialog() {
    if (this.selectedPicking.length === 0) {
      this.toastr.error("Kindly select any row", "Norification");
      return;
    }
    if (this.selectedPicking[0].statusId != 48) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.deleterecord(this.selectedPicking[0]);
  ...
  openDialog(data: any = 'new', type?: any): void {
    if (type) {
      this.selectedPicking.push(type);
    }
    if (data != 'New')
      if (this.selectedPicking.length === 0) {
    ...
    if (this.selectedPicking.length > 0) {
      paramdata = this.cs.encrypt({ code: this.selectedPicking[0], pageflow: data });
  ...
  updatePicker(data: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        let data: any[] = [];
        this.selectedPicking.forEach((x: any) => {
          data.push({
            itemCode: x.itemCode,
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        if (objArray.length > 0) {
          this.spin.show();
          this.selectedPicking.forEach((x) => {
            x.assignedPickerId = result;
          })
          this.spin.show();

          this.sub.add(this.service.multipleUpdatePicker(this.selectedPicking).subscribe(res => {
            this.spin.hide();
  ...
  isSelected(item: any): boolean {
    return this.selectedPicking.includes(item);
  }
}
