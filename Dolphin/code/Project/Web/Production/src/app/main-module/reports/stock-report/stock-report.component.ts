...
export class StockReportComponent implements OnInit {
  ...
  applyFilter(event: Event) {
    ...
    if(this.dataSource.filter.length == 0){
      this.dataSource.data.forEach(element => {
        this.totalonhandqty = this.totalonhandqty + (element.onHandQty != null ? element.onHandQty : 0);
        this.totaldamqty = this.totaldamqty + (element.damageQty != null ? element.damageQty : 0);
        this.totalholdqty = this.totalholdqty + (element.holdQty != null ? element.holdQty : 0);
        this.totalavaiqty = this.totalavaiqty + (element.availableQty != null ? element.availableQty : 0);
      })
    }
  else{
    for (let i=0;i<this.dataSource.filteredData.length;i++){
      this.totalonhandqty = this.totalonhandqty + (this.dataSource.filteredData[i].onHandQty != null ? this.dataSource.filteredData[i].onHandQty : 0);
      this.totaldamqty = this.totaldamqty + (this.dataSource.filteredData[i].damageQty != null ? this.dataSource.filteredData[i].damageQty : 0);
      this.totalholdqty = this.totalholdqty + (this.dataSource.filteredData[i].holdQty != null ? this.dataSource.filteredData[i].holdQty : 0);
      this.totalavaiqty = this.totalavaiqty + (this.dataSource.filteredData[i].availableQty != null ? this.dataSource.filteredData[i].availableQty : 0);
  ...
  onItemType(searchKey) {
    ...
    if (searchVal !== '' && searchVal !== null) {
      forkJoin({
        itemList: this.reportService.getItemCodeDropDown2(searchVal.trim(),this.auth.companyId,this.auth.plantId,this.selectedItems,this.auth.languageId).pipe(catchError(err => of(err))),
      })
        .subscribe(({ itemList }) => {
