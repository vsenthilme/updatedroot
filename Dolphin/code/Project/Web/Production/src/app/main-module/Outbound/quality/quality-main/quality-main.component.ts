...
export class QualityMainComponent implements OnInit {
  @ViewChild('qualityTag') qualityTag: Table | undefined;
  ...
  callTableHeader() {
    this.cols = [
      ...
      { field: 'actualHeNo', header: 'HE No' },
      { field: 'referenceField4', header: 'Item Code' },
      { field: 'referenceField1', header: 'Picked Storage Bin' },
      { field: 'referenceField2', header: 'Pallet ID' },
      { field: 'refDocNumber', header: 'Order No', format:'extra' },
      { field: 'partnerCode', header: 'Store' },
      { field: 'qualityQty', header: 'Accepted Qty' },
      { field: 'rejectQty', header: 'Rejected Qty' },
      { field: 'qualityInspectionNo', header: 'QC No' },
      { field: 'manufacturerPartNo', header: 'Mfr Part No' },
      { field: 'qualityCreatedBy', header: 'Quality By' },
      { field: 'qualityCreatedOn', header: 'Quality Date', format: 'date' }
    ];
  }
  ...
  multipleQualityConfirm() {
    ...
    if (this.selectedQuality.length < this.dataSource.data.length) {
      //  thi
    }
    if (this.selectedQuality.length == 0) {
      this.toastr.error(
    ...
    if (this.dataSource.connect().value.length < this.selectedQuality.length) {
  ...
  downloadexcel() {
    const exportData = this.quality.map(item => {
    ...
    this.cs.exportAsExcel(exportData, 'Quality');
  }
  ...
  isSelected(item: any): boolean {
    return this.selectedQuality.includes(item);
  }
}
