...
export class ContainerMainComponent implements OnInit {
  ...
  @ViewChild('containerTag') containerTag: Table | undefined;
  ...
  ngOnInit(): void {
    ...
    oneMonthBefore.setDate(oneMonthBefore.getDate() - 31);

    this.searhform.controls.endContainerReceivedDate.patchValue(new Date);
    this.searhform.controls.startContainerReceivedDate.patchValue(oneMonthBefore);
  ...
  callTableHeader() {
    this.cols = [
      ...
      { field: 'containerNo', header: 'Container No', format: 'extra'  },
      { field: 'containerReceiptNo', header: 'Container Receipt No' },
      { field: 'refDocNumber', header: 'Order No' },
      { field: 'partnerCode', header: 'Supplier Name' },
      { field: 'createdBy', header: 'Created By' },
      { field: 'referenceField2', header: 'Container Unloaded On', format: 'date' },
      { field: 'referenceField5', header: 'Actual Receipt Date', format: 'date' },
    ];
  }
  ...
  downloadexcel() {
    ...
    this.cs.exportAsExcel(exportData, 'Container Receipt');
  }
  deleteDialog() {
  ...
  openDialog(data: any = 'new',type?:any): void {
    if(type){
      this.selectedContainer.push(type);
    }
    if (data != 'New')
      if (this.selectedContainer.length === 0) {
