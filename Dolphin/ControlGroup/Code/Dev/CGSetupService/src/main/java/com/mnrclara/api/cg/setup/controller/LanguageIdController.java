...
import com.mnrclara.api.cg.setup.service.LanguageIdService;
...
public class LanguageIdController {
	...
	public ResponseEntity<?> postLanguageId(@Valid @RequestBody AddLanguageId newLanguageId,
		...
		LanguageId createdLanguageId =
				languageidService.createLanguageId(newLanguageId, loginUserID);
	...
	@PostMapping("/find")
	public ResponseEntity<?> findLanguageId(@Valid @RequestBody FindLanguageId findLanguageId) throws Exception {
		List<LanguageId> createdLanguageId = languageidService.findLanguageId(findLanguageId);
		return new ResponseEntity<>(createdLanguageId, HttpStatus.OK);
	}
}