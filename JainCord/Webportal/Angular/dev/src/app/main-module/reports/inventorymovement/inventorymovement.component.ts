...
export class InventorymovementComponent implements OnInit {
  ...
  @ViewChild('inventorymovTag') inventorymovTag: Table | any;
    ...
    cols: any[] = [
      ...
      { header: 'Barcode Id', field: 'barcodeId', format: 'normal', showFooter: false },
      { header: 'Storage Bin', field: 'storageBin', format: 'normal', showFooter: false },
      { header: 'Movement', field: 'movementType', format: 'movement', showFooter: false },
      { header: 'Sub Movement', field: 'subMovementText', format: 'normal', showFooter: false },
      { header: 'Order No', field: 'refDocNumber', format: 'normal', showFooter: false },
      { header: 'Opening Qty', field: 'referenceField2', format: 'normal', showFooter: true },
      { header: 'Movement Qty', field: 'movementQty', format: 'normal', showFooter: true },
      { header: 'Closing Qty', field: 'balanceOHQty', format: 'normal', showFooter: true },
      { header: 'UOM', field: 'inventoryUom', format: 'normal', showFooter: false },
      { header: 'Created On', field: 'createdOn', format: 'date', showFooter: false }
    ];
    ...
    initialSearch(){
      ...
      if(this.form.controls.movementTypeFE.value != null){
      this.form.controls.movementType.patchValue([this.form.controls.movementTypeFE.value])
      }
      else{
        this.form.controls.movementType.patchValue(this.form.controls.movementTypeFE.value)
      }
    ...
    movementChange(){
      this.submovementList = []
        ...
        if(x.movemenTypeID === this.form.controls.movementTypeFE.value){
...
this.spin.hide();
    ...
    downloadexcel() {
      ...
      this.cs.exportAsExcel(exportData, 'Total Stock Movement');
    }
    ...
    handleSearch1(filterInput){
      ...
      if (filterInput) {
        this.calculateTotals(this.inventorymovTag.filteredValue);
      }
      else {
        this.calculateTotals(this.binner);
  ...
  totalQty1:number =0;
    calculateTotals(data?: any[]): void {
     ...
     this.totalQty1=0;
      if (data && data.length > 0) {
        this.totalInventoryQuantity = data.reduce((total, item) => total + Number(item.referenceField2 != null ? item.referenceField2 : 0), 0);
        this.totalAllocatedQuantity =data.reduce((total, item) => total + (item.movementQty != null ? item.movementQty : 0), 0);
        this.totalQty1=data.reduce((total, item) => total + (item.balanceOHQty != null ? item.balanceOHQty : 0), 0);
      } else {
    ...
    getopenQty(){
      let total = 0;
      this.binner.forEach(x =>{
        total = total + (x.referenceField2 != null ? Number(x.referenceField2) : 0);
