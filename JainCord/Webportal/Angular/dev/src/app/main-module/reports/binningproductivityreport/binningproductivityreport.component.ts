...
export class BinningproductivityreportComponent implements OnInit {
  ...
  cols: any[] = [
    { header: 'Branch', field: 'plant_text', format: 'normal' },
    { header: 'Warehouse', field: 'wh_text', format: 'normal' },
    { header: 'Status', field: 'status_text', format: 'normal' },
    { header: 'Mfr Name', field: 'mfr_name', format: 'normal' },
    { header: 'Part No', field: 'itm_code', format: 'normal' },
    { header: 'Order No', field: 'ref_doc_no', format: 'normal' },
    { header: 'Order Type', field: 'ref_doc_type', format: 'normal' },
    { header: 'Proposed Storage Bin', field: 'prop_st_bin', format: 'normal' },
    { header: 'Confirmed Storage Bin', field: 'cnf_st_bin', format: 'normal' },
    { header: 'Barcode', field: 'barcode_id', format: 'normal' },
    { header: 'Order Qty', field: 'pa_qty', format: 'normal' },
    { header: 'Binned Qty', field: 'pa_cnf_qty', format: 'normal' },
    { header: 'Binner', field: 'pa_cnf_by', format: 'normal' },
    { header: 'Assigned On', field: 'pa_ctd_on', format: 'date' },
    { header: 'Binned On', field: 'pa_cnf_on', format: 'date' },
    { header: 'Lead Time', field: 'leadTime', format: 'lead' },
    { header: 'Company', field: 'c_text', format: 'normal' }
  ];
...
binnernew:any[]=[];
  ...
  downloadexcel() {
    ...
    this.binner.forEach(x => {
      res.push({
        ...
        "Lead Time": this.cs.getDataDiff(x.pa_ctd_on ,x.pa_cnf_on),
        "Company":x.c_text,
  ...
  getProductivity() {
    ...
    this.binner.forEach(element => {
      ...
      if (element.pa_ctd_on && element.pa_cnf_on) {
        const confirmedDate: Date = new Date(element.pa_cnf_on);
        const createdDate: Date = new Date(element.pa_ctd_on); 
        const leadTimeInMilliseconds = (confirmedDate as any) - (createdDate as any);
          const leadTimeInMinutes = leadTimeInMilliseconds / (1000 * 60); // Convert to minutes directly
          totalLeadTimeInMinutes += leadTimeInMinutes;
      }
    });

    let percentage = (totalLeadTimeInMinutes > 0) ? (total2 / totalLeadTimeInMinutes) : 0;
...
getAvgLeadTime() {
  ...
  let averageLeadTime = (count > 0) ? totalLeadTimeInMinutes / total2 : 0;
