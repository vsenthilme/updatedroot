...
@Component({
  selector: 'app-perpetualreport',
  templateUrl: './perpetualreport.component.html',
  styleUrls: ['./perpetualreport.component.scss']
})
export class PerpetualreportComponent implements OnInit {
  screenid=3166;
    binner: any[] = [];
    selectedbinner: any[] = [];
    ...
    cols: any[] =[
      { header: 'Plant', field: 'plantDescription', format: 'normal' },
      { header: 'Warehouse', field: 'warehouseDescription', format: 'normal' },
      { header: 'Cycle Count', field: 'cycleCountNo', format: 'normal' },
      { header: 'Mfr Name', field: 'manufacturerName', format: 'normal' },
      { header: 'Part No', field: 'itemCode', format: 'normal' },
      { header: 'Description', field: 'itemDesc', format: 'normal' },
      { header: 'Section', field: 'storageSectionId', format: 'normal' },
      { header: 'Bin Location', field: 'storageBin', format: 'normal' },
      { header: 'Barcode Id', field: 'barcodeId', format: 'normal' },
      { header: 'Stock Type', field: 'stockTypeId', format: 'normal' },
      { header: 'Inventory Qty', field: 'inventoryQuantity', format: 'normal' },
      { header: 'Counted Qty', field: 'countedQty', format: 'normal' },
      { header: 'First Counted Qty', field: 'firstCountedQty', format: 'normal' },
      { header: 'Second Counted Qty', field: 'secondCountedQty', format: 'normal' },
      { header: 'Outbound Qty', field: 'outboundQuantity', format: 'normal' },
      { header: 'Inbound Qty', field: 'inboundQuantity', format: 'normal' },
      { header: 'Frozen Qty', field: 'frozenQty', format: 'normal' },
      { header: 'UOM', field: 'inventoryUom', format: 'normal' },
      { header: 'User Id', field: 'cycleCounterId', format: 'normal' },
      { header: 'Status', field: 'statusDescription', format: 'normal' },
      { header: 'Company', field: 'companyDescription', format: 'normal' }
    ];
    ...
    multicycleList:any[]=[];
    getDropdown() {
      this.sub.add(this.reportService.getperpetualheaderv2({ warehouseId: [this.auth.warehouseId],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId] }).subscribe(res => {
    ...
    itemCodeChanged(e){
      ...
      let SelectedArray2:any;
        ...
        this.searhform.controls.manufacturerPartNo.patchValue([SelectedArray2]);
    }
    ...
    filtersearch() {
     ...
     if(this.searhform.controls.storageBinFE.value == null){
      ...
      this.sub.add(this.reportService.getperpertuallineSpark(this.searhform.getRawValue()).subscribe(res => {
        this.binner = res;
    ...
    downloadexcel() {
      ...
      this.cs.exportAsExcel(exportData, 'Perpetual Report');
    }
