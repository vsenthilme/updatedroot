...
export class NewStockMovementComponent implements OnInit {
  ...
  cols: any[] = [
    ...
    { header: 'MFR Name', field: 'manufacturerSKU', format: 'normal' },
    { header: 'Part No', field: 'itemCode', format: 'normal' },
    { header: 'Description', field: 'itemText', format: 'normal' },
    { header: 'Document Type', field: 'documentType', format: 'normal' },
    { header: 'Document No', field: 'documentNumber', format: 'normal' },
    { header: 'Movement Qty', field: 'movementQty', format: 'normal' },
    { header: 'Opening Qty', field: 'openingStock', format: 'number' },
    { header: 'Balance OH Qty', field: 'balanceOHQty', format: 'number' },
    { header: 'Date', field: 'confirmedOn', format: 'date' },
    { header: 'Time', field: 'confirmedOn', format: 'date' } // Same field is used for Date and Time, handle separately in the template
  ];
...
this.searhform.controls.manufacturerName.patchValue(this.searhform.controls.manufacturerName.value);
...
this.searhform.controls.toDeliveryDate.patchValue(this.cs.day_callapiSearch(this.searhform.controls.toCreatedOn.value));
  ...
  //  / obj.warehouseId = [this.selectedWarehouse];
    ...
    this.reportService.getStockMovementDetailsNewV2(this.searhform.getRawValue()).subscribe(
      result => {
        this.spin.hide();
        let obj: any = {};
        obj.itemCode = this.searhform.controls.itemCode.value;
        obj.warehouseId = this.selectedItems;
        if(result){
  ...
  downloadexcel() {
    const exportData = this.transfer.map(item => {
    ...
    this.cs.exportAsExcel(exportData, 'Stock Movement Report');
  }
