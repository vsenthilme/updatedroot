...
multiOrderNo: any[] = [];
  getDropdown(){
    this.sub.add(this.reportService.searchstream({warehouseId : [this.auth.warehouseId],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId]}).subscribe(res => {
      res.forEach((x: any) => this.multiOrderNo.push({ value: x.refDocNumber, label: x.refDocNumber }));
      res.forEach((x: any) => this.multiordertype.push({ value: x.inboundOrderTypeId, label: x.referenceDocumentType }));
      res.forEach((x: { containerNo: string; }) => {
        // Check if 'containerNo' is not null before pushing to 'containerNoList'.
          ...
          this.multiordertype=this.cs.removeDuplicatesFromArrayNewstatus(this.multiordertype);
...
filtersearch() {
  ...
  if(this.form.controls.inboundOrderTypeId.value == null){
    obj.inboundOrderTypeId=null;
  }
  else{
    obj.inboundOrderTypeId=this.form.controls.inboundOrderTypeId.value;
  }
  this.sub.add(this.service.searchstream(obj).subscribe(res => {
    this.inbound = res;
    this.spin.hide()
...
downloadexcel() {
    ...
    this.cs.exportAsExcel(exportData, 'Inbound Order Summary Report');
  }
