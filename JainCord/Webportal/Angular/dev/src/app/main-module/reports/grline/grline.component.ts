...
export class GrlineComponent implements OnInit {
  ...
  cols: any[] = [
    ...
    { header: 'Status', field: 'statusId', format: 'normal' },
    { header: 'Mfr Name', field: 'manufacturerName', format: 'normal' },
    { header: 'Part No', field: 'itemCode', format: 'normal' },
    { header: 'Batch Serial Number', field: 'batchSerialNumber', format: 'normal' },
    { header: 'Assigned User Id', field: 'assignedUserId', format: 'normal' },
    { header: 'Order No', field: 'refDocNumber', format: 'normal' },
    { header: 'Order Type', field: 'referenceDocumentType', format: 'normal' },
    { header: 'Barcode', field: 'barcodeId', format: 'normal' },
    { header: 'Inventory Qty', field: 'inventoryQuantity', format: 'normal' },
    { header: 'Released Qty', field: 'acceptedQty', format: 'normal' },
    { header: 'Damaged Qty', field: 'damageQty', format: 'normal' },
    { header: 'Remaining Qty', field: 'remainingQty', format: 'normal' },
    { header: 'Expiration Date', field: 'expiryDate', format: 'date' },
    { header: 'Manufactured Date', field: 'manufacturerDate', format: 'date' },
    { header: 'Charge Value', field: 'threePLCbm', format: 'normal' },
    { header: 'Charge Unit', field: 'threePLUom', format: 'normal' },
    { header: 'Quantity Type', field: 'quantityType', format: 'normal' },
    { header: 'Created On', field: 'createdOn', format: 'date' },
    { header: 'Confirmed On', field: 'confirmedOn', format: 'date' },
    { header: 'Lead Time', field: 'leadTime', format: 'lead' },
    { header: 'Released By', field: 'createdBy', format: 'normal' },
  ...
  filtersearch() {
    ...
    this.sub.add(this.reportService.getgrlinev2spark(this.searhform.getRawValue()).subscribe(res => {
      this.binner = res;
  ...
  downloadexcel() {
    ...
    this.binner.forEach(x => {
      res.push({
        ...
        "Part No": x.itemCode,
       "Batch Serial Number":x.batchSerialNumber,
        ...
        "Expiration Date":this.cs.dateExcel(x.expiryDate),
        "Manufactured Date":this.cs.dateExcel(x.manufacturerDate),
        "Charge Value":x.threePLCbm,
        "Charge Unit":x.threePLUom,
        "Quantity Type":x.quantityType,
        "Created On":this.cs.dateExcel(x.createdOn),
        "Confirmed On":this.cs.dateExcel(x.confirmedOn),
        "Lead Time":this.cs.getDataDiff(x.createdOn, x.confirmedOn),
        "Released By" :x.createdBy,
    ...
    this.cs.exportAsExcel(res, "Staging Report");
  }
