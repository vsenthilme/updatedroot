...
@ViewChild('inboundTag') inboundTag: Table | any;
  ...
  form = this.fb.group({
   ...
   cols: any[] = [
    ...
    { header: 'Sales Invoice Number', field: 'salesInvoiceNumber', format: 'normal', showFooter: false },
    { header: 'Sales Order No', field: 'salesOrderNumber', format: 'normal', showFooter: false },
    { header: 'Target Branch', field: 'targetBranchCode', format: 'normal', showFooter: false },
    ...
    { header: 'Order Qty', field: 'orderQty', format: 'normal', showFooter: true  },
    { header: 'Picked Qty', field: 'referenceField9', format: 'number', showFooter: true },
    { header: 'QA Qty', field: 'referenceField10', format: 'number', showFooter: true },
    { header: 'Delivery Qty', field: 'deliveryQty', format: 'normal', showFooter: true },
    { header: 'Created Date', field: 'createdOn', format: 'date', showFooter: false },
...
inboundnew:any[]=[];
   ...
   filtersearch(){
     ...
     this.form.controls.plantId.patchValue([this.auth.plantId]);
 this.spin.show();
 this.form.controls.toDeliveryDate.patchValue(this.cs.day_callapiSearch(this.form.controls.toCreatedOn.value));
 this.form.controls.fromDeliveryDate.patchValue(this.cs.day_callapiSearch(this.form.controls.fromCreatedOn.value));
...
totalQty1:number =0;
  calculateTotals(data?: any[]): void {
   ...
   downloadexcel() {
    ...
    this.cs.exportAsExcel(exportData, 'Outbound Order Details Report');
  }
