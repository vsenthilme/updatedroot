...
import { InhouseTransferService } from "../../make&change/inhouse-transfer/inhouse-transfer.service";
  ...
  export class TransferReportComponent implements OnInit {
    ...
    @ViewChild('transferTag') transferTag: Table | any;
     ...
     cols: any[] = [
      ...
      { header: 'Source Part No', field: 'sourceItemCode' , format: 'normal', showFooter: false },
      { header: 'Source Storage Bin', field: 'sourceStorageBin' , format: 'normal', showFooter: false },
      { header: 'Target Part No', field: 'targetItemCode' , format: 'normal', showFooter: false },
      { header: 'Target Storage Bin', field: 'targetStorageBin' , format: 'normal', showFooter: false },
      { header: 'Transfer Confirmed Qty', field: 'transferConfirmedQty' , format: 'normal', showFooter: true },
      { header: 'Barcode', field: 'barcodeId' , format: 'normal', showFooter: false },
      { header: 'Confirmed By', field: 'confirmedBy' , format: 'normal', showFooter: false },
      { header: 'Confirmed On', field: 'confirmedOn' , format: 'date', showFooter: false }
    ];
    ...
    calculateTotals(data?: any[]): void {
      ...
      if (data && data.length > 0) {
        this.totalInventoryQuantity = data.reduce((total, item) => total + (item.transferConfirmedQty != null ? item.transferConfirmedQty : 0), 0);
      } else {
        this.totalInventoryQuantity = 0;
      }
    }
  ...
  obj.stockTypeId=this.targetStockTypeId;
      ...
      downloadexcel() {
        ...
        this.cs.exportAsExcel(exportData, 'Transfer');
      }
