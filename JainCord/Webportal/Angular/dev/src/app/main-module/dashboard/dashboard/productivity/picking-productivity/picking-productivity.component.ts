...
@Component({
  ...
  templateUrl: './picking-productivity.component.html',
  styleUrls: ['./picking-productivity.component.scss']
})
export class PickingProductivityComponent implements OnInit {
  ...
  dateChoosed(event: any = null) {
    ...
    this.spin.show();
    this.report.pickingReportDashboard(obj).subscribe((res: any[]) => {
      if (res) {
  ...
  bindChart(chartData) {
    ...
    Highcharts.chart('container123', this.chartOptions); 
  }
}
