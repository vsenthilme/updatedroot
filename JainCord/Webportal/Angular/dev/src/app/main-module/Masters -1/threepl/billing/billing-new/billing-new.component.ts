...
// this.moduleList=this.cas.forLanguageFilter(results[3],this.cas.dropdownlist.setup.moduleid.key);
 ...
 this.languageList=this.cas.foreachlist2(results[9],this.cas.dropdownlist.setup.languageid.key); 
    ...
    onpartnertytpeChange(value) {
      console.log(value); 
      console.log(this.businesspartnerList);
      console.log(this.businesspartnerList.find(partner=>partner.partnerCode))
      const partner = this.businesspartnerList.find(partner => partner.value === value);
  
      console.log(partner); 
  
      if (partner) {
         
          this.form.controls.referenceField9.patchValue(partner.label);
      } else {
  ...
  if (this.js.code) {
    this.sub.add(this.service.Update(this.form.getRawValue(),this.js.code,this.js.warehouseId,this.js.languageId,this.js.plantId,this.js.companyCodeId,this.js.moduleId).subscribe(res => {
      this.toastr.success(res.billModeId + " updated successfully!","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
