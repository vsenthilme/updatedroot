...
export class PartnerComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog,
   ...
   // private excel: ExcelService,
    ...
    cols: any[] = [
      { header: 'Language', field: 'languageId', format: 'normal'},
      { header: 'Company', field: 'companyCodeId', format: 'normal'},
      { header: 'Plant', field: 'plantId', format: 'normal'},
      { header: 'Warehouse', field: 'warehouseId', format: 'normal'},
      { header: 'Partner', field: 'businessPartnerCode', format: 'normal'},
      { header: 'Mfr Name', field: 'manufacturerName', format: 'name'},
      { header: 'Partner Type', field: 'businessPartnerType', format: 'normal'},
      { header: 'Part No', field: 'referenceField10', format: 'no'},
      { header: 'Barcode', field: 'partnerItemBarcode', format: 'normal'},
      { header: 'Created By', field: 'createdBy', format: 'normal'},
  ...
  deleterecord(result) {
    ...
    obj.languageId = this.auth.languageId;
    obj.warehouseId = this.auth.warehouseId;
    obj.itemCode=this.selecteditemtype[0].itemCode;
    obj.manufacturerName=this.selecteditemtype[0].manufacturerName;
    obj.partnerItemBarcode=this.selecteditemtype[0].partnerItemBarcode;
    this.spin.show();
    this.sub.add(this.service.DeleteV2([obj]).subscribe((res) => {
      this.toastr.success(this.selecteditemtype[0].businessPartnerCode + " Deleted successfully.","Notification",{
        timeOut: 2000,
        progressBar: false,
      });
      this.spin.hide();
