...
@Component({
  selector: 'app-masteroperations',
  templateUrl: './masteroperations.component.html',
  styleUrls: ['./masteroperations.component.scss']
})
...
selectedcurrency : any;
  ...
  deleterecord(id: any) {
    ...
    this.sub.add(this.service.DeleteMasterOperations(id.operationNumber,id.warehouseId,id.plantId,id.companyCodeId,id.languageId).subscribe((res) => {
      
      console.log(id);
      ;
      this.toastr.success(id.operationNumber + " Deleted successfully.","",{
        timeOut: 2000,
        progressBar: false,
      });
  ...
  openDialog(data: any = 'New'): void {
    ...
    let paramdata = "";
    paramdata = this.cs.encrypt({ pageflow: data, code: data != 'New' ? this.selectedcurrency[0].operationNumber : null,warehouseId: data != 'New' ? this.selectedcurrency[0].warehouseId : null,languageId: data != 'New' ? this.selectedcurrency[0].languageId : null, plantId: data != 'New' ? this.selectedcurrency[0].plantId : null,companyCodeId: data != 'New' ? this.selectedcurrency[0].companyCodeId : null});
    this.router.navigate(['/main/other-masters/masteroperationnew/' + paramdata]);
  }
  ngOnDestroy() {
    if (this.sub != null) {
...
search(ispageload = false) {
 ...
 this.service.searchMasterOperation(this.searhform.value).subscribe(res => {
...
if(res){
  res = this.cs.removeDuplicatesFromArrayList(res , 'operationNumber')
 this.currrencys = res;
