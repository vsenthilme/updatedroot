...
cols :any;
  ...
  this.cols = [
    ...
    { header: 'Preinbound No', field: 'preInboundNo', format: 'normal' },
    { header: 'Order Received By', field: 'createdBy', format: 'normal' },
    { header: 'Order Received On', field: 'createdOn', format: 'date' }
  ]
  if (this.activeUrl[3] !== 'palletization-main') {
    this.cols.splice(4, 0,{ header: 'Pallet', field: 'palletId', format: 'link' });
  }
  if (this.activeUrl[3] == 'palletization-main') {
    this.cols.splice(4, 0,{ header: 'Order No', field: 'refDocNumber', format: 'link' });
  }
}
  activeUrl: any;
  ngOnInit(): void {
  ...
  downloadexcel() {
    const exportData = this.itemReceipt.map(item => {
    ...
    this.cs.exportAsExcel(exportData, 'GR Release');
  }
  ...
  openDialog(data: any = 'new', type?: any): void {
    ...
    if (this.activeUrl[3] == 'palletization-main') {
      ...
      if (this.selecteditemReceipt.length > 0) {
        ...
        this.router.navigate(['/main/inbound/palletization-create/' + paramdata]);
      }
      else {
  ...
  search(ispageload = false) {
    ...
    this.spin.show();
this.searhform.controls.statusId.patchValue([101]);
    this.grservice.searchLineSpark(this.searhform.value).subscribe(res => {
      let result = res.filter((x: any) => x.warehouseId == this.warehouseId);
      if (ispageload) {
        let tempcaseCodeList: any[] = []
        const caseCode = [...new Set(res.map(item => item.caseCode))].filter(x => x != null)
      ...
      this.itemReceipt = result;
      this.itemReceipt = this.cs.removeDuplicatesFromArrayList(this.itemReceipt, 'palletId');
      this.spin.hide();
    }, err => {
  ...
  printLabel1() {
    ...
    this.grservice.searchLineSpark({
      stagingNo: [this.selecteditemReceipt[0].stagingNo],
      caseCode: [this.selecteditemReceipt[0].caseCode],
      companyCodeId: [this.auth.companyId],
      plantId: [this.auth.plantId],
      warehouseId: [this.auth.warehouseId],
      languageId: [this.auth.languageId],
      ...
      let dataa: any[] = [];
      filteredRes.forEach(x => {
        for (let j = 0; j < (1); j++) { // for (let j = 0; j < (x.orderQty ?? 1); j++) {
          let obj: any = {};
