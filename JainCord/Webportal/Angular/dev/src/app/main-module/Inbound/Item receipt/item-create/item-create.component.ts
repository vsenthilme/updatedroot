...
export class ItemCreateComponent implements OnInit {
    ...
    if (data.pageflow != 'New') {
      ...
      this.sub.add(this.grservice.searchLineSpark({
       // stagingNo: [data.code.stagingNo],
        ...
        const filteredRes = res; //.filter((x: any) => x.statusId != 17 && x.statusId != 17);
  ...
  submitInterim() {
    this.spin.show();
    this.grNew.forEach(line => {
      if (!this.selectedgrNew.some(item2 => item2.itemCode === line.itemCode)) {
        console.log(line.itemCode)
        line.grLineFlag = 2;
      }
    })

    this.service.CreateLine(this.grNew).subscribe(res => {
      this.toastr.success("GR Line created successfully", "Notification",
        {
          timeOut: 2000,
          progressBar: false,
        });
