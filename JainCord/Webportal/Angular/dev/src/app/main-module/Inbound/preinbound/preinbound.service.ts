...
export class PreinboundService {
  ...
  createinterwarehouseTransferOrder(obj: any) {
    return this.http.post<any>('/wms-transaction-service/warehouse/inbound/interWarehouseTransferIn/v2', obj);
  }
  ...
  public inboundUpload(file: File,  apiName:any) {
    let formParams = new FormData();
    formParams.append('file', file)
    return this.http.post(`/wms-transaction-service/warehouse/inbound/upload/v4` +'?languageId='+this.auth.languageId+'&companyCodeId='+this.auth.companyId+'&plantId='+this.auth.plantId+'&warehouseId='+this.auth.warehouseId+'&loginUserID='+this.auth.userID 
      +'&inboundOrderTypeId='+apiName, formParams)
  }
