...
export class LabelGenerateComponent implements OnInit {
    ...
    this.batchService.search(obj).subscribe(res => {
      ...
      if (res.length != 0) {
        ...
        if (res[0].shelfLifeIndicator == true) {
          ...
          this.data.line['reminder'] = res[0].referenceField1;
  ...
  findDimensions() {
    ...
    this.basicdataService.search1(obj).subscribe(res => {
      ...
      if (res[0].shelfLifeIndicator == true) {
        this.showexpirtaiondate = true;
        this.data.line['period'] = res[0].referenceField1;
        this.data.line['numberPeriod'] = res[0].referenceField2;
      }
  ...
  manufactureDateChanged(e: any) {

  }

  addDuration(date: any): void {

    const tempTimestamp = new Date(date.value);
    const unit = this.data.line.period;
    const duration = this.data.line.numberPeriod;

    switch (unit) {
      case 'Months':
        tempTimestamp.setMonth(tempTimestamp.getMonth() + parseInt(duration));
        break;
      case 'Weeks':
        tempTimestamp.setDate(tempTimestamp.getDate() + (parseInt(duration) * 7));
        break;
      case 'Years':
        tempTimestamp.setFullYear(tempTimestamp.getFullYear() + parseInt(duration));
        break;
      case 'Days':
        tempTimestamp.setDate(tempTimestamp.getDate() + parseInt(duration));
        break;
      default:
        console.error('Invalid unit');
        return;
    }
    this.data.line['expiryDate'] =  this.cs.day_callapi(tempTimestamp);

    if(this.data.line.reminder){
      const reminderDate = new Date(tempTimestamp);
      reminderDate.setDate(reminderDate.getDate() - parseInt(this.data.line.reminder));
      this.data.line['reminderDate'] = this.cs.day_callapi(reminderDate);
    }
  }
}
