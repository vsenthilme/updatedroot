...
screenid=3044;
  ...
  callTableHeader() {
  this.cols = [
      { header: 'Actions', field: 'action', format: 'action' },
      ...
      { header: 'Order Details', field: 'details', format: 'details'}, // Only shown for userType 1
    ];
    }
    downloadexcel() {
      const exportData = this.preInbound.map(item => {
      ...
      this.cs.exportAsExcel(exportData, 'Preinbound');
    }
...
location:any;
  ...
  async download(element) {
    this.spin.show()
  if((element == 'RMINBOUND') ||(element == 'SUPPLIERINVOICE') ){
    this.fileName='ASNV2.xlsx'
    this.location='/document/rminbound'
  }
    const blob = await this.service.download(this.fileName,this.location)
  ...
  uploadDocument(event){
    this.spin.show();
    this.service.inboundUpload(this.file, this.selectedContext).subscribe(   //[this.selectedContext]
      (resp: any) => {
  ...
  openOrderSatus(){
      ...
      const dialogRef = this.dialog.open(DynamicPopupComponent, {
        ...
        maxWidth: '80%',
        position: { top: '9%', },
        data: { cols: cols, title: 'Inbound Order Status', pageFrom: 'inboundOrder' },
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          this.search();
