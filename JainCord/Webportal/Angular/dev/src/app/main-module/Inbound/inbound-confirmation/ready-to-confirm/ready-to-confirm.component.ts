...
import { DatePipe } from "@angular/common";
  ...
  export class ReadyToConfirmComponent implements OnInit {
    ...
    cols: any[] = [
      ...
      { header: 'Putaway Lines', field: 'receivedLines', format: 'normal' },
  ...
  this.spin.show();
      this.service.search(this.searhform.value).subscribe(res => {
        ...
        if (ispageload) {
          ...
          const inboundOrderTypeId = [...new Set(result.map(item => item.inboundOrderTypeId))] 
            .filter(x => x != null); // Remove any null values
          
          // Create an object for each unique inboundOrderTypeId value with 'value' set to the ID and 'label' set to the corresponding referenceDocumentType
          inboundOrderTypeId.forEach(x => {
            const correspondingItem = result.find(item => item.inboundOrderTypeId === x);
            if (correspondingItem) {
              tempInboundOrderTypeIdList.push({ value: x, label: correspondingItem.referenceDocumentType });
            }
          });
