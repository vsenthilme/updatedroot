...
export class PutawayMainComponent implements OnInit {
  ...
  cols: any[] = [
    ...
    { header: 'Order Type', field: 'referenceDocumentType' ,format: 'type' }, 
    { header: 'Part No', field: 'referenceField5',format: 'normal' }, 
    { header: 'Order No', field: 'refDocNumber' ,format: 'link' }, 
    { header: 'Putaway No', field: 'putAwayNumber',format: 'type' }, 
    { header: 'Barcode', field: 'barcodeId',format: 'type' }, 
    { header: 'Storage Bin', field: 'proposedStorageBin',format: 'type' }, 
    { header: 'Assign User', field: 'assignedUserId',format: 'type' }, 
    { header: 'Level', field: 'levelId',format: 'type' }, 
    { header: 'TO Qty', field: 'putAwayQuantity',format: 'type' }, 
    { header: 'Created By', field: 'createdBy' ,format: 'normal' }, 
    { header: 'Created On', field: 'createdOn' ,format: 'date' }, 
    { header: 'Remarks', field: 'remark' ,format: 'normal' }
  ];
  ngOnInit(): void {
  ...
  downloadexcel() {
    const exportData = this.putaway.map(item => {
    ...
    this.cs.exportAsExcel(exportData, 'Putaway');
  }
  ...
  search(ispageload = false) {
    ...
    this.service.searchSpark(this.searhform.value).subscribe(
      (res) => {
        ...
        this.spin.hide();
          res = this.cs.removeDuplicatesFromArrayList(res , 'palletId')
          this.putaway = res;
      },
      (err) => {
