...
export class InboundCreateComponent implements OnInit {
  ...
  cols: any[] = [
    ...
    { header: "Line No", field: "lineNo" ,format: "normal" },
    { header: "Invoice No", field: "refDocNumber" ,format: "normal" },
    { field: 'manufacturerName', header: 'Mfr Name', format: 'normal' },
    { header: "Part No", field: "itemCode" ,format: "normal" },
    { field: 'description', header: 'Description', format: 'normal' }, 
    { field: 'batchSerialNumber', header: 'Batch Serial Number', format: 'normal' },  
    { header: "Order Qty", field: "orderQty" ,format: "normal" },
    { header: "Received Qty", field: "acceptedQty" ,format: "normal" },
    { header: "Rejected Qty", field: "damageQty" ,format: "normal" },
    { header: "Variance", field: "varianceQty" ,format: "normal" },
    { header: "Barcode ID", field: "barcodeId" ,format: "normal" },
    { header: "Final Confirmation", field: "referenceField2" ,format: "checkbox" },
    { header: "Remarks", field: "referenceField1" ,format: "remark" }, // Only if `pageflow1` is 'GR Confirmation'
    { header: "Putaway Details", field: "putaway" ,format: "putaway" },
    { header: "% of Quantity", field: "qtyreceived" ,format: "normal" },
  ];
    ...
    if (data.pageflow != 'New') {
      ...
      this.sub.add(this.service.Get(data.code.warehouseId, data.code.preInboundNo, data.code.refDocNumber, this.auth.companyId, data.code.languageId, data.code.plantId).subscribe(res => {
        ...
        res.inboundLine.forEach((x: any) => {
          ...
          x.referenceField2 = x.varianceQty == 0 || x.statusId == 24 ? true : false;
  ...
  confirmPopup(dataErrorArray) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result == 'Yes') {
        ...
        this.updateInboundLine();
      }
    });
  }

  confirmPartial(event, line, rowIndex) {
    if(event.checked ==  true){
      const dialogRef = this.dialog.open(CustomdialogComponent, {
        ...
        data: { title: "GR Confirm", body: "Final Confirmation cannot be reverted, Do you still want to confirm?" },
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result == 'Yes') {
        }else{
          console.log(rowIndex);
          line.referenceField2 = false;
          this.inbound.splice(rowIndex,1);
          console.log(this.inbound)
          this.inbound.splice(rowIndex, 0, line);
          console.log(this.inbound)
          console.log(line)
        }
      });
  ...
  updateInboundLine() {
    ...
    let dataOrderQtyValidation: any = [];
    this.inbound.forEach(data => {
      if (data.statusId != 20 && data.statusId != 24) {
        dataErrorArray.push(data.lineNo);
      }
      if ((data.acceptedQty + data.damageQty) > data.orderQty) {
        dataOrderQtyValidation.push(data.lineNo)
      }
    });
    ...
    if (dataOrderQtyValidation.length > 0) {
      this.toastr.error(
        "Putaway Qty is greater than order qty in line no : " + dataOrderQtyValidation,
        "Notification", {
        timeOut: 2000,
        progressBar: false,
      }
    ...
    let updateConfirmedLines: any[] = [];
    updateConfirmedLines = this.inbound.filter(x => x.referenceField2 == true && x.statusId != 24);
    
    if (updateConfirmedLines.length == 0) {
      this.toastr.error(
        "No lines selected for confirmation",
        "Notification", {
        timeOut: 2000,
        progressBar: false,
      }
    ...
    this.spin.show();
    this.service.partialConfirmationv2(updateConfirmedLines[0],updateConfirmedLines).subscribe(res => {
      this.toastr.success(this.code.refDocNumber + " Confirm Successfully.", "", {
        timeOut: 2000,
        progressBar: false,
      })
