...
cols: any[] =  [
  { header: 'Actions', field: 'actions', format: 'actions' },
  { header: 'Branch', field: 'plantDescription', format: 'normal' },
  { header: 'Warehouse', field: 'warehouseDescription', format: 'normal' },
  { header: 'Target Branch', field: 'targetBranchCode', format: 'target' }, // Conditionally hide
  ...
  search(ispageload = false) {
    ...
    this.service.searchSpark(this.searhform.value).subscribe(res => {
      ...
      res = this.cs.removeDuplicatesFromArrayList(res , 'preOutboundNo')
      this.quality = res;
