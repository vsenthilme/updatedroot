...
export class DeliveryMainComponent implements OnInit {
  ...
  cols: any[] = [
    ...
    { header: 'Target Branch', field: 'targetBranchCode', format: 'normal' },
    { header: 'Sales Order No', field: 'salesOrderNumber', format: 'normal' },
    { header: 'Customer Code', field: 'partnerCode', format: 'normal' },
    { header: 'Customer Name', field: 'partnerName', format: 'normal' },
    { header: 'Status', field: 'statusDescription', format: 'normal' },
    { header: 'Order No', field: 'refDocNumber', format: 'link' },
    { header: 'Doc', field: 'doc', format: 'doc' },
    { header: 'Order Type', field: 'referenceDocumentType', format: 'normal' },
    { header: 'Order Lines', field: 'referenceField10', format: 'normal' },
    { header: 'Order Qty', field: 'referenceField9', format: 'number' },
    { header: 'Delivered Lines', field: 'referenceField8', format: 'normal' },
    { header: 'Delivered Qty', field: 'referenceField7', format: 'number' },
    { header: 'Order Date', field: 'refDocDate', format: 'date' },
    { header: 'Delivery Date', field: 'deliveryConfirmedOn', format: 'date' },
    { header: 'Lead Time', field: 'dd', format: 'dd' },
    { header: 'Order Category', field: 'referenceField1', format: 'normal' }
  ];
  ...
  searhform = this.fb.group({
    ...
    endDeliveryConfirmedOnFE: [new Date()],
    endOrderDate: [],
    endOrderDateFE: [],
    ...
    startDeliveryConfirmedOnFE: [new Date()],
    startOrderDate: [],
    startOrderDateFE: [],
  ...
  openDialog(data: any = 'new', type ? : any): void {
    if (type && type != undefined) {
      ...
      this.selecteddelivery.push(type);
    }
    if (data != 'New')
    ...
    if (this.selecteddelivery.length > 0) {
      paramdata = this.cs.encrypt({ code: this.selecteddelivery[0], pageflow: data,pageflow1:'Delivered' });
      this.router.navigate(['/main/outbound/delivery-confirm/' + paramdata]);
    }
    else {
      paramdata = this.cs.encrypt({ pageflow: data,pageflow1:'Delivered' });
      this.router.navigate(['/main/outbound/delivery-confirm/' + paramdata]);
    }
  }
  openDialog1(data: any = 'New'): void {
    console.log(this.selecteddelivery)
      ...
      const dialogRef = this.dialog.open(DeliveryUpdateComponent, {
        disableClose: true,
        width: '30%',
        maxWidth: '80%',
        data: { pageflow: data, code:this.selecteddelivery}
      });
  ...
  openDialogDisplay(data: any = 'new'): void {
    let paramdata = "";
  ...
  openConfirm(data: any) {
    ...
    let paramdata = this.cs.encrypt({ code: data, pageflow: 'Edit',pageflow1: 'Delivered' });
    this.router.navigate(['/main/outbound/delivery-confirm/' + paramdata]);
  ...
  downloadexcel() {
    const exportData = this.delivery.map(item => {
    ...
    this.cs.exportAsExcel(exportData, 'Delivered');
  }
  ...
  generatePdf(element: any) {
    this.spin.show();
    this.sub.add(this.service.getOtBoundDeliveryFormPdfv2(element.refDocNumber, this.auth.warehouseId,this.auth.companyId,this.auth.plantId,this.auth.languageId,element.preOutboundNo).subscribe((res: any) => {
      //this.spin.hide();
      ...
      if (res != null) {
        ...
        let headerTable: any[] = [];
        headerTable.push([
          { image: indusLogo.headerLogo, fit: [70, 70], bold: true, fontSize: 9, border: [false, false, false, false] },
        ...
        headerTable.push([
          { text: 'Order Category :' + res[0].orderType == null ? 'Order Category:' : 'Order Category:' + res[0].orderType , bold: true, fontSize: 9, border: [false, false, false, false] },
          { text: '', bold: true, fontSize: 9, border: [false, false, false, false] },
        ...
        let bodyArray: any[] = [];
        bodyArray.push([
          { text: 'Order No', bold: true, fontSize: 9, border: [false, false, false, true] },
          { text: 'Part No', bold: true, fontSize: 9, border: [false, false, false, true] },
          { text: 'Description', bold: true, fontSize: 9, border: [false, false, false, true] },
          { text: 'Mfr Name', bold: true, fontSize: 9, border: [false, false, false, true] },
            ...
            columns: [
              {
                stack: [
                  ...
                  { text: 'Almailem Name & Signature :', fontSize: 9, bold: true, lineHeight: 2 },
