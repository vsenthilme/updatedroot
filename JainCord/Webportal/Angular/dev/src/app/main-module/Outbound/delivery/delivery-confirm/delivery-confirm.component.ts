...
export class DeliveryConfirmComponent implements OnInit {
 ...
 cols: any[] = [
    ...
    { header: 'Quality Date', field: 'qualityCreatedOn', format: 'date' },
...
this.pageflow1=data.pageflow1;
     console.log(this.pageflow1);
      ...
      this.spin.show();
      this.sub.add(this.service.searchLinev2({ preOutboundNo: [this.code.preOutboundNo], refDocNumber: [this.code.refDocNumber], warehouseId: [this.code.warehouseId] ,companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],languageId:[this.auth.languageId]}).subscribe(res => {
  ...
  confirm() {
    ...
    this.spin.show();
    this.sub.add(this.service.confirmv2(this.code).subscribe(res => {
