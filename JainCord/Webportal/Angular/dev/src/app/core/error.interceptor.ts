...
export class ErrorInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    return next.handle(request).pipe(catchError(error => {
      if (error.status === 401 || error.status == 400 || error.status == 415) {
        if (request.url.includes('/auth-token') || (request.url.includes('wms-spark-service')) || (request.url.includes('mnr-spark-service'))|| (error.error != null && error.error.error.includes("Order is not completely Processed"))) {
  ...
  private addTokenHeader(request: HttpRequest<any>, token: string) {
   ...
   if (request.url.includes('upload') &&  request.method == 'POST')
      return request.clone({  url: `${request}${request.url}` });
