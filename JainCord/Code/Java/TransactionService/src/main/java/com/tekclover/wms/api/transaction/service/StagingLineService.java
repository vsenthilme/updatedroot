...
//                }
    ...
    public List<StagingLineEntityV2> caseConfirmationV2(List<CaseConfirmation> caseConfirmations,
        ...
        if (!updatedStagingLineEntityList.isEmpty()) {
            ...
            if (createdGrHeader != null && createdGrHeader.getInboundOrderTypeId() != null && createdGrHeader.getInboundOrderTypeId() == 9) {  //ReturnOrderJainCord
                createGrLineV5(createdGrHeader, loginUserID);
            }
        }
...
//            }
    ...
    // Update_Staging_Line for Jain_Cord
    public List<StagingLineEntityV2> updateStagingLine(List<StagingLineEntityV2> stagingLineEntityV2List, String loginUserID) throws Exception {
        try {
<<<<<<< HEAD
        List<StagingLineEntityV2> stagingLineEntityList = new ArrayList<>();
...
<<<<<<< HEAD
        ...
        String refDocNo = stagingLineEntityList.stream().findFirst().get().getRefDocNumber();
        List<StagingLineEntityV2> records = stagingLineV2Repository.findByRefDocNumberAndDeletionIndicator(refDocNo, 0L);
=======
                stagingLineEntityList.add(staging);
            });
            ...
            List<StagingLineEntityV2> records = stagingLineV2Repository.findByRefDocNumberAndDeletionIndicator(refDocNo, 0L);
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e
            boolean allHaveStatus101 = records.stream().allMatch(record -> record.getStatusId() == 101L);
            if (allHaveStatus101) {
                // Get common values from the first record
                StagingLineEntityV2 firstRecord = records.get(0);

                // Update GR Header Status
                grHeaderV2Repository.updateGrHeaderStatus(
                        firstRecord.getWarehouseId(),
                        firstRecord.getCompanyCode(),
                        firstRecord.getPlantId(),
                        firstRecord.getLanguageId(),
                        refDocNo,
                        101L,
                        firstRecord.getStatusDescription()
                );
                log.info("GrHeader Status Update Successfully for refDocNo: " + refDocNo);
            } else {
                log.warn("Not all records for refDocNo " + refDocNo + " have statusId = 101. Skipping GR Header update.");
            }
<<<<<<< HEAD
        return stagingLineEntityList;
=======
...
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e
     ...
     * @throws Exception
     */
    public void createGrLineV5(GrHeaderV2 grHeader, String loginUserId) throws Exception {

        String companyCodeId = grHeader.getCompanyCode();
        String plantId = grHeader.getPlantId();
        String languageId = grHeader.getLanguageId();
        String warehouseId = grHeader.getWarehouseId();
        String refDocNo = grHeader.getRefDocNumber();
        String preInboundNo = grHeader.getPreInboundNo();
<<<<<<< HEAD
        log.info("GrLine Create Started -----------------------------------------> " + grHeader);
=======
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e

        List<StagingLineEntityV2> stagingLineEntityList = getStagingLineForGrLine(companyCodeId, plantId, languageId, warehouseId, grHeader.getStagingNo(), grHeader.getCaseCode());

        Long statusId = 101L;
        statusDescription = getStatusDescription(statusId, languageId);
        stagingLineV2Repository.updateStagingLineStatus(companyCodeId, plantId, languageId, warehouseId, refDocNo, preInboundNo, statusId, statusDescription, loginUserId, new Date());
        grHeaderV2Repository.updateGrHeaderStatus(warehouseId, companyCodeId, plantId, languageId, refDocNo, statusId, statusDescription);

        String hhtUser = getHHTUser(companyCodeId, plantId, languageId, warehouseId);
        List<AddGrLineV2> newGrLineList = new ArrayList<>();
        ...
        for (StagingLineEntityV2 dbStagingLine : stagingLineEntityList) {
            ...
            PickupLineV2 dbPickupLine = pickupLineService.getPickupLineV5(companyCodeId, plantId, languageId, warehouseId, grHeader.getRefDocNumber(), grHeader.getParentProductionOrderNo());
            log.info("PickupLine : " + dbPickupLine);

            BeanUtils.copyProperties(dbStagingLine, newGrLine, CommonUtils.getNullPropertyNames(dbStagingLine));

            String idMasterToken = getIDMasterAuthToken();
            long NUM_RAN_ID = 6;
            String nextRangeNumber = getNextRangeNumber(NUM_RAN_ID, companyCodeId, plantId, languageId, warehouseId, idMasterToken);
            ...
            newGrLine.setReferenceDocumentType(dbStagingLine.getReferenceDocumentType());
            newGrLine.setPurchaseOrderNumber(dbStagingLine.getPurchaseOrderNumber());
            newGrLine.setInterimStorageBin(dbStagingLine.getMiddlewareTable());
            newGrLine.setAssignedUserId(hhtUser);
            newGrLine.setGrLineFlag("1");

            newGrLine.setPackBarcodes(packBarcodeList);
            if (dbPickupLine != null) {
                newGrLine.setBarcodeId(dbPickupLine.getBarcodeId());
//                newGrLine.setInterimStorageBin(dbPickupLine.getPickedStorageBin());
                newGrLine.setPalletId(dbPickupLine.getPalletId());
                newGrLine.setLevelId(dbPickupLine.getLevelId());
                dbStagingLine.setBarcodeId(dbPickupLine.getBarcodeId());
            }

            newGrLineList.add(newGrLine);

            inventoryService.createInventoryNonCBMStagingLineV2(dbStagingLine, loginUserId);
        }
        grLineService.createGrLineNonCBMV2(newGrLineList, loginUserId);
        log.info("GrLine Created Successfully!");
    }
