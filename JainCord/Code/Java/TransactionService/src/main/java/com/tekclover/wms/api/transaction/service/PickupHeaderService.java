                ...
                for (IKeyValuePair pickup : notification) {
                    ...
                    if (deviceToken != null && !deviceToken.isEmpty()) {
                        ...
                        String response = pushNotificationService.sendPushNotificationWithUserId(deviceToken, title, message,
                                pickupHeaderV2.getLanguageId(), pickupHeaderV2.getCompanyCodeId(), pickupHeaderV2.getPlantId(),
                                pickupHeaderV2.getWarehouseId(), pickupHeaderV2.getRefDocNumber(), pickup.getAssignPicker());
...
//        }
    ...
    public PickupHeaderV2 updatePickupHeaderV2(String companyCodeId, String plantId, String languageId, String warehouseId, String preOutboundNo, String refDocNumber,
        ...
        if (dbPickupHeader != null) {
            ...
            // send Notification
            if (pickup != null) {
                sendNotificationForUpdate(pickup.getRefDocNumber(), pickup.getAssignedPickerId(),
                        pickup.getLanguageId(), pickup.getCompanyCodeId(), pickup.getPlantId(),
                        pickup.getWarehouseId(), pickup.getReferenceDocumentType());
            }
            return pickup;
        }
        return null;
...
//    }
        ...
        try {
            ...
            for (PickupHeaderV2 data : updatePickupHeaderList) {
                ...
                if (dbPickupHeader != null) {
                    ...
                    // Send Notification
                    sendNotificationForUpdate(data.getRefDocNumber(), data.getAssignedPickerId(),
                            data.getLanguageId(), data.getCompanyCodeId(), data.getPlantId(),
                            data.getWarehouseId(), data.getReferenceDocumentType());
                    pickupHeaderList.add(pickupHeader);
                } else {
                    log.info("No record for PickupHeader object from db for data : " + data);
    ...
    // SendNotification
    public void sendNotificationForUpdate(String refDocNo, String assignPickerId, String languageId, String companyId,
                                          String plantId, String warehouseId, String refDocType)
            throws FirebaseMessagingException {

        //withoutInput
        List<IKeyValuePair> notification = pickupHeaderV2Repository.findByStatusIdAndNotificationStatusAndDeletionIndicatorDistinctRefDocNo();
        ...
        if (deviceToken != null && !deviceToken.isEmpty()) {
            ...
            String response = pushNotificationService.sendPushNotificationWithUserId(deviceToken, title, message, languageId,
                    companyId, plantId, warehouseId, refDocNo, assignPickerId);
            if (response.equals("OK")) {
        ...
        if (notification != null) {
            for (IKeyValuePair pickupHeaderV2 : notification) {
                ...
                if (token != null && !token.isEmpty()) {
                    ...
                    String response = pushNotificationService.sendPushNotificationWithUserId(token, title, message, languageId,
                            companyId, plantId, warehouseId, refDocNo, pickupHeaderV2.getAssignPicker());
                    if (response.equals("OK")) {
                        pickupHeaderV2Repository.updateNotificationStatus(
     ...
     * @return
     */
    public boolean getPickupHeaderCountForDeliveryConfirmationV5(String companyCodeId, String plantId, String languageId,
                                                                 String warehouseId, String refDocNumber, String preOutboundNo, Long statusId) {
