...
//                        }
    ...
    private void fireBaseNotification(PutAwayLineV2 putAwayLine, String loginUserID) {
        try {
            List<String> deviceToken = pickupHeaderV2Repository.getDeviceToken(putAwayLine.getCompanyCode(), putAwayLine.getPlantId(), putAwayLine.getLanguageId(), putAwayLine.getWarehouseId());
...
//        }
    ...
    public List<PutAwayLineV2> putAwayLineConfirmNonCBMV2(@Valid List<PutAwayLineV2> newPutAwayLines, String loginUserID) {
        ...
        if(newPutAwayLines == null || newPutAwayLines.isEmpty()) {
            throw new BadRequestException("Empty Lines cannot be Processed");
        }
        ...
        try {
            for (PutAwayLineV2 newPutAwayLine : newPutAwayLines) {
                ...
                if (dbStagingLineEntity != null) {
                    ...
                    newPutAwayLine.setColor(dbStagingLineEntity.getColor());
                    newPutAwayLine.setGrade(dbStagingLineEntity.getGrade());
                    newPutAwayLine.setGsm(dbStagingLineEntity.getGsm());
                    newPutAwayLine.setLotNo(dbStagingLineEntity.getLotNo());
                    newPutAwayLine.setMeter(dbStagingLineEntity.getMeter());
                    newPutAwayLine.setPieceId(dbStagingLineEntity.getPieceId());
                    newPutAwayLine.setSortNo(dbStagingLineEntity.getSortNo());
                }
                ...
                if (putAwayHeader != null) {
                    ...
                    if(dbPutAwayLine.getBarcodeId() == null) {
                        dbPutAwayLine.setBarcodeId(putAwayHeader.getBarcodeId());
                    }
                } else {
                    dbPutAwayLine.setCreatedOn(new Date());
                }
...
//                    fireBaseNotification(createdPutAwayLine, loginUserID);
                    ...
                    // Inventory_Update
//                    createInventoryNonCBMV2(createdPutAwayLine);
//                    createInventoryMovementV2(createdPutAwayLine);
                    if (createdPutAwayLine != null && createdPutAwayLine.getPutawayConfirmedQty() > 0L) {

                        inventoryService.createInventoryNonCBMPutAwayLineV2(createdPutAwayLine, loginUserID);
                        ...
                        if (putAwayHeader != null) {
                            ...
                            putAwayHeader.setStatusId(20L);
                            putAwayHeader.setStatusDescription(statusDescription);
                            putAwayHeader = putAwayHeaderV2Repository.saveAndFlush(putAwayHeader);
...
//                                    Double PUTAWAY_QTY = (putAwayHeader.getPutAwayQuantity() != null ? putAwayHeader.getPutAwayQuantity() : 0) - (createdPutAwayLine.getPutawayConfirmedQty() != null ? createdPutAwayLine.getPutawayConfirmedQty() : 0);
                        ...
                        grLineV2Repository.updateGrLineStatusV5(companyCode, plantId, languageId, warehouseId, refDocNumber, preInboundNo,
                                createdPutAwayLine.getLineNo(), createdPutAwayLine.getItemCode(), 20L, statusDescription);
                    }
                } else {
                    log.info("Putaway Line already exist : " + existingPutAwayLine);
                }
    ...
    // Create_Inventory
    public InventoryV2 createInventoryNonCBMV2(String companyCode, String plantId, String languageId, String warehouseId, String itemCode,
                                               String manufacturerName, String refDocNumber, PutAwayLineV2 putAwayLine, String loginUserID) {

        try {
            InventoryV2 dbInventory = null;
            String packparcode = putAwayLine.getBatchSerialNumber();
            if (putAwayLine.getBatchSerialNumber() != null) {
                dbInventory = inventoryV2Repository.findTopByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndItemCodeAndManufacturerNameAndPackBarcodesAndBinClassIdAndBatchSerialNumberAndDeletionIndicatorOrderByInventoryIdDesc(
                        companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName, packparcode, 3L, putAwayLine.getBatchSerialNumber(), 0L);
            } else {
                dbInventory = inventoryV2Repository.findTopByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndItemCodeAndManufacturerNameAndPackBarcodesAndBinClassIdAndDeletionIndicatorOrderByInventoryIdDesc(
                        companyCode, plantId, languageId, warehouseId, itemCode, manufacturerName,
                        "99999", 3L, 0L);
            }
            ...
            if (dbInventory != null) {
                ...
                inventory.setInventoryQuantity(dbInventory.getInventoryQuantity() + putAwayLine.getInventoryQuantity());
                log.info("Inventory Qty = inv_qty + gr_qty: " + dbInventory.getInventoryQuantity() + ", " + putAwayLine.getInventoryQuantity());
                Double totalQty = 0D;
                if (inventory.getReferenceField4() != null) {
                    totalQty = inventory.getReferenceField4() + putAwayLine.getInventoryQuantity();
                }
                if (inventory.getReferenceField4() == null) {
                    totalQty = putAwayLine.getInventoryQuantity();
                }
...
//            if (dbInventory == null) {
            try {
                ...
                inventory.setBatchSerialNumber(manufacturerName);
//                inventory.setManufacturerDate(stagingLine.getManufacturerDate());
//                inventory.setExpiryDate(stagingLine.getExpiryDate());
                ...
                StorageBinV2 storageBin = storageBinService.getStorageBinByBinClassIdV2(
                        warehouseId, 3L, companyCode, plantId, languageId);
                ...
                // INV_QTY
                if (dbInventory != null) {
                    inventory.setInventoryQuantity(round(dbInventory.getInventoryQuantity() + putAwayLine.getInventoryQuantity()));
                    log.info("Inventory Qty = inv_qty + gr_qty: " + dbInventory.getInventoryQuantity() + ", " + putAwayLine.getInventoryQuantity());
                ...
                if (dbInventory == null) {
                    inventory.setInventoryQuantity(round(putAwayLine.getInventoryQuantity()));
                    log.info("Inventory Qty = gr_qty: " + putAwayLine.getInventoryQuantity());
                    inventory.setReferenceField4(round(inventory.getInventoryQuantity()));
                    log.info("Inventory Total Qty: " + inventory.getInventoryQuantity());   //Allocated Qty is always 0 for BinClassId 3
                }
                //packbarcode
...
//            createGrLineLog7(stagingLine, e.toString());
    ...
    private InventoryV2 createInventoryNonCBMV2(PutAwayLineV2 putAwayLine) {
        ...
        try {
            if (putAwayLine.getBarcodeId() != null) {
                existinginventory = inventoryV2Repository.findTopByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndItemCodeAndBarcodeIdAndManufacturerNameAndPackBarcodesAndBinClassIdAndDeletionIndicatorOrderByInventoryIdDesc(
                ...
                existinginventory = inventoryService.getInventoryForStockAdjustmentDamageV2(
                        putAwayLine.getCompanyCode(),
                        putAwayLine.getPlantId(),
                        putAwayLine.getLanguageId(),
                        putAwayLine.getWarehouseId(),
...
//            log.info("Create Inventory bin Class Id 1 Initiated: " + new Date());
            log.info("Create Inventory BinClassId 1 <---------------------> ");
            InventoryV2 inventory = new InventoryV2();
            BeanUtils.copyProperties(putAwayLine, inventory, CommonUtils.getNullPropertyNames(putAwayLine));
