...
public interface StagingLineRepository extends JpaRepository<StagingLineEntity, Long>,
    ...
    @Query(value = "SELECT TOP 1 * FROM tblstagingline WHERE REF_DOC_NO = :refDocNumber and is_deleted =0 ", nativeQuery = true)
    StagingLineEntityV2 findDistinctByRefDocNumberAndDeletionIndicator(@Param("refDocNumber") String refDocNumber);
}