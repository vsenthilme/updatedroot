...
<<<<<<< HEAD
                inboundIntegrationHeader.setPieceNo(dbOBOrder.getPieceNo());
=======
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e
...
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e
                    ...
                    inboundIntegrationLine.setBarcodeId(line.getPieceId());
...
<<<<<<< HEAD
                String pieceNo = inbound.getPieceNo();
                try {
                    log.info("InboundOrder ID : " + inbound.getRefDocumentNo());
                    if (pieceNo != null) {
                        InboundHeaderV2 inboundHeader = inboundOrderProcessingService.processInboundReceivedV2(inbound.getRefDocumentNo(), inbound, pieceNo);
                        if (inboundHeader != null) {
                            // Updating the Processed Status
//                        updateProcessedInboundOrderV2(refDocNumber, inboundOrderTypeId, 10L);
                            inboundOrderV2Repository.updateProcessStatusId(inbound.getRefDocumentNo(), pieceNo, 10L);
                            // Update ERP Record t_inbound
                            inboundEntityRepository.updateInbound(10L, refDocNumber);
                            inboundList.remove(inbound);
                            warehouseApiResponse = successResponse(warehouseApiResponse);
                        } else {
                            log.info("Piece No is null");
                            inboundList.remove(inbound);
                        }
                    } else {
                        updateProcessedInboundOrderV2(refDocNumber, inboundOrderTypeId, 100L);
                        inboundOrderV2Repository.updateProcessStatusId(inbound.getRefDocumentNo(), pieceNo, 100L);
                        // Update ERP Record t_inbound
                        inboundEntityRepository.updateInbound(100L, refDocNumber);
                        inboundList.remove(inbound);
...
=======
                try {
                    log.info("InboundOrder ID : " + inbound.getRefDocumentNo());
                    InboundHeaderV2 inboundHeader = inboundOrderProcessingService.processInboundReceivedV2(inbound.getRefDocumentNo(), inbound);
                    if (inboundHeader != null) {
                        // Updating the Processed Status
                        updateProcessedInboundOrderV2(refDocNumber, inboundOrderTypeId, 10L);
                        inboundList.remove(inbound);
...
//                        updateProcessedInboundOrderV2(refDocNumber, inboundOrderTypeId, 900L);
                        inboundOrderV2Repository.updateProcessStatusId(inbound.getRefDocumentNo(), pieceNo, 900L);
                    } else {
                        inboundOrderV2Repository.updateProcessStatusId(inbound.getRefDocumentNo(), pieceNo, 100L);
//                        updateProcessedInboundOrderV2(refDocNumber, inboundOrderTypeId, 100L);
                    }
            ...
            if (outboundList != null && !outboundList.isEmpty()) {
                ...
                for (OutboundIntegrationHeaderV2 outbound : spOutboundList) {
                    ...
                    try {
                        ...
                        OutboundHeaderV2 outboundHeader = outboundOrderProcessingService.processOutboundReceivedV5(outbound);
