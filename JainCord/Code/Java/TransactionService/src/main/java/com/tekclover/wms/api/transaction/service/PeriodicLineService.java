...
//					createInventoryMovement (updatedPeriodicLine) ;
     ...
     * @return
     */
    public List<PeriodicLineV2> updateAssingHHTUserV2(List<AssignHHTUserCC> assignHHTUsers, String loginUserID) throws FirebaseMessagingException {
        log.info("assignHHTUsers : " + assignHHTUsers);
...
<<<<<<< HEAD
//        if (dbPeriodicHeader != null) {
//            sendNotification(dbPeriodicHeader.getLanguageId(), dbPeriodicHeader.getCompanyCode(), dbPeriodicHeader.getPlantId(),
//                    dbPeriodicHeader.getWarehouseId(), dbPeriodicHeader.getCycleCountNo());
//        }
=======
        if (dbPeriodicHeader != null) {
            sendNotification(dbPeriodicHeader.getLanguageId(), dbPeriodicHeader.getCompanyCode(), dbPeriodicHeader.getPlantId(),
...
//        inventory.setInventoryId(System.currentTimeMillis());
        ...
        StorageBinV2 storageBin =
                storageBinService.getStorageBinByBinClassIdV2(
                        updatePeriodicLine.getWarehouseId(), 5L,
                        updatePeriodicLine.getCompanyCode(),
                        updatePeriodicLine.getPlantId(),
                        updatePeriodicLine.getLanguageId());
        inventory.setStorageBin(storageBin.getStorageBin());
...
//                                dbPeriodicLine.getManufacturerName());
    ...
    public void sendNotification(String languageId, String companyId, String plantId, String warehouseId, String processId) throws FirebaseMessagingException {
        List<IKeyValuePair> notification = periodicLineV2Repository.findByStatusIdAndNotificationStatusAndDeletionIndicatorDistinctRefDocNo();

        if (notification != null) {
            for (IKeyValuePair periodicHeader : notification) {
                // Get NotificationId
                IKeyValuePair notifyId = periodicLineV2Repository.getNotificationId(companyId, languageId, "2");
                ...
                List<String> userIds = periodicLineV2Repository.getUserId(companyId, languageId, notifyId.getUserRole());
                if (userIds.isEmpty()) {
                    log.warn("No Users found for the specified role: {}, companyId: {}", notifyId.getUserRole(), companyId);
                    return;
                }

                List<String> deviceToken = periodicLineV2Repository.getDeviceToken(languageId, companyId, plantId, warehouseId, userIds);

                if (deviceToken != null && !deviceToken.isEmpty()) {
                    String title = "PERIODIC";
                    String message = periodicHeader.getRefDocType() + " ORDER - " + periodicHeader.getRefDocNumber() + " - IS ASSIGNED";
                    String response = pushNotificationService.sendPushNotificationWithUserIdList(deviceToken, title, message,
                            languageId, companyId, plantId, warehouseId, processId, userIds);
                    if (response.equals("OK")) {
                        periodicLineV2Repository.updateNotificationStatus(periodicHeader.getRefDocNumber(), periodicHeader.getWarehouseId());
...
<<<<<<< HEAD
        log.info("ExecuteStockCount API Inventory Response <--------------------->" + dbInventoryList);
=======
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e
        if (dbInventoryList != null && !dbInventoryList.isEmpty()) {
            ...
            for (IInventoryImpl dbInventory : dbInventoryList) {
                ...
                periodicLineV2.setItemDesc(dbInventory.getItemTypeDescription());
