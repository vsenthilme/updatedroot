...
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e
    ...
    // GrLine Create Generate PackBarCode
    public List<GrLineV2> createGrLineGenerateBarCodes(List<AddGrLineV2> newGrLines, String loginUserID) throws Exception {
        List<AddGrLineV2> grLineV2List = new ArrayList<>();
<<<<<<< HEAD
        log.info("Entering createGrLineGenerateBarCodes with {} GR lines for user: {}", newGrLines.size(), loginUserID);
            for (AddGrLineV2 grLineV2 : newGrLines) {
                Double acceptedQty = null;
                Double damagedQty = null;

                log.info("Processing GR line with RefDocNumber: {}", grLineV2.getRefDocNumber());
                StagingLineEntityV2 stagingLine = stagingLineRepository.findDistinctByRefDocNumberAndDeletionIndicator(grLineV2.getRefDocNumber());
                GrHeader gr = grHeaderV2Repository.findByRefDocNumberAndPreInboundNoAndDeletionIndicator(grLineV2.getRefDocNumber(), grLineV2.getPreInboundNo(), 0L);

                // Accepted_Qty
                if (grLineV2.getAcceptedQty() != null && grLineV2.getAcceptedQty() != 0) {
                    acceptedQty = grLineV2.getAcceptedQty();
                } else {
                    acceptedQty = stagingLine.getOrderQty() != null ? stagingLine.getOrderQty() : 0;
                }
                // Damaged_Qty
                if (grLineV2.getDamageQty() != null && grLineV2.getDamageQty() != 0) {
                    damagedQty = grLineV2.getDamageQty();
                } else {
                    damagedQty = 0.0;
                }
                // Generate_Barcode
                List<PackBarcode> packBarcodeList = generatePackBarcodeV2(grLineV2.getCompanyCode(), grLineV2.getLanguageId(),
                        grLineV2.getPlantId(), acceptedQty, damagedQty, grLineV2.getWarehouseId(), loginUserID);
                log.info("Generated {} pack barcodes for RefDocNumber: {}", packBarcodeList.size(), grLineV2.getRefDocNumber());
                grLineV2.setPackBarcodes(packBarcodeList);
                if (grLineV2.getGoodsReceiptNo() == null) {
                    grLineV2.setGoodsReceiptNo(gr.getGoodsReceiptNo());
                }
                grLineV2.setBatchSerialNumber("1");
                grLineV2List.add(grLineV2);
                log.info("GR line processed: {}", grLineV2);
            }
        log.info("Exiting createGrLineGenerateBarCodes. Passing {} GR lines to createGrLineNonCBMV2.", grLineV2List.size());
        try {
            return createGrLineNonCBMV2(grLineV2List, loginUserID);
        } catch (Exception e) {
            log.error("Error creating GR lines in createGrLineNonCBMV2: {}", e.getMessage(), e);
            throw e;
        }
=======
        for(AddGrLineV2 grLineV2 : newGrLines) {
            ...
            Double damagedQty = null;
                StagingLineEntityV2 stagingLine = stagingLineRepository.findDistinctByRefDocNumberAndDeletionIndicator(grLineV2.getRefDocNumber());
                GrHeader gr = grHeaderV2Repository.findByRefDocNumberAndDeletionIndicator(grLineV2.getRefDocNumber(), 0L);
            ...
            grLineV2.setBatchSerialNumber("1");
            grLineV2List.add(grLineV2);
            log.info("Create GrLine Generate Backbarcodes -------- " + grLineV2);
        }
        return createGrLineNonCBMV2(grLineV2List, loginUserID);
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e
    }
...
<<<<<<< HEAD
        log.info("Entering createGrLineNonCBMV2 with {} GR lines for user: {}", newGrLines.size(), loginUserID);
...
<<<<<<< HEAD
                log.info("Processing GR line with RefDocNumber: {}, GrLineFlag: {}", newGrLine.getRefDocNumber(), newGrLine.getGrLineFlag());

                if (newGrLine.getGrLineFlag() != null && newGrLine.getGrLineFlag().equalsIgnoreCase("2")) {
                    for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
                        log.info("Processing PackBarcode with QuantityType: {}", packBarcode.getQuantityType());
=======
                log.info("GrLine Flag ---------------> " + newGrLine.getGrLineFlag());
                if (newGrLine.getGrLineFlag() != null && newGrLine.getGrLineFlag().equalsIgnoreCase("2")) {
                    for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e

                        if(newGrLine.getManufacturerName() == null) {
                            newGrLine.setManufacturerName(MFR_NAME);
                        }
                        //=========================================================================================================//

                        // GR_QTY
                        if (packBarcode.getQuantityType().equalsIgnoreCase("A")) {
                            newGrLine.setGoodReceiptQty(newGrLine.getAcceptedQty());
                        } else if (packBarcode.getQuantityType().equalsIgnoreCase("D")) {
                            newGrLine.setGoodReceiptQty(newGrLine.getDamageQty());
                        }

<<<<<<< HEAD
                    // Create Inventory record only
//                        inventoryService.updateInventory(newGrLine, loginUserID);
                        inventoryService.createInventoryNonCBMGrLineV2(newGrLine, loginUserID);
                        log.info("Inventory record created for PackBarcode.");
=======
                        // Create Inventory record only
//                        inventoryService.updateInventory(newGrLine, loginUserID);
                        inventoryService.createInventoryNonCBMGrLineV2(newGrLine, loginUserID);
...
<<<<<<< HEAD
                        log.error("PackBarcodes missing for RefDocNumber: {}", newGrLine.getRefDocNumber());
=======
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e
...
>>>>>>> 81dc64bb940937a238631ec1ef8d3af347b8ec9e
                            newGrLine.setManufacturerName(MFR_NAME);
                            dbGrLine.setManufacturerName(MFR_NAME);
                            dbGrLine.setManufacturerPartNo(MFR_NAME);
                            dbGrLine.setManufacturerCode(MFR_NAME);
                        }
                        //=========================================================================================================//
                        ...
                        //cubic root formula
                        if (newGrLine.getThreePLCbm() != null) {
                            Double cubicRootValue = Math.cbrt(newGrLine.getThreePLCbm());

                            dbGrLine.setThreePLLength(cubicRootValue);
                            dbGrLine.setThreePLWidth(cubicRootValue);
                            dbGrLine.setThreePLHeight(cubicRootValue);
                        }

                        dbGrLine.setThreePLBillStatus("GR");
                        dbGrLine.setQuantityType(packBarcode.getQuantityType());
                        dbGrLine.setPackBarcodes(packBarcode.getBarcode());
...
//                        }
                        ...
                        if (dbStagingLineEntity != null) {
                            ...
                            dbGrLine.setCompanyDescription(dbStagingLineEntity.getCompanyDescription());
                            dbGrLine.setPlantDescription(dbStagingLineEntity.getPlantDescription());
                            dbGrLine.setWarehouseDescription(dbStagingLineEntity.getWarehouseDescription());
                        }

                        //===================================================================================
                        if (packBarcode.getCbm() != null && packBarcode.getCbmQuantity() != null) {
                            dbGrLine.setCbm(packBarcode.getCbm());
                            dbGrLine.setCbmQuantity(packBarcode.getCbmQuantity());
                        }
                        //=================================================================================================
...
//                inventory.setInventoryId(System.currentTimeMillis());
    ...
    // Update_GrLine(Pallet_Id_Pass)
    public GrLineV2 updateGrLineV2(String companyCode, String plantId, String languageId,
                                   String warehouseId, String refDocNumber,
                                   String palletId, String loginUserID, GrLineV2 updateGrLine)
            throws IllegalAccessException, InvocationTargetException, java.text.ParseException {
        GrLineV2 dbGrLine = grLineV2Repository.findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndRefDocNumberAndPalletIdAndDeletionIndicator(
                languageId, companyCode, plantId, warehouseId, refDocNumber, palletId, 0L);
        BeanUtils.copyProperties(updateGrLine, dbGrLine, CommonUtils.getNullPropertyNames(updateGrLine));
        dbGrLine.setUpdatedBy(loginUserID);
        dbGrLine.setUpdatedOn(new Date());
        return grLineV2Repository.save(dbGrLine);
    }
    ...
    @Transactional
    public List<GrLineV2> createGrLineForMobileApp(@Valid List<StagingLineEntityV2> stagingLineEntity, String loginUserID) throws Exception {
        List<GrLineV2> createdGRLines = new ArrayList<>();
        try {
            AuthToken authTokenForMastersService = authTokenService.getMastersServiceAuthToken();

            for (StagingLineEntityV2 stagingLineEntityV2 : stagingLineEntity) {

                // Flag for Inventory Create
                log.info("--------------------------->GR_LINE FLAG " + stagingLineEntityV2.getGrLineFlag());
                if (stagingLineEntityV2.getGrLineFlag() != null && stagingLineEntityV2.getGrLineFlag().equalsIgnoreCase("2")) {
                    // Create Inventory record only
//                    inventoryService.updateInventoryForMobileApp(stagingLineEntityV2, loginUserID);
                    inventoryService.createInventoryNonCBMGrLineV2(stagingLineEntityV2, loginUserID);
                } else {
                    // Inserting multiple records
                    GrLineV2 dbGrLine = new GrLineV2();

                    GrHeader gr = grHeaderRepository.findByRefDocNumberAndDeletionIndicator(stagingLineEntityV2.getRefDocNumber(), 0L);
                    log.info("newGrLine : " + stagingLineEntityV2);
                    BeanUtils.copyProperties(stagingLineEntityV2, dbGrLine, CommonUtils.getNullPropertyNames(stagingLineEntityV2));
                    dbGrLine.setCompanyCode(stagingLineEntityV2.getCompanyCode());
                    dbGrLine.setGoodsReceiptNo(gr.getGoodsReceiptNo());
                    // GR_QTY
                    Double grQty = stagingLineEntityV2.getOrderQty();
                    dbGrLine.setGoodReceiptQty(grQty);
                    dbGrLine.setAcceptedQty(grQty);
                    dbGrLine.setDamageQty(0D);
                    log.info("A-------->: " + dbGrLine);
                    dbGrLine.setQuantityType("A");
                    dbGrLine.setPackBarcodes(stagingLineEntityV2.getPalletId());
                    dbGrLine.setStatusId(14L);

                    //===========================================Handle MfrName==============================================================//
                    if (stagingLineEntityV2.getCompanyCode() != null && stagingLineEntityV2.getManufacturerName() == null) {
                        stagingLineEntityV2.setManufacturerName(getMfrName(stagingLineEntityV2.getCompanyCode()));
                        dbGrLine.setManufacturerName(getMfrName(stagingLineEntityV2.getCompanyCode()));
                        if (dbGrLine.getManufacturerPartNo() == null) {
                            dbGrLine.setManufacturerPartNo(getMfrName(stagingLineEntityV2.getCompanyCode()));
                        }
                        if (dbGrLine.getManufacturerCode() == null) {
                            dbGrLine.setManufacturerCode(getMfrName(stagingLineEntityV2.getCompanyCode()));
                        }
                    }
                    //=========================================================================================================//
                    ...
                    //GoodReceipt Qty should be less than or equal to ordered qty---> if GrQty > OrdQty throw Exception
                    Double dbGrQty = grLineV2Repository.getGrLineQuantity(
                            stagingLineEntityV2.getCompanyCode(),
                            stagingLineEntityV2.getPlantId(),
                            stagingLineEntityV2.getLanguageId(),
                            stagingLineEntityV2.getWarehouseId(),
                            stagingLineEntityV2.getRefDocNumber(),
                            stagingLineEntityV2.getPreInboundNo(),
                            gr.getGoodsReceiptNo(),
                            stagingLineEntityV2.getPalletCode(),
                            stagingLineEntityV2.getCaseCode(),
                            stagingLineEntityV2.getItemCode(),
                            stagingLineEntityV2.getManufacturerName(),
                            stagingLineEntityV2.getLineNo()
                    );
                    log.info("dbGrQty, newGrQty, OrdQty: " + dbGrQty + ", " + dbGrLine.getGoodReceiptQty() + ", " + stagingLineEntityV2.getOrderQty());
                    if (dbGrQty != null) {
                        Double totalGrQty = dbGrLine.getGoodReceiptQty() + dbGrQty;
                        if (stagingLineEntityV2.getOrderQty() < totalGrQty) {
                            throw new BadRequestException("Gr Qty is greater than Order Qty ");
                        }
                    }

                    //V2 Code
                    IKeyValuePair description = stagingLineV2Repository.getDescription(stagingLineEntityV2.getCompanyCode(),
                            stagingLineEntityV2.getLanguageId(),
                            stagingLineEntityV2.getPlantId(),
                            stagingLineEntityV2.getWarehouseId());

                    statusDescription = stagingLineV2Repository.getStatusDescription(dbGrLine.getStatusId(), stagingLineEntityV2.getLanguageId());
                    dbGrLine.setStatusDescription(statusDescription);
                    ...
                    dbGrLine.setManufacturerFullName(stagingLineEntityV2.getManufacturerFullName());
                    dbGrLine.setReferenceDocumentType(stagingLineEntityV2.getReferenceDocumentType());
                    dbGrLine.setPurchaseOrderNumber(stagingLineEntityV2.getPurchaseOrderNumber());
                    ...
                    if (stagingLineEntityV2.getOrderQty() != null) {
                        invoiceQty = stagingLineEntityV2.getOrderQty();
                    }
                    if (stagingLineEntityV2.getOrderQty() != null) {
                        acceptQty = stagingLineEntityV2.getOrderQty();
                    }
                    ...
                    StagingLineEntityV2 dbStagingLineEntity = stagingLineService.getStagingLineForPutAwayLineV2(stagingLineEntityV2.getCompanyCode(),
                            stagingLineEntityV2.getPlantId(),
                            stagingLineEntityV2.getLanguageId(),
                            stagingLineEntityV2.getWarehouseId(),
                            stagingLineEntityV2.getPreInboundNo(),
                    ...
                    if (variance == 0D) {
                        dbGrLine.setStatusId(17L);
                        statusDescription = stagingLineV2Repository.getStatusDescription(17L, stagingLineEntityV2.getLanguageId());
                        dbGrLine.setStatusDescription(statusDescription);
                    }
                    ...
                    dbGrLine.setCbm(cbm);
                    dbGrLine.setCbmQuantity(cbmPerQty);
//            dbGrLine.setAcceptTotalCbm(stagingLineEntityV2.getAcceptTotalCbm());

                    dbGrLine.setBranchCode(stagingLineEntityV2.getBranchCode());
                    dbGrLine.setTransferOrderNo(stagingLineEntityV2.getTransferOrderNo());
                    dbGrLine.setIsCompleted(stagingLineEntityV2.getIsCompleted());

                    dbGrLine.setBarcodeId(stagingLineEntityV2.getBarcodeId());
                    dbGrLine.setDeletionIndicator(0L);
                    dbGrLine.setCreatedBy(loginUserID);
                    dbGrLine.setUpdatedBy(loginUserID);
                    dbGrLine.setConfirmedBy(loginUserID);
                    ...
                    createdGRLinesStatusId17Count = grLineV2Repository.getGrLineStatus17Count(createdGRLines.get(0).
                                    getCompanyCode(), createdGRLines.get(0).getPlantId(), createdGRLines.get(0).getLanguageId(),
                            createdGRLines.get(0).getWarehouseId(),
                            createdGRLines.get(0).getRefDocNumber(),
                            createdGRLines.get(0).getPreInboundNo(), 17L);
                    log.info("createdGRLinesStatusId17Count: " + createdGRLinesStatusId17Count);
...
//            createdGRLinesStatusId17Count = createdGRLinesStatusId17Count + (createdGRLines.stream().filter(n -> n.getStatusId() == 17L).count());
                    ...
                    for (GrLineV2 grLine : createdGRLines) {
                        ...
                        List<StagingLineEntityV2> stagingLineEntityList =
                                stagingLineService.getStagingLineV2(
                                        ...
                                        grLine.getItemCode(),
                                        grLine.getCaseCode());
                        for (StagingLineEntityV2 stageLine : stagingLineEntityList) {

                            //v2 code
                            if (stageLine.getRec_accept_qty() == null) {
                                stageLine.setRec_accept_qty(0D);
                            }
                            if (grLine.getAcceptedQty() == null) {
                                grLine.setAcceptedQty(0D);
                            }
                            if (stageLine.getRec_damage_qty() == null) {
                                stageLine.setRec_damage_qty(0D);
                            }
                            if (grLine.getDamageQty() == null) {
                                grLine.setDamageQty(0D);
                            }

                            Double rec_accept_qty = stageLine.getRec_accept_qty() + grLine.getAcceptedQty();
                            Double rec_damage_qty = stageLine.getRec_damage_qty() + grLine.getDamageQty();

                            stageLine.setRec_accept_qty(rec_accept_qty);
                            stageLine.setRec_damage_qty(rec_damage_qty);

                            if (grLine.getStatusId() == 17L) {
                                stageLine.setStatusId(17L);
                                statusDescription = stagingLineV2Repository.getStatusDescription(17L, grLine.getLanguageId());
                                stageLine.setStatusDescription(statusDescription);
                            }
                            stageLine = stagingLineV2Repository.save(stageLine);
                            log.info("stagingLineEntity updated: " + stageLine);
                        }
            ...
            //Exception Log
//            createGrLineLog10(stagingLineEntityV2, e.toString());
