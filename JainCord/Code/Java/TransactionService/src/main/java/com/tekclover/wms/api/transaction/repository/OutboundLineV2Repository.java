...
//            "AND im.WH_ID in (:warehouseId) " +
    ...
    //==============================================JainCord===========================================================================================
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("UPDATE OutboundLineV2 ob SET ob.statusId = :statusId, ob.statusDescription = :statusDescription, ob.assignedPickerId = :assignedPickerId \r\n"
            + " WHERE C_ID = :companyCodeId AND PLANT_ID = :plantId AND LANG_ID = :languageId AND ob.warehouseId = :warehouseId AND \r\n "
    ...
    "SELECT 1 FROM tbloutboundline as t1 \n"+
    "JOIN (SELECT pre_ob_no, COUNT(ob_line_no) AS st57 \n"+
    "FROM tbloutboundline \n"+
            "WHERE status_id in (57,51,47) and pre_ob_no = :preOutboundNo and c_id = :companyCodeId AND plant_id = :plantId AND lang_id = :languageId AND wh_id = :warehouseId \n"+
            "GROUP BY pre_ob_no) as t2 \n"+
            "ON t1.pre_ob_no = t2.pre_ob_no \n"+
            "where t2.st57 = (SELECT COUNT(ob_line_no) \n"+
            "FROM tbloutboundline WHERE pre_ob_no = t2.pre_ob_no)\n"+
            ") THEN 1 ELSE 0 END ", nativeQuery = true)
    Long doesMatchingRecordExist(@Param("companyCodeId") String companyCodeId,
                                 @Param("plantId") String plantId,
                                 @Param("languageId") String languageId,
                                 @Param("warehouseId") String warehouseId,
                                 @Param("preOutboundNo") String preOutboundNo);
    ...
    @Query("UPDATE OutboundLineV2 ob SET ob.statusId = :statusId, ob.statusDescription = :statusDescription \r\n"
            + " WHERE C_ID = :companyCodeId AND PLANT_ID = :plantId AND LANG_ID = :languageId AND ob.warehouseId = :warehouseId AND \r\n "
            + " ob.refDocNumber = :refDocNumber AND ob.preOutboundNo = :preOutboundNo AND ob.lineNumber = :lineNumber AND ob.itemCode = :itemCode")
    public void updateOutboundLineUnAllocateStatusV5(@Param("companyCodeId") String companyCodeId,
                                                     @Param("plantId") String plantId,
                                                     @Param("languageId") String languageId,
                                                     @Param("warehouseId") String warehouseId,
                                                     @Param("refDocNumber") String refDocNumber,
