...
//            apiHeader.setBranchCode(asnV2Header.getBranchCode());
            ...
            apiHeader.setTransferOrderNumber(asnV2Header.getAsnNumber());
...
//				apiHeader.setRefDocumentType("Production Order");
            apiHeader.setRefDocumentType(getInboundOrderTypeDesc(apiHeader.getCompanyCode(), apiHeader.getBranchCode(),
                    apiHeader.getLanguageId(), apiHeader.getWarehouseID(), apiHeader.getInboundOrderTypeId()));
//			apiHeader.setInboundOrderHeaderId(System.currentTimeMillis());
...
//			StockReceiptHeader stockReceiptHeader = stockReceipt.getStockReceiptHeader();
            ...
            InboundOrderV2 apiHeader = new InboundOrderV2();
            BeanUtils.copyProperties(stockReceipt, apiHeader, CommonUtils.getNullPropertyNames(stockReceipt));
...
//			apiHeader.setRefDocumentType("DirectReceipt");
            ...
            apiHeader.setLanguageId(stockReceipt.getLanguageId() != null ? stockReceipt.getLanguageId() : LANG_ID);

            if (stockReceipt.getWarehouseId() != null && !stockReceipt.getWarehouseId().isBlank()) {
                apiHeader.setWarehouseID(stockReceipt.getWarehouseId());
            } else {
<<<<<<< HEAD
            // Get Warehouse
            Optional<Warehouse> dbWarehouse =
...
//			apiHeader.setInboundOrderHeaderId(System.currentTimeMillis());
            ...
            for (StockReceiptLine stockReceiptLine : stockReceiptLines) {
                InboundOrderLinesV2 apiLine = new InboundOrderLinesV2();
                BeanUtils.copyProperties(stockReceiptLine, apiLine, CommonUtils.getNullPropertyNames(stockReceiptLine));
...
//			apiHeader.setRefDocumentType("SalesReturn");
            ...
            apiHeader.setLanguageId(soReturnHeaderV2.getLanguageId() != null ? soReturnHeaderV2.getLanguageId() : LANG_ID);
...
//     * @return
//     */
//    public InboundOrderV2 postPurchaseOrder(PurchaseOrder purchaseOrder) {
//        log.info("PurchaseOrder received from External: " + purchaseOrder);
//        InboundOrderV2 receivedPurchaseOrder = savePurchaseOrderUpload(purchaseOrder);
//        log.info("PurchaseOrder: " + purchaseOrder);
//        return receivedPurchaseOrder;
//    }

    // InterWarehouseTransferInV2
//    private InboundOrderV2 savePurchaseOrderUpload(PurchaseOrder purchaseOrder) {
//        try {
//            PurchaseHeader header = purchaseOrder.getPurchaseHeader();
//            List<PurchaseLine> purchaseLines = purchaseOrder.getPurchaseLines();
//
//            InboundOrderV2 apiHeader = new InboundOrderV2();
//            BeanUtils.copyProperties(header, apiHeader, CommonUtils.getNullPropertyNames(header));
//            apiHeader.setRefDocumentNo(header.getTransferOrderNumber());
//            apiHeader.setOrderId(header.getTransferOrderNumber());
//            apiHeader.setCompanyCode(header.getCompanyCode());
//            apiHeader.setLanguageId(header.getLanguageId());
//            apiHeader.setTransferOrderNumber(header.getTransferOrderNumber());
//            apiHeader.setInboundOrderTypeId(5L);                // Hardcoded Value 3
//			apiHeader.setRefDocumentType("WMS to WMS");
//            apiHeader.setOrderReceivedOn(new Date());
//            apiHeader.setUpdatedOn(header.getUpdatedOn());
//
//            // Get Warehouse
//            Optional<com.tekclover.wms.api.transaction.model.warehouse.Warehouse> dbWarehouse =
//                    warehouseRepository.findByCompanyCodeIdAndPlantIdAndLanguageIdAndDeletionIndicator(
//                            header.getCompanyCode(),
//                            header.getBranchCode(),
//                            "EN",
//                            0L
//                    );
//            log.info("dbWarehouse : " + dbWarehouse);
//            apiHeader.setWarehouseID(dbWarehouse.get().getWarehouseId());
//            apiHeader.setRefDocumentType(getInboundOrderTypeDesc(apiHeader.getCompanyCode(), apiHeader.getBranchCode(),
//                    "EN", apiHeader.getWarehouseID(), apiHeader.getInboundOrderTypeId()));
////			apiHeader.setInboundOrderHeaderId(System.currentTimeMillis());
//
//            Set<InboundOrderLinesV2> orderLinesV2 = new HashSet<>();
//            for (PurchaseLine purchaseLine : purchaseLines) {
//                InboundOrderLinesV2 apiLine = new InboundOrderLinesV2();
//                BeanUtils.copyProperties(purchaseLine, apiLine, CommonUtils.getNullPropertyNames(purchaseLine));
//                apiLine.setLineReference(purchaseLine.getLineReference());                 // IB_LINE_NO
//                apiLine.setItemCode(purchaseLine.getSku());                                // ITM_CODE
//                apiLine.setItemText(purchaseLine.getSkuDescription());                     // ITEM_TEXT
//                apiLine.setSupplierPartNumber(purchaseLine.getSupplierPartNumber());        // PARTNER_ITM_CODE
//                apiLine.setExpectedQty(purchaseLine.getExpectedQty());
//                apiLine.setUom(purchaseLine.getUom());
//                if (purchaseLine.getManufacturerCode() != null) {
//                    apiLine.setManufacturerCode(purchaseLine.getManufacturerCode());
//                } else {
//                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
//                        apiLine.setManufacturerCode(getMfrName(COMPANY_CODE));
//                    }
//                }
//                if (purchaseLine.getManufacturerName() != null) {
//                    apiLine.setManufacturerName(purchaseLine.getManufacturerName());
//                } else {
//                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
...
//                try {
//                    ZoneId defaultZoneId = ZoneId.systemDefault();
//                    String sdate = purchaseLine.getExpectedDate();
//                    String firstHalf = sdate.substring(0, sdate.lastIndexOf("/"));
//                    String secondHalf = sdate.substring(sdate.lastIndexOf("/") + 1);
//                    secondHalf = "/20" + secondHalf;
//                    sdate = firstHalf + secondHalf;
...
//            apiHeader.setLine(orderLinesV2);
//            apiHeader.setOrderProcessedOn(new Date());
//            if (purchaseOrder.getPurchaseLines() != null &&
//                    !purchaseOrder.getPurchaseLines().isEmpty()) {
//                apiHeader.setProcessedStatusId(0L);
//                log.info("apiHeader : " + apiHeader);
//                InboundOrderV2 createdOrderV2 = orderService.createInboundOrdersV2(apiHeader);
//                log.info("Purchase Order Success: " + createdOrderV2);
//                return createdOrderV2;
//            } else if (purchaseOrder.getPurchaseLines() == null ||
//                    purchaseOrder.getPurchaseLines().isEmpty()) {
//                // throw the error as Lines are Empty and set the Indicator as '100'
//                apiHeader.setProcessedStatusId(100L);
//                log.info("apiHeader : " + apiHeader);
//                InboundOrderV2 createdOrderV2 = orderService.createInboundOrdersV2(apiHeader);
//                log.info("Purchase Order Failed : " + createdOrderV2);
//                throw new BadRequestException("Purchase Order doesn't contain any Lines.");
//            }
//        } catch (Exception e) {
//            throw e;
//        }
...
//			apiHeader.setRefDocumentType("WMS to WMS");
            ...
            apiHeader.setLanguageId(interWarehouseTransferInHeaderV2.getLanguageId() != null ? interWarehouseTransferInHeaderV2.getLanguageId() : LANG_ID);

            if (interWarehouseTransferInHeaderV2.getWarehouseId() != null && !interWarehouseTransferInHeaderV2.getWarehouseId().isBlank()) {
                apiHeader.setWarehouseID(interWarehouseTransferInHeaderV2.getWarehouseId());
            } else {
<<<<<<< HEAD
            // Get Warehouse
            Optional<com.tekclover.wms.api.transaction.model.warehouse.Warehouse> dbWarehouse =
...
//				apiLine.setInboundOrderHeaderId(apiHeader.getInboundOrderHeaderId());
    ...
    private InboundOrderV2 saveB2BTransferIn(B2bTransferIn b2bTransferIn) {
        try {
            ...
            InboundOrderV2 apiHeader = new InboundOrderV2();
            BeanUtils.copyProperties(b2BTransferInHeader, apiHeader, CommonUtils.getNullPropertyNames(b2BTransferInHeader));
...
//			apiHeader.setRefDocumentType("Non-WMS to WMS");
            ...
            apiHeader.setLanguageId(b2BTransferInHeader.getLanguageId() != null ? b2BTransferInHeader.getLanguageId() : LANG_ID);

            if (b2BTransferInHeader.getWarehouseId() != null && !b2BTransferInHeader.getWarehouseId().isBlank()) {
                apiHeader.setWarehouseID(b2BTransferInHeader.getWarehouseId());
            } else {
<<<<<<< HEAD
            // Get Warehouse
            Optional<com.tekclover.wms.api.transaction.model.warehouse.Warehouse> dbWarehouse =
...
//			apiHeader.setInboundOrderHeaderId(System.currentTimeMillis());
            ...
            for (B2bTransferInLine b2bTransferInLine : b2bTransferInLines) {
                InboundOrderLinesV2 apiLine = new InboundOrderLinesV2();
                BeanUtils.copyProperties(b2bTransferInLine, apiLine, CommonUtils.getNullPropertyNames(b2bTransferInLine));
...
//				apiLine.setInboundOrderHeaderId(apiHeader.getInboundOrderHeaderId());
    ...
    private CycleCountHeader savePerpetual(Perpetual perpetual) {
        try {
            ...
            BeanUtils.copyProperties(perpetualHeaderV1, apiHeader, CommonUtils.getNullPropertyNames(perpetualHeaderV1));
            ...
            for (PerpetualLineV1 perpetualLineV1 : perpetualLineV1List) {
                ...
                BeanUtils.copyProperties(perpetualLineV1, apiLine, CommonUtils.getNullPropertyNames(perpetualLineV1));
...
//				apiLine.setManufacturerName(perpetualLineV1.getManufacturerName());                        // MANU_FAC_NM
                if (perpetualLineV1.getManufacturerCode() != null) {
                    apiLine.setManufacturerCode(perpetualLineV1.getManufacturerCode());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
                        apiLine.setManufacturerCode(getMfrName(COMPANY_CODE));
                    }
                }
                if (perpetualLineV1.getManufacturerName() != null) {
                    apiLine.setManufacturerName(perpetualLineV1.getManufacturerName());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
                        apiLine.setManufacturerName(getMfrName(COMPANY_CODE));
                    }
    ...
    private CycleCountHeader savePeriodic(Periodic periodic) {
        try {
            ...
            CycleCountHeader apiHeader = new CycleCountHeader();
            BeanUtils.copyProperties(periodicHeaderV1, apiHeader, CommonUtils.getNullPropertyNames(periodicHeaderV1));
            ...
            for (PeriodicLineV1 periodicLineV1 : periodicLineV1List) {
                CycleCountLine apiLine = new CycleCountLine();
                BeanUtils.copyProperties(periodicLineV1, apiLine, CommonUtils.getNullPropertyNames(periodicLineV1));
                ...
                if (periodicLineV1.getManufacturerCode() != null) {
                    apiLine.setManufacturerCode(periodicLineV1.getManufacturerCode());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
                        apiLine.setManufacturerCode(getMfrName(COMPANY_CODE));
                    }
                }
                if (periodicLineV1.getManufacturerName() != null) {
                    apiLine.setManufacturerName(periodicLineV1.getManufacturerName());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
                        apiLine.setManufacturerName(getMfrName(COMPANY_CODE));
                    }
...
//
            ...
            BeanUtils.copyProperties(soHeader, apiHeader, CommonUtils.getNullPropertyNames(soHeader));
...
//			apiHeader.setRefDocumentType("WMS to Non-WMS");                        // Hardcoded value "SO"
            ...
            apiHeader.setRefDocumentType(getOutboundOrderTypeDesc(apiHeader.getCompanyCode(), apiHeader.getBranchCode(),
                    "EN", apiHeader.getWarehouseID(), apiHeader.getOutboundOrderTypeID()));
...
//				apiLine.setManufacturerName(soLine.getManufacturerName());
                if (soLine.getManufacturerCode() != null) {
                    apiLine.setManufacturerCode(soLine.getManufacturerCode());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
                        apiLine.setManufacturerCode(getMfrName(COMPANY_CODE));
                ...
                if (soLine.getManufacturerName() != null) {
                    apiLine.setManufacturerName(soLine.getManufacturerName());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
                        apiLine.setManufacturerName(getMfrName(COMPANY_CODE));
...
//	}
    ...
    private OutboundOrderV2 saveSalesOrderV2(@Valid SalesOrderV2 salesOrder) throws ParseException {
        try {
            ...
            if (salesOrderHeader.getWarehouseId() != null) {
                apiHeader.setWarehouseID(salesOrderHeader.getWarehouseId());
            } else {
                Optional<Warehouse> warehouse =
                        warehouseRepository.findByCompanyCodeIdAndPlantIdAndLanguageIdAndDeletionIndicator(
                                salesOrderHeader.getCompanyCode(), salesOrderHeader.getBranchCode(),
                                apiHeader.getLanguageId(), 0L);
                // Warehouse ID Validation
                validateWarehouseId(warehouse.get().getWarehouseId());
                apiHeader.setWarehouseID(warehouse.get().getWarehouseId());
            }
...
//			apiHeader.setRefDocumentType("PICK LIST");                              // Hardcoded value "SaleOrder"
            apiHeader.setRefDocumentType(getOutboundOrderTypeDesc(apiHeader.getCompanyCode(), apiHeader.getBranchCode(),
                    apiHeader.getLanguageId(), apiHeader.getWarehouseID(), apiHeader.getOutboundOrderTypeID()));
...
//			apiHeader.setOutboundOrderHeaderId(System.currentTimeMillis());
            ...
            for (InterWarehouseTransferOutLineV2 iwhTransferLine : interWarehouseTransferOutLines) {
                OutboundOrderLineV2 apiLine = new OutboundOrderLineV2();
                BeanUtils.copyProperties(iwhTransferLine, apiLine, CommonUtils.getNullPropertyNames(iwhTransferLine));
...
//				apiLine.setManufacturerName(iwhTransferLine.getManufacturerName());
                if (iwhTransferLine.getManufacturerCode() != null) {
                    apiLine.setManufacturerCode(iwhTransferLine.getManufacturerCode());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
                        apiLine.setManufacturerCode(getMfrName(COMPANY_CODE));
                ...
                if (iwhTransferLine.getManufacturerName() != null) {
                    apiLine.setManufacturerName(iwhTransferLine.getManufacturerName());
                } else {
                    if (apiHeader.getCompanyCode().equalsIgnoreCase(COMPANY_CODE)) {
                        apiLine.setManufacturerName(getMfrName(COMPANY_CODE));
...
//			}
            ...
            OutboundOrderV2 apiHeader = new OutboundOrderV2();
            BeanUtils.copyProperties(salesInvoice, apiHeader, CommonUtils.getNullPropertyNames(salesInvoice));
...
//			}
    ...
    private StockAdjustment saveStockAdjustment(StockAdjustment stockAdjustment) {
        try {
            ...
            if (stockAdjustment.getWarehouseId() != null) {
                dbStockAdjustment.setWarehouseId(stockAdjustment.getWarehouseId());
            } else {
                // Get Warehouse
                Optional<Warehouse> dbWarehouse =
                        warehouseRepository.findByCompanyCodeIdAndPlantIdAndLanguageIdAndDeletionIndicator(
                                stockAdjustment.getCompanyCode(), stockAdjustment.getBranchCode(),
                                stockAdjustment.getLanguageId() != null ? stockAdjustment.getLanguageId() : "EN", 0L);
                log.info("dbWarehouse: " + dbWarehouse);
                validateWarehouseId(dbWarehouse.get().getWarehouseId());
...
//			dbStockAdjustment.setOrderProcessedOn(new Date());
            ...
            IKeyValuePair iKeyValuePair = outboundOrderV2Repository.getV2Description(
                    stockAdjustment.getCompanyCode(), stockAdjustment.getBranchCode(), dbStockAdjustment.getWarehouseId());
            if (iKeyValuePair != null) {
