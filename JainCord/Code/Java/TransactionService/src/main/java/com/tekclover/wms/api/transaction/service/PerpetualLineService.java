...
public class PerpetualLineService extends BaseService {
    ...
    public PerpetualLineService() {super();}
    //=======================================================================================================================
    ...
    public List<PerpetualLine> updatePerpetualLineForMobileCount(List<UpdatePerpetualLine> updatePerpetualLines,
        ...
        for (UpdatePerpetualLine updatePerpetualLine : updatePerpetualLines) {
            ...
            if (dbPerpetualLine != null) { /* Update */
                ...
                dbPerpetualLine.setInventoryQuantity(round(INV_QTY));
...
//					createInventoryMovement (updatedPerpetualLine);
     ...
     * @return
     */
    public List<PerpetualLineV2> updateAssingHHTUserV2(List<AssignHHTUserCC> assignHHTUsers, String loginUserID) throws FirebaseMessagingException {
        ...
        if (dbPerpetualHeader != null) {
            sendNotification(dbPerpetualHeader.getLanguageId(), dbPerpetualHeader.getCompanyCodeId(), dbPerpetualHeader.getPlantId(),
                    dbPerpetualHeader.getWarehouseId(), dbPerpetualHeader.getCycleCountNo());
        }
        return responseList;
    }
...
//                                dbPerpetualLine.getManufacturerName());
    ...
    public void sendNotification(String languageId, String companyId, String plantId, String warehouseId, String processId) throws FirebaseMessagingException {
        List<IKeyValuePair> notification = perpetualLineV2Repository.findByStatusIdAndNotificationStatusAndDeletionIndicatorDistinctRefDocNo();

        if (notification != null) {
            for (IKeyValuePair perpetualHeader : notification) {

                try {
                    // Get NotificationId
                    IKeyValuePair notifyId = perpetualLineV2Repository.getNotificationId(companyId, languageId, "3");
                    ...
                    List<String> userIds = perpetualLineV2Repository.getUserId(companyId, languageId, notifyId.getUserRole());
                    if (userIds.isEmpty()) {
                        log.warn("No Users found for the specified role: {}, companyId: {}", notifyId.getUserRole(), companyId);
                        return;
                    }

                    List<String> deviceToken = perpetualLineV2Repository.getDeviceToken(languageId, companyId, plantId, warehouseId, userIds);

                    if (deviceToken != null && !deviceToken.isEmpty()) {
                        String title = "PERPETUAL";
                        String message = perpetualHeader.getRefDocType() + " ORDER - " + perpetualHeader.getRefDocNumber() + " - IS ASSIGNED";
                        ...
                        if (response.equals("OK")) {
                            perpetualLineV2Repository.updateNotificationStatus(perpetualHeader.getRefDocNumber(), perpetualHeader.getWarehouseId());
