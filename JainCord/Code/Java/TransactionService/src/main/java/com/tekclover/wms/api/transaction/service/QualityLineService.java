...
<<<<<<< HEAD
     ...
     */
    @Transactional
    public List<QualityLineV2> createQualityLineV4(List<AddQualityLineV2> newQualityLines, String loginUserID)
            throws Exception {
        try {
            log.info("-------createQualityLine--------called-------> " + newQualityLines);
...
//                    qualityHeaderV2Repository.updateQualityHeader(statusDescription, dbQualityLine.getQualityInspectionNo());
            ...
            for (QualityLineV2 dbQualityLine : createdQualityLineList) {
               ...
               List<InventoryV2> inventoryList =  pickupLineService.modifyInventoryForQualityConfirm(dbQualityLine.getCompanyCodeId(), dbQualityLine.getPlantId(), dbQualityLine.getLanguageId(),
                        dbQualityLine.getWarehouseId(), dbQualityLine.getItemCode(), dbQualityLine.getRefDocNumber(), dbQualityLine, loginUserID);

               log.info("Return InboundOrder Starting ----------------> ");
                returnInboundOrderService.triggerInboundOrder(dbQualityLine.getCompanyCodeId(), dbQualityLine.getPlantId(), dbQualityLine.getLanguageId(), dbQualityLine.getWarehouseId(), dbQualityLine, inventoryList, loginUserID);
