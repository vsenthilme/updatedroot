        ...
        for (InboundOrderProcessV4 listUploadedData : allRowsList) {
            ...
            if (isSameOrder) {
                ...
                if (oneTimeAllow) {
                    ...
                    header.setWarehouseId(warehouseId);
                    header.setLoginUserId(loginUserId);
                    String asnNumber = listUploadedData.getAsnNumber();
                    String removeSpecialCHASN = asnNumber.replaceAll("[^a-zA-Z0-9]", "");

                    header.setAsnNumber(removeSpecialCHASN);
//                    header.setAsnNumber(listUploadedData.getAsnNumber());
                    header.setInboundOrderTypeId(listUploadedData.getInboundOrderTypeId());
                    header.setSystem(listUploadedData.getSystem());
                    header.setRefDocumentType(listUploadedData.getRefDocumentType());
                }
                oneTimeAllow = false;
...
//    // Purchase_Order
//    public List<PurchaseOrder> prepPurchaseOrder(String companyCodeId, String plantId,
//                                                             String languageId, String warehouseId, String loginUserId,
//                                                             List<InboundOrderProcessV4> allRowsList) {
...
//        List<PurchaseLine> listLines = new ArrayList<>();
//        for (InboundOrderProcessV4 listUploadedData : allRowsList) {
//            if (orderNumber != null) {
//                isSameOrder = orderNumber.equalsIgnoreCase(listUploadedData.getTransferOrderNumber());
//            }
...
//                orders.setPurchaseLines(listLines);
//                orderList.add(orders);
//
//                //reset to create new order
//                oneTimeAllow = true;
