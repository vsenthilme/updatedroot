...
/*            isScanned =
    ...
    private fun barcodeValidation(periodicLine: PeriodicLine?, decodedData: String?) {
        val viewModel = ViewModelProvider(this)[FindImPartnerViewModel::class.java]
        viewModel.itemCode = periodicLine?.itemCode ?: ""
        viewModel.businessPartnerCode = periodicLine?.manufacturerName ?: ""
        ...
        viewModel.findImPartner.observe(this) { imPartnerList ->
            if (imPartnerList.isNullOrEmpty().not()) {
                val imPartnerResponse: FindImPartnerResponse? =
                    imPartnerList.find { it.partnerItemBarcode == periodicLine?.barcodeId }
