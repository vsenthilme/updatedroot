...
//                ).saveStockInfo(data[position].perpetualLine)
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val cycleCountNo = itemView.txt_cell_1
        val rCycleCountNo = itemView.txt_cell_2
        val date = itemView.txt_cell_3


        fun bindItems(model: PerpetualResponse) {

            cycleCountNo.text = model.cycleCountNo
            rCycleCountNo.text = model.referenceCycleCountNo
            date.text = DateUtil.getDateYYYYMMDD(model.createdOn)
