...
import com.tvhht.myapplication.putaway.adapter.PutAwayConfirmAdapter
...
import com.tvhht.myapplication.putaway.utils.BinCustomDialog
import com.tvhht.myapplication.putaway.utils.ChangeBinCustomDialog
import com.tvhht.myapplication.putaway.utils.ConfirmWarningDialog
import com.tvhht.myapplication.putaway.utils.QuantityCustomDialogActivity
...
import kotlinx.android.synthetic.main.activity_putaway_details_confirm.Recycler_view
...
import kotlinx.android.synthetic.main.activity_putaway_details_confirm.barcode_value
...
class PutAwayDetailsActivity : AppCompatActivity(), ReasonDialog.ReasonListener {
    ...
    lateinit var proposedBin: String
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        addNewBIN.setOnClickListener {
            ...
            if (dataBinQtyListfinal.size == 0) {
                val cdd = ConfirmWarningDialog(
                    this@PutAwayDetailsActivity
                )
                cdd.show()
            } else {
    ...
    fun loadAddBinData() {
        ...
        val myIntent = Intent(this@PutAwayDetailsActivity, AddBinCustomDialog::class.java)
    ...
    private fun loadData() {
        ...
        pd = ProgressDialog(this@PutAwayDetailsActivity)
        ...
        proposedBin = putAwayInfo.proposedStorageBin.toString()
        ...
        Recycler_view.adapter = PutAwayConfirmAdapter(this, dataBinQtyList)
            ...
            if (dataBinQtyList[i].isSelected == true && dataBinQtyList[i].putawayConfirmedQty!! > 0) {
                /*request.add(
                    PutAwaySubmit(
                        ...
                        putAwayInfo.putAwayQuantity?:0.0,
    ...
    private fun processRequest(vDataList: PutAwayConfirmResponse?) {
        ...
        if (vDataList != null) {
            ToastUtils.showToast(
                WMSApplication.getInstance().context,
                "PutAway Details Submitted successfully"
            )
            finish()
            PutAwayListActivity.getInstance()?.reload()
        } else {
            ToastUtils.showToast(
                WMSApplication.getInstance().context,
    ...
    fun getCurrentSum() {
        ...
        totalQTY.text = String.format(Locale.getDefault(),"%d",(putAwayInfo?.putAwayQuantity ?:0).toInt())
        balanceQty.text = String.format(Locale.getDefault(),"%d",balanceQty1?.toInt())
    ...
    fun launchChangeBin() {
        val myIntent = Intent(this@PutAwayDetailsActivity, ChangeBinCustomDialog::class.java)
        startActivity(myIntent)
    }


    companion object {
        private var instances2: PutAwayDetailsActivity? = null
        fun getInstance(): PutAwayDetailsActivity? {
            return instances2
        }
    }
        ...
        model.authToken().observe(this) {
            it?.let { token ->
                model.binValidation(bin, token).observe(this) { response ->
                    ...
                    if (response != null) {
                        val myIntent = Intent(this, QuantityCustomDialogActivity::class.java)
                        startActivity(myIntent)
                    } else {
                        ToastUtils.showToast(
                            WMSApplication.getInstance().context,
