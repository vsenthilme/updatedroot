...
class LoginActivity : AppCompatActivity() {
        ...
        model.getUserStatus().observe(this) { userList ->
            ...
            if (userList.equals("ERROR")) {
                ...
                if (userList.equals("FAILED")) {
                    ...
                    val filter = userList?.filter { it.userId?.lowercase(Locale.getDefault()) == stringUserID?.lowercase(Locale.getDefault()) }

                    WMSSharedPref.getAppPrefs(
                        WMSApplication.getInstance().context
                    ).saveUserInfoVO(filter)

                    if (model.warehouseId == ApiConstant.WAREHOUSE_ID_200 || filter?.size == 1) {
                        WMSSharedPref.getAppPrefs(
                            WMSApplication.getInstance().context
                        ).saveStringValue(PrefConstant.USER_DETAILS_INDEX, filter!![0].plantId)
    ...
    fun loadGroupDetails() {
        ...
        val model = ViewModelProviders.of(this)[LoginLiveData::class.java]
        model.getWarehouseList(wareHouseId,companyCodeId,plantId,languageId).observe(this) { warelist ->
            // update UI
            if (pd != null)
                pd.dismiss()
