package com.tvhht.myapplication.reports.utils;
...
public class ScanPalletCustomDialog extends AppCompatActivity implements
        View.OnClickListener {

    public Activity c;
    public String title = "Please Scan Barcode";
    public String value = "";
    public Dialog d;
...
/*
        if (decodedData.toString().length() > ApiConstant.INSTANCE.getCode_length_three()) {
            ...
            textValue.setText(value);
            textMessage.setText("Barcode is Scanned");
...
*/
        if (!TextUtils.isEmpty(decodedData)) {
            value = decodedData.toString();
            textValue.setText(value);
            textMessage.setText("Barcode Scanned");
            imageIcon.setBackgroundResource(R.drawable.icon_tick_success);
        }
    }
                ...
                if (value.length() > 0) {
                   finish();
                    WMSSharedPref.getAppPrefs(WMSApplication.getInstance().getContext())
                            .saveStringValue("REPORTS_ID_VALUE", value);
                    ReportListFromPalletIDActivity.Companion.getInstance().verifyToken();

                } else {

                    ToastUtils.Companion.showToast(getApplicationContext(), "Please scan the Barcode");
                }
