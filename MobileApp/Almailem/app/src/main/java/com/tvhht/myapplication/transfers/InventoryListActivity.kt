...
class InventoryListActivity : AppCompatActivity() {
    ...
    fun loadInventoryList() {
        ...
        model.getInventoryList(scannedBinList).observe(this) { vDataList ->
            ...
            if (vDataList != null) {
                ...
                dataList = if (warehouseId == ApiConstant.WAREHOUSE_ID_200) {
                    dataList2
                } else {
                    dataList2.filter { it.warehouseId == warehouseId && it.companyCodeId == companyCodeId && it.plantId == plantId } as MutableList<InventoryModel>
                }
            }

            //putaway_txt_count_val.text = dataList!!.size.toString()

            for (inventory in dataList) {
                str_list.add(inventory.barcodeId.toString())
            }
    ...
    fun getUpdatedData() {
        ...
        if (qtyInfo != null) {
            for (i in dataList.indices) {
                if (dataList[i].barcodeId == qtyInfo.barcodeId && qtyInfo.isSelected) {
                    dataList.removeAt(i);
    ...
    private fun isScanned(barcode: String): Boolean {
        var returnState = false
        /*val csDetails: InventoryModel? =
            dataList.find {
                it.barcodeId == (barcode)
            }

        for (csModel: InventoryModel in dataList) {
            if (csModel.barcodeId == barcode) {
                val csDetails: InventoryModel? =
                    dataList.find {
                        it.barcodeId == (barcode)
                    }
        ...
        val inventoryModel: InventoryModel? =
            dataList.find { it.barcodeId == barcode }

        if (inventoryModel != null) {
            returnState = true
            inventoryModel.isSelected = true
            WMSSharedPref.getAppPrefs(
                WMSApplication.getInstance().context
            ).saveInventoryInfo(inventoryModel)
        }
        val adapter = InventoryAdapter(this, dataList)
        ...
        for (i in selecteddataList?.indices!!) {
            createList.add(
                InhouseTransferLine(
                    ...
                    selecteddataList[i].manufacturerName
                )
            )
        }
