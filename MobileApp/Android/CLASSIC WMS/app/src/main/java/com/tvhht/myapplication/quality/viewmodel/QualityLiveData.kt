...
class QualityLiveData : ViewModel() {
    ...
    internal fun getQualityListNew(he_n0: String): MutableLiveData<List<QualityListResponse>> {
        if (qualityList == null) {
            qualityList = MutableLiveData()
        }
        getQA(he_n0)
    ...
    private fun loadDetails() {
        ...
        val requestData = QualityRequest(wareIDList, statusID)
...
//                    strAuthenticator?.postValue("ERROR")
    ...
    private fun getQA(he_n0: String) {
        viewModelScope.launch {

            try {
                val dbHelper =
                    DatabaseHelperImpl(DatabaseBuilder.getInstance(WMSApplication.getInstance().applicationContext))
                val qaList = dbHelper.getQAList(he_n0)
