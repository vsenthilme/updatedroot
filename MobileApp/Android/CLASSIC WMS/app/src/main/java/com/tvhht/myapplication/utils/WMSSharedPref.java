...
public class WMSSharedPref {
    ...
    public void savePickingInfo(PickingCombineResponse putawayList) {
        if (putawayList != null) {
            SharedPreferences.Editor editor = Objects.requireNonNull(preferences).edit();
            String json = getGson().toJson(putawayList);
    ...
    @Nullable
    public PickingCombineResponse getPickingInfo() {
        String json = Objects.requireNonNull(preferences).getString("PICKING_COMB_LIST", "");
        if (TextUtils.isEmpty(json)) {
            return null;
        } else {
            Type founderListType = new TypeToken<PickingCombineResponse>() {
            }.getType();
