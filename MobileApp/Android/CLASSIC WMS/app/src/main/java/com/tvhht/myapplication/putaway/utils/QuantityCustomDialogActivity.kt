...
class QuantityCustomDialogActivity : AppCompatActivity() {
    var c: Activity? = null
    var title = ""
    var value = ""
    var d: Dialog? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        yes!!.setOnClickListener { view: View? ->
            ...
            if (s.isEmpty()) {
                ...
                qtyInfo?.putawayConfirmedQty = s.toInt()
                qtyInfo?.isSelected = true
                WMSSharedPref.getAppPrefs(
                    WMSApplication.getInstance().context
                ).saveQTYInfo(qtyInfo)
                finish()
                   PutAwayDetailsActivity.getInstance()?.loadFromCache();
            }
        ...
        imageIcon!!.setOnClickListener { view: View? ->
            ...
            finish()
            PutAwayDetailsActivity.getInstance()?.launchChangeBin();
        }
        textTitle!!.text = "Bin Number: " + qtyInfo!!.binLocation
        textMessage!!.text = title
        qty_one!!.setText(qtyInfo.putawayTotalQty.toString())
        qty_two!!.setText(qtyInfo.putawayTotalQty.toString())
